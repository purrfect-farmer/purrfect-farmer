import { DEBUG, MEDIA_CACHE_MAX_BYTES, MEDIA_PROGRESSIVE_CACHE_DISABLED, MEDIA_PROGRESSIVE_CACHE_NAME, } from '../config';
...
export async function respondForProgressive(e) {
    const { url } = e.request;
...
export async function requestPart(e, params) {
...
async function getClientForRequest(url) {
    ...
    return clients.find((c) => (c.type === 'window' && c.frameType === 'top-level' && getAccountSlot(c.url) === urlAccountSlot));
}
