...
export const processDeepLink = (url) => {
    ...
    const { protocol, searchParams, hostname, } = new URL(urlToParse);
    if (protocol !== 'tg:')
        return false;
    const method = hostname;
    const params = Object.fromEntries(searchParams);
    const { checkChatInvite, openChatByUsername, openChatByPhoneNumber, openStickerSet, joinVoiceChatByLink, openInvoice, openChatWithDraft, checkChatlistInvite, openStoryViewerByUsername, checkGiftCode, openStarsBalanceModal, } = actions;
    switch (method) {
        case 'resolve': {
            const { domain, phone, post, comment, voicechat, livestream, start, startattach, attach, thread, topic, appname, startapp, mode, story, text, } = params;
        ...
        case 'stars_topup': {
            ...
            if (!balanceNeeded || balanceNeeded < 0)
                return true;
...
export function formatShareText(url, text, title) {
    return {
...
function parseChooseParameter(choose) {
    ...
    return types.filter((type) => API_CHAT_TYPES.includes(type));
}
