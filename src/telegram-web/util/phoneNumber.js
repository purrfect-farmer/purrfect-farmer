...
export function getCountryCodeByIso(phoneCodeList, iso) {
        ...
        while (pattern[j] !== PATTERN_PLACEHOLDER && j < pattern.length) {
            ...
            if (pattern[j] === phoneNumber[i]) { // If pattern contains digits, move input position too
                ...
                if (i === phoneNumber.length)
                    break; // But don't overdo it, or it will insert full pattern unexpectedly
            }
            j++;
        }
...
function getBestPattern(numberWithoutCode, patterns) {
    ...
    const bestMatches = patterns.filter((pattern) => {
        ...
        if (stripped.startsWith(PATTERN_PLACEHOLDER))
            return false; // Don't consider default number format here
