...
export function register(trackId, trackType, handler, onForcePlay, onTrackChange) {
    if (!tracks.has(trackId)) {
...
function cleanUpQueue(type, trackId) {
    if (selectTabState(getGlobal()).globalSearch.currentContent === GlobalSearchContent.Music)
        return;
...
function findNextInQueue(currentId, origin = AudioOrigin.Inline, isReverseOrder) {
...
export function makeTrackId(message) {
