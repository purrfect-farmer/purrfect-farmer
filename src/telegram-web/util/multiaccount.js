import { ACCOUNT_QUERY, DATA_BROADCAST_CHANNEL_PREFIX, ESTABLISH_BROADCAST_CHANNEL_PREFIX, GLOBAL_STATE_CACHE_PREFIX, MULTIACCOUNT_MAX_SLOTS, MULTITAB_LOCALSTORAGE_KEY_PREFIX, SESSION_ACCOUNT_PREFIX, } from '../config';
...
export const ACCOUNT_SLOT = WORKER_ACCOUNT_SLOT || (IS_MULTIACCOUNT_SUPPORTED ? getAccountSlot(globalThis.location.href) : undefined);
...
export function getAccountSlot(url) {
    const params = new URL(url).searchParams;
    const slot = params.get(ACCOUNT_QUERY);
    const slotNumber = slot ? Number(slot) : 1;
    if (!slotNumber || Number.isNaN(slotNumber) || slotNumber === 1)
        return undefined;
    return slotNumber;
}
...
function getAccountInfo(slot) {
    ...
    const { userId, avatarUri, color, emojiStatusId, firstName, lastName, isPremium, isTest, phone, } = sessionData || {};
    if (!userId)
        return undefined;
    return {
        userId,
...
export function loadSlotSession(slot) {
    try {
        const data = JSON.parse(localStorage.getItem(`${SESSION_ACCOUNT_PREFIX}${slot || 1}`) || '{}');
        if (!data.dcId)
            return undefined;
        return data;
    }
...
export function storeAccountData(slot, data) {
    ...
    if (!updatedSharedData.userId)
        return;
    writeSlotSession(slot, updatedSharedData);
}
export function writeSlotSession(slot, data) {
    localStorage.setItem(`${SESSION_ACCOUNT_PREFIX}${slot || 1}`, JSON.stringify(data));
}
export function getAccountSlotUrl(slot, forLogin) {
