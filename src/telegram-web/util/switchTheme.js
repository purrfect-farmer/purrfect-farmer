...
export function lerpRgb(start, end, interpolationRatio) {
...
function applyColorAnimationStep(startIndex, endIndex, interpolationRatio = 1) {
  colors.forEach(({ property, colors: propertyColors }) => {
    const { r, g, b, a } = lerpRgb(
      ...
      propertyColors[endIndex],
      interpolationRatio
    );
    const roundedA =
      a !== undefined
        ? Math.round((a / 255) * 10 ** DECIMAL_PLACES) / 10 ** DECIMAL_PLACES
        : undefined;
    document.documentElement.style.setProperty(
      property,
      `rgb(${r},${g},${b}${roundedA ? `,${roundedA}` : ""})`
    );
