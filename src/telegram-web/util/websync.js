...
const WEBSYNC_URLS = ["t.me", "telegram.me"].map(
  (domain) => `https://${domain}/_websync_?`
);
...
let lastTimeout;
export const forceWebsync = (authed) => {
  if (!ENABLE_WEB_SYNC) return undefined;
  if (IS_MOCKED_CLIENT || IS_ELECTRON) return undefined;
  const currentTs = getTs();
  const { canRedirect, ts } = JSON.parse(
    localStorage.getItem(WEBSYNC_KEY) || "{}"
  );
  if (canRedirect !== authed || ts + WEBSYNC_TIMEOUT <= currentTs) {
    return Promise.all(
      WEBSYNC_URLS.map((url) => {
        return new Promise((resolve, reject) => {
          const script = document.createElement("script");
          const removeElement = () =>
            Boolean(document.body.removeChild(script));
          script.src =
            url +
            new URLSearchParams({
...
export function startWebsync() {
  ...
  lastTimeout = setTimeout(() => {
    ...
    const authed =
      authState === "authorizationStateReady" || hasStoredSession();
    forceWebsync(authed);
