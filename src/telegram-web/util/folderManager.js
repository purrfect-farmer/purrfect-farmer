...
import { ALL_FOLDER_ID, ARCHIVED_FOLDER_ID, DEBUG, SAVED_FOLDER_ID, SERVICE_NOTIFICATIONS_USER_ID, } from '../config';
import { getIsChatMuted } from '../global/helpers/notifications';
import { selectChatLastMessage, selectNotifyDefaults, selectTabState, selectTopics, } from '../global/selectors';
...
export function getOrderedIds(folderId) {
    if (!inited)
        init();
    return results.orderedIdsByFolderId[folderId];
}
export function getPinnedChatsCount(folderId) {
    if (!inited)
        init();
...
export function addChatsCountCallback(callback) {
    return callbacks.chatsCountByFolderId.addCallback(callback);
}
export function addUnreadChatsByFolderIdCallback(callback) {
    return callbacks.unreadChatIdsByFolderId.addCallback(callback);
}
export function addUnreadCountersCallback(callback) {
...
function updateFolderManager(global) {
    ...
    if (!(isAllFolderChanged || isArchivedFolderChanged || isSavedFolderChanged || areFoldersChanged
    ...
    affectedFolderIds = affectedFolderIds.concat(updateChats(global, areFoldersChanged || isAllFolderChanged || isArchivedFolderChanged || isSavedFolderChanged, areNotifyDefaultsChanged, areNotifyExceptionsChanged, prevAllFolderListIds, prevArchivedFolderListIds, prevSavedFolderListIds));
...
function updateFolders(global, isAllFolderChanged, isArchivedFolderChanged, isSavedFolderChanged, areFoldersChanged) {
    ...
    if (isAllFolderChanged) {
        ...
        prepared.folderSummariesById[ALL_FOLDER_ID] = buildFolderSummaryFromMainList(ALL_FOLDER_ID, newListIds, newPinnedIds);
    ...
    if (isArchivedFolderChanged) {
        ...
        prepared.folderSummariesById[ARCHIVED_FOLDER_ID] = buildFolderSummaryFromMainList(ARCHIVED_FOLDER_ID, newListIds, newPinnedIds);
    ...
    if (isSavedFolderChanged) {
        ...
        prepared.folderSummariesById[SAVED_FOLDER_ID] = buildFolderSummaryFromMainList(SAVED_FOLDER_ID, newListIds, newPinnedIds);
...
function updateChats(global, areFoldersChanged, areNotifyDefaultsChanged, areNotifyExceptionsChanged, prevAllFolderListIds, prevArchivedFolderListIds, prevSavedFolderListIds) {
    ...
    if (newAllFolderListIds !== prevAllFolderListIds || newArchivedFolderListIds !== prevArchivedFolderListIds
        ...
        allIds = unique(allIds.concat(prevAllFolderListIds || [], prevArchivedFolderListIds || [], prevSavedFolderListIds || []));
    }
    allIds.forEach((chatId) => {
        ...
        if (chat) {
            ...
            const newSummary = buildChatSummary(global, chat, newNotifyDefaults, newNotifyExceptions, newUsersById[chatId], isRemovedFromAll, isRemovedFromSaved);
...
function buildChatSummary(global, chat, notifyDefaults, notifyExceptions, user, isRemovedFromAll, isRemovedFromSaved) {
    const { id, type, isRestricted, isNotJoined, migratedTo, folderId, unreadCount: chatUnreadCount, unreadMentionsCount: chatUnreadMentionsCount, hasUnreadMark, isForum, } = chat;
    ...
    const shouldHideServiceChat = chat.id === SERVICE_NOTIFICATIONS_USER_ID && (!lastMessage || lastMessage.content.action?.type === 'historyClear');
...
function isChatInFolder(chatSummary, folderSummary) {
    ...
    if (folderSummary.listIds) {
        if ((chatSummary.isArchived && folderSummary.id === ALL_FOLDER_ID)
...
function updateResults(affectedFolderIds) {
    ...
    Array.from(affectedFolderIds).forEach((folderId) => {
        ...
        const areOrderedIdsChanged = (!currentOrderedIds
        ...
        if (!wasUnreadCountersChanged) {
            wasUnreadCountersChanged = (!currentUnreadCounters || !arePropsShallowEqual(newUnreadCounters, currentUnreadCounters));
        }
        ...
        if (!wasUnreadChatsChanged) {
            wasUnreadChatsChanged = (!currentUnreadChats || !areSortedArraysEqual(newUnreadChats, currentUnreadChats));
        }
...
function buildFolderOrderedIds(folderId) {
    ...
    const { chatIdsByFolderId: { [folderId]: chatIds }, } = prepared;
    const { orderedIdsByFolderId: { [folderId]: prevOrderedIds }, } = results;
...
function buildFolderByUnreadChats(folderId) {
    const { chatSummariesById } = prepared;
    const { orderedIdsByFolderId: { [folderId]: orderedIds } } = results;
