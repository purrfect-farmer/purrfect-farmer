...
const notificationSound = new Audio("/telegram-web/public/notification.mp3");
notificationSound.setAttribute("mozaudiochannel", "notification");
export async function playNotifySound(id, volume) {
...
export async function subscribe() {
  ...
  try {
    ...
    if (Notification.permission === "denied") {
      ...
      if (
        [DOMException.ABORT_ERR, DOMException.NOT_SUPPORTED_ERR].includes(
          error.code
        )
      ) {
...
function checkIfShouldNotify(chat, message) {
  ...
  const isMuted =
    topicMutedUntil === undefined
      ? isChatMuted
      : topicMutedUntil > getServerTime();
  const shouldIgnoreMute = message.isMentioned;
  const shouldNotifyAboutMessage =
    message.content?.action?.type !== "phoneCall";
  if (
...
function getNotificationContent(chat, message, reaction) {
  ...
  if (
    ...
    const isChat =
      chat && (isChatChannel(chat) || message.senderId === message.chatId);
    // TODO[forums] Support ApiChat
    const sender = isChat ? messageSenderChat : messageSenderUser;
    const senderName = sender
      ? getMessageSenderName(oldTranslate, chat.id, sender)
      : undefined;
    let summary =
      jsxToHtml(
        <span>
  ...
  let title = isScreenLocked
    ? APP_NAME
    : getChatTitle(oldTranslate, chat, isSelf);
  if (message.isSilent) {
    title += " 🔕";
  }
  return { title, body };
...
export async function notifyAboutCall({ call, user }) {
...
export async function notifyAboutMessage({
  ...
  const isChatSilent = getIsChatSilent(
    chat,
    selectNotifyDefaults(getGlobal()),
    selectNotifyException(getGlobal(), chat.id)
  );
  const topic = selectTopicFromMessage(global, message);
  const isSilent =
    topic?.notifySettings.hasSound === undefined
      ? isChatSilent
      : !topic.notifySettings.hasSound;
  const areNotificationsSupported = checkIfNotificationsSupported();
  ...
  if (
    isReaction &&
    activeReaction &&
    activeReaction.reaction.type === "custom"
  ) {
    await loadCustomEmoji(activeReaction.reaction.documentId);
  }
  const icon = await getAvatar(chat);
  const { title, body } = getNotificationContent(chat, message, activeReaction);
  if (checkIfPushSupported()) {
...
export function closeMessageNotifications(payload) {
