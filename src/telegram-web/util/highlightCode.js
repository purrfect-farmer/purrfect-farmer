...
export default async function highlightCode(text, language) {
...
function getLanguageName(alias) {
  return Object.entries(SUPPORTED_LANGUAGES).find(
    ([langName, aliases]) => langName === alias || aliases.includes(alias)
  )?.[0];
}
async function ensureLanguage(language) {
  ...
  const languagePromise = THIRD_PARTY_LANGUAGES.includes(langCode)
    ? loadThirdPartyLanguage(langCode)
    : loadFirstPartyLanguage(langCode);
