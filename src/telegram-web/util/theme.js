...
export function setVariable(name, lightValueOrGeneral, darkValue) {
    if (!darkValue) {
...
export function updatePeerColors(peerColors) {
...
function setPeerColor(n, colors, darkColors) {
    ...
    if (colors.length > 1) {
        const lightGradientColors = colors.map((color, i) => (`${color} ${i * PEER_COLOR_GRADIENT_STEP}px, ${color} ${(i + 1) * PEER_COLOR_GRADIENT_STEP}px`));
        const darkGradientColors = darkColors?.map((color, i) => (`${color} ${i * PEER_COLOR_GRADIENT_STEP}px, ${color} ${(i + 1) * PEER_COLOR_GRADIENT_STEP}px`));
