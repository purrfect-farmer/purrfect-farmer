...
export function captureControlledSwipe(element, options) {
...
export function allowSwipeControlForTransition(currentSlide, nextSlide, onCancelForTransition) {
    ...
    requestMeasure(() => {
        ...
        const targetPositionPx = targetPosition.units === 'px'
            ...
            : ((targetPosition.value / 100) * (targetPosition.axis === 'X' ? currentSlide.offsetWidth : currentSlide.offsetHeight));
...
function extractAnimationEndPosition(element) {
    for (const animation of element.getAnimations()) {
        ...
        for (const keyframe of animation.effect.getKeyframes()) {
            if (keyframe.offset !== 1 || !keyframe.transform)
                continue;
