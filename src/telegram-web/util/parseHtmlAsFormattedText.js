...
export const ENTITY_CLASS_BY_NODE_NAME = {
...
export default function parseHtmlAsFormattedText(html, withMarkdownLinks = false, skipMarkdown = false) {
...
export function fixImageContent(fragment) {
...
function parseMarkdown(html) {
    ...
    parsedHtml = parsedHtml.replace(/(?!<(code|pre)[^<]*|<\/)[~]{2}([^~\n]+)[~]{2}(?![^<]*<\/(code|pre)>)/g, '<s>$2</s>');
    parsedHtml = parsedHtml.replace(/(?!<(code|pre)[^<]*|<\/)[|]{2}([^|\n]+)[|]{2}(?![^<]*<\/(code|pre)>)/g, `<span data-entity-type="${ApiMessageEntityTypes.Spoiler}">$2</span>`);
    return parsedHtml;
}
