...
export function getDayStart(datetime) {
    const date = new Date(datetime);
    date.setHours(0, 0, 0, 0);
    return date;
}
export function getDayStartAt(datetime) {
    return getDayStart(datetime).getTime();
}
export function toYearMonth(timestamp) {
...
export function formatTime(lang, datetime) {
...
export function formatPastTimeShort(lang, datetime, alwaysShowTime = false) {
    const date = typeof datetime === 'number' ? new Date(datetime) : datetime;
...
export function formatFullDate(lang, datetime) {
    return formatDateToString(datetime, lang.code, false, 'numeric');
}
export function formatMonthAndYear(lang, date, isShort = false) {
    return formatDateToString(date, lang.code, false, isShort ? 'short' : 'long', true);
}
export function formatCountdown(lang, secondsLeft) {
...
export function formatCountdownShort(lang, msLeft) {
    if (msLeft < 60 * 1000) {
...
export function formatLocationLastUpdate(lang, currentTime, lastUpdated = currentTime) {
...
export function formatRelativePastTime(lang, currentTime, lastUpdated = currentTime) {
    const seconds = currentTime - lastUpdated;
    if (seconds < 60) {
...
export function formatPastDatetime(lang, pastTime, currentTime = getServerTime()) {
...
export function formatTimeDuration(lang, duration, showLast = 2) {
    if (!duration) {
        return undefined;
    }
...
export function formatHumanDate(lang, datetime, isShort = false, noWeekdays = false, isUpperFirst) {
    const date = typeof datetime === 'number' ? new Date(datetime) : datetime;
    const today = getDayStart(new Date());
    if (!noWeekdays) {
...
export function formatVoiceRecordDuration(durationInMs) {
    const parts = [];
...
const formatDayToStringWithCache = withCache((dayStartAt, locale, noYear, monthFormat = 'short', noDay) => {
...
export function formatDateToString(datetime, locale = 'en-US', noYear = false, monthFormat = 'short', noDay = false) {
    const date = typeof datetime === 'number' ? new Date(datetime) : datetime;
    const dayStartAt = getDayStartAt(date);
    return formatDayToStringWithCache(dayStartAt, locale, noYear, monthFormat, noDay);
}
export function formatDateTimeToString(datetime, locale = 'en-US', noSeconds, timeFormat) {
    const date = typeof datetime === 'number' ? new Date(datetime) : datetime;
...
export function formatDateAtTime(lang, datetime) {
    const date = typeof datetime === 'number' ? new Date(datetime) : datetime;
    const today = getDayStart(new Date());
    const time = formatTime(lang, date);
    if (toIsoString(date) === toIsoString(today)) {
...
export function formatShortDuration(lang, duration) {
    if (duration < 0) {
...
export function parseDateString(query = '') {
...
export function timestampPlusDay(timestamp) {
...
export function formatRegistrationMonth(lang, dateString) {
