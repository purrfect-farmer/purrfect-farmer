...
export const copyHtmlToClipboard = (html, text) => {
...
export const copyImageToClipboard = (imageUrl) => {
    if (!imageUrl)
        return;
...
export const copyTextToClipboardFromPromise = async (getTextPromise, onSuccess, onFailure) => {
    ...
    if (!CLIPBOARD_ITEM_SUPPORTED || !navigator.clipboard.write) {
        if (await copyTextToClipboardFallback())
            onSuccess();
        return;
    }
    try {
