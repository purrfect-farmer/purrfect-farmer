...
export function isDeepLink(link) {
    return Boolean(link.match(RE_TME_LINK) || link.match(RE_TG_LINK));
}
export function tryParseDeepLink(link) {
    if (!isDeepLink(link)) {
        return undefined;
    }
    try {
...
export function getUsernameFromDeepLink(url) {
...
function parseTgLink(url) {
    ...
    switch (deepLinkType) {
        case 'publicMessageLink': {
            const { domain, post, single, thread, comment, t, } = queryParams;
        ...
        case 'privateMessageLink': {
            const { channel, post, single, thread, comment, t, } = queryParams;
...
function parseHttpLink(url) {
    ...
    switch (deepLinkType) {
        case 'publicMessageLink': {
            const { single, comment, t, } = queryParams;
            const { username, thread, messageId, } = pathParams.length === 2 ? {
        ...
        case 'privateMessageLink': {
            const { single, comment, t, } = queryParams;
            const { channelId, thread, messageId, } = pathParams.length === 3 ? {
...
function getHttpDeepLinkType(queryParams, pathParams) {
    ...
    if (len === 1) {
        if (method === 'share')
            return 'shareLink';
        if (method === 'boost' || queryParams.boost !== undefined)
        ...
        if (method === 'c') {
            if (queryParams.boost !== undefined)
                return 'chatBoostLink';
...
function getTgDeepLinkType(queryParams, pathParams, method) {
    switch (method) {
        case 'resolve': {
            const { domain, post, bot_id, scope, public_key, nonce, } = queryParams;
...
function buildPublicMessageLink(params) {
    const { messageId, threadId, commentId, username, single, timestamp, } = params;
...
function buildPrivateMessageLink(params) {
    const { messageId, threadId, commentId, channelId, single, timestamp, } = params;
...
function buildTelegramPassportLink(params) {
    const { botId, scope, publicKey, nonce, callbackUrl, payload, } = params;
...
function buildPublicUsernameOrBotLink(params) {
    const { username, start, text, startApp, mode, startAttach, attach, appName, choose, ref, } = params;
    if (!username) {
        return undefined;
    }
...
export function buildChatBoostLink(params) {
...
function buildSettingsScreen(screenParam) {
    switch (screenParam) {
        case 'devices':
            return 'devices';
        case 'folders':
...
function buildSettingsScreenLink(params) {
    const { screen, } = params;
    return {
        type: 'settings',
