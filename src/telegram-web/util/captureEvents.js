...
export function captureEvents(element, options) {
    ...
    function onCapture(e) {
        ...
        const { excludedClosestSelector, includedClosestSelector, withNativeDrag, withCursor, onDrag, } = options;
        ...
        if ((excludedClosestSelector && (target.matches(excludedClosestSelector) || target.closest(excludedClosestSelector)))
    ...
    function onWheel(e) {
        if (!options.onZoom && !options.onDrag)
            return;
        if (options.excludedClosestSelector && (e.target.matches(options.excludedClosestSelector)
            || e.target.closest(options.excludedClosestSelector))) {
            return;
        }
        e.preventDefault();
        e.stopPropagation();
