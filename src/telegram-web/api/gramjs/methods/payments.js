...
import { buildApiBoost, buildApiBoostsStatus, buildApiCheckedGiftCode, buildApiGiveawayInfo, buildApiMyBoost, buildApiPaymentForm, buildApiPremiumGiftCodeOption, buildApiPremiumPromo, buildApiReceipt, buildShippingOptions, } from '../apiBuilders/payments';
import { buildApiPeerId } from '../apiBuilders/peers';
import { buildInputInvoice, buildInputPeer, buildInputStorePaymentPurpose, buildInputThemeParams, buildShippingInfo, DEFAULT_PRIMITIVES, } from '../gramjsBuilders';
import { deserializeBytes, serializeBytes, } from '../helpers/misc';
import localDb from '../localDb';
import { sendApiUpdate } from '../updates/apiUpdateEmitter';
import { handleGramJsUpdate, invokeRequest } from './client';
import { getTemporaryPaymentPassword } from './twoFaSettings';
export async function validateRequestedInfo({ inputInvoice, requestInfo, shouldSave, }) {
...
export async function sendPaymentForm({ inputInvoice, formId, requestedInfoId, shippingOptionId, credentials, savedCredentialId, temporaryPassword, tipAmount, }) {
...
export async function sendStarPaymentForm({ formId, inputInvoice, }) {
    ...
    if (inputInvoice.type === 'chatInviteSubscription') {
        ...
        const mtpChannelId = updates?.find((update) => (update instanceof GramJs.UpdateChannel))?.channelId;
        if (!mtpChannelId) {
            return undefined;
        }
        return {
...
export async function getPaymentForm(inputInvoice, theme) {
    try {
...
export async function getReceipt(chat, msgId) {
...
export async function fetchTemporaryPaymentPassword(password) {
...
export async function applyBoost({ chat, slots, }) {
...
export async function fetchBoostStatus({ chat, }) {
...
export async function fetchBoostList({ chat, isGifts, offset = DEFAULT_PRIMITIVES.STRING, limit = DEFAULT_PRIMITIVES.INT, }) {
...
export async function fetchGiveawayInfo({ peer, messageId, }) {
...
export async function checkGiftCode({ slug, }) {
...
export function applyGiftCode({ slug, }) {
...
export async function getPremiumGiftCodeOptions({ chat, }) {
...
export function launchPrepaidGiveaway({ chat, giveawayId, paymentPurpose, }) {
