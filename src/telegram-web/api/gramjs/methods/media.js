...
import { DOWNLOAD_WORKERS, MEDIA_CACHE_DISABLED, MEDIA_CACHE_MAX_BYTES, MEDIA_CACHE_NAME, MEDIA_CACHE_NAME_AVATARS, } from '../../../config';
...
export default async function downloadMedia({ url, mediaFormat, start, end, isHtmlAllowed, }, client, onProgress) {
    const { data, mimeType, fullSize, } = await download(url, client, onProgress, start, end, isHtmlAllowed) || {};
    if (!data) {
        return undefined;
    }
    ...
    const canCache = mediaFormat !== ApiMediaFormat.Progressive && (mediaFormat !== ApiMediaFormat.BlobUrl || parsed.size <= MEDIA_CACHE_MAX_BYTES);
    if (!MEDIA_CACHE_DISABLED && cacheApi && canCache) {
        const cacheName = url.startsWith('avatar') ? MEDIA_CACHE_NAME_AVATARS : MEDIA_CACHE_NAME;
        void cacheApi.save(cacheName, url, parsed);
    }
    const dataBlob = mediaFormat === ApiMediaFormat.Progressive ? '' : parsed;
    const arrayBuffer = mediaFormat === ApiMediaFormat.Progressive ? parsed : undefined;
    return {
        dataBlob,
        arrayBuffer,
        mimeType,
...
async function download(url, client, onProgress, start, end, isHtmlAllowed) {
    ...
    if (!parsed)
        return undefined;
    const { entityType, entityId, sizeType, params, mediaMatchType, } = parsed;
...
function parseMedia(data, mediaFormat, mimeType) {
...
function getMimeType(data, fallbackMimeType = 'image/jpeg') {
    if (data.length < 4) {
...
export function parseMediaUrl(url) {
    const mediaMatch = url.startsWith('staticMap')
        ...
        : url.startsWith('webDocument')
            ...
            : url.match(/(avatar|profile|photo|stickerSet|sticker|wallpaper|document)([-\d\w./]+)(?::\d+)?(\?size=\w+)?/);
    if (!mediaMatch) {
        return undefined;
    }
