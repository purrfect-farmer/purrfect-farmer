...
import { buildInputContact, buildInputEmojiStatus, buildInputPeer, buildInputUser, buildMtpPeerId, DEFAULT_PRIMITIVES, getEntityTypeById, } from '../gramjsBuilders';
...
export async function fetchFullUser({ id, accessHash, }) {
...
export async function fetchCommonChats(user, maxId) {
...
export async function fetchPaidMessagesStarsAmount(user) {
...
export async function fetchTopUsers() {
    ...
    const users = topPeers.users.map(buildApiUser).filter((user) => Boolean(user) && !user.isSelf);
...
export async function importContact({ phone = DEFAULT_PRIMITIVES.STRING, firstName = DEFAULT_PRIMITIVES.STRING, lastName = DEFAULT_PRIMITIVES.STRING, }) {
...
export function updateContact({ id, accessHash, phoneNumber = DEFAULT_PRIMITIVES.STRING, firstName = DEFAULT_PRIMITIVES.STRING, lastName = DEFAULT_PRIMITIVES.STRING, shouldSharePhoneNumber = false, }) {
...
export async function deleteContact({ id, accessHash, }) {
    const input = buildInputUser(id, accessHash);
    if (!(input instanceof GramJs.InputUser)) {
        return;
    }
...
export async function fetchProfilePhotos({ peer, offset = 0, limit = 0, }) {
    const chat = 'title' in peer ? peer : undefined;
    const user = !chat ? peer : undefined;
    if (user) {
        ...
        return {
            ...
            photos: result.photos
                .filter((photo) => photo instanceof GramJs.Photo)
    ...
    const { messages, totalCount, nextOffsetId, } = result;
    return {
        count: totalCount,
...
export function reportSpam(userOrChat) {
...
export function updateEmojiStatus(emojiStatus) {
...
export function saveCloseFriends(userIds) {
