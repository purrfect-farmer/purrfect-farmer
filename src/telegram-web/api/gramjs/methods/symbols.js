...
import { buildStickerSet, buildStickerSetCovered, processStickerPackResult, processStickerResult, } from '../apiBuilders/symbols';
import { buildInputDocument, buildInputStickerSet, buildInputStickerSetShortName, DEFAULT_PRIMITIVES, } from '../gramjsBuilders';
import localDb from '../localDb';
import { sendApiUpdate } from '../updates/apiUpdateEmitter';
import { invokeRequest } from './client';
export async function fetchCustomEmojiSets({ hash }) {
...
export async function fetchStickerSets({ hash }) {
...
export async function fetchRecentStickers({ hash }) {
...
export async function fetchFavoriteStickers({ hash }) {
...
export async function fetchFeaturedStickers({ hash }) {
...
export async function faveSticker({ sticker, unfave, }) {
...
export function removeRecentSticker({ sticker, }) {
    const id = buildInputDocument(sticker);
    if (!id)
        return;
...
export async function fetchStickers({ stickerSetInfo }) {
...
export async function fetchCustomEmoji({ documentId }) {
...
export async function fetchCollectibleEmojiStatuses({ hash }) {
    const result = await invokeRequest(new GramJs.account.GetCollectibleEmojiStatuses({ hash: hash ? BigInt(hash) : DEFAULT_PRIMITIVES.BIGINT }));
...
export async function searchStickers({ query, hash }) {
...
export async function fetchSavedGifs({ hash }) {
...
export function saveGif({ gif, shouldUnsave }) {
...
export async function installStickerSet({ stickerSetId, accessHash }) {
...
export async function uninstallStickerSet({ stickerSetId, accessHash }) {
...
let inputGifBot;
export async function searchGifs({ query, offset = DEFAULT_PRIMITIVES.STRING, username = DEFAULT_GIF_SEARCH_BOT_USERNAME, }) {
    if (!inputGifBot) {
...
export async function fetchStickersForEmoji({ emoji, hash, }) {
...
export async function fetchEmojiKeywords({ language, fromVersion = DEFAULT_PRIMITIVES.INT, }) {
...
export async function fetchRecentEmojiStatuses(hash) {
