...
import { buildApiAttachBot, buildApiBotInlineMediaResult, buildApiBotInlineResult, buildApiMessagesBotApp, buildBotSwitchPm, buildBotSwitchWebview, } from '../apiBuilders/bots';
...
import { buildInputBotApp, buildInputPeer, buildInputReplyTo, buildInputThemeParams, buildInputUser, DEFAULT_PRIMITIVES, generateRandomBigInt, } from '../gramjsBuilders';
import { addDocumentToLocalDb, addPhotoToLocalDb, addUserToLocalDb, addWebDocumentToLocalDb, } from '../helpers/localDb';
import { deserializeBytes } from '../helpers/misc';
import { sendApiUpdate } from '../updates/apiUpdateEmitter';
import { invokeRequest } from './client';
export async function answerCallbackButton({ chatId, accessHash, messageId, data, isGame, }) {
...
export async function fetchInlineBot({ username }) {
...
export async function fetchInlineBotResults({ bot, chat, query, offset = DEFAULT_PRIMITIVES.STRING, }) {
...
export async function sendInlineBotResult({ chat, replyInfo, resultId, queryId, sendAs, isSilent, scheduleDate, allowPaidStars, }) {
...
export async function startBot({ bot, startParam, }) {
    const randomId = generateRandomBigInt();
...
export async function requestWebView({ isSilent, peer, bot, url, startParam, replyInfo, theme, sendAs, isFromBotMenu, isFullscreen, }) {
...
export async function requestMainWebView({ peer, bot, startParam, mode, theme, }) {
...
export async function requestSimpleWebView({ bot, url, theme, startParam, isFromSwitchWebView, isFromSideMenu, }) {
...
export async function fetchBotApp({ bot, appName, }) {
...
export async function requestAppWebView({ peer, app, startParam, mode, theme, isWriteAllowed, }) {
...
export function prolongWebView({ isSilent, peer, bot, queryId, replyInfo, sendAs, }) {
...
export async function sendWebViewData({ bot, buttonText, data, }) {
    const randomId = generateRandomBigInt();
...
export async function loadAttachBots({ hash, }) {
...
export async function loadAttachBot({ bot, }) {
...
export function toggleAttachBot({ bot, isWriteAllowed, isEnabled, }) {
...
export async function requestBotUrlAuth({ chat, buttonId, messageId, }) {
...
export async function acceptBotUrlAuth({ chat, messageId, buttonId, isWriteAllowed, }) {
...
export async function requestLinkUrlAuth({ url }) {
    const result = await invokeRequest(new GramJs.messages.RequestUrlAuth({
        url,
    }));
    if (!result)
...
export async function acceptLinkUrlAuth({ url, isWriteAllowed }) {
    const result = await invokeRequest(new GramJs.messages.AcceptUrlAuth({
        url,
        writeAllowed: isWriteAllowed || undefined,
    }));
...
export function fetchBotCanSendMessage({ bot }) {
    return invokeRequest(new GramJs.bots.CanSendMessage({
        bot: buildInputUser(bot.id, bot.accessHash),
    }));
}
export function allowBotSendMessages({ bot }) {
...
export async function invokeWebViewCustomMethod({ bot, customMethod, parameters, }) {
    try {
...
export async function fetchPreviewMedias({ bot }) {
    const result = await invokeRequest(new GramJs.bots.GetPreviewMedias({
        bot: buildInputUser(bot.id, bot.accessHash),
    }));
    if (!result)
        return undefined;
    const previews = result.map((preview) => {
        return {
...
export function checkBotDownloadFileParams({ bot, fileName, url, }) {
...
export function toggleUserEmojiStatusPermission({ bot, isEnabled }) {
...
export function setBotInfo({ bot, langCode, name, about, description, }) {
...
export async function fetchPopularAppBots({ offset = DEFAULT_PRIMITIVES.STRING, limit, }) {
...
export async function fetchBotsRecommendations({ user }) {
    if (!user)
        return undefined;
