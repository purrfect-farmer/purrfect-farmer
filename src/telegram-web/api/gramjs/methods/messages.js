...
import { API_GENERAL_ID_LIMIT, DEBUG, GIF_MIME_TYPE, MAX_INT_32, MENTION_UNREAD_SLICE, MESSAGE_ID_REQUIRED_ERROR, PINNED_MESSAGES_LIMIT, REACTION_UNREAD_SLICE, SUPPORTED_PHOTO_CONTENT_TYPES, SUPPORTED_VIDEO_CONTENT_TYPES, } from '../../../config';
import { fetchFile } from '../../../util/files';
import { compact, split } from '../../../util/iteratees';
import { getMessageKey } from '../../../util/keys/messageKey';
import { getServerTime } from '../../../util/serverTime';
import { interpolateArray } from '../../../util/waveform';
import { buildApiChatFromPreview, buildApiSendAsPeerId, buildApiSponsoredMessageReportResult, } from '../apiBuilders/chats';
import { buildApiFormattedText } from '../apiBuilders/common';
import { buildMessageMediaContent, buildMessageTextContent, buildPollFromMedia, buildWebPage, } from '../apiBuilders/messageContent';
import { buildApiFactCheck, buildApiMessage, buildApiQuickReply, buildApiReportResult, buildApiSponsoredMessage, buildApiThreadInfo, buildLocalForwardedMessage, buildLocalMessage, buildPreparedInlineMessage, buildUploadingMedia, } from '../apiBuilders/messages';
import { getApiChatIdFromMtpPeer } from '../apiBuilders/peers';
import { buildApiUser, buildApiUserStatuses } from '../apiBuilders/users';
import { buildInputChannel, buildInputMediaDocument, buildInputPeer, buildInputPoll, buildInputPollFromExisting, buildInputReaction, buildInputReplyTo, buildInputStory, buildInputTextWithEntities, buildInputTodo, buildInputUser, buildMessageFromUpdate, buildMtpMessageEntity, buildPeer, buildSendMessageAction, DEFAULT_PRIMITIVES, generateRandomBigInt, getEntityTypeById, } from '../gramjsBuilders';
import { deserializeBytes, resolveMessageApiChatId, } from '../helpers/misc';
...
export async function fetchMessages({ chat, threadId, offsetId, isSavedDialog, addOffset, limit, }) {
...
export async function fetchMessage({ chat, messageId }) {
    const isChannel = getEntityTypeById(chat.id) === 'channel';
    let result;
    try {
        result = await invokeRequest(isChannel
...
export function sendMessageLocal(params) {
    const { chat, lastMessageId, text, entities, replyInfo, attachment, sticker, story, gif, poll, todo, contact, scheduledAt, groupedId, sendAs, wasDrafted, isInvertedMedia, effectId, isPending, messagePriceInStars, } = params;
    if (!chat)
        return undefined;
    const { message: localMessage, poll: localPoll, } = buildLocalMessage(chat, lastMessageId, text, entities, replyInfo, attachment, sticker, gif, poll, todo, contact, groupedId, scheduledAt, sendAs, story, isInvertedMedia, effectId, isPending, messagePriceInStars);
    sendApiUpdate({
...
export function sendApiMessage(params, localMessage, onProgress) {
    const { chat, text, entities, replyInfo, attachment, sticker, story, gif, poll, todo, contact, isSilent, scheduledAt, groupedId, noWebPage, sendAs, shouldUpdateStickerSetOrder, isInvertedMedia, effectId, webPageMediaSize, webPageUrl, messagePriceInStars, } = params;
    if (!chat)
        return undefined;
...
export async function sendMessage(params, onProgress) {
...
function sendGroupedMedia({ chat, text = DEFAULT_PRIMITIVES.STRING, entities, replyInfo, attachment, groupedId, isSilent, scheduledAt, sendAs, messagePriceInStars, }, randomId, localMessage, onProgress) {
    let groupIndex = -1;
    ...
    mediaQueue = (async () => {
        ...
        const inputMedia = await fetchInputMedia(buildInputPeer(chat.id, chat.accessHash), media);
        await prevMediaQueue;
        if (!inputMedia) {
            groupedUploads[groupedId].counter--;
            if (DEBUG) {
...
async function fetchInputMedia(peer, uploadedMedia) {
    ...
    if ((messageMedia instanceof GramJs.MessageMediaPhoto
        && messageMedia.photo
    ...
    if ((messageMedia instanceof GramJs.MessageMediaDocument
...
export async function editMessage({ chat, message, text, entities, attachment, noWebPage, }, onProgress) {
...
export async function editTodo({ chat, message, todo, }) {
...
export async function appendTodoList({ chat, message, items, }) {
...
export async function rescheduleMessage({ chat, message, scheduledAt, }) {
    await invokeRequest(new GramJs.messages.EditMessage({
        peer: buildInputPeer(chat.id, chat.accessHash),
        id: message.id,
        scheduleDate: scheduledAt,
...
async function uploadMedia(message, attachment, onProgress) {
    const { filename, blobUrl, mimeType, quick, voice, audio, previewBlobUrl, shouldSendAsFile, shouldSendAsSpoiler, ttlSeconds, } = attachment;
    ...
    const attributes = [new GramJs.DocumentAttributeFilename({ fileName: filename })];
    if (!shouldSendAsFile) {
        if (quick) {
...
export async function pinMessage({ chat, messageId, isUnpin, isOneSide, isSilent, }) {
...
export async function unpinAllMessages({ chat, threadId }) {
...
export async function deleteMessages({ chat, messageIds, shouldDeleteForAll, }) {
    const isChannel = getEntityTypeById(chat.id) === 'channel';
    const result = await invokeRequest(isChannel
...
export async function deleteParticipantHistory({ chat, peer, isRepeat = false, }) {
    const result = await invokeRequest(new GramJs.channels.DeleteParticipantHistory({
        channel: buildInputChannel(chat.id, chat.accessHash),
...
export function deleteScheduledMessages({ chat, messageIds, }) {
    invokeRequest(new GramJs.messages.DeleteScheduledMessages({
        peer: buildInputPeer(chat.id, chat.accessHash),
        id: messageIds,
    }));
}
export async function deleteHistory({ chat, shouldDeleteForAll, maxId, }) {
    const isChannel = getEntityTypeById(chat.id) === 'channel';
    const result = await invokeRequest(isChannel
...
export async function deleteSavedHistory({ chat, }) {
...
export async function reportMessages({ peer, messageIds, description, option, }) {
    try {
...
export function reportChannelSpam({ peer, chat, messageIds, }) {
...
export async function sendMessageAction({ peer, threadId, action, }) {
...
export async function markMessageListRead({ chat, threadId, maxId = 0, }) {
    const isChannel = getEntityTypeById(chat.id) === 'channel';
    // Workaround for local message IDs overflowing some internal `Buffer` range check
...
export async function markMessagesRead({ chat, messageIds, }) {
    const isChannel = getEntityTypeById(chat.id) === 'channel';
    const result = await invokeRequest(isChannel
...
export async function fetchMessageViews({ chat, ids, shouldIncrement, }) {
    const chunks = split(ids, API_GENERAL_ID_LIMIT);
    const results = await Promise.all(chunks.map((chunkIds) => (invokeRequest(new GramJs.messages.GetMessagesViews({
        peer: buildInputPeer(chat.id, chat.accessHash),
        id: chunkIds,
...
export async function fetchFactChecks({ chat, ids, }) {
    const chunks = split(ids, API_GENERAL_ID_LIMIT);
    const results = await Promise.all(chunks.map((chunkIds) => (invokeRequest(new GramJs.messages.GetFactCheck({
        peer: buildInputPeer(chat.id, chat.accessHash),
        msgId: chunkIds,
    })))));
    if (!results || results.some((result) => !result))
...
export function reportMessagesDelivery({ chat, messageIds, }) {
    return invokeRequest(new GramJs.messages.ReportMessagesDelivery({
        peer: buildInputPeer(chat.id, chat.accessHash),
        id: messageIds,
    }));
}
export async function fetchDiscussionMessage({ chat, messageId, }) {
...
export async function searchMessagesInChat({ peer, isSavedDialog, savedTag, type, query = DEFAULT_PRIMITIVES.STRING, threadId, minDate, maxDate, offsetId, addOffset, limit, }) {
    let filter;
    switch (type) {
        case 'media':
...
export async function searchMessagesGlobal({ query, offsetRate = 0, offsetPeer, offsetId, limit, type = 'text', minDate, maxDate, context = 'all', }) {
    let filter;
    switch (type) {
        case 'media':
            filter = new GramJs.InputMessagesFilterPhotoVideo();
...
export async function searchHashtagPosts({ hashtag, offsetRate, offsetPeer, offsetId, limit, }) {
    const peer = (offsetPeer && buildInputPeer(offsetPeer.id, offsetPeer.accessHash)) || new GramJs.InputPeerEmpty();
...
export async function sendPollVote({ chat, messageId, options, }) {
...
export async function toggleTodoCompleted({ chat, messageId, completedIds, incompletedIds, }) {
    const { id, accessHash } = chat;
...
export async function closePoll({ chat, messageId, poll, }) {
    const { id, accessHash } = chat;
    await invokeRequest(new GramJs.messages.EditMessage({
        peer: buildInputPeer(id, accessHash),
        id: messageId,
...
export async function loadPollOptionResults({ chat, messageId, option, offset, limit, shouldResetVoters, }) {
    const { id, accessHash } = chat;
...
export async function fetchExtendedMedia({ chat, ids, }) {
    await invokeRequest(new GramJs.messages.GetExtendedMedia({
        peer: buildInputPeer(chat.id, chat.accessHash),
        id: ids,
    }));
}
export function forwardMessagesLocal(params) {
    const { toChat, toThreadId, messages, scheduledAt, sendAs, noAuthors, noCaptions, isCurrentUserPremium, wasDrafted, lastMessageId, } = params;
...
export async function forwardApiMessages(params) {
    const { fromChat, toChat, toThreadId, isSilent, scheduledAt, sendAs, withMyScore, noAuthors, noCaptions, forwardedLocalMessagesSlice, messagePriceInStars, } = params;
    if (!forwardedLocalMessagesSlice)
        return;
...
export async function findFirstMessageIdAfterDate({ chat, timestamp, }) {
...
export async function fetchScheduledHistory({ chat }) {
    const { id, accessHash } = chat;
...
export async function sendScheduledMessages({ chat, ids }) {
    const { id, accessHash } = chat;
...
export async function fetchPinnedMessages({ chat, threadId }) {
    const result = await invokeRequest(new GramJs.messages.Search({
        peer: buildInputPeer(chat.id, chat.accessHash),
...
export async function fetchSeenBy({ chat, messageId }) {
...
export async function fetchSendAs({ chat, isForPaidReactions, }) {
...
export async function viewSponsoredMessage({ random }) {
    await invokeRequest(new GramJs.messages.ViewSponsoredMessage({
        randomId: deserializeBytes(random),
    }));
}
export function clickSponsoredMessage({ random, isMedia, isFullscreen, }) {
...
export async function reportSponsoredMessage({ randomId, option, }) {
    try {
...
export async function readAllMentions({ chat, threadId, }) {
...
export async function readAllReactions({ chat, threadId, }) {
...
export async function fetchUnreadMentions({ chat, threadId, offsetId, addOffset, maxId, minId, }) {
...
export async function fetchUnreadReactions({ chat, threadId, offsetId, addOffset, maxId, minId, }) {
...
export async function transcribeAudio({ chat, messageId, }) {
...
function handleMultipleLocalMessagesUpdate(localMessages, update) {
    ...
    const updateMessageIds = update.updates.filter((u) => (u instanceof GramJs.UpdateMessageID));
    ...
    updateMessageIds.forEach((updateMessageId) => {
        const updateNewScheduledMessage = update.updates
            .find((scheduledUpdate) => {
            if (!(scheduledUpdate instanceof GramJs.UpdateNewScheduledMessage))
                return false;
    ...
    const otherUpdates = update.updates.filter((u) => {
        if (u instanceof GramJs.UpdateMessageID)
            return false;
        if (u instanceof GramJs.UpdateNewScheduledMessage)
            return false;
        return true;
    });
...
function handleLocalMessageUpdate(localMessage, update, scheduledMessageUpdate) {
    let messageUpdate;
    if (update instanceof GramJs.UpdateShortSentMessage || update instanceof GramJs.UpdateMessageID) {
        messageUpdate = update;
    }
    else if ('updates' in update) {
        messageUpdate = update.updates.find((u) => u instanceof GramJs.UpdateMessageID);
        scheduledMessageUpdate = update.updates.find((u) => (u instanceof GramJs.UpdateNewScheduledMessage));
    }
    if (!messageUpdate) {
        handleGramJsUpdate(update);
        return;
...
export async function fetchOutboxReadDate({ chat, messageId }) {
    const { id, accessHash } = chat;
...
export async function fetchQuickReplies() {
    ...
    if (!result || result instanceof GramJs.messages.QuickRepliesNotModified)
        return undefined;
    const messages = result.messages.map(buildApiMessage).filter(Boolean);
...
export async function sendQuickReply({ chat, shortcutId, }) {
    ...
    if (!messages || messages instanceof GramJs.messages.MessagesNotModified)
        return;
    ...
    if ('updates' in result) {
        const filteredUpdates = result.updates
            .filter((u) => !(u instanceof GramJs.UpdateMessageID));
        result.updates = filteredUpdates;
    }
    handleGramJsUpdate(result);
}
export async function exportMessageLink({ id, chat, shouldIncludeThread, shouldIncludeGrouped, }) {
...
export async function fetchPreparedInlineMessage({ bot, id, }) {
