...
import { buildApiPeerStories, buildApiStealthMode, buildApiStory, buildApiStoryView, buildApiStoryViews, } from '../apiBuilders/stories';
import { buildInputPeer, buildInputPrivacyRules, buildInputReaction, DEFAULT_PRIMITIVES, } from '../gramjsBuilders';
import { addStoryToLocalDb } from '../helpers/localDb';
import { deserializeBytes } from '../helpers/misc';
import { invokeRequest } from './client';
export async function fetchAllStories({ stateHash, isFirstRequest = false, isHidden = false, }) {
    ...
    const allUserStories = result.peerStories.reduce((acc, peerStories) => {
        ...
        const { profileIds, orderedIds, lastUpdatedAt } = Object.values(stories).reduce((dataAcc, story) => {
    ...
    result.peerStories.forEach((peerStories) => (peerStories.stories.forEach((story) => addStoryToLocalDb(story, getApiChatIdFromMtpPeer(peerStories.peer)))));
    return {
...
export async function fetchPeerStories({ peer, }) {
    ...
    const stories = buildCollectionByCallback(result.stories.stories, (story) => ([story.id, buildApiStory(peer.id, story)]));
    // Add after building stories to avoid overwriting repair info
...
export function fetchPeerProfileStories({ peer, offsetId = DEFAULT_PRIMITIVES.INT, }) {
...
export function fetchStoriesArchive({ peer, offsetId = DEFAULT_PRIMITIVES.INT, }) {
    return fetchCommonStoriesRequest({
...
export async function fetchPeerStoriesByIds({ peer, ids }) {
...
export function viewStory({ peer, storyId }) {
    return invokeRequest(new GramJs.stories.IncrementStoryViews({
        peer: buildInputPeer(peer.id, peer.accessHash),
        id: [storyId],
    }));
}
export function markStoryRead({ peer, storyId }) {
    return invokeRequest(new GramJs.stories.ReadStories({
        peer: buildInputPeer(peer.id, peer.accessHash),
        maxId: storyId,
    }));
}
export function deleteStory({ peer, storyId }) {
    return invokeRequest(new GramJs.stories.DeleteStories({
        peer: buildInputPeer(peer.id, peer.accessHash),
        id: [storyId],
    }));
}
export function toggleStoryInProfile({ peer, storyId, isInProfile, }) {
...
export function toggleStoryPinnedToTop({ peer, storyIds, }) {
    return invokeRequest(new GramJs.stories.TogglePinnedToTop({
        peer: buildInputPeer(peer.id, peer.accessHash),
        id: storyIds,
    }));
}
export async function fetchStoryViewList({ peer, storyId, areJustContacts, query, areReactionsFirst, limit = STORY_LIST_LIMIT, offset = DEFAULT_PRIMITIVES.STRING, }) {
...
export async function fetchStoriesViews({ peer, storyIds, }) {
...
export async function fetchStoryLink({ peer, storyId }) {
...
export async function reportStory({ peer, storyId, description, option, }) {
    try {
...
export function editStoryPrivacy({ peer, id, privacy, }) {
...
export function toggleStoriesHidden({ peer, isHidden, }) {
    return invokeRequest(new GramJs.stories.TogglePeerStoriesHidden({
        peer: buildInputPeer(peer.id, peer.accessHash),
        hidden: isHidden,
    }));
}
export function fetchStoriesMaxIds({ peers, }) {
...
async function fetchCommonStoriesRequest({ method, peerId }) {
    ...
    const stories = buildCollectionByCallback(result.stories, (story) => ([story.id, buildApiStory(peerId, story)]));
    // Add after building stories to avoid overwriting repair info
...
export function sendStoryReaction({ peer, storyId, reaction, shouldAddToRecent, }) {
...
export function activateStealthMode({ isForPast, isForFuture, }) {
