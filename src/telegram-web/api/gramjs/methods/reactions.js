...
import { API_GENERAL_ID_LIMIT, REACTION_LIST_LIMIT, RECENT_REACTIONS_LIMIT, TOP_REACTIONS_LIMIT, } from '../../../config';
import { split } from '../../../util/iteratees';
import { buildApiAvailableEffect, buildApiAvailableReaction, buildApiReaction, buildApiSavedReactionTag, buildMessagePeerReaction, } from '../apiBuilders/reactions';
import { buildStickerFromDocument } from '../apiBuilders/symbols';
import { buildInputPaidReactionPrivacy, buildInputPeer, buildInputReaction, DEFAULT_PRIMITIVES, generateRandomTimestampedBigInt, } from '../gramjsBuilders';
import localDb from '../localDb';
import { invokeRequest } from './client';
export function sendWatchingEmojiInteraction({ chat, emoticon, }) {
...
export function sendEmojiInteraction({ chat, emoticon, messageId, timestamps, }) {
    return invokeRequest(new GramJs.messages.SetTyping({
        peer: buildInputPeer(chat.id, chat.accessHash),
...
export function sendReaction({ chat, messageId, reactions, shouldAddToRecent, }) {
...
export function sendPaidReaction({ chat, messageId, count, isPrivate, peerId, }) {
...
export function fetchMessageReactions({ ids, chat, }) {
...
export async function fetchMessageReactionsList({ chat, messageId, reaction, offset, }) {
...
export async function fetchRecentReactions({ hash }) {
...
export async function fetchDefaultTagReactions({ hash }) {
...
export async function fetchSavedReactionTags({ hash }) {
...
export function updateSavedReactionTag({ reaction, title, }) {
