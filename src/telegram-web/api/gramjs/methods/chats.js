...
import { ALL_FOLDER_ID, ARCHIVED_FOLDER_ID, DEBUG, GENERAL_TOPIC_ID, GLOBAL_SEARCH_CONTACTS_LIMIT, MAX_INT_32, MEMBERS_LOAD_SLICE, SERVICE_NOTIFICATIONS_USER_ID, TOPICS_SLICE, } from '../../../config';
import { buildCollectionByKey, omitUndefined } from '../../../util/iteratees';
import { buildApiChatBotCommands, buildApiChatFolder, buildApiChatFolderFromSuggested, buildApiChatFromDialog, buildApiChatFromPreview, buildApiChatFromSavedDialog, buildApiChatInviteInfo, buildApiChatlistExportedInvite, buildApiChatlistInvite, buildApiChatReactions, buildApiMissingInvitedUser, buildApiSponsoredPeer, buildApiTopic, buildChatMember, buildChatMembers, getPeerKey, } from '../apiBuilders/chats';
...
import { buildChatAdminRights, buildChatBannedRights, buildFilterFromApiFolder, buildInputChannel, buildInputChat, buildInputChatReactions, buildInputPeer, buildInputPhoto, buildInputReplyTo, buildInputUser, buildMtpMessageEntity, DEFAULT_PRIMITIVES, generateRandomBigInt, getEntityTypeById, } from '../gramjsBuilders';
import { addPhotoToLocalDb, } from '../helpers/localDb';
import { isChatFolder } from '../helpers/misc';
import { scheduleMutedChatUpdate } from '../scheduleUnmute';
import { sendApiUpdate } from '../updates/apiUpdateEmitter';
import { applyState, processAffectedHistory, updateChannelState, } from '../updates/updateManager';
import { handleGramJsUpdate, invokeRequest, uploadFile } from './client';
export async function fetchChats({ limit, offsetDate, offsetPeer, offsetId, archived, withPinned, lastLocalServiceMessageId, }) {
...
export async function fetchSavedChats({ limit, offsetDate, offsetPeer, offsetId, withPinned, }) {
    const peer = (offsetPeer && buildInputPeer(offsetPeer.id, offsetPeer.accessHash)) || new GramJs.InputPeerEmpty();
...
const fullChatRequestDedupe = new Map();
export async function fetchFullChat(chat) {
    const { id } = chat;
...
export async function fetchPeerSettings(peer) {
...
export async function fetchChat({ type, user, }) {
    let mtpUser;
...
export async function requestChatUpdate({ chat, lastLocalMessage, noLastMessage, }) {
...
export function saveDraft({ chat, draft, }) {
...
async function getFullChatInfo(chatId) {
    ...
    const { about, participants, exportedInvite, botInfo, call, availableReactions, recentRequesters, requestsPending, chatPhoto, translationsDisabled, reactionsLimit, hasScheduled, } = result.fullChat;
    if (chatPhoto) {
...
async function getFullChannelInfo(chat) {
    ...
    const { about, onlineCount, exportedInvite, slowmodeSeconds, slowmodeNextSendDate, migratedFromChatId, migratedFromMaxId, canViewParticipants, canViewStats, linkedChatId, hiddenPrehistory, call, botInfo, availableReactions, reactionsLimit, defaultSendAs, requestsPending, recentRequesters, statsDc, participantsCount, stickerset, chatPhoto, participantsHidden, translationsDisabled, storiesPinnedAvailable, viewForumAsMessages, emojiset, boostsApplied, boostsUnrestrict, botVerification, canViewRevenue: canViewMonetization, paidReactionsAvailable, hasScheduled, stargiftsCount, stargiftsAvailable, paidMessagesAvailable, } = result.fullChat;
    if (chatPhoto) {
        addPhotoToLocalDb(chatPhoto);
    }
    ...
    const { members: kickedMembers, userStatusesById: bannedStatusesById } = (canLoadParticipants && adminRights && await fetchMembers({ chat, memberFilter: 'kicked' })) || {};
    const { members: adminMembers, userStatusesById: adminStatusesById } = (canLoadParticipants && await fetchMembers({ chat, memberFilter: 'admin' })) || {};
    const botCommands = botInfo ? buildApiChatBotCommands(botInfo) : undefined;
...
export function updateChatNotifySettings({ chat, settings, }) {
...
export function updateTopicMutedState({ chat, topicId, isMuted, mutedUntil = 0, }) {
...
export async function createChannel({ title, about = DEFAULT_PRIMITIVES.STRING, users, isBroadcast, isMegagroup, }) {
...
export function joinChannel({ channelId, accessHash, }) {
...
export function deleteChatUser({ chat, user, shouldRevokeHistory, }) {
...
export function leaveChannel({ channelId, accessHash, }) {
...
export function deleteChannel({ channelId, accessHash, }) {
...
export async function createGroupChat({ title, users, }) {
...
export async function editChatPhoto({ chatId, accessHash, photo, }) {
    ...
    return invokeRequest(chatType === 'channel'
...
export async function toggleChatPinned({ chat, shouldBePinned, }) {
    const { id, accessHash } = chat;
...
export async function toggleSavedDialogPinned({ chat, shouldBePinned, }) {
    const { id, accessHash } = chat;
...
export function toggleChatArchived({ chat, folderId, }) {
    const { id, accessHash } = chat;
...
export async function fetchChatFolders() {
    ...
    return {
        byId: buildCollectionByKey(dialogFilters
            .map(buildApiChatFolder), 'id'),
        orderedIds,
    };
}
export async function fetchPinnedDialogs({ listType, }) {
...
export async function editChatFolder({ id, folderUpdate, }) {
...
export async function deleteChatFolder(id) {
    const isActionSuccessful = await invokeRequest(new GramJs.messages.UpdateDialogFilter({
        id,
        filter: undefined,
    }));
...
export function sortChatFolders(ids) {
    return invokeRequest(new GramJs.messages.UpdateDialogFiltersOrder({
        order: ids,
    }));
}
export async function toggleDialogUnread({ chat, hasUnreadMark, }) {
    const { id, accessHash } = chat;
...
export async function getChatByPhoneNumber(phoneNumber) {
    const result = await invokeRequest(new GramJs.contacts.ResolvePhone({
        phone: phoneNumber,
    }));
    return processResolvedPeer(result);
}
export async function getChatByUsername(username, referrer) {
...
export function togglePreHistoryHidden({ chat, isEnabled, }) {
    const { id, accessHash } = chat;
...
export function updateChatDefaultBannedRights({ chat, bannedRights, }) {
    const { id, accessHash } = chat;
...
export function updateChatMemberBannedRights({ chat, user, bannedRights, untilDate, }) {
...
export function updateChatAdmin({ chat, user, adminRights, customTitle = DEFAULT_PRIMITIVES.STRING, }) {
    const channel = buildInputChannel(chat.id, chat.accessHash);
...
export async function updateChatTitle(chat, title) {
    const type = getEntityTypeById(chat.id);
    await invokeRequest(type === 'channel'
...
export async function updateChatAbout(chat, about) {
...
export function toggleSignatures({ chat, areSignaturesEnabled, areProfilesEnabled, }) {
    const { id, accessHash } = chat;
    const channel = buildInputChannel(id, accessHash);
...
export async function fetchMembers({ chat, memberFilter = 'recent', offset, query = DEFAULT_PRIMITIVES.STRING, }) {
    let filter;
    switch (memberFilter) {
        case 'kicked':
...
export async function fetchMember({ chat, peer, }) {
...
export function setDiscussionGroup({ channel, chat, }) {
...
export async function migrateChat(chat) {
    const result = await invokeRequest(new GramJs.messages.MigrateChat({ chatId: buildInputChat(chat.id) }), {
        shouldThrow: true,
    });
    ...
    const newChannelId = result.updates
        .find((update) => update instanceof GramJs.UpdateChannel).channelId;
    const newChannel = result.chats.find((c) => (c instanceof GramJs.Channel && c.id.toString() === newChannelId.toString()));
    return buildApiChatFromPreview(newChannel);
}
export async function checkChatInvite(hash) {
...
export async function addChatMembers(chat, users) {
    try {
...
export function deleteChatMember(chat, user) {
    if (chat.type === 'chatTypeChannel' || chat.type === 'chatTypeSuperGroup') {
...
export function toggleJoinToSend(chat, isEnabled) {
...
export function toggleJoinRequest(chat, isEnabled) {
...
export async function importChatInvite({ hash }) {
...
export function setChatEnabledReactions({ chat, enabledReactions, reactionsLimit, }) {
...
export function toggleIsProtected({ chat, isProtected, }) {
    const { id, accessHash } = chat;
...
export function toggleParticipantsHidden({ chat, isEnabled, }) {
    const { id, accessHash } = chat;
...
export function toggleForum({ chat, isEnabled, }) {
    const { id, accessHash } = chat;
...
export async function createTopic({ chat, title, iconColor, iconEmojiId, sendAs, }) {
    const { id, accessHash } = chat;
    ...
    return updates.updates?.find((update) => (update instanceof GramJs.UpdateMessageID))?.id;
}
export async function fetchTopics({ chat, query, offsetTopicId, offsetId, offsetDate, limit = TOPICS_SLICE, }) {
    const { id, accessHash } = chat;
...
export async function fetchTopicById({ chat, topicId, }) {
    const { id, accessHash } = chat;
...
export async function deleteTopic({ chat, topicId, }) {
    const { id, accessHash } = chat;
...
export function togglePinnedTopic({ chat, topicId, isPinned, }) {
    const { id, accessHash } = chat;
...
export function editTopic({ chat, topicId, title, iconEmojiId, isClosed, isHidden, }) {
    const { id, accessHash } = chat;
...
export async function checkChatlistInvite({ slug, }) {
...
export function joinChatlistInvite({ slug, peers, }) {
...
export async function fetchLeaveChatlistSuggestions({ folderId, }) {
...
export function leaveChatlist({ folderId, peers, }) {
...
export async function createChalistInvite({ folderId, title = DEFAULT_PRIMITIVES.STRING, peers, }) {
    ...
    if (!result || result.filter instanceof GramJs.DialogFilterDefault)
        return undefined;
    return {
...
export function deleteChatlistInvite({ folderId, slug, }) {
...
export async function editChatlistInvite({ folderId, slug, title, peers, }) {
...
export async function fetchChatlistInvites({ folderId, }) {
...
export function togglePeerTranslations({ chat, isEnabled, }) {
    return invokeRequest(new GramJs.messages.TogglePeerTranslations({
        disabled: isEnabled ? undefined : true,
        peer: buildInputPeer(chat.id, chat.accessHash),
    }));
}
export function setViewForumAsMessages({ chat, isEnabled }) {
    const { id, accessHash } = chat;
    const channel = buildInputChannel(id, accessHash);
...
export async function fetchChannelRecommendations({ chat }) {
...
export function updatePaidMessagesPrice({ chat, paidMessagesStars, }) {
...
export async function fetchSponsoredPeer({ query }) {
    const result = await invokeRequest(new GramJs.contacts.GetSponsoredPeers({ q: query }));
    if (!result || result instanceof GramJs.contacts.SponsoredPeersEmpty)
        return undefined;
    return buildApiSponsoredPeer(result.peers[0]);
}
export function toggleAutoTranslation({ chat, isEnabled, }) {
