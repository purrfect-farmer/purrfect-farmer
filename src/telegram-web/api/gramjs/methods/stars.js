...
import { buildApiStarsAmount, buildApiStarsGiftOptions, buildApiStarsGiveawayOptions, buildApiStarsSubscription, buildApiStarsTransaction, buildApiStarTopupOption, } from '../apiBuilders/payments';
...
export async function fetchStarGifts() {
    ...
    return result.gifts.map(buildApiStarGift).filter((gift) => gift.type === 'starGift');
}
export async function fetchResaleGifts({ giftId, offset = DEFAULT_PRIMITIVES.STRING, limit = DEFAULT_PRIMITIVES.INT, attributesHash, filter, }) {
    const attributes = [
...
export async function fetchSavedStarGifts({ peer, offset = DEFAULT_PRIMITIVES.STRING, limit = DEFAULT_PRIMITIVES.INT, filter, }) {
    const params = {
...
export function saveStarGift({ inputGift, shouldUnsave, }) {
...
export async function fetchStarsTransactions({ peer, offset = DEFAULT_PRIMITIVES.STRING, limit = DEFAULT_PRIMITIVES.INT, isInbound, isOutbound, }) {
...
export async function fetchStarsTransactionById({ id, peer, }) {
    const inputPeer = peer ? buildInputPeer(peer.id, peer.accessHash) : new GramJs.InputPeerSelf();
...
export async function fetchStarsSubscriptions({ offset = DEFAULT_PRIMITIVES.STRING, peer, }) {
    const inputPeer = peer ? buildInputPeer(peer.id, peer.accessHash) : new GramJs.InputPeerSelf();
...
export async function changeStarsSubscription({ peer, subscriptionId, isCancelled, }) {
...
export async function fulfillStarsSubscription({ peer, subscriptionId, }) {
...
export async function fetchStarGiftUpgradePreview({ giftId, }) {
...
export function upgradeStarGift({ inputSavedGift, shouldKeepOriginalDetails, }) {
...
export function transferStarGift({ inputSavedGift, toPeer, }) {
...
export function toggleSavedGiftPinned({ inputSavedGifts, peer, }) {
...
export function updateStarGiftPrice({ inputSavedGift, price, }) {
...
export async function fetchStarGiftWithdrawalUrl({ inputGift, password, }) {
    try {
