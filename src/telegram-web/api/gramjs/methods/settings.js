...
import { ACCEPTABLE_USERNAME_ERRORS, BLOCKED_LIST_LIMIT, LANG_PACK, MAX_INT_32, } from '../../../config';
...
import { buildApiConfig, buildApiCountryList, buildApiLanguage, buildApiPeerColors, buildApiPeerNotifySettings, buildApiSession, buildApiTimezone, buildApiWallpaper, buildApiWebSession, buildLangStrings, oldBuildLangPack, } from '../apiBuilders/misc';
import { getApiChatIdFromMtpPeer } from '../apiBuilders/peers';
import { buildDisallowedGiftsSettings, buildInputChannel, buildInputPeer, buildInputPhoto, buildInputPrivacyKey, buildInputPrivacyRules, buildInputUser, DEFAULT_PRIMITIVES, } from '../gramjsBuilders';
...
export function updateProfile({ firstName, lastName, about, }) {
...
export async function updateProfilePhoto(photo, isFallback) {
...
export async function uploadProfilePhoto(file, isFallback, isVideo = false, videoTs = 0, bot) {
...
export async function uploadContactProfilePhoto({ file, isSuggest, user, }) {
...
export async function deleteProfilePhotos(photos) {
...
export async function uploadWallpaper(file) {
    ...
    localDb.documents[String(result.document.id)] = result.document;
    return { wallpaper };
}
export async function fetchBlockedUsers({ isOnlyStories, }) {
...
export function blockUser({ user, isOnlyStories, }) {
    return invokeRequest(new GramJs.contacts.Block({
        id: buildInputPeer(user.id, user.accessHash),
        myStoriesFrom: isOnlyStories,
    }));
}
export function unblockUser({ user, isOnlyStories, }) {
...
export function terminateAuthorization(hash) {
...
export function terminateWebAuthorization(hash) {
...
export async function fetchNotifyDefaultSettings() {
    const [usersSettings, groupsSettings, channelsSettings,] = await Promise.all([
...
export function updateContactSignUpNotification(isSilent) {
    return invokeRequest(new GramJs.account.SetContactSignUpNotification({ silent: isSilent }));
}
export function updateNotificationSettings(peerType, { isMuted, shouldShowPreviews, }) {
    let peer;
    if (peerType === 'users') {
        peer = new GramJs.InputNotifyUsers();
    }
...
export async function fetchLangPack({ langPack, langCode, }) {
...
export async function fetchLangDifference({ langPack, langCode, fromVersion, }) {
...
export async function fetchLanguage({ langPack, langCode, }) {
...
export async function fetchPrivacySettings(privacyKey) {
...
export async function setPrivacySettings(privacyKey, rules) {
    const key = buildInputPrivacyKey(privacyKey);
...
export async function updateIsOnline(isOnline) {
...
export async function fetchPeerColors(hash) {
...
export async function fetchTimezones(hash) {
    const result = await invokeRequest(new GramJs.help.GetTimezonesList({
        hash: hash ?? DEFAULT_PRIMITIVES.INT,
    }));
    if (!result || result instanceof GramJs.help.TimezonesListNotModified)
        return undefined;
...
export async function fetchCountryList({ langCode = 'en' }) {
...
export async function updateGlobalPrivacySettings({ shouldArchiveAndMuteNewNonContact, shouldHideReadMarks, shouldNewNonContactPeersRequirePremium, nonContactPeersPaidStars, shouldDisplayGiftsButton, disallowedGifts, }) {
...
export function toggleUsername({ chatId, accessHash, username, isActive, }) {
    if (chatId) {
