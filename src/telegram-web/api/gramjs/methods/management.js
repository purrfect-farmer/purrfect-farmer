...
export async function checkChatUsername({ username }) {
    try {
...
export async function deactivateAllUsernames({ chat }) {
...
export async function updatePrivateLink({ chat, usageLimit, expireDate, }) {
    const result = await invokeRequest(new GramJs.messages.ExportChatInvite({
        peer: buildInputPeer(chat.id, chat.accessHash),
        usageLimit,
        expireDate,
    }));
    if (!(result instanceof GramJs.ChatInviteExported))
        return undefined;
    sendApiUpdate({
...
export async function fetchExportedChatInvites({ peer, admin, limit = 0, isRevoked, }) {
    ...
    const invites = (exportedInvites.invites
        .filter((invite) => invite instanceof GramJs.ChatInviteExported))
        .map(buildApiExportedInvite);
    return {
        invites,
    };
}
export async function editExportedChatInvite({ peer, isRevoked, link, expireDate, usageLimit, isRequestNeeded, title, }) {
...
export async function exportChatInvite({ peer, expireDate, usageLimit, isRequestNeeded, title, }) {
    ...
    if (!(invite instanceof GramJs.ChatInviteExported))
        return undefined;
    return buildApiExportedInvite(invite);
}
export async function deleteExportedChatInvite({ peer, link, }) {
...
export async function deleteRevokedExportedChatInvites({ peer, admin, }) {
...
export async function fetchChatInviteImporters({ peer, link, offsetDate = 0, offsetUser, limit = 0, isRequested, }) {
...
export function hideChatJoinRequest({ peer, user, isApproved, }) {
...
export function hideAllChatJoinRequests({ peer, isApproved, link, }) {
...
export function hidePeerSettingsBar(peer) {
