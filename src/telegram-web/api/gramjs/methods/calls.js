...
import { buildApiGroupCall, buildApiGroupCallParticipant, buildCallProtocol, buildPhoneCall, } from '../apiBuilders/calls';
import { buildInputGroupCall, buildInputPeer, buildInputPhoneCall, buildInputUser, DEFAULT_PRIMITIVES, generateRandomInt, } from '../gramjsBuilders';
import { sendApiUpdate } from '../updates/apiUpdateEmitter';
import { invokeRequest, invokeRequestBeacon } from './client';
export async function getGroupCall({ call, }) {
...
export function discardGroupCall({ call, }) {
...
export function editGroupCallParticipant({ call, participant, muted, presentationPaused, videoStopped, videoPaused, volume, raiseHand, }) {
...
export function editGroupCallTitle({ groupCall, title, }) {
...
export async function exportGroupCallInvite({ call, canSelfUnmute, }) {
...
export async function fetchGroupCallParticipants({ call, offset = DEFAULT_PRIMITIVES.STRING, }) {
...
export function leaveGroupCall({ call, isPageUnload, }) {
...
export async function joinGroupCall({ call, inviteHash, params, }) {
...
export async function createGroupCall({ peer, }) {
...
export function joinGroupCallPresentation({ call, params, }) {
...
export function toggleGroupCallStartSubscription({ call, subscribed, }) {
...
export function leaveGroupCallPresentation({ call, }) {
...
export async function getDhConfig() {
    ...
    if (!dhConfig || dhConfig instanceof GramJs.messages.DhConfigNotModified)
        return undefined;
    return {
        g: dhConfig.g,
...
export function discardCall({ call, isBusy, isPageUnload, }) {
...
export async function requestCall({ user, gAHash, isVideo, }) {
...
export function setCallRating({ call, rating, comment, }) {
...
export function receivedCall({ call, }) {
    return invokeRequest(new GramJs.phone.ReceivedCall({
        peer: buildInputPhoneCall(call),
    }));
}
export async function acceptCall({ call, gB, }) {
...
export async function confirmCall({ call, gA, keyFingerprint, }) {
...
export function sendSignalingData({ data, call, }) {
