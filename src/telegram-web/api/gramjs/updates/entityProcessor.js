...
export function processAndUpdateEntities(response) {
    if (!response || typeof response !== 'object')
        return;
    if (!('users' in response || 'chats' in response || 'messages' in response))
        return;
    let userById;
    let chatById;
    const threadInfos = [];
    ...
    if (!userById && !chatById && !threadInfos?.length)
        return;
...
export function processMessageAndUpdateThreadInfo(message) {
    addMessageToLocalDb(message);
    const threadInfo = buildApiThreadInfoFromMessage(message);
    if (!threadInfo)
        return;
