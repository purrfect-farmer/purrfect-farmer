...
import { buildLocalUpdatePts } from './UpdatePts';
...
const SEQ_QUEUE = new SortedQueue(seqComparator);
const PTS_QUEUE = new Map();
export async function init(invokeReq) {
    invoke = invokeReq;
...
export function processUpdate(update, isFromDifference, shouldOnlySave) {
...
export function updateChannelState(channelId, pts) {
...
function savePtsUpdate(update, shouldOnlySave) {
    ...
    const ptsQueue = PTS_QUEUE.get(channelId) || new SortedQueue(ptsComparator);
    ptsQueue.add(update);
...
export function scheduleGetChannelDifference(channelId) {
    if (PTS_TIMEOUTS.has(channelId))
        return;
...
export function processAffectedHistory(chat, affected) {
