...
import { buildApiGroupCall, buildApiGroupCallParticipant, buildPhoneCall, getGroupCallId, } from '../apiBuilders/calls';
import { buildApiChatFolder, buildApiChatFromPreview, buildChatMember, buildChatMembers, buildChatTypingStatus, } from '../apiBuilders/chats';
import { buildApiPhoto, buildApiUsernames, buildPrivacyRules, } from '../apiBuilders/common';
import { omitVirtualClassFields } from '../apiBuilders/helpers';
import { buildApiMessageExtendedMediaPreview, buildBoughtMediaContent, buildPoll, buildPollFromMedia, buildPollResults, } from '../apiBuilders/messageContent';
import { buildApiMessage, buildApiMessageFromNotification, buildApiMessageFromShort, buildApiMessageFromShortChat, buildApiQuickReply, buildMessageDraft, } from '../apiBuilders/messages';
import { buildApiPeerNotifySettings, buildLangStrings, buildPrivacyKey, } from '../apiBuilders/misc';
...
import { buildApiPaidReactionPrivacy, buildApiReaction, buildMessageReactions, } from '../apiBuilders/reactions';
...
import { buildApiPeerSettings, buildApiUserStatus, } from '../apiBuilders/users';
import { buildChatPhotoForLocalDb, buildMessageFromUpdate, } from '../gramjsBuilders';
import { addPhotoToLocalDb, addStoryToLocalDb, } from '../helpers/localDb';
import { isChatFolder, log, resolveMessageApiChatId, serializeBytes, } from '../helpers/misc';
...
export function updater(update) {
    if (update instanceof UpdateServerTimeOffset) {
    ...
    else if (update instanceof GramJs.UpdateNewMessage
        ...
        if (update.message instanceof GramJs.MessageService) {
            ...
            if (action instanceof GramJs.MessageActionChatEditTitle) {
                ...
                if (update._entities && update._entities.some((e) => (e instanceof GramJs.User && Boolean(e.self) && e.id === action.userId))) {
                    sendApiUpdate({
                        '@type': 'updateChat',
                        id: message.chatId,
                        chat: {
                ...
                if (update._entities && update._entities.some((e) => (e instanceof GramJs.User && Boolean(e.self) && action.users.includes(e.id)))) {
                    sendApiUpdate({
                        '@type': 'updateChatJoin',
                        id: message.chatId,
                    });
                ...
                const replyTo = update.message.replyTo instanceof GramJs.MessageReplyHeader
                    ...
                    : undefined;
                const { replyToMsgId, replyToTopId, forumTopic: isTopicReply, } = replyTo || {};
    ...
    else if (update instanceof GramJs.UpdateEditMessage
        ...
        const previewMedia = !isBought ? update.extendedMedia
            .filter((m) => m instanceof GramJs.MessageExtendedMediaPreview)
        ...
        const ids = update.order?.filter((dp) => dp instanceof GramJs.DialogPeer)
    ...
    else if (update instanceof GramJs.UpdateSavedDialogPinned
        && update.peer instanceof GramJs.DialogPeer) {
        sendApiUpdate({
        ...
        const ids = update.order
            ? update.order
                .filter((dp) => dp instanceof GramJs.DialogPeer)
                .map((dp) => getApiChatIdFromMtpPeer(dp.peer))
            : [];
        sendApiUpdate({
        ...
        const addedMember = buildChatMember(pick(update, ['userId', 'inviterId', 'date']));
        sendApiUpdate({
            '@type': 'updateChatMembers',
    ...
    else if (update instanceof GramJs.UpdatePinnedMessages
    ...
    else if (update instanceof GramJs.UpdateUserTyping
        ...
        const channel = _entities.find((e) => (e instanceof GramJs.Channel || e instanceof GramJs.ChannelForbidden));
    ...
    else if (update instanceof GramJs.UpdateDialogUnreadMark
        && update.peer instanceof GramJs.DialogPeer) {
        sendApiUpdate({
            '@type': 'updateChat',
            id: getApiChatIdFromMtpPeer(update.peer.peer),
        ...
        const { notifySettings, peer: notifyPeer, } = update;
