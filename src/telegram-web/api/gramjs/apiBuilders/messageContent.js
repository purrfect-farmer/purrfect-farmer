...
import { addMediaToLocalDb, addStoryToLocalDb, } from '../helpers/localDb';
import { serializeBytes } from '../helpers/misc';
import { buildApiFormattedText, buildApiMessageEntity, buildApiPhoto, buildApiPhotoPreviewSizes, buildApiPhotoSize, buildApiThumbnailFromPath, buildApiThumbnailFromStripped, } from './common';
import { buildApiStarGift } from './gifts';
import { buildApiPeerId, getApiChatIdFromMtpPeer } from './peers';
import { buildStickerFromDocument, processStickerResult } from './symbols';
export function buildMessageContent(mtpMessage) {
    let content = {};
    if (mtpMessage.media) {
...
export function buildMessageTextContent(message, entities) {
    return {
        text: message,
        ...(entities && { entities: entities.map(buildApiMessageEntity) }),
    };
}
export function buildMessageMediaContent(media, context) {
...
export function buildVideoFromDocument(document, params) {
    if (document instanceof GramJs.DocumentEmpty) {
        return undefined;
    }
    const { isSpoiler, timestamp } = params || {};
    const { id, mimeType, thumbs, size, videoThumbs, attributes, } = document;
    if (mimeType === VIDEO_WEBM_TYPE && !self.isWebmSupported) {
        return undefined;
    }
    const videoAttr = attributes
        .find((a) => a instanceof GramJs.DocumentAttributeVideo);
    if (!videoAttr) {
        return undefined;
    }
    const gifAttr = attributes
        .find((a) => a instanceof GramJs.DocumentAttributeAnimated);
    ...
    const { duration, w: width, h: height, supportsStreaming = false, roundMessage: isRound = false, nosound, } = videoAttr;
...
export function buildAudioFromDocument(document) {
    if (document instanceof GramJs.DocumentEmpty) {
        return undefined;
    }
    const { id, mimeType, size, attributes, } = document;
    const audioAttributes = attributes
        .find((a) => a instanceof GramJs.DocumentAttributeAudio);
    if (!audioAttributes) {
        return undefined;
    }
    const { duration, title, performer, } = audioAttributes;
    return {
        mediaType: 'audio',
        id: String(id),
        mimeType,
...
function buildAltVideos(media) {
    if (!(media instanceof GramJs.MessageMediaDocument) || !media.altDocuments) {
        return undefined;
    }
    const altVideos = media.altDocuments.filter((d) => (d instanceof GramJs.Document && d.mimeType.startsWith('video'))).map((alt) => buildVideoFromDocument(alt, { isSpoiler: media.spoiler }))
        .filter(Boolean);
    if (!altVideos.length) {
        return undefined;
    }
...
export function buildApiDocument(document) {
    if (!(document instanceof GramJs.Document)) {
        return undefined;
    }
    const { id, size, mimeType, date, thumbs, attributes, } = document;
    const photoSize = thumbs && thumbs.find((s) => s instanceof GramJs.PhotoSize);
    let thumbnail = thumbs && buildApiThumbnailFromStripped(thumbs);
    if (!thumbnail && thumbs && photoSize) {
        const photoPath = thumbs.find((s) => s instanceof GramJs.PhotoPathSize);
        if (photoPath) {
    ...
    if (photoSize) {
        ...
        if (SUPPORTED_PHOTO_CONTENT_TYPES.has(mimeType)) {
            ...
            const imageAttribute = attributes
                .find((a) => a instanceof GramJs.DocumentAttributeImageSize);
            if (imageAttribute) {
...
function buildContact(media) {
    ...
    const { firstName, lastName, phoneNumber, userId, } = media;
    return {
        mediaType: 'contact',
        firstName,
        lastName,
...
export function buildGeoPoint(geo) {
    if (geo instanceof GramJs.GeoPointEmpty)
        return undefined;
...
function buildGame(media) {
    const { id, accessHash, shortName, title, description, photo: apiPhoto, document: apiDocument, } = media.game;
...
function buildGiveaway(media) {
    const { channels, months, stars, quantity, untilDate, countriesIso2, onlyNewSubscribers, prizeDescription, } = media;
...
function buildGiveawayResults(media) {
    const { months, untilDate, onlyNewSubscribers, launchMsgId, unclaimedCount, winners, winnersCount, additionalPeersCount, prizeDescription, refunded, channelId, } = media;
...
export function buildMessageStoryData(media) {
...
export function buildPoll(poll, pollResults) {
...
export function buildTodoItem(item) {
    return {
        id: item.id,
        title: buildApiFormattedText(item.title),
    };
}
export function buildTodo(todo, completions) {
...
export function buildMediaInvoice(media) {
    const { description, title, photo, test, totalAmount, currency, receiptMsgId, extendedMedia, } = media;
...
export function buildPollResults(pollResults) {
    const { results: rawResults, totalVoters, recentVoters, solution, solutionEntities: entities, min, } = pollResults;
    const results = rawResults?.map(({ option, chosen, correct, voters, }) => ({
        isChosen: chosen,
        isCorrect: correct,
...
export function buildWebPage(media) {
    if (!(media instanceof GramJs.MessageMediaWebPage)
        || !(media.webpage instanceof GramJs.WebPage)) {
        return undefined;
    }
    const { id, photo, document, attributes, } = media.webpage;
    let video;
    let audio;
    ...
    const attributeStory = attributes
        ?.find((a) => a instanceof GramJs.WebPageAttributeStory);
    const attributeGift = attributes
        ?.find((a) => a instanceof GramJs.WebPageAttributeUniqueStarGift);
    if (attributeStory) {
    ...
    const attributeStickers = attributes?.find((a) => (a instanceof GramJs.WebPageAttributeStickerSet));
...
function buildPaidMedia(media) {
    ...
    return {
        ...
        extendedMedia: extendedMedia
            .filter((paidMedia) => (paidMedia instanceof GramJs.MessageExtendedMediaPreview))
            .map((paidMedia) => buildApiMessageExtendedMediaPreview(paidMedia)),
    };
}
function getFilenameFromDocument(document, defaultBase = 'file') {
    const { mimeType, attributes } = document;
    const filenameAttribute = attributes
        .find((a) => a instanceof GramJs.DocumentAttributeFilename);
...
export function buildApiMessageExtendedMediaPreview(preview) {
    const { w, h, thumb, videoDuration, } = preview;
    return {
...
export function buildApiWebDocument(document) {
    if (!document)
        return undefined;
    const { url, size, mimeType, } = document;
    const accessHash = document instanceof GramJs.WebDocument ? document.accessHash.toString() : undefined;
    const sizeAttr = document.attributes.find((attr) => (attr instanceof GramJs.DocumentAttributeImageSize));
...
export function buildBoughtMediaContent(media) {
    const boughtMedia = media
        .filter((m) => m instanceof GramJs.MessageExtendedMedia)
