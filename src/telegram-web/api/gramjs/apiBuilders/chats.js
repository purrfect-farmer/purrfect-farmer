...
import { buildApiBotVerification, buildApiFormattedText, buildApiPhoto, buildApiUsernames, buildAvatarPhotoId, } from './common';
import { omitVirtualClassFields } from './helpers';
import { buildApiPeerNotifySettings } from './misc';
import { buildApiEmojiStatus, buildApiPeerColor, buildApiPeerId, getApiChatIdFromMtpPeer, isMtpPeerChat, isMtpPeerUser, } from './peers';
import { buildApiReaction } from './reactions';
function buildApiChatFieldsFromPeerEntity(peerEntity, isSupport = false) {
...
export function buildApiChatFromDialog(dialog, peerEntity) {
    const { peer, folderId, unreadMark, unreadCount, unreadMentionsCount, unreadReactionsCount, readOutboxMaxId, readInboxMaxId, draft, viewForumAsMessages, } = dialog;
    return {
...
export function buildApiChatFromSavedDialog(dialog, peerEntity) {
    const { peer } = dialog;
    return {
        id: getApiChatIdFromMtpPeer(peer),
        type: getApiChatTypeFromPeerEntity(peerEntity),
...
export function buildApiChatFromPreview(preview, isSupport = false) {
    if (preview instanceof GramJs.ChatEmpty || preview instanceof GramJs.UserEmpty) {
        return undefined;
    }
    const id = buildApiPeerId(preview.id, preview instanceof GramJs.User ? 'user'
...
export function getApiChatTypeFromPeerEntity(peerEntity) {
...
export function getApiChatTitleFromMtpPeer(peer, peerEntity) {
    if (isMtpPeerUser(peer)) {
...
export function buildChatMember(member) {
...
export function buildChatMembers(participants) {
...
export function buildChatTypingStatus(update) {
    let action = '';
    let emoticon;
...
export function buildApiChatFolder(filter) {
...
export function buildApiChatFolderFromSuggested({ filter, description, }) {
    if (!(filter instanceof GramJs.DialogFilter || filter instanceof GramJs.DialogFilterChatlist))
        return undefined;
    return {
...
export function buildApiChatBotCommands(botInfos) {
...
export function buildApiExportedInvite(invite) {
    const { revoked, date, expireDate, link, permanent, startDate, usage, usageLimit, requested, requestNeeded, title, adminId, } = invite;
    return {
        isRevoked: revoked,
        date,
        expireDate,
...
export function buildChatInviteImporter(importer) {
    const { userId, date, about, requested, viaChatlist, } = importer;
    return {
...
export function buildApiChatReactions(chatReactions) {
...
export function buildApiSendAsPeerId(sendAs) {
    return {
        id: getApiChatIdFromMtpPeer(sendAs.peer),
        isPremium: sendAs.premiumRequired,
    };
}
export function buildApiTopic(forumTopic) {
    if (forumTopic instanceof GramJs.ForumTopicDeleted) {
        return undefined;
    }
    const { id, my, closed, pinned, hidden, short, date, title, iconColor, iconEmojiId, topMessage, unreadCount, unreadMentionsCount, unreadReactionsCount, fromId, notifySettings, } = forumTopic;
    return {
        id,
        isClosed: closed,
        isPinned: pinned,
...
export function buildApiChatlistInvite(invite, slug) {
...
export function buildApiChatlistExportedInvite(invite) {
    if (!(invite instanceof GramJs.ExportedChatlistInvite))
        return undefined;
    const { title, url, peers, } = invite;
    return {
        title,
        url,
...
export function buildApiMissingInvitedUser(user) {
    return {
...
export function buildApiSponsoredMessageReportResult(result) {
...
export function buildApiChatInviteInfo(invite) {
    const { color, participants, participantsCount, photo, title, about, scam, fake, verified, megagroup, channel, broadcast, requestNeeded, subscriptionFormId, subscriptionPricing, canRefulfillSubscription, botVerification, } = invite;
    let apiPhoto;
...
export function buildApiStarsSubscriptionPricing(pricing) {
    return {
        period: pricing.period,
        amount: pricing.amount.toJSNumber(),
    };
}
export function buildApiSponsoredPeer(sponsoredPeer) {
    const { peer, randomId, additionalInfo, sponsorInfo, } = sponsoredPeer;
    return {
