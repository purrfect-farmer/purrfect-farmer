...
export function buildApiStarGift(starGift) {
    if (starGift instanceof GramJs.StarGiftUnique) {
        const { id, num, ownerId, ownerName, title, attributes, availabilityIssued, availabilityTotal, slug, ownerAddress, giftAddress, resellStars, } = starGift;
        return {
            type: 'starGiftUnique',
            id: id.toString(),
            number: num,
    ...
    const { id, limited, stars, availabilityRemains, availabilityTotal, convertStars, firstSaleDate, lastSaleDate, soldOut, birthday, upgradeStars, resellMinStars, title, availabilityResale, } = starGift;
...
export function buildApiStarGiftAttribute(attribute) {
    ...
    if (attribute instanceof GramJs.StarGiftAttributeBackdrop) {
        const { name, rarityPermille, centerColor, edgeColor, patternColor, textColor, backdropId, } = attribute;
        return {
            type: 'backdrop',
            backdropId,
            name,
    ...
    if (attribute instanceof GramJs.StarGiftAttributeOriginalDetails) {
        const { date, recipientId, message, senderId, } = attribute;
        return {
...
export function buildApiSavedStarGift(userStarGift, peerId) {
    const { gift, date, convertStars, fromId, message, msgId, nameHidden, unsaved, upgradeStars, transferStars, canUpgrade, savedId, canExportAt, pinnedToTop, canResellAt, canTransferAt, } = userStarGift;
    const inputGift = savedId && peerId
...
export function buildApiDisallowedGiftsSettings(result) {
    const { disallowUnlimitedStargifts, disallowLimitedStargifts, disallowUniqueStargifts, disallowPremiumGifts, } = result;
    return {
...
export function buildApiStarGiftAttributeId(result) {
...
export function buildApiStarGiftAttributeCounter(result) {
    const { count, } = result;
...
export function buildApiResaleGifts(result) {
    const { count, nextOffset, attributesHash, } = result;
...
export function buildInputResaleGiftsAttributes(attributes) {
