...
import { buildApiDocument, buildApiWebDocument, buildAudioFromDocument, buildGeoPoint, buildVideoFromDocument, } from './messageContent';
import { buildSvgPath } from './pathBytesToSvg';
import { buildApiPeerId } from './peers';
import { buildStickerFromDocument } from './symbols';
export function buildReplyButtons(replyMarkup, receiptMessageId) {
    ...
    if (markup.every((row) => !row.length))
        return undefined;
    return {
...
export function buildBotInlineMessage(sendMessage, type, document, photo) {
    const content = {};
...
export function buildApiBotInlineResult(result, queryId) {
    const { id, type, title, description, url, thumb, content, sendMessage, } = result;
    return {
        id,
        queryId,
        type: type,
...
export function buildApiBotInlineMediaResult(result, queryId) {
    const { id, type, title, description, sendMessage, photo, document, } = result;
    return {
        id,
        queryId,
        type: type,
...
export function buildBotSwitchPm(switchPm) {
    return switchPm ? pick(switchPm, ['text', 'startParam']) : undefined;
}
export function buildBotSwitchWebview(switchWebview) {
    return switchWebview ? pick(switchWebview, ['text', 'url']) : undefined;
}
export function buildApiAttachBot(bot) {
    return {
...
function buildApiAttachMenuPeerType(peerTypes) {
    return peerTypes.flatMap((peerType) => {
        if (peerType instanceof GramJs.AttachMenuPeerTypeBotPM)
            return ['bots'];
        if (peerType instanceof GramJs.AttachMenuPeerTypePM)
            return ['users'];
        if (peerType instanceof GramJs.AttachMenuPeerTypeChat)
            return ['chats', 'groups'];
        if (peerType instanceof GramJs.AttachMenuPeerTypeBroadcast)
            return ['channels'];
        if (peerType instanceof GramJs.AttachMenuPeerTypeSameBotPM)
            return ['self'];
        return [];
    });
}
function buildApiAttachMenuIcon(icon) {
    if (!(icon.icon instanceof GramJs.Document))
        return undefined;
...
export function buildApiBotInfo(botInfo, chatId) {
    const { description, descriptionPhoto, descriptionDocument, userId, commands, menuButton, privacyPolicyUrl, hasPreviewMedias, appSettings, } = botInfo;
...
export function buildBotAppSettings(settings) {
...
export function buildApiBotCommand(botId, command) {
    return {
        botId,
        ...omitVirtualClassFields(command),
    };
}
export function buildApiBotMenuButton(menuButton) {
...
export function buildApiBotApp(app) {
    if (app instanceof GramJs.BotAppNotModified)
        return undefined;
    const { id, accessHash, title, description, shortName, photo, document, } = app;
...
export function buildApiMessagesBotApp(botApp) {
...
export function buildApiInlineQueryPeerType(peerType) {
    if (peerType instanceof GramJs.InlineQueryPeerTypeBotPM)
        return 'bots';
    if (peerType instanceof GramJs.InlineQueryPeerTypePM)
        return 'users';
    if (peerType instanceof GramJs.InlineQueryPeerTypeChat)
        return 'chats';
    if (peerType instanceof GramJs.InlineQueryPeerTypeMegagroup)
        return 'supergroups';
    if (peerType instanceof GramJs.InlineQueryPeerTypeBroadcast)
        return 'channels';
    if (peerType instanceof GramJs.InlineQueryPeerTypeSameBotPM)
        return 'self';
