...
export function buildApiMessageAction(action) {
    ...
    if (action instanceof GramJs.MessageActionPaymentSent) {
        const { recurringInit, recurringUsed, currency, totalAmount, invoiceSlug, subscriptionUntilDate, } = action;
        return {
            mediaType: 'action',
            type: 'paymentSent',
    ...
    if (action instanceof GramJs.MessageActionPhoneCall) {
        const { video, callId, reason, duration, } = action;
        return {
            mediaType: 'action',
            type: 'phoneCall',
            isVideo: video,
    ...
    if (action instanceof GramJs.MessageActionBotAllowed) {
        const { attachMenu, fromRequest, domain, app, } = action;
        return {
            mediaType: 'action',
            type: 'botAllowed',
    ...
    if (action instanceof GramJs.MessageActionGiftPremium) {
        const { currency, amount, months, cryptoCurrency, cryptoAmount, message, } = action;
        return {
            mediaType: 'action',
            type: 'giftPremium',
            currency,
    ...
    if (action instanceof GramJs.MessageActionTopicEdit) {
        const { title, iconEmojiId, closed, hidden, } = action;
        return {
            mediaType: 'action',
            type: 'topicEdit',
            title,
    ...
    if (action instanceof GramJs.MessageActionGiftCode) {
        const { viaGiveaway, unclaimed, boostPeer, months, slug, currency, amount, cryptoCurrency, cryptoAmount, message, } = action;
        return {
            mediaType: 'action',
            type: 'giftCode',
    ...
    if (action instanceof GramJs.MessageActionPaymentRefunded) {
        const { peer, currency, totalAmount, } = action;
        return {
            mediaType: 'action',
    ...
    if (action instanceof GramJs.MessageActionGiftStars) {
        const { currency, amount, stars, cryptoCurrency, cryptoAmount, transactionId, } = action;
        return {
            mediaType: 'action',
            type: 'giftStars',
            currency,
    ...
    if (action instanceof GramJs.MessageActionPrizeStars) {
        const { unclaimed, stars, transactionId, boostPeer, giveawayMsgId, } = action;
        return {
            mediaType: 'action',
            type: 'prizeStars',
            isUnclaimed: unclaimed,
    ...
    if (action instanceof GramJs.MessageActionStarGiftUnique) {
        const { upgrade, transferred, saved, refunded, gift, canExportAt, transferStars, fromId, peer, savedId, resaleStars, } = action;
        const starGift = buildApiStarGift(gift);
        if (starGift.type !== 'starGiftUnique')
            return UNSUPPORTED_ACTION;
        return {
            mediaType: 'action',
            type: 'starGiftUnique',
    ...
    if (action instanceof GramJs.MessageActionPaidMessagesPrice) {
        const { stars, broadcastMessagesAllowed, } = action;
        return {
            mediaType: 'action',
    ...
    if (action instanceof GramJs.MessageActionTodoCompletions) {
        const { completed, incompleted, } = action;
        return {
            mediaType: 'action',
...
export function buildApiPhoneCallDiscardReason(reason) {
