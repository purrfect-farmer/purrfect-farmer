...
export function buildApiUserFullInfo(mtpUserFull) {
    const { fullUser: { about, commonChatsCount, pinnedMsgId, botInfo, blocked, profilePhoto, voiceMessagesForbidden, hasScheduled, fallbackPhoto, personalPhoto, translationsDisabled, storiesPinnedAvailable, contactRequirePremium, businessWorkHours, businessLocation, businessIntro, birthday, personalChannelId, personalChannelMessage, sponsoredEnabled, stargiftsCount, botVerification, botCanManageEmojiStatus, settings, sendPaidMessagesStars, displayGiftsButton, disallowedGifts, }, users, } = mtpUserFull;
...
export function buildApiPeerSettings({ autoarchived, reportSpam, addContact, blockContact, registrationMonth, phoneCountry, nameChangeDate, photoChangeDate, chargePaidMessageStars, }) {
    return {
...
export function buildApiUser(mtpUser) {
    if (!(mtpUser instanceof GramJs.User)) {
        return undefined;
    }
    const { id, firstName, lastName, fake, scam, support, closeFriend, storiesUnavailable, storiesMaxId, bot, botActiveUsers, botVerificationIcon, botInlinePlaceholder, botAttachMenu, botCanEdit, sendPaidMessagesStars, username, } = mtpUser;
...
export function buildApiUserStatus(mtpStatus) {
...
export function buildApiUserStatuses(mtpUsers) {
...
export function buildApiBirthday(birthday) {
