...
import { CONTENT_NOT_SUPPORTED, LOTTIE_STICKER_MIME_TYPE, RE_LINK_TEMPLATE, SERVICE_NOTIFICATIONS_USER_ID, SUPPORTED_AUDIO_CONTENT_TYPES, SUPPORTED_PHOTO_CONTENT_TYPES, SUPPORTED_VIDEO_CONTENT_TYPES, TME_LINK_PREFIX, VERIFICATION_CODES_USER_ID, VIDEO_STICKER_MIME_TYPE, } from '../../config';
...
export function getMessageHtmlId(messageId, index) {
    const parts = ['message', messageId.toString().replace('.', '-'), index].filter(Boolean);
    return parts.join('-');
}
export function getMessageOriginalId(message) {
    return message.previousLocalId || message.id;
}
export function getMessageTranscription(message) {
    const { transcriptionId } = message;
    const global = getGlobal();
    return transcriptionId && global.transcriptions[transcriptionId]?.text;
}
export function hasMessageText(message) {
    const { action, text, sticker, photo, video, audio, voice, document, pollId, todo, webPage, contact, invoice, location, game, storyData, giveaway, giveawayResults, paidMedia, } = message.content;
    return Boolean(text) || !(sticker || photo || video || audio || voice || document || contact || pollId || todo || webPage
        || invoice || location || game || storyData || giveaway || giveawayResults
        || paidMedia || action?.type === 'phoneCall');
}
export function getMessageStatefulContent(global, message) {
...
export function groupStatefulContent({ poll, story, }) {
    return {
        poll,
...
export function getMessageCustomShape(message) {
    const { text, sticker, photo, video, audio, voice, document, pollId, webPage, contact, action, game, invoice, location, storyData, } = message.content;
...
export function getMessageSingleRegularEmoji(message) {
...
export function getMessageSingleCustomEmoji(message) {
    const { text } = message.content;
...
export function getFirstLinkInMessage(message) {
    const { text } = message.content;
    let match;
    if (text?.entities) {
        const firstTextUrl = text.entities.find((entity) => (entity.type === ApiMessageEntityTypes.TextUrl));
        if (firstTextUrl) {
...
export function matchLinkInMessageText(message) {
    const { text } = message.content;
...
export function isReplyToMessage(message) {
...
export function getMessageSingleInlineButton(message) {
    return message.inlineButtons?.length === 1
        && message.inlineButtons[0].length === 1
        && message.inlineButtons[0][0];
}
export function orderHistoryIds(listedIds) {
    return listedIds.sort((a, b) => a - b);
}
export function orderPinnedIds(pinnedIds) {
    return pinnedIds.sort((a, b) => b - a);
}
export function mergeIdRanges(ranges, idsUpdate) {
...
export function extractMessageText(message, inChatList = false) {
...
export function isExpiredMessage(message) {
    return message.content.action?.type === 'expired';
}
export function hasMessageTtl(message) {
    return message.content?.ttlSeconds !== undefined;
}
export function getAttachmentMediaType(attachment) {
...
export function isUploadingFileSticker(attachment) {
...
export function getMessageLink(peer, topicId, messageId) {
...
export function splitMessagesForForwarding(messages, limit) {
    const result = [];
    let currentArr = [];
