...
export function getRequestInputInvoice(global, inputInvoice) {
    if (inputInvoice.type === 'slug')
        return inputInvoice;
    if (inputInvoice.type === 'stargiftResale') {
        const { slug, peerId, } = inputInvoice;
    ...
    if (inputInvoice.type === 'stargift') {
        const { peerId, shouldHideName, giftId, message, shouldUpgrade, } = inputInvoice;
        const peer = selectPeer(global, peerId);
        if (!peer)
            return undefined;
        return {
    ...
    if (inputInvoice.type === 'starsgift') {
        const { userId, stars, amount, currency, } = inputInvoice;
    ...
    if (inputInvoice.type === 'stars') {
        const { stars, amount, currency, } = inputInvoice;
        return {
            type: 'stars',
            purpose: {
                type: 'stars',
    ...
    if (inputInvoice.type === 'premiumGiftStars') {
        const { months, userId, message, } = inputInvoice;
        const user = selectUser(global, userId);
        if (!user)
            return undefined;
        return {
    ...
    if (inputInvoice.type === 'giftcode') {
        const { userIds, boostChannelId, amount, currency, option, message, } = inputInvoice;
    ...
    if (inputInvoice.type === 'starsgiveaway') {
        const { chatId, additionalChannelIds, amount, currency, untilDate, areWinnersVisible, countries, isOnlyForNewSubscribers, prizeDescription, stars, users, } = inputInvoice;
    ...
    if (inputInvoice.type === 'giveaway') {
        const { chatId, additionalChannelIds, amount, currency, option, untilDate, areWinnersVisible, countries, isOnlyForNewSubscribers, prizeDescription, } = inputInvoice;
        const chat = selectChat(global, chatId);
        if (!chat) {
            return undefined;
        }
...
export function getRequestInputSavedStarGift(global, inputGift) {
...
export function buildStarsTransactionCustomPeer(peer) {
...
export function formatStarsTransactionAmount(lang, starsAmount) {
...
export function formatStarsAmount(lang, starsAmount) {
    return lang.number(starsAmount.amount + starsAmount.nanos / 1e9);
}
export function getStarsTransactionFromGift(message) {
...
export function getPrizeStarsTransactionFromGiveaway(message) {
    const { action } = message.content;
    if (action?.type !== 'prizeStars')
        return undefined;
...
export function areInputSavedGiftsEqual(one, two) {
