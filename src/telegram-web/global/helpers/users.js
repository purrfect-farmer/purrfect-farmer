...
export function getUserFirstOrLastName(user) {
    if (!user) {
        return undefined;
    }
    switch (user.type) {
...
export function getUserStatus(lang, user, userStatus) {
...
export function isUserOnline(user, userStatus, withSelfOnline = false) {
    const { id, type } = user;
    if (!userStatus) {
        return false;
    }
...
export function isUserBot(user) {
    return user.type === 'userTypeBot';
}
export function getCanAddContact(user) {
    return !user.isSelf && !user.isContact && !isUserBot(user) && user.id !== ANONYMOUS_USER_ID;
}
export function sortUserIds(userIds, usersById, userStatusesById, priorityIds) {
...
export function getMainUsername(userOrChat) {
    return userOrChat.usernames?.find((u) => u.isActive)?.username;
}
export function getPeerStoryHtmlId(userId) {
