...
import { IS_OPFS_SUPPORTED, IS_OPUS_SUPPORTED, IS_PROGRESSIVE_SUPPORTED, IS_SAFARI, MAX_BUFFER_SIZE, } from '../../util/browser/windowEnvironment';
...
export function hasMessageMedia(message) {
    return Boolean((getMessagePhoto(message)
...
export function canEditMedia(message) {
    const { video, ...otherMedia } = message.content;
    return !video?.isRound && !Object.keys(otherMedia).length;
}
export function getMessagePhoto(message) {
    return message.content.photo;
}
export function getMessageActionPhoto(message) {
    return message.content.action?.type === 'suggestProfilePhoto' ? message.content.action.photo : undefined;
}
export function getMessageVideo(message) {
    return message.content.video;
}
export function getMessageRoundVideo(message) {
...
export function getMessageAudio(message) {
    return message.content.audio;
}
export function getMessageVoice(message) {
    return message.content.voice;
}
export function getMessageSticker(message) {
    return message.content.sticker;
}
export function getMessageDocument(message) {
    return message.content.document;
}
export function getMessageWebPageDocument(message) {
...
export function isMessageDocumentSticker(message) {
    const document = getMessageDocument(message);
    return document ? document.mimeType === 'image/webp' : undefined;
}
export function getMessageContact(message) {
    return message.content.contact;
}
export function getMessagePollId(message) {
    return message.content.pollId;
}
export function getMessageTodo(message) {
    return message.content.todo;
}
export function getMessageInvoice(message) {
    return message.content.invoice;
}
export function getMessageLocation(message) {
    return message.content.location;
}
export function getMessageWebPage(message) {
    return message.content.webPage;
}
export function getMessagePaidMedia(message) {
    return message.content.paidMedia;
}
export function getMessageWebPagePhoto(message) {
    return getMessageWebPage(message)?.photo;
}
export function getMessageDocumentPhoto(message) {
    const document = getMessageDocument(message);
    return document && isDocumentPhoto(document) ? document : undefined;
}
export function getMessageWebPageVideo(message) {
    return getMessageWebPage(message)?.video;
}
export function getMessageWebPageAudio(message) {
    return getMessageWebPage(message)?.audio;
}
export function getMessageDocumentVideo(message) {
    const document = getMessageDocument(message);
    return document && isDocumentVideo(document) ? document : undefined;
}
export function getMessageDownloadableMedia(message) {
...
export function getMessageMediaThumbDataUri(message) {
    return getMessageMediaThumbnail(message)?.dataUri;
}
export function getMediaThumbUri(media) {
    return media.thumbnail?.dataUri;
}
export function getMessageIsSpoiler(message) {
    const media = getMessagePhoto(message)
        || getMessageVideo(message);
    const invoiceMedia = getMessageInvoice(message)?.extendedMedia;
    return Boolean(invoiceMedia || media?.isSpoiler);
}
export function buildStaticMapHash(geo, width, height, zoom, scale) {
...
export function getMessageMediaHash(message, target) {
...
export function getPhotoMediaHash(photo, target, isAction) {
...
export function getProfilePhotoMediaHash(photo) {
    return `photo${photo.id}?size=c`;
}
export function getVideoProfilePhotoMediaHash(photo) {
    if (!photo.isVideo)
        return undefined;
    return `photo${photo.id}?size=u`;
}
export function getVideoMediaHash(video, target) {
...
export function getVideoPreviewMediaHash(video) {
    return video.hasVideoPreview ? `document${video.id}?size=v` : undefined;
}
export function getDocumentMediaHash(document, target) {
...
export function getAudioMediaHash(audio, target) {
...
export function getVoiceMediaHash(voice, target) {
...
export function getWebDocumentHash(webDocument) {
    if (!webDocument)
        return undefined;
    return `webDocument:${webDocument.url}`;
}
export function getStickerMediaHash(sticker, target) {
...
export function getMediaHash(media, target) {
...
export function getGamePreviewPhotoHash(game) {
    const { photo } = game;
    if (photo) {
...
export function getGamePreviewVideoHash(game) {
    const { document } = game;
    if (document) {
...
export function appendProgressiveQueryParameters(media, base) {
...
export function getAudioHasCover(media) {
    return media.thumbnailSizes && media.thumbnailSizes.length > 0;
}
export function getMediaFormat(media, target) {
...
export function getMediaFileSize(media) {
    return 'size' in media ? media.size : undefined;
}
export function hasMediaLocalBlobUrl(media) {
    if ('blobUrl' in media) {
...
export function getMediaTransferState(progress, isLoadNeeded = false, isUploading = false) {
...
export function getMessageContentIds(messages, messageIds, contentType) {
    let validator;
    switch (contentType) {
        case 'media':
            validator = (message) => {
        ...
        case 'documents':
            ...
            break;
        case 'links':
            validator = (message) => getMessageWebPage(message) || matchLinkInMessageText(message);
            break;
        case 'audio':
...
export function getMediaDuration(message) {
...
export function canReplaceMessageMedia(message, attachment) {
    ...
    return ((isPhotoOrVideo && (fileType === 'photo' || fileType === 'video'))
        || (isFile && (fileType === 'audio' || fileType === 'file')));
}
export function isMediaLoadableInViewer(newMessage) {
    if (!newMessage.content)
        return false;
    if (newMessage.content.photo)
        return true;
    if (newMessage.content.video && !newMessage.content.video.isRound && !newMessage.content.video.isGif)
        return true;
    return false;
}
...
export function getIsDownloading(activeDownloads, media) {
    const hash = getMediaHash(media, 'download');
    if (!hash)
        return false;
    return Boolean(activeDownloads[hash]);
}
export function getTimestampableMedia(message) {
