...
initCache();
addActionHandler('initShared', async (prevGlobal, actions, payload) => {
    const { force } = payload || {};
    await initGlobal(force, prevGlobal);
});
addActionHandler('init', (global, actions, payload) => {
    ...
    Object.keys(global.messages.byChatId).forEach((chatId) => {
        ...
        Object.keys(threadsById).forEach((thread) => {
            ...
            global = replaceTabThreadParam(global, chatId, threadId, 'viewportIds', lastViewportIds, tabId);
        });
    });
    // Temporary state fix
    Object.keys(global.messages.byChatId).forEach((chatId) => {
...
addActionHandler('requestMasterAndCallAction', async (global, actions, payload) => {
...
addActionHandler('clearMultitabNextAction', (global, actions, payload) => {
    const { tabId = getCurrentTabId() } = payload || {};
    return updateTabState(global, {
