...
import { ALL_FOLDER_ID, ARCHIVED_FOLDER_ID, DEBUG, DEFAULT_LIMITS, GLOBAL_STATE_CACHE_ARCHIVED_CHAT_LIST_LIMIT, GLOBAL_STATE_CACHE_CHAT_LIST_LIMIT, GLOBAL_STATE_CACHE_CUSTOM_EMOJI_LIMIT, GLOBAL_STATE_CACHE_DISABLED, GLOBAL_STATE_CACHE_USER_LIST_LIMIT, IS_SCREEN_LOCKED_CACHE_KEY, SAVED_FOLDER_ID, SHARED_STATE_CACHE_KEY, } from '../config';
...
import { selectChatLastMessageId, selectChatMessages, selectCurrentMessageList, selectTopics, selectViewportIds, selectVisibleUsers, } from './selectors';
...
export async function loadCache(initialState) {
    if (GLOBAL_STATE_CACHE_DISABLED) {
        return undefined;
    }
...
export function migrateCache(cached, initialState) {
    try {
...
function unsafeMigrateCache(cached, initialState) {
    ...
    if (cached.cacheVersion < 2) {
        if (untypedCached.settings.themes.dark) {
            untypedCached.settings.themes.dark.patternColor = initialState.settings.themes.dark.patternColor;
        }
        if (untypedCached.settings.themes.light) {
            untypedCached.settings.themes.light.patternColor = initialState.settings.themes.light.patternColor;
        }
...
function reduceSharedState(sharedState) {
    return {
        ...sharedState,
        settings: {
...
export function serializeGlobal(global) {
...
function reduceUsers(global) {
    const { users: { byId, statusesById, fullInfoById, botAppPermissionsById, }, currentUserId, } = global;
    const currentChatIds = compact(Object.values(global.byTabId)
        .map(({ id: tabId }) => selectCurrentMessageList(global, tabId))).map(({ chatId }) => chatId).filter((chatId) => isUserId(chatId));
    ...
    const chatStoriesUserIds = currentChatIds
        ...
        .filter((id) => Boolean(id) && isUserId(id));
...
function reduceMessages(global) {
    ...
    const byChatId = {};
    const currentChatIds = compact(Object.values(global.byTabId)
        .map(({ id: tabId }) => selectCurrentMessageList(global, tabId))).map(({ chatId }) => chatId);
    const forumPanelChatIds = compact(Object.values(global.byTabId)
...
function reduceSettings(global) {
    const { byKey, botVerificationShownPeerIds, notifyDefaults, lastPremiumBandwithNotificationDate, themes, accountDaysTtl, } = global.settings;
    return {
        byKey,
        privacy: {},
