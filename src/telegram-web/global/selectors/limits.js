...
export function selectCurrentLimit(global, limit) {
    ...
    if (limit === 'dialogFilters')
        return value + 1; // Server does not count "All" as folder, but we need to
    return value;
}
export function selectPremiumLimit(global, limit) {
    const { appConfig } = global;
    if (!appConfig)
        return DEFAULT_LIMITS[limit][1];
    const { limits } = appConfig;
