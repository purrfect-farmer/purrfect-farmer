...
import { addStories, addStoriesForPeer, removePeerStory, updateLastReadStoryForPeer, updateLastViewedStoryForPeer, updatePeer, updatePeerProfileStory, updatePeerStoriesFullyLoaded, updatePeerStoriesHidden, updatePeerStory, updatePeerStoryViews, updatePeersWithStories, updateSentStoryReaction, updateStealthMode, updateStoryViews, updateStoryViewsLoading, } from '../../reducers';
import { updateTabState } from '../../reducers/tabs';
import { selectIsCurrentUserFrozen, selectPeer, selectPeerStories, selectPeerStory, selectPinnedStories, selectTabState, } from '../../selectors';
const INFINITE_LOOP_MARKER = 100;
addActionHandler('loadAllStories', async (global) => {
    let i = 0;
...
addActionHandler('loadAllHiddenStories', async (global) => {
    let i = 0;
...
addActionHandler('loadPeerSkippedStories', async (global, actions, payload) => {
...
addActionHandler('viewStory', async (global, actions, payload) => {
...
addActionHandler('deleteStory', async (global, actions, payload) => {
...
addActionHandler('toggleStoryInProfile', async (global, actions, payload) => {
...
addActionHandler('toggleStoryPinnedToTop', async (global, actions, payload) => {
    const { peerId, storyId } = payload;
    const peer = selectPeer(global, peerId);
    const peerStories = selectPeerStories(global, peerId);
    if (!peer || !peerStories) {
...
addActionHandler('loadPeerStories', async (global, actions, payload) => {
...
addActionHandler('loadPeerProfileStories', async (global, actions, payload) => {
    if (selectIsCurrentUserFrozen(global))
        return;
...
addActionHandler('loadStoriesArchive', async (global, actions, payload) => {
    if (selectIsCurrentUserFrozen(global))
        return;
    const { peerId, offsetId } = payload;
    const peer = selectPeer(global, peerId);
    let peerStories = selectPeerStories(global, peerId);
    if (!peer || peerStories?.isArchiveFullyLoaded)
        return;
...
addActionHandler('loadPeerStoriesByIds', async (global, actions, payload) => {
...
addActionHandler('loadStoryViews', async (global, actions, payload) => {
    const { peerId, storyId } = payload;
    const peer = selectPeer(global, peerId);
    if (!peer) {
        return;
...
addActionHandler('loadStoryViewList', async (global, actions, payload) => {
    const { peerId, storyId, offset, areReactionsFirst, areJustContacts, query, limit, tabId = getCurrentTabId(), } = payload;
    const peer = selectPeer(global, peerId);
    if (!peer) {
        return;
    }
...
addActionHandler('reportStory', async (global, actions, payload) => {
    const { peerId, storyId, description = '', option = '', tabId = getCurrentTabId(), } = payload;
    const peer = selectPeer(global, peerId);
    if (!peer) {
        return;
    }
...
addActionHandler('editStoryPrivacy', (global, actions, payload) => {
    const { peerId, storyId, privacy, } = payload;
    const peer = selectPeer(global, peerId);
    if (!peer) {
        return;
    }
...
addActionHandler('toggleStoriesHidden', async (global, actions, payload) => {
...
addActionHandler('loadStoriesMaxIds', async (global, actions, payload) => {
...
addActionHandler('sendStoryReaction', async (global, actions, payload) => {
    const { peerId, storyId, containerId, reaction, shouldAddToRecent, tabId = getCurrentTabId(), } = payload;
    const peer = selectPeer(global, peerId);
    if (!peer)
        return;
    const story = selectPeerStory(global, peerId, storyId);
...
addActionHandler('activateStealthMode', (global, actions, payload) => {
