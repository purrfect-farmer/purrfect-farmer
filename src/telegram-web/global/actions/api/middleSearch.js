...
import { CHAT_MEDIA_SLICE, MESSAGE_SEARCH_SLICE, SHARED_MEDIA_SLICE, } from '../../../config';
...
import { addChatMessagesById, addMessages, addUserStatuses, initializeChatMediaSearchResults, mergeWithChatMediaSearchSegment, setChatMediaSearchLoading, updateChatMediaSearchResults, updateMiddleSearch, updateMiddleSearchResults, updateSharedMediaSearchResults, } from '../../reducers';
import { selectChat, selectCurrentChatMediaSearch, selectCurrentMessageList, selectCurrentMiddleSearch, selectCurrentSharedMediaSearch, selectPeer, } from '../../selectors';
const MEDIA_PRELOAD_OFFSET = 9;
addActionHandler('performMiddleSearch', async (global, actions, payload) => {
    const { query, chatId, threadId = MAIN_THREAD_ID, tabId = getCurrentTabId(), } = payload || {};
    if (!chatId)
        return;
    ...
    const { results, savedTag, type, isHashtag, } = currentSearch;
    ...
    const { userStatusesById, messages, totalCount, nextOffsetId, nextOffsetRate, nextOffsetPeerId, } = result;
...
addActionHandler('searchHashtag', (global, actions, payload) => {
...
addActionHandler('searchSharedMediaMessages', (global, actions, payload) => {
...
addActionHandler('searchChatMediaMessages', (global, actions, payload) => {
    const { chatId, threadId, currentMediaMessageId, limit, direction, tabId = getCurrentTabId(), } = payload;
...
addActionHandler('searchMessagesByDate', async (global, actions, payload) => {
...
async function searchSharedMedia(global, peer, threadId, type, offsetId, isBudgetPreload = false, isSavedDialog, ...[tabId = getCurrentTabId()]) {
    ...
    const { userStatusesById, messages, totalCount, nextOffsetId, } = result;
    ...
    global = updateSharedMediaSearchResults(global, resultChatId, threadId, type, newFoundIds, totalCount, nextOffsetId, tabId);
    setGlobal(global);
    if (!isBudgetPreload) {
...
function selectCurrentChatMediaSearchSegment(params, currentMediaMessageId) {
    ...
    const index = params.segments.findIndex((segment) => isInsideSortedArrayRange(currentMediaMessageId, segment.foundIds));
    if (index === -1) {
...
function calcChatMediaSearchOffsetId(direction, currentMessageId, segment) {
    ...
    if (direction === LoadMoreDirection.Backwards)
        return segment.foundIds[0];
    if (direction === LoadMoreDirection.Forwards)
        return segment.foundIds[segment.foundIds.length - 1];
    return currentMessageId;
}
function calcLoadMoreDirection(currentMessageId, currentSegment) {
    if (!currentSegment)
        return LoadMoreDirection.Around;
    ...
    if (idIndexInSegment >= lastMediaIndex - preloadOffset) {
        if (currentSegment.loadingState.areAllItemsLoadedForwards)
            return undefined;
...
async function searchChatMedia(global, peer, threadId, currentMediaMessageId, chatMediaSearchParams, direction, isSavedDialog, limit = CHAT_MEDIA_SLICE, ...[tabId = getCurrentTabId()]) {
    ...
    const { messages, userStatusesById, } = result;
    const byId = buildCollectionByKey(messages, 'id');
    const newFoundIds = Object.keys(byId).map(Number);
    global = addUserStatuses(global, userStatusesById);
    global = addChatMessagesById(global, resultChatId, byId);
    ...
    currentSegment = mergeWithChatMediaSearchSegment(filteredIds, loadingState, currentSegment);
    global = updateChatMediaSearchResults(global, resultChatId, threadId, currentSegment, chatMediaSearchParams, tabId);
    global = setChatMediaSearchLoading(global, resultChatId, threadId, false, tabId);
    setGlobal(global);
}
