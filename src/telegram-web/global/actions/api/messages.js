...
import { GIF_MIME_TYPE, MAX_MEDIA_FILES_FOR_ALBUM, MESSAGE_ID_REQUIRED_ERROR, MESSAGE_LIST_SLICE, RE_TELEGRAM_LINK, SERVICE_NOTIFICATIONS_USER_ID, SUPPORTED_AUDIO_CONTENT_TYPES, SUPPORTED_PHOTO_CONTENT_TYPES, SUPPORTED_VIDEO_CONTENT_TYPES, } from '../../../config';
import { ensureProtocol, isMixedScriptUrl } from '../../../util/browser/url';
import { IS_IOS } from '../../../util/browser/windowEnvironment';
import { copyTextToClipboardFromPromise } from '../../../util/clipboard';
import { isDeepLink } from '../../../util/deepLinkParser';
import { getCurrentTabId } from '../../../util/establishMultitabRole';
import { areSortedArraysIntersecting, buildCollectionByKey, omit, partition, split, unique, } from '../../../util/iteratees';
...
addActionHandler('loadViewportMessages', (global, actions, payload) => {
    const { direction = LoadMoreDirection.Around, isBudgetPreload = false, shouldForceRender = false, onLoaded, onError, tabId = getCurrentTabId(), } = payload || {};
    ...
    if (!viewportIds || !viewportIds.length || direction === LoadMoreDirection.Around) {
        ...
        if (!areAllLocal) {
            onTickEnd(() => {
                void loadViewportMessages(global, chat, threadId, offsetId, LoadMoreDirection.Around, isOutlying, isBudgetPreload, onLoaded, tabId);
            });
        }
        else {
            onLoaded?.();
        ...
        if (historyIds?.length) {
            const { newViewportIds, areSomeLocal, areAllLocal, } = getViewportSlice(historyIds, offsetId, direction);
            if (areSomeLocal) {
                global = safeReplaceViewportIds(global, chatId, threadId, newViewportIds, tabId);
            }
            onTickEnd(() => {
...
async function loadWithBudget(global, actions, areAllLocal, isOutlying, isBudgetPreload, chat, threadId, direction, offsetId, onLoaded, ...[tabId = getCurrentTabId()]) {
    if (!areAllLocal) {
...
addActionHandler('loadMessage', async (global, actions, payload) => {
    ...
    if (message && threadUpdate) {
        ...
        global = getGlobal();
        global = updateThreadUnreadFromForwardedMessage(global, message, chatId, lastMessageId, isDeleting);
        setGlobal(global);
    }
});
addActionHandler('sendMessage', async (global, actions, payload) => {
    ...
    if (isSingle) {
        ...
        const { text, entities, attachments, replyInfo: replyToForFirstMessage, ...commonParams } = params;
        if (text) {
            const sendParams = {
                ...commonParams,
                text,
    ...
    if (localMessages?.length)
        sendMessagesWithNotification(global, localMessages);
});
addActionHandler('sendInviteMessages', async (global, actions, payload) => {
...
addActionHandler('editMessage', (global, actions, payload) => {
    const { messageList, text, entities, attachments, tabId = getCurrentTabId(), } = payload;
    if (!messageList) {
        return;
    }
...
addActionHandler('editTodo', (global, actions, payload) => {
    const { chatId, todo, messageId, } = payload;
    const chat = selectChat(global, chatId);
...
addActionHandler('cancelUploadMedia', (global, actions, payload) => {
...
addActionHandler('clearDraft', (global, actions, payload) => {
    const { chatId, threadId = MAIN_THREAD_ID, isLocalOnly, shouldKeepReply, } = payload;
    const currentDraft = selectDraft(global, chatId, threadId);
    if (!currentDraft) {
        return;
    }
...
addActionHandler('updateDraftReplyInfo', (global, actions, payload) => {
    ...
    if (!updatedReplyInfo.replyToMsgId)
        return;
    const newDraft = {
        ...currentDraft,
...
addActionHandler('resetDraftReplyInfo', (global, actions, payload) => {
...
addActionHandler('saveEffectInDraft', (global, actions, payload) => {
    const { chatId, threadId, effectId, } = payload;
    const chat = selectChat(global, chatId);
    const currentDraft = selectDraft(global, chatId, threadId);
    if (chat?.isMonoforum && !currentDraft?.replyInfo) {
        return; // Monoforum doesn't support drafts outside threads
...
addActionHandler('updateInsertingPeerIdMention', (global, actions, payload) => {
    const { peerId, tabId = getCurrentTabId() } = payload || {};
    return updateTabState(global, {
        insertingPeerIdMention: peerId,
    }, tabId);
});
async function saveDraft({ global, chatId, threadId, draft, isLocalOnly, noLocalTimeUpdate, }) {
    const chat = selectChat(global, chatId);
    const user = selectUser(global, chatId);
    if (!chat || (user && isDeletedUser(user)))
        return;
...
addActionHandler('unpinAllMessages', async (global, actions, payload) => {
...
addActionHandler('deleteMessages', (global, actions, payload) => {
    const { messageIds, shouldDeleteForAll, messageList: payloadMessageList, tabId = getCurrentTabId(), } = payload;
    const currentMessageList = selectCurrentMessageList(global, tabId);
...
addActionHandler('resetLocalPaidMessages', (global, actions, payload) => {
    const { tabId = getCurrentTabId() } = payload || {};
...
addActionHandler('deleteParticipantHistory', (global, actions, payload) => {
...
addActionHandler('deleteSavedHistory', async (global, actions, payload) => {
...
addActionHandler('reportMessages', async (global, actions, payload) => {
    const { messageIds, description = '', option = '', chatId, tabId = getCurrentTabId(), } = payload;
    const chat = selectChat(global, chatId);
...
addActionHandler('sendMessageAction', async (global, actions, payload) => {
    ...
    if (selectIsChatWithSelf(global, chatId))
        return;
    const chat = selectChat(global, chatId);
    if (!chat || chat.isMonoforum)
        return;
    const user = selectUser(global, chatId);
    if (user && (isUserBot(user) || isDeletedUser(user)))
        return;
    await callApi('sendMessageAction', {
        peer: chat, threadId, action,
    });
});
addActionHandler('reportChannelSpam', (global, actions, payload) => {
...
addActionHandler('loadWebPagePreview', async (global, actions, payload) => {
...
addActionHandler('clearWebPagePreview', (global, actions, payload) => {
    const { tabId = getCurrentTabId() } = payload || {};
...
addActionHandler('toggleTodoCompleted', (global, actions, payload) => {
...
addActionHandler('appendTodoList', (global, actions, payload) => {
    const { chatId, items, messageId, } = payload;
    const chat = selectChat(global, chatId);
    const message = selectChatMessage(global, chatId, messageId);
    if (!chat || !message) {
        return;
...
addActionHandler('loadPollOptionResults', async (global, actions, payload) => {
    const { chat, messageId, option, offset, limit, shouldResetVoters, tabId = getCurrentTabId(), } = payload;
...
addActionHandler('loadScheduledHistory', async (global, actions, payload) => {
    if (selectIsCurrentUserFrozen(global))
        return;
...
addActionHandler('rescheduleMessage', (global, actions, payload) => {
    const { chatId, messageId, scheduledAt, } = payload;
    const chat = selectChat(global, chatId);
...
addActionHandler('forwardMessages', (global, actions, payload) => {
...
async function executeForwardMessages(global, sendParams, tabId) {
    const { fromChatId, messageIds, toChatId, withMyScore, noAuthors, noCaptions, toThreadId = MAIN_THREAD_ID, } = selectTabState(global, tabId).forwardMessages;
...
async function loadViewportMessages(global, chat, threadId, offsetId, direction, isOutlying = false, isBudgetPreload = false, onLoaded, ...[tabId = getCurrentTabId()]) {
...
function findClosestIndex(sourceIds, offsetId) {
    ...
    return sourceIds.findIndex((id, i) => (id === offsetId
...
export async function getPeerStarsForMessage(global, peerId) {
    const peer = selectPeer(global, peerId);
    if (!peer)
        return undefined;
...
addActionHandler('sendMessages', async (global, actions, payload) => {
    ...
    if (sendParams.length > 0 && sendParams[0].messagePriceInStars)
        actions.loadStarStatus();
});
addActionHandler('loadPinnedMessages', async (global, actions, payload) => {
    const { chatId, threadId } = payload;
    const chat = selectChat(global, chatId);
    if (!chat || getIsSavedDialog(chatId, threadId, global.currentUserId)) {
        return;
...
addActionHandler('loadSendPaidReactionsAs', async (global, actions, payload) => {
    const { chatId } = payload;
    const chat = selectChat(global, chatId);
    if (!chat) {
        return;
...
addActionHandler('viewSponsored', (global, actions, payload) => {
    const { randomId } = payload;
    void callApi('viewSponsoredMessage', { random: randomId });
});
addActionHandler('clickSponsored', (global, actions, payload) => {
    const { randomId, isMedia, isFullscreen } = payload;
    void callApi('clickSponsoredMessage', {
        random: randomId, isMedia, isFullscreen,
    });
});
addActionHandler('reportSponsored', async (global, actions, payload) => {
    const { peerId, randomId, option = '', tabId = getCurrentTabId(), } = payload;
...
addActionHandler('hideSponsored', async (global, actions, payload) => {
    const { tabId = getCurrentTabId() } = payload || {};
    const isCurrentUserPremium = selectIsCurrentUserPremium(global);
...
addActionHandler('focusNextMention', async (global, actions, payload) => {
    const { tabId = getCurrentTabId() } = payload || {};
    ...
    if (!chat.unreadMentions) {
        ...
        if (!chat?.unreadMentions || previousChatId !== chat.id)
            return;
    }
    actions.focusMessage({ chatId: chat.id, messageId: chat.unreadMentions[0], tabId });
});
addActionHandler('readAllMentions', (global, actions, payload) => {
...
addActionHandler('openUrl', (global, actions, payload) => {
    const { url, shouldSkipModal, ignoreDeepLinks, tabId = getCurrentTabId(), } = payload;
...
addActionHandler('openChatOrTopicWithReplyInDraft', (global, actions, payload) => {
    ...
    if (!currentReplyInfo.replyToPeerId && toChatId === currentChat.id)
        return;
    const getPeerId = () => {
        if (!currentReplyInfo?.replyToPeerId)
...
addActionHandler('forwardToSavedMessages', (global, actions, payload) => {
    const { tabId = getCurrentTabId() } = payload || {};
    global = updateTabState(global, {
        forwardMessages: {
            ...selectTabState(global, tabId).forwardMessages,
...
addActionHandler('forwardStory', (global, actions, payload) => {
    ...
    const { text, entities } = story.content.text || {};
...
addActionHandler('requestMessageTranslation', (global, actions, payload) => {
    const { chatId, id, toLanguageCode = selectTranslationLanguage(global), tabId = getCurrentTabId(), } = payload;
...
addActionHandler('showOriginalMessage', (global, actions, payload) => {
    const { chatId, id, tabId = getCurrentTabId(), } = payload;
    global = removeRequestedMessageTranslation(global, chatId, id, tabId);
    return global;
});
addActionHandler('markMessagesTranslationPending', (global, actions, payload) => {
    const { chatId, messageIds, toLanguageCode = selectLanguageCode(global), } = payload;
...
addActionHandler('scheduleForViewsIncrement', (global, actions, payload) => {
...
addActionHandler('loadMessageViews', async (global, actions, payload) => {
...
addActionHandler('loadFactChecks', async (global, actions, payload) => {
    const { chatId, ids } = payload;
    const chat = selectChat(global, chatId);
    if (!chat)
        return;
...
addActionHandler('loadPaidReactionPrivacy', () => {
    callApi('fetchPaidReactionPrivacy');
    return undefined;
});
addActionHandler('loadOutboxReadDate', async (global, actions, payload) => {
    const { chatId, messageId } = payload;
    const chat = selectChat(global, chatId);
    if (!chat)
        return;
...
addActionHandler('loadQuickReplies', async (global) => {
...
addActionHandler('sendQuickReply', (global, actions, payload) => {
...
addActionHandler('copyMessageLink', async (global, actions, payload) => {
    const { chatId, messageId, shouldIncludeThread, shouldIncludeGrouped, tabId = getCurrentTabId(), } = payload;
    const chat = selectChat(global, chatId);
    if (!chat) {
        actions.showNotification({
    ...
    await copyTextToClipboardFromPromise(callApiExportMessageLinkPromise, showLinkCopiedNotification, showErrorOccurredNotification);
});
const MESSAGES_TO_REPORT_DELIVERY = new Map();
let reportDeliveryTimeout;
addActionHandler('reportMessageDelivery', (global, actions, payload) => {
    const { chatId, messageId } = payload;
...
addActionHandler('openPreparedInlineMessageModal', async (global, actions, payload) => {
...
addActionHandler('openSharePreparedMessageModal', (global, actions, payload) => {
...
function getAttachmentType(attachment) {
    const { shouldSendAsFile, mimeType, } = attachment;
    ...
    if (mimeType === GIF_MIME_TYPE)
        return 'gif';
    if (SUPPORTED_PHOTO_CONTENT_TYPES.has(mimeType) || SUPPORTED_VIDEO_CONTENT_TYPES.has(mimeType))
        return 'media';
    if (attachment.voice)
        return 'voice';
    return 'file';
