...
import { CUSTOM_BG_CACHE_NAME, LANG_CACHE_NAME, LOCK_SCREEN_ANIMATION_DURATION_MS, MEDIA_CACHE_NAME, MEDIA_CACHE_NAME_AVATARS, MEDIA_PROGRESSIVE_CACHE_NAME, } from '../../../config';
import { updateAppBadge } from '../../../util/appBadge';
import { PASSCODE_IDB_STORE } from '../../../util/browser/idb';
import { IS_WEBM_SUPPORTED, MAX_BUFFER_SIZE, PLATFORM_ENV, } from '../../../util/browser/windowEnvironment';
...
import { callApi, callApiLocal, initApi, setShouldEnableDebugLog, } from '../../../api/gramjs';
import { removeGlobalFromCache, removeSharedStateFromCache, serializeGlobal } from '../../cache';
import { addActionHandler, getGlobal, setGlobal, } from '../../index';
import { clearGlobalForLockScreen, updateManagementProgress, updatePasscodeSettings, } from '../../reducers';
...
addActionHandler('initApi', (global, actions) => {
    ...
    const { shouldAllowHttpTransport, shouldForceHttpTransport, shouldDebugExportedSenders, shouldCollectDebugLogs, language, } = selectSharedSettings(global);
...
addActionHandler('uploadProfilePhoto', async (global, actions, payload) => {
    const { file, isFallback, isVideo, videoTs, bot, tabId = getCurrentTabId(), } = payload;
...
addActionHandler('requestChannelDifference', (global, actions, payload) => {
    const { chatId } = payload;
    void callApi('requestChannelDifference', chatId);
});
addActionHandler('reset', (global, actions) => {
...
addActionHandler('destroyConnection', () => {
...
addActionHandler('setDeviceToken', (global, actions, payload) => {
    const { token } = payload;
    return {
        ...global,
        push: {
