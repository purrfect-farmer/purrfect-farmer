...
import { ChatCreationProgress, ManagementProgress, NewChatMembersProgress, SettingsScreens, } from '../../../types';
import { ALL_FOLDER_ID, ARCHIVED_FOLDER_ID, CHAT_LIST_LOAD_SLICE, DEBUG, GLOBAL_SUGGESTED_CHANNELS_ID, MAX_INT_32, RE_TG_LINK, SAVED_FOLDER_ID, SERVICE_NOTIFICATIONS_USER_ID, TME_WEB_DOMAINS, TMP_CHAT_ID, TOP_CHAT_MESSAGES_PRELOAD_LIMIT, TOPICS_SLICE, TOPICS_SLICE_SECOND_LOAD, } from '../../../config';
...
import { buildCollectionByKey, omit, pick, unique, } from '../../../util/iteratees';
import { isLocalMessageId } from '../../../util/keys/messageKey';
import * as langProvider from '../../../util/oldLangProvider';
import { debounce, pause, throttle } from '../../../util/schedulers';
import { extractCurrentThemeParams } from '../../../util/themeStyle';
import { callApi } from '../../../api/gramjs';
import { getIsSavedDialog, isChatArchived, isChatBasicGroup, isChatChannel, isChatMonoforum, isChatSuperGroup, isUserBot, } from '../../helpers';
import { addActionHandler, getGlobal, setGlobal, } from '../../index';
import { addChatListIds, addChatMembers, addChats, addMessages, addNotifyExceptions, addSimilarBots, addUsers, addUserStatuses, deleteChatMessages, deletePeerPhoto, deleteTopic, leaveChat, removeChatFromChatLists, replaceChatFullInfo, replaceChatListIds, replaceChatListLoadingParameters, replaceMessages, replaceNotifyExceptions, replaceSimilarChannels, replaceThreadParam, replaceUserStatuses, toggleSimilarChannels, updateChat, updateChatFullInfo, updateChatLastMessageId, updateChatListSecondaryInfo, updateChats, updateChatsLastMessageId, updateListedTopicIds, updateManagementProgress, updateMissingInvitedUsers, updatePeerFullInfo, updateThread, updateThreadInfo, updateTopic, updateTopics, updateUser, updateUsers, } from '../../reducers';
import { updateGroupCall } from '../../reducers/calls';
import { updateTabState } from '../../reducers/tabs';
import { selectChat, selectChatByUsername, selectChatFolder, selectChatFullInfo, selectChatLastMessageId, selectChatListLoadingParameters, selectChatListType, selectChatMessages, selectCurrentChat, selectCurrentMessageList, selectDraft, selectIsChatPinned, selectIsChatWithSelf, selectIsCurrentUserFrozen, selectLastServiceNotification, selectPeer, selectSimilarChannelIds, selectStickerSet, selectSupportChat, selectTabState, selectThread, selectThreadInfo, selectTopic, selectTopics, selectTopicsInfo, selectUser, selectUserByPhoneNumber, } from '../../selectors';
...
function abortChatRequestsForCurrentChat(global, newChatId, newThreadId, ...[tabId = getCurrentTabId()]) {
...
addActionHandler('openChat', (global, actions, payload) => {
    const { id, type, noForumTopicPanel, shouldReplaceHistory, shouldReplaceLast, tabId = getCurrentTabId(), } = payload;
...
addActionHandler('openSavedDialog', (global, actions, payload) => {
...
addActionHandler('openThread', async (global, actions, payload) => {
    const { type, isComments, noForumTopicPanel, shouldReplaceHistory, shouldReplaceLast, focusMessageId, tabId = getCurrentTabId(), } = payload;
    let { chatId } = payload;
    let threadId;
    let loadingChatId;
    let loadingThreadId;
...
addActionHandler('openSupportChat', async (global, actions, payload) => {
...
addActionHandler('loadPinnedDialogs', async (global, actions, payload) => {
...
addActionHandler('loadFullChat', (global, actions, payload) => {
    const { chatId, force, withPhotos, } = payload;
...
addActionHandler('invalidateFullInfo', (global, actions, payload) => {
...
addActionHandler('requestSavedDialogUpdate', async (global, actions, payload) => {
    const { chatId } = payload;
    const chat = selectChat(global, chatId);
    if (!chat) {
        return;
...
addActionHandler('updateChatSilentPosting', (global, actions, payload) => {
...
addActionHandler('updateTopicMutedState', (global, actions, payload) => {
    const { chatId, topicId, isMuted, mutedUntil, } = payload;
    const chat = selectChat(global, chatId);
    if (!chat) {
        return;
    }
...
addActionHandler('createChannel', async (global, actions, payload) => {
    const { title, about, photo, memberIds, discussionChannelId, tabId = getCurrentTabId(), } = payload;
    ...
    try {
        ...
        if (error.message === 'CHANNELS_TOO_MUCH') {
            ...
            actions.showDialog({ data: { ...error, hasErrorKey: true }, tabId });
        }
    }
    if (!createdChannel) {
        return;
...
addActionHandler('joinChannel', async (global, actions, payload) => {
...
addActionHandler('deleteChatUser', (global, actions, payload) => {
    const { chatId, userId, shouldRevokeHistory, tabId = getCurrentTabId(), } = payload;
    const chat = selectChat(global, chatId);
...
addActionHandler('leaveChannel', async (global, actions, payload) => {
    const { chatId, tabId = getCurrentTabId() } = payload;
    const chat = selectChat(global, chatId);
    if (!chat) {
        return;
...
addActionHandler('createGroupChat', async (global, actions, payload) => {
    const { title, memberIds, photo, tabId = getCurrentTabId(), } = payload;
...
addActionHandler('toggleSavedDialogPinned', (global, actions, payload) => {
    const { id, tabId = getCurrentTabId() } = payload;
    if (selectIsCurrentUserFrozen(global)) {
        actions.openFrozenAccountModal({ tabId });
        return;
...
addActionHandler('loadChatFolders', async (global) => {
...
addActionHandler('loadRecommendedChatFolders', async (global) => {
...
addActionHandler('editChatFolders', (global, actions, payload) => {
    const { chatId, idsToRemove, idsToAdd, tabId = getCurrentTabId(), } = payload;
...
addActionHandler('deleteChatFolder', async (global, actions, payload) => {
    const { id } = payload;
    const folder = selectChatFolder(global, id);
    if (folder) {
...
addActionHandler('markChatUnread', (global, actions, payload) => {
    const { id } = payload;
    if (selectIsCurrentUserFrozen(global)) {
        actions.openFrozenAccountModal({ tabId: getCurrentTabId() });
        return;
...
addActionHandler('markChatMessagesRead', async (global, actions, payload) => {
    if (selectIsCurrentUserFrozen(global))
        return;
    const { id } = payload;
    if (selectIsCurrentUserFrozen(global)) {
    ...
    while (hasMoreTopics) {
        ...
        result.topics.forEach((topic) => {
            if (!topic.unreadCount && !topic.unreadMentionsCount && !topic.unreadReactionsCount)
                return;
...
addActionHandler('markChatRead', (global, actions, payload) => {
    const { id } = payload;
    const chat = selectChat(global, id);
    if (!chat)
        return;
...
addActionHandler('checkChatInvite', async (global, actions, payload) => {
...
addActionHandler('openChatByPhoneNumber', async (global, actions, payload) => {
    const { phoneNumber, startAttach, attach, text, tabId = getCurrentTabId(), } = payload;
...
addActionHandler('openTelegramLink', async (global, actions, payload) => {
    const { url, shouldIgnoreCache, tabId = getCurrentTabId(), } = payload;
    const { openChatByPhoneNumber, checkChatInvite, openStickerSet, openChatWithDraft, joinVoiceChatByLink, openInvoice, checkChatlistInvite, openChatByUsername: openChatByUsernameAction, openStoryViewerByUsername, checkGiftCode, } = actions;
    if (isDeepLink(url)) {
...
addActionHandler('processBoostParameters', async (global, actions, payload) => {
...
addActionHandler('acceptChatInvite', async (global, actions, payload) => {
    const { hash, tabId = getCurrentTabId() } = payload;
...
addActionHandler('openChatByUsername', async (global, actions, payload) => {
    const { username, messageId, commentId, startParam, startAttach, attach, threadId, originalParts, startApp, mode, text, onChatChanged, choose, ref, timestamp, tabId = getCurrentTabId(), } = payload;
    ...
    if (!commentId) {
        ...
        if (startApp !== undefined && !webAppName) {
            ...
            if (!chatByUsername || !chat || !user?.hasMainMiniApp)
                return;
...
addActionHandler('openPrivateChannel', (global, actions, payload) => {
    const { id, commentId, messageId, threadId, timestamp, tabId = getCurrentTabId(), } = payload;
    const chat = selectChat(global, id);
    if (!chat) {
        actions.showNotification({
            message: {
...
addActionHandler('updateChatMemberBannedRights', async (global, actions, payload) => {
    const { chatId, userId, bannedRights, tabId = getCurrentTabId(), } = payload;
    const user = selectUser(global, userId);
    if (!user) {
        return;
    }
...
addActionHandler('updateChatAdmin', async (global, actions, payload) => {
    if (selectIsCurrentUserFrozen(global))
        return;
    const { chatId, userId, adminRights, customTitle, tabId = getCurrentTabId(), } = payload;
    const user = selectUser(global, userId);
    if (!user) {
        return;
    }
...
addActionHandler('updateChat', async (global, actions, payload) => {
    const { chatId, title, about, photo, tabId = getCurrentTabId(), } = payload;
    const chat = selectChat(global, chatId);
...
addActionHandler('resetOpenChatWithDraft', (global, actions, payload) => {
    const { tabId = getCurrentTabId() } = payload || {};
    return updateTabState(global, {
        requestedDraft: undefined,
    }, tabId);
});
addActionHandler('loadMoreMembers', async (global, actions, payload) => {
    const { chatId } = payload || {};
    if (selectIsCurrentUserFrozen(global))
        return;
    const chat = selectChat(global, chatId);
    ...
    if (offset !== undefined && chat.membersCount !== undefined && offset >= chat.membersCount)
        return;
...
addActionHandler('setChatEnabledReactions', async (global, actions, payload) => {
    const { chatId, enabledReactions, reactionsLimit, } = payload;
    const chat = selectChat(global, chatId);
    if (!chat)
        return;
...
addActionHandler('fetchChat', (global, actions, payload) => {
    const { chatId } = payload;
    const chat = selectChat(global, chatId);
    if (chat) {
        return;
...
addActionHandler('closeForumPanel', (global, actions, payload) => {
    const { tabId = getCurrentTabId() } = payload || {};
    return updateTabState(global, {
...
addActionHandler('processAttachBotParameters', async (global, actions, payload) => {
    const { username, filter, startParam, tabId = getCurrentTabId(), } = payload;
...
addActionHandler('createTopic', async (global, actions, payload) => {
    const { chatId, title, iconColor, iconEmojiId, tabId = getCurrentTabId(), } = payload;
    const chat = selectChat(global, chatId);
    if (!chat)
        return;
...
addActionHandler('editTopic', async (global, actions, payload) => {
    const { chatId, topicId, tabId = getCurrentTabId(), ...rest } = payload;
    const chat = selectChat(global, chatId);
    const topic = selectTopic(global, chatId, topicId);
    if (!chat || !topic)
        return;
...
addActionHandler('toggleTopicPinned', (global, actions, payload) => {
    const { chatId, topicId, isPinned, tabId = getCurrentTabId(), } = payload;
    const { topicsPinnedLimit } = global.appConfig || {};
    const chat = selectChat(global, chatId);
    const topics = selectTopics(global, chatId);
    if (!chat || !topics || !topicsPinnedLimit)
        return;
...
addActionHandler('checkChatlistInvite', async (global, actions, payload) => {
...
addActionHandler('joinChatlistInvite', async (global, actions, payload) => {
...
addActionHandler('leaveChatlist', async (global, actions, payload) => {
...
addActionHandler('loadChatlistInvites', async (global, actions, payload) => {
...
addActionHandler('createChatlistInvite', async (global, actions, payload) => {
...
addActionHandler('editChatlistInvite', async (global, actions, payload) => {
    const { folderId, peerIds, url, tabId = getCurrentTabId(), } = payload;
...
addActionHandler('deleteChatlistInvite', async (global, actions, payload) => {
...
addActionHandler('openDeleteChatFolderModal', async (global, actions, payload) => {
...
addActionHandler('updateChatDetectedLanguage', (global, actions, payload) => {
...
addActionHandler('togglePeerTranslations', async (global, actions, payload) => {
    const { chatId, isEnabled } = payload;
    const chat = selectChat(global, chatId);
    if (!chat)
        return;
...
addActionHandler('setViewForumAsMessages', (global, actions, payload) => {
    const { chatId, isEnabled } = payload;
    const chat = selectChat(global, chatId);
...
addActionHandler('loadChannelRecommendations', async (global, actions, payload) => {
    if (selectIsCurrentUserFrozen(global))
        return;
    const { chatId } = payload;
...
addActionHandler('loadBotRecommendations', async (global, actions, payload) => {
    if (selectIsCurrentUserFrozen(global))
        return;
...
addActionHandler('toggleChannelRecommendations', (global, actions, payload) => {
    const { chatId } = payload;
    const chat = selectChat(global, chatId);
    if (!chat) {
        return;
...
addActionHandler('updatePaidMessagesPrice', async (global, actions, payload) => {
...
addActionHandler('toggleAutoTranslation', async (global, actions, payload) => {
    const { chatId, isEnabled } = payload;
    const chat = selectChat(global, chatId);
    if (!chat)
        return;
...
addActionHandler('resolveBusinessChatLink', async (global, actions, payload) => {
    const { slug, tabId = getCurrentTabId() } = payload;
...
addActionHandler('requestCollectibleInfo', async (global, actions, payload) => {
    const { type, collectible, peerId, tabId = getCurrentTabId(), } = payload;
...
async function loadChats(listType, isFullDraftSync, shouldIgnorePagination) {
    ...
    if (!shouldIgnorePagination) {
        global = replaceChatListLoadingParameters(global, listType, result.nextOffsetId, result.nextOffsetPeerId, result.nextOffsetDate);
    }
    ...
    idsToUpdateDraft.forEach((chatId) => {
        ...
        if (!selectDraft(global, chatId, MAIN_THREAD_ID)?.isLocal) {
            global = replaceThreadParam(global, chatId, MAIN_THREAD_ID, 'draft', draft);
        }
    });
...
export async function loadFullChat(global, actions, chat) {
    if (selectIsCurrentUserFrozen(global))
        return undefined;
    ...
    const { chats, userStatusesById, fullInfo, groupCall, membersCount, isForumAsMessages, } = result;
    global = getGlobal();
    global = updateChats(global, buildCollectionByKey(chats, 'id'));
    if (userStatusesById) {
        global = addUserStatuses(global, userStatusesById);
    ...
    if (groupCall) {
        ...
        global = updateGroupCall(global, groupCall.id, omit(groupCall, ['connectionState', 'isLoaded']), undefined, existingGroupCall ? undefined : groupCall.participantsCount);
    }
...
export async function migrateChat(global, actions, chat, ...[tabId = getCurrentTabId()]) {
    try {
...
export async function fetchChatByUsername(global, username, referrer) {
    global = getGlobal();
...
export async function checkWebAppExists(global, botName, appName) {
    if (!botName || !appName)
        return false;
    global = getGlobal();
...
export async function fetchChatByPhoneNumber(global, phoneNumber) {
    global = getGlobal();
...
async function getAttachBotOrNotify(global, actions, username, ...[tabId = getCurrentTabId()]) {
...
async function openChatByUsername(global, actions, params, ...[tabId = getCurrentTabId()]) {
    const { username, threadId, channelPostId, startParam, ref, startAttach, attach, text, timestamp, } = params;
...
async function openChatWithParams(global, actions, chat, params, ...[tabId = getCurrentTabId()]) {
    const { isCurrentChat, threadId, messageId, startParam, referrer, startAttach, attach, text, timestamp, } = params;
    if (messageId) {
...
async function openAttachMenuFromLink(global, actions, chatId, attach, startAttach, ...[tabId = getCurrentTabId()]) {
    global = getGlobal();
...
export async function ensureIsSuperGroup(global, actions, chatId, ...[tabId = getCurrentTabId()]) {
    const chat = selectChat(global, chatId);
