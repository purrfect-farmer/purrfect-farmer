...
import { addUserStatuses, closeNewContactDialog, replaceUserStatuses, updateChats, updateManagementProgress, updatePeerPhotos, updatePeerPhotosIsLoading, updateUser, updateUserCommonChats, updateUserFullInfo, updateUsers, updateUserSearch, updateUserSearchFetchingStatus, } from '../../reducers';
import { updateTabState } from '../../reducers/tabs';
import { selectChat, selectChatFullInfo, selectIsCurrentUserFrozen, selectIsCurrentUserPremium, selectPeer, selectPeerPhotos, selectTabState, selectUser, selectUserCommonChats, selectUserFullInfo, } from '../../selectors';
...
addActionHandler('loadTopUsers', async (global) => {
...
addActionHandler('loadContactList', async (global) => {
...
addActionHandler('loadCommonChats', async (global, actions, payload) => {
    const { userId } = payload;
...
addActionHandler('toggleNoPaidMessagesException', async (global, actions, payload) => {
    const { userId, shouldRefundCharged } = payload;
    const user = selectUser(global, userId);
    if (!user) {
        return;
    }
    const result = await callApi('toggleNoPaidMessagesException', { user, shouldRefundCharged });
    if (!result) {
        return;
    }
    global = getGlobal();
...
addActionHandler('openChatRefundModal', async (global, actions, payload) => {
...
addActionHandler('updateContact', async (global, actions, payload) => {
    const { userId, isMuted = false, firstName, lastName, shouldSharePhoneNumber, tabId = getCurrentTabId(), } = payload;
    const user = selectUser(global, userId);
    if (!user) {
        return;
    }
...
addActionHandler('deleteContact', async (global, actions, payload) => {
    const { userId } = payload;
    const user = selectUser(global, userId);
    if (!user) {
        return;
...
addActionHandler('loadMoreProfilePhotos', async (global, actions, payload) => {
    if (selectIsCurrentUserFrozen(global))
        return;
    ...
    if (profilePhotos && !shouldInvalidateCache && (isPreload || !profilePhotos.nextOffset))
        return;
    ...
    global = getGlobal();
    const { photos, count, nextOffsetId, } = result;
...
addActionHandler('setUserSearchQuery', (global, actions, payload) => {
    ...
    void runThrottledForSearch(async () => {
        ...
        const { accountResultIds, globalResultIds, } = result;
...
addActionHandler('importContact', async (global, actions, payload) => {
    const { phoneNumber: phone, firstName, lastName, tabId = getCurrentTabId(), } = payload;
...
addActionHandler('setEmojiStatus', async (global, actions, payload) => {
    const { emojiStatus, referrerWebAppKey, tabId = getCurrentTabId(), } = payload;
...
addActionHandler('saveCloseFriends', async (global, actions, payload) => {
...
addActionHandler('openSuggestedStatusModal', async (global, actions, payload) => {
    const { customEmojiId, duration, botId, webAppKey, tabId = getCurrentTabId(), } = payload;
...
addActionHandler('loadPeerSettings', async (global, actions, payload) => {
...
addActionHandler('markBotVerificationInfoShown', (global, actions, payload) => {
    const { peerId } = payload;
