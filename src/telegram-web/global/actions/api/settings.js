...
import { addBlockedUser, addNotifyException, addNotifyExceptions, deletePeerPhoto, removeBlockedUser, replaceSettings, updateChat, updateSharedSettings, updateUser, updateUserFullInfo, } from '../../reducers';
import { updateTabState } from '../../reducers/tabs';
import { selectChat, selectIsCurrentUserFrozen, selectTabState, selectUser, } from '../../selectors';
import { selectSharedSettings } from '../../selectors/sharedState';
addActionHandler('updateProfile', async (global, actions, payload) => {
    const { photo, firstName, lastName, bio: about, username, tabId = getCurrentTabId(), } = payload;
    ...
    if (firstName || lastName || about) {
        ...
        if (result) {
            ...
            if (currentUser) {
                global = updateUser(global, currentUser.id, {
                    firstName,
                    lastName,
                });
    ...
    if (username !== undefined) {
        ...
        if (result && currentUser) {
            ...
            const usernames = shouldUsernameUpdate
                ...
                : [{ username, isEditable: true, isActive: true }, ...currentUser.usernames || []];
...
addActionHandler('loadWallpapers', async (global) => {
...
addActionHandler('loadBlockedUsers', async (global) => {
...
addActionHandler('blockUser', async (global, actions, payload) => {
...
addActionHandler('unblockUser', async (global, actions, payload) => {
    const { userId, isOnlyStories } = payload;
    const user = selectUser(global, userId);
    if (!user)
        return;
...
addActionHandler('updateWebNotificationSettings', async (global, actions, payload) => {
...
addActionHandler('loadLanguages', async (global) => {
...
addActionHandler('loadPrivacySettings', async (global) => {
    ...
    const [phoneNumberSettings, addByPhoneSettings, lastSeenSettings, profilePhotoSettings, forwardsSettings, chatInviteSettings, phoneCallSettings, phoneP2PSettings, voiceMessagesSettings, bioSettings, birthdaySettings, giftsSettings, noPaidMessagesSettings,] = result;
    global = getGlobal();
    global = {
        ...global,
        settings: {
...
addActionHandler('setPrivacyVisibility', async (global, actions, payload) => {
    ...
    const { privacy: { [privacyKey]: settings }, } = global.settings;
    if (!settings) {
        return;
    }
...
addActionHandler('setPrivacySettings', async (global, actions, payload) => {
    const { privacyKey, isAllowList, updatedIds, isPremiumAllowed, botsPrivacy, } = payload;
    const { privacy: { [privacyKey]: settings }, } = global.settings;
    if (!settings) {
        return;
    }
...
addActionHandler('loadContentSettings', async (global) => {
    if (selectIsCurrentUserFrozen(global))
        return;
...
addActionHandler('loadAppConfig', async (global, actions, payload) => {
...
addActionHandler('loadPeerColors', async (global) => {
...
addActionHandler('loadTimezones', async (global) => {
...
addActionHandler('loadGlobalPrivacySettings', async (global) => {
...
addActionHandler('toggleUsername', async (global, actions, payload) => {
...
addActionHandler('toggleChatUsername', async (global, actions, payload) => {
    const { chatId, username, isActive, } = payload;
...
addActionHandler('sortUsernames', async (global, actions, payload) => {
...
addActionHandler('sortChatUsernames', async (global, actions, payload) => {
