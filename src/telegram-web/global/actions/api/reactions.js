...
import { buildCollectionByCallback, buildCollectionByKey, omit, partition, unique, } from '../../../util/iteratees';
import { getMessageKey } from '../../../util/keys/messageKey';
import * as mediaLoader from '../../../util/mediaLoader';
import requestActionTimeout from '../../../util/requestActionTimeout';
import { callApi } from '../../../api/gramjs';
import { addPaidReaction, getDocumentMediaHash, getReactionKey, getUserReactions, isMessageLocal, isSameReaction, } from '../../helpers';
import { addActionHandler, getGlobal, setGlobal } from '../../index';
import { addChatMessagesById, updateChat, updateChatMessage, } from '../../reducers';
import { addMessageReaction, subtractXForEmojiInteraction, updateUnreadReactions } from '../../reducers/reactions';
import { updateTabState } from '../../reducers/tabs';
import { selectChat, selectChatMessage, selectCurrentChat, selectDefaultReaction, selectIsChatWithSelf, selectIsCurrentUserFrozen, selectMaxUserReactions, selectMessageIdsByGroupId, selectPerformanceSettingsValue, selectTabState, } from '../../selectors';
const INTERACTION_RANDOM_OFFSET = 40;
let interactionLocalId = 0;
addActionHandler('loadAvailableReactions', async (global) => {
...
addActionHandler('loadAvailableEffects', async (global) => {
...
addActionHandler('interactWithAnimatedEmoji', (global, actions, payload) => {
    const { emoji, x, y, startSize, isReversed, tabId = getCurrentTabId(), } = payload;
...
addActionHandler('sendEmojiInteraction', (global, actions, payload) => {
    const { messageId, chatId, emoji, interactions, } = payload;
...
addActionHandler('sendDefaultReaction', (global, actions, payload) => {
    ...
    if (!reaction || !message || isMessageLocal(message))
        return;
...
addActionHandler('toggleReaction', async (global, actions, payload) => {
    const { chatId, reaction, shouldAddToRecent, tabId = getCurrentTabId(), } = payload;
...
addActionHandler('addLocalPaidReaction', (global, actions, payload) => {
    const { chatId, messageId, count, shouldIgnoreDefaultPrivacy = false, tabId = getCurrentTabId(), } = payload;
...
addActionHandler('sendPaidReaction', async (global, actions, payload) => {
    const { chatId, messageId, forcedAmount, tabId = getCurrentTabId(), } = payload;
    const chat = selectChat(global, chatId);
    const message = selectChatMessage(global, chatId, messageId);
    if (!chat || !message) {
        return;
...
addActionHandler('sendWatchingEmojiInteraction', (global, actions, payload) => {
    const { chatId, emoticon, x, y, startSize, isReversed, id, tabId = getCurrentTabId(), } = payload;
    const chat = selectChat(global, chatId);
    const tabState = selectTabState(global, tabId);
...
addActionHandler('focusNextReaction', (global, actions, payload) => {
...
addActionHandler('readAllReactions', (global, actions, payload) => {
...
addActionHandler('loadTopReactions', async (global) => {
...
addActionHandler('loadRecentReactions', async (global) => {
...
addActionHandler('clearRecentReactions', async (global) => {
...
addActionHandler('loadDefaultTagReactions', async (global) => {
...
addActionHandler('loadSavedReactionTags', async (global) => {
...
addActionHandler('editSavedReactionTag', async (global, actions, payload) => {
