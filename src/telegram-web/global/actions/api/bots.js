...
import { removeBlockedUser, updateBotAppPermissions, updateManagementProgress, updateSharedSettings, updateUser, updateUserFullInfo, } from '../../reducers';
import { activateWebAppIfOpen, addWebAppToOpenList, replaceInlineBotSettings, replaceInlineBotsIsLoading, } from '../../reducers/bots';
import { updateTabState } from '../../reducers/tabs';
import { selectBot, selectChat, selectChatLastMessageId, selectChatMessage, selectCurrentChat, selectCurrentMessageList, selectDraft, selectIsCurrentUserFrozen, selectIsTrustedBot, selectMessageReplyInfo, selectPeer, selectSendAs, selectSender, selectTabState, selectUser, selectUserFullInfo, } from '../../selectors';
...
addActionHandler('clickBotInlineButton', (global, actions, payload) => {
    const { chatId, messageId, button, tabId = getCurrentTabId(), } = payload;
...
addActionHandler('sendBotCommand', (global, actions, payload) => {
    ...
    void sendBotCommand(chat, command, selectDraft(global, chat.id, threadId)?.replyInfo, selectSendAs(global, chat.id), lastMessageId);
});
...
addActionHandler('loadTopBotApps', async (global) => {
...
addActionHandler('queryInlineBot', async (global, actions, payload) => {
    const { chatId, username, query, offset, tabId = getCurrentTabId(), } = payload;
...
addActionHandler('switchBotInline', (global, actions, payload) => {
    const { query, isSamePeer, messageId, filter, tabId = getCurrentTabId(), } = payload;
    let { botId, } = payload;
    const chat = selectCurrentChat(global, tabId);
    if (!chat) {
        return undefined;
...
addActionHandler('sendInlineBotApiResult', async (global, actions, payload) => {
    const { chat, id, queryId, replyInfo, sendAs, isSilent, scheduledAt, allowPaidStars, } = payload;
...
addActionHandler('sendInlineBotResult', async (global, actions, payload) => {
    const { id, queryId, isSilent, scheduledAt, threadId, chatId, tabId = getCurrentTabId(), } = payload;
    if (!id) {
        return;
    }
    const chat = selectChat(global, chatId);
...
addActionHandler('resetInlineBot', (global, actions, payload) => {
    ...
    if (!force && Date.now() < inlineBotData.cacheTime)
        return;
    inlineBotData = {
        id: inlineBotData.id,
        query: '',
...
addActionHandler('resetAllInlineBots', (global, actions, payload) => {
...
addActionHandler('sharePhoneWithBot', async (global, actions, payload) => {
    const { botId } = payload;
    const bot = selectUser(global, botId);
    if (!bot) {
        return;
...
addActionHandler('requestSimpleWebView', async (global, actions, payload) => {
    const { url, botId, theme, buttonText, isFromSideMenu, isFromSwitchWebView, startParam, tabId = getCurrentTabId(), } = payload;
    if (checkIfOpenOrActivate(global, botId, tabId, url))
        return;
    const bot = selectUser(global, botId);
    if (!bot)
        return;
...
addActionHandler('requestWebView', async (global, actions, payload) => {
    const { url, botId, peerId, theme, isSilent, buttonText, isFromBotMenu, startParam, isFullscreen, tabId = getCurrentTabId(), } = payload;
    if (checkIfOpenOrActivate(global, botId, tabId, url))
        return;
    const bot = selectUser(global, botId);
    if (!bot)
...
addActionHandler('requestMainWebView', async (global, actions, payload) => {
    const { botId, peerId, theme, startParam, mode, shouldMarkBotTrusted, tabId = getCurrentTabId(), } = payload;
    if (selectIsCurrentUserFrozen(global)) {
        actions.openFrozenAccountModal({ tabId });
        return;
    }
    if (checkIfOpenOrActivate(global, botId, tabId))
        return;
    const bot = selectUser(global, botId);
    if (!bot)
        return;
...
addActionHandler('loadPreviewMedias', async (global, actions, payload) => {
    const { botId, } = payload;
    const bot = selectUser(global, botId);
    if (!bot)
        return;
...
addActionHandler('openWebAppsCloseConfirmationModal', (global, actions, payload) => {
    const { tabId = getCurrentTabId(), } = payload || {};
    return updateTabState(global, {
        isWebAppsCloseConfirmationModalOpen: true,
    }, tabId);
});
addActionHandler('closeWebAppsCloseConfirmationModal', (global, actions, payload) => {
...
addActionHandler('requestAppWebView', async (global, actions, payload) => {
    const { botId, appName, startApp, mode, theme, isWriteAllowed, isFromConfirm, shouldSkipBotTrustRequest, tabId = getCurrentTabId(), } = payload;
    if (checkIfOpenOrActivate(global, botId, tabId, appName))
        return;
    const bot = selectUser(global, botId);
    if (!bot)
        return;
...
addActionHandler('prolongWebView', async (global, actions, payload) => {
    const { botId, peerId, isSilent, replyInfo, queryId, tabId = getCurrentTabId(), } = payload;
    const bot = selectUser(global, botId);
    if (!bot)
        return;
...
addActionHandler('sendWebViewData', (global, actions, payload) => {
    const { bot, data, buttonText, } = payload;
...
addActionHandler('loadAttachBots', async (global) => {
...
export function isWepAppOpened(global, webApp, tabId) {
...
export function checkIfOpenOrActivate(global, botId, tabId, requestUrl, webAppName) {
...
addActionHandler('callAttachBot', (global, actions, payload) => {
    const { bot, startParam, isFromConfirm, tabId = getCurrentTabId(), } = payload;
...
addActionHandler('requestBotUrlAuth', async (global, actions, payload) => {
    const { chatId, buttonId, messageId, url, tabId = getCurrentTabId(), } = payload;
    const chat = selectChat(global, chatId);
    if (!chat) {
        return;
    }
...
addActionHandler('acceptBotUrlAuth', async (global, actions, payload) => {
    ...
    if (!tabState.urlAuth?.button)
        return;
    const { button, url, } = tabState.urlAuth;
...
addActionHandler('closeUrlAuthModal', (global, actions, payload) => {
    const { tabId = getCurrentTabId() } = payload || {};
    return updateTabState(global, {
        urlAuth: undefined,
    }, tabId);
});
function handleUrlAuthResult(global, actions, url, result, ...[tabId = getCurrentTabId()]) {
...
async function searchInlineBot(global, { username, inlineBotData, chatId, query, offset, }, ...[tabId = getCurrentTabId()]) {
    global = getGlobal();
...
let gameePopups;
async function answerCallbackButton(global, actions, chat, messageId, data, isGame = false, ...[tabId = getCurrentTabId()]) {
    const { showDialog, showNotification, openUrl, openGame, } = actions;
    if (isGame) {
        if (!gameePopups) {
...
addActionHandler('setBotInfo', async (global, actions, payload) => {
    const { bot, name, description: about, tabId = getCurrentTabId(), } = payload;
    ...
    if (name || about) {
        ...
        if (result) {
            global = getGlobal();
            global = updateUser(global, bot.id, {
                firstName: name,
            });
...
addActionHandler('toggleUserEmojiStatusPermission', async (global, actions, payload) => {
    const { botId, isEnabled, isBotAccessEmojiGranted, } = payload;
...
addActionHandler('toggleUserLocationPermission', (global, actions, payload) => {
    const { botId, isAccessGranted, } = payload;
    const bot = selectUser(global, botId);
    if (!bot)
        return;
    global = getGlobal();
...
addActionHandler('startBotFatherConversation', async (global, actions, payload) => {
    const { param, tabId = getCurrentTabId(), } = payload;
    if (!botFatherId) {
