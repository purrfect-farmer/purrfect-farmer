...
import { rebuildStickersForEmoji, replaceAnimatedEmojis, updateCustomEmojiForEmoji, updateCustomEmojiSets, updateGifSearch, updateRecentStatusCustomEmojis, updateStickerSearch, updateStickerSet, updateStickerSets, updateStickersForEmoji, } from '../../reducers';
import { updateTabState } from '../../reducers/tabs';
import { selectIsCurrentUserFrozen, selectIsCurrentUserPremium, selectStickerSet, selectTabState, } from '../../selectors';
...
addActionHandler('loadStickerSets', async (global, actions) => {
    ...
    global = getGlobal();
    global = updateStickerSets(global, 'added', addedStickers.hash, addedStickers.sets);
    global = updateCustomEmojiSets(global, addedCustomEmojis.hash, addedCustomEmojis.sets);
    setGlobal(global);
...
addActionHandler('loadAddedStickers', async (global, actions) => {
    const { added: { setIds: addedSetIds = [], }, setsById: cached, } = global.stickers;
    const { added: { setIds: customEmojiSetIds = [], }, } = global.customEmojis;
...
addActionHandler('loadFavoriteStickers', async (global) => {
...
addActionHandler('loadFeaturedStickers', async (global) => {
    ...
    global = getGlobal();
    global = updateStickerSets(global, 'featured', featuredStickers.hash, featuredStickers.sets);
    setGlobal(global);
});
addActionHandler('loadPremiumGifts', async (global) => {
...
addActionHandler('loadDefaultStatusIcons', async (global) => {
...
addActionHandler('loadUserCollectibleStatuses', async (global, actions) => {
    setGlobal(global);
...
addActionHandler('loadStickers', (global, actions, payload) => {
    ...
    if (cachedSet && cachedSet.count === cachedSet?.stickers?.length)
...
addActionHandler('loadBirthdayNumbersStickers', async (global) => {
...
addActionHandler('loadRestrictedEmojiStickers', async (global) => {
    const emojis = await callApi('fetchStickers', {
        stickerSetInfo: {
...
addActionHandler('loadSavedGifs', async (global) => {
...
addActionHandler('saveGif', async (global, actions, payload) => {
    const { gif, shouldUnsave, tabId = getCurrentTabId(), } = payload;
    ...
    if (!shouldUnsave && length && length >= limit) {
        actions.showNotification({
            ...
            message: isPremium ? langProvider.oldTranslate('LimitReachedFavoriteGifsSubtitlePremium')
                : langProvider.oldTranslate('LimitReachedFavoriteGifsSubtitle', premiumLimit.toString()),
            ...(!isPremium && {
                action: {
    ...
    if (current >= limit) {
        actions.showNotification({
            ...
            message: isPremium ? langProvider.oldTranslate('LimitReachedFavoriteStickersSubtitlePremium')
                : langProvider.oldTranslate('LimitReachedFavoriteStickersSubtitle', premiumLimit.toString()),
            ...(!isPremium && {
                action: {
                    action: 'openPremiumModal',
                    payload: { tabId },
...
addActionHandler('removeRecentSticker', async (global, actions, payload) => {
    const { sticker } = payload;
...
addActionHandler('loadEmojiKeywords', async (global, actions, payload) => {
...
async function loadStickers(global, actions, stickerSetInfo) {
    let stickerSet;
    try {
        stickerSet = await callApi('fetchStickers', { stickerSetInfo });
    }
    catch (error) {
...
addActionHandler('setStickerSearchQuery', (global, actions, payload) => {
    ...
    if (query) {
        void searchThrottled(async () => {
            ...
            global = updateStickerSets(global, 'search', result.hash, result.sets);
...
addActionHandler('searchMoreGifs', (global, actions, payload) => {
...
addActionHandler('loadRecentEmojiStatuses', async (global) => {
...
async function searchGifs(global, query, botUsername, offset, ...[tabId = getCurrentTabId()]) {
