...
import { appendStarsSubscriptions, appendStarsTransactions, replacePeerSavedGifts, updateChats, updateStarsBalance, updateStarsSubscriptionLoading, updateUsers, } from '../../reducers';
import { updateTabState } from '../../reducers/tabs';
import { selectGiftProfileFilter, selectPeer, selectPeerSavedGifts, selectTabState, } from '../../selectors';
addActionHandler('loadStarStatus', async (global) => {
...
addActionHandler('loadStarsTransactions', async (global, actions, payload) => {
    const { type } = payload;
...
addActionHandler('loadStarGifts', async (global) => {
...
addActionHandler('updateResaleGiftsFilter', (global, actions, payload) => {
...
addActionHandler('loadResaleGifts', async (global, actions, payload) => {
    const { giftId, shouldRefresh, tabId = getCurrentTabId(), } = payload;
    let tabState = selectTabState(global, tabId);
    if (tabState.resaleGifts.isLoading || (tabState.resaleGifts.isAllLoaded && !shouldRefresh))
        return;
    global = updateTabState(global, {
        resaleGifts: {
            ...tabState.resaleGifts,
...
addActionHandler('resetResaleGifts', (global, actions, payload) => {
...
addActionHandler('loadPeerSavedGifts', async (global, actions, payload) => {
    const { peerId, shouldRefresh, tabId = getCurrentTabId(), } = payload;
    ...
    if (!shouldRefresh && currentGifts && !localNextOffset)
        return; // Already loaded all
...
addActionHandler('reloadPeerSavedGifts', (global, actions, payload) => {
...
addActionHandler('loadStarsSubscriptions', async (global) => {
    const subscriptions = global.stars?.subscriptions;
    const offset = subscriptions?.nextOffset;
    if (subscriptions && !offset)
        return; // Already loaded all
...
addActionHandler('changeStarsSubscription', async (global, actions, payload) => {
...
addActionHandler('fulfillStarsSubscription', async (global, actions, payload) => {
...
addActionHandler('changeGiftVisibility', async (global, actions, payload) => {
...
addActionHandler('convertGiftToStars', async (global, actions, payload) => {
...
addActionHandler('openGiftUpgradeModal', async (global, actions, payload) => {
    const { giftId, gift, peerId, tabId = getCurrentTabId(), } = payload;
...
addActionHandler('toggleSavedGiftPinned', async (global, actions, payload) => {
    ...
    const newPinnedGifts = gift.isPinned
        ? currentPinnedGifts.filter((g) => g.gift.slug !== gift.gift.slug)
...
addActionHandler('updateStarGiftPrice', async (global, actions, payload) => {
    const { gift, price, } = payload;
