...
import { getRequestInputInvoice, getRequestInputSavedStarGift, } from '../../helpers/payments';
import { addActionHandler, getActions, getGlobal, setGlobal, } from '../../index';
import { closeInvoice, openStarsTransactionFromReceipt, setPaymentStep, setReceipt, setRequestInfoId, setSmartGlocalCardInfo, setStripeCardInfo, updateChatFullInfo, updatePayment, updateShippingOptions, updateStarsPayment, } from '../../reducers';
import { updateTabState } from '../../reducers/tabs';
import { selectChat, selectChatFullInfo, selectIsCurrentUserFrozen, selectPaymentInputInvoice, selectPaymentRequestId, selectProviderPublicToken, selectProviderPublishableKey, selectSmartGlocalCredentials, selectStarsPayment, selectStripeCredentials, selectTabState, } from '../../selectors';
...
addActionHandler('sendStarGift', (global, actions, payload) => {
    const { gift, peerId, message, shouldHideName, shouldUpgrade, tabId = getCurrentTabId(), } = payload;
    const inputInvoice = {
        type: 'stargift',
        peerId,
        giftId: gift.id,
...
addActionHandler('buyStarGift', (global, actions, payload) => {
    const { slug, peerId, stars, tabId = getCurrentTabId(), } = payload;
    const inputInvoice = {
        type: 'stargiftResale',
        slug,
        peerId,
...
addActionHandler('sendPremiumGiftByStars', (global, actions, payload) => {
    const { userId, months, amount, message, tabId = getCurrentTabId(), } = payload;
    const inputInvoice = {
...
addActionHandler('getReceipt', async (global, actions, payload) => {
...
addActionHandler('clearPaymentError', (global, actions, payload) => {
...
addActionHandler('clearReceipt', (global, actions, payload) => {
    const { tabId = getCurrentTabId() } = payload || {};
...
addActionHandler('sendPaymentForm', async (global, actions, payload) => {
    const { shippingOptionId, saveCredentials, savedCredentialId, tipAmount, tabId = getCurrentTabId(), } = payload;
...
addActionHandler('sendStarPaymentForm', async (global, actions, payload) => {
...
async function sendStripeCredentials(global, data, publishableKey, ...[tabId = getCurrentTabId()]) {
...
async function sendSmartGlocalCredentials(global, data, publicToken, ...[tabId = getCurrentTabId()]) {
    const params = {
        card: {
    ...
    if (result.status !== 'ok') {
        ...
        const error = { descriptionKey: { key: 'ErrorUnexpected' } };
        global = getGlobal();
        global = updateTabState(global, {
            payment: {
                ...selectTabState(global, tabId).payment,
...
addActionHandler('setSmartGlocalCardInfo', (global, actions, payload) => {
...
addActionHandler('openPremiumModal', async (global, actions, payload) => {
    const { initialSection, fromUserId, isSuccess, isGift, monthsAmount, toUserId, tabId = getCurrentTabId(), } = payload || {};
...
addActionHandler('openGiveawayModal', async (global, actions, payload) => {
    const { chatId, prepaidGiveaway, tabId = getCurrentTabId(), } = payload;
...
addActionHandler('openGiftModal', async (global, actions, payload) => {
...
addActionHandler('openStarsGiftModal', async (global, actions, payload) => {
    const { forUserId, tabId = getCurrentTabId(), } = payload || {};
    if (selectIsCurrentUserFrozen(global)) {
        actions.openFrozenAccountModal({ tabId });
        return;
...
addActionHandler('validatePaymentPassword', async (global, actions, payload) => {
...
async function validateRequestedInfo(global, inputInvoice, requestInfo, shouldSave, ...[tabId = getCurrentTabId()]) {
...
addActionHandler('openBoostModal', async (global, actions, payload) => {
    const { chatId, tabId = getCurrentTabId() } = payload;
    const chat = selectChat(global, chatId);
    if (!chat || !(isChatChannel(chat) || isChatSuperGroup(chat)))
        return;
    global = updateTabState(global, {
        boostModal: {
            chatId,
...
addActionHandler('openBoostStatistics', async (global, actions, payload) => {
    const { chatId, tabId = getCurrentTabId() } = payload;
    const chat = selectChat(global, chatId);
    if (!chat)
        return;
    ...
    setGlobal(global);
    const [boostListResult, boostListGiftResult, boostStatusResult] = await Promise.all([
...
addActionHandler('openMonetizationStatistics', (global, actions, payload) => {
    const { chatId, tabId = getCurrentTabId() } = payload;
    const chat = selectChat(global, chatId);
    if (!chat)
        return;
...
addActionHandler('loadMoreBoosters', async (global, actions, payload) => {
    const { isGifts, tabId = getCurrentTabId() } = payload || {};
    let tabState = selectTabState(global, tabId);
    if (!tabState.boostStatistics)
        return;
...
addActionHandler('applyBoost', async (global, actions, payload) => {
...
addActionHandler('checkGiftCode', async (global, actions, payload) => {
...
addActionHandler('applyGiftCode', async (global, actions, payload) => {
...
addActionHandler('launchPrepaidGiveaway', async (global, actions, payload) => {
    const { chatId, giveawayId, paymentPurpose, tabId = getCurrentTabId(), } = payload;
    const chat = selectChat(global, chatId);
    if (!chat)
        return;
...
addActionHandler('launchPrepaidStarsGiveaway', async (global, actions, payload) => {
    const { chatId, giveawayId, paymentPurpose, tabId = getCurrentTabId(), } = payload;
    const chat = selectChat(global, chatId);
    if (!chat)
        return;
...
addActionHandler('upgradeGift', (global, actions, payload) => {
    const { gift, shouldKeepOriginalDetails, upgradeStars, tabId = getCurrentTabId(), } = payload;
...
addActionHandler('transferGift', (global, actions, payload) => {
    const { gift, recipientId, transferStars, tabId = getCurrentTabId(), } = payload;
...
async function payInputStarInvoice(global, inputInvoice, price, ...[tabId = getCurrentTabId()]) {
...
addActionHandler('openUniqueGiftBySlug', async (global, actions, payload) => {
    const { slug, tabId = getCurrentTabId(), } = payload;
...
addActionHandler('processStarGiftWithdrawal', async (global, actions, payload) => {
    const { gift, password, tabId = getCurrentTabId(), } = payload;
    let giftWithdrawModal = selectTabState(global, tabId).giftWithdrawModal;
    if (!giftWithdrawModal)
        return;
    global = updateTabState(global, {
        giftWithdrawModal: {
            ...giftWithdrawModal,
