...
addActionHandler('openMediaViewer', (global, actions, payload) => {
    const { chatId, threadId = MAIN_THREAD_ID, messageId, timestamp, mediaIndex, isAvatarView, isSponsoredMessage, origin, withDynamicLoading, standaloneMedia, tabId = getCurrentTabId(), } = payload;
...
addActionHandler('closeMediaViewer', (global, actions, payload) => {
    const { tabId = getCurrentTabId() } = payload || {};
    const { volume, isMuted, playbackRate, isHidden, } = selectTabState(global, tabId).mediaViewer;
    return updateTabState(global, {
        mediaViewer: {
            volume,
            isMuted,
...
addActionHandler('openMediaFromTimestamp', (global, actions, payload) => {
    const { chatId, messageId, threadId, timestamp, tabId = getCurrentTabId(), } = payload;
    ...
    if (maxMessageDuration) {
        if (maxMessageDuration <= timestamp)
            return;
    ...
    if (!maxReplyMessageDuration || maxReplyMessageDuration <= timestamp)
        return;
...
addActionHandler('updateLastPlaybackTimestamp', (global, actions, payload) => {
...
addActionHandler('setMediaViewerPlaybackRate', (global, actions, payload) => {
    const { playbackRate, tabId = getCurrentTabId(), } = payload;
    global = {
        ...global,
        mediaViewer: {
            ...global.mediaViewer,
