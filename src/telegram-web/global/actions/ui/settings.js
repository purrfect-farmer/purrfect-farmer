...
addCallback((global) => {
    ...
    if (sharedSettings.language !== oldSharedSettings.language) {
        oldSetLanguage(sharedSettings.language);
    }
    ...
    if (sharedSettings.messageTextSize !== oldSharedSettings.messageTextSize) {
        document.documentElement.style.setProperty('--composer-text-size', `${Math.max(sharedSettings.messageTextSize, IS_IOS ? 16 : 15)}px`);
        document.documentElement.style.setProperty('--message-meta-height', `${Math.floor(sharedSettings.messageTextSize * 1.3125)}px`);
...
addActionHandler('updateShouldEnableDebugLog', (global) => {
...
addActionHandler('updateShouldDebugExportedSenders', (global) => {
    const settings = selectSharedSettings(global);
    callApi('setShouldDebugExportedSenders', Boolean(settings.shouldDebugExportedSenders));
});
addActionHandler('setSettingOption', (global, actions, payload) => {
    return replaceSettings(global, payload);
});
addActionHandler('setSharedSettingOption', (global, actions, payload) => {
    return updateSharedSettings(global, payload);
});
addActionHandler('updatePerformanceSettings', (global, actions, payload) => {
    const settings = selectSharedSettings(global);
    global = updateSharedSettings(global, {
        performance: {
...
addActionHandler('setThemeSettings', (global, actions, payload) => {
    const { theme, ...settings } = payload;
    return updateThemeSettings(global, theme, settings);
});
addActionHandler('requestNextFoldersAction', (global, actions, payload) => {
    const { foldersAction, tabId = getCurrentTabId() } = payload;
    return updateTabState(global, {
        nextFoldersAction: foldersAction,
    }, tabId);
});
addActionHandler('openLeftColumnContent', (global, actions, payload) => {
...
addActionHandler('openSettingsScreen', (global, actions, payload) => {
    ...
    if (payload.screen)
        actions.openLeftColumnContent({ contentKey: LeftColumnContent.Settings, tabId });
    return updateTabState(global, {
        leftColumn: {
            ...tabState.leftColumn,
...
addActionHandler('openEditChatFolder', (global, actions, payload) => {
...
addActionHandler('openShareChatFolderModal', (global, actions, payload) => {
    const { folderId, url, noRequestNextScreen, tabId = getCurrentTabId(), } = payload;
    const chatFolder = selectChatFolder(global, folderId);
    ...
    if (!noRequestNextScreen)
        actions.openSettingsScreen({ screen: SettingsScreens.FoldersShare, tabId });
    return updateTabState(global, {
        shareFolderScreen: {
            folderId,
...
addActionHandler('closeShareChatFolderModal', (global, actions, payload) => {
