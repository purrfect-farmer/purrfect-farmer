...
import { IS_ANDROID, IS_ELECTRON, IS_IOS, IS_LINUX, IS_MAC_OS, IS_SAFARI, IS_TOUCH_ENV, IS_WINDOWS, } from '../../../util/browser/windowEnvironment';
...
import { selectCanAnimateInterface, selectPerformanceSettings, selectSettingsKeys, selectTabState, selectTheme, } from '../../selectors';
...
addActionHandler('switchMultitabRole', async (global, actions, payload) => {
...
addActionHandler('onSomeTabSwitchedMultitabRole', async (global) => {
    if (global.passcode.hasPasscode && !global.passcode.isScreenLocked) {
        const { sessionJson } = await decryptSessionByCurrentHash();
        const session = JSON.parse(sessionJson);
        storeSession(session);
...
addActionHandler('initShared', () => {
    startWebsync();
});
addActionHandler('initMain', (global) => {
...
addCallback((global) => {
    ...
    requestMutation(() => {
        document.documentElement.style.setProperty('--composer-text-size', `${Math.max(messageTextSize, IS_IOS ? 16 : 15)}px`);
...
addActionHandler('clearAuthErrorKey', (global) => {
    return {
        ...global,
        authErrorKey: undefined,
    };
});
addActionHandler('disableHistoryAnimations', (global, actions, payload) => {
