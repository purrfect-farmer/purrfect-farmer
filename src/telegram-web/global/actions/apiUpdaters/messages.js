...
import { buildCollectionByKey, omit, pickTruthy, unique, } from '../../../util/iteratees';
import { getMessageKey, isLocalMessageId } from '../../../util/keys/messageKey';
import { notifyAboutMessage } from '../../../util/notifications';
import { onTickEnd } from '../../../util/schedulers';
import { getServerTime } from '../../../util/serverTime';
import { addPaidReaction, checkIfHasUnreadReactions, getIsSavedDialog, getMessageContent, getMessageText, isActionMessage, isMessageLocal, } from '../../helpers';
import { getMessageReplyInfo, getStoryReplyInfo } from '../../helpers/replies';
import { addActionHandler, getGlobal, setGlobal, } from '../../index';
import { addMessages, addViewportId, clearMessageTranslation, deleteChatMessages, deleteChatScheduledMessages, deletePeerPhoto, deleteQuickReply, deleteQuickReplyMessages, deleteTopic, removeChatFromChatLists, replaceThreadParam, updateChat, updateChatLastMessageId, updateChatMediaLoadingState, updateChatMessage, updateListedIds, updateMessageTranslations, updatePeerFullInfo, updatePoll, updatePollVote, updateQuickReplies, updateQuickReplyMessage, updateScheduledMessage, updateThreadInfo, updateThreadInfos, updateThreadUnreadFromForwardedMessage, updateTopic, } from '../../reducers';
import { updateUnreadReactions } from '../../reducers/reactions';
import { updateTabState } from '../../reducers/tabs';
import { selectCanAnimateSnapEffect, selectChat, selectChatLastMessageId, selectChatMessage, selectChatMessages, selectChatScheduledMessages, selectCommonBoxChatId, selectCurrentMessageList, selectFirstUnreadId, selectIsChatListed, selectIsChatWithSelf, selectIsMessageInCurrentMessageList, selectIsServiceChatReady, selectIsViewportNewest, selectListedIds, selectPerformanceSettingsValue, selectPinnedIds, selectSavedDialogIdFromMessage, selectScheduledIds, selectScheduledMessage, selectTabState, selectThreadByMessage, selectThreadIdFromMessage, selectThreadInfo, selectTopic, selectTopicFromMessage, selectViewportIds, } from '../../selectors';
...
addActionHandler('apiUpdate', (global, actions, update) => {
    switch (update['@type']) {
        case 'newMessage': {
            const { chatId, id, message, shouldForceReply, wasDrafted, poll, } = update;
        ...
        case 'updateStartEmojiInteraction': {
            Object.values(global.byTabId).forEach(({ id: tabId }) => {
                ...
                if (getMessageText(message)?.text !== update.emoji)
                    return;
        ...
        case 'updateVideoProcessingPending': {
            ...
            Object.values(global.byTabId).forEach(({ id: tabId }) => {
                const currentMessageList = selectCurrentMessageList(global, tabId);
                if (currentMessageList?.chatId !== chatId)
                    return;
                const now = Date.now();
        ...
        case 'deleteScheduledMessages': {
            ...
            const hadVideoProcessing = ids?.some((id) => (selectScheduledMessage(global, chatId, id)?.isVideoProcessingPending));
        ...
        case 'updateMessageExtendedMedia': {
            ...
            if (message.content.paidMedia) {
                const paidMediaUpdate = isBought ? { isBought, extendedMedia }
                    : { extendedMedia: extendedMedia, isBought: undefined };
                global = updateChatMessage(global, chatId, id, {
                    content: {
                        ...message.content,
                        paidMedia: {
...
export function updateWithLocalMedia(global, chatId, id, messageUpdate, isScheduled = false) {
    const currentMessage = isScheduled
        ? selectScheduledMessage(global, chatId, id)
        : selectChatMessage(global, chatId, id);
    // Preserve locally uploaded media.
    if (currentMessage && messageUpdate.content && !isLocalMessageId(id)) {
        const { photo, video, sticker, document, } = getMessageContent(currentMessage);
...
function findLastMessage(global, chatId, threadId = MAIN_THREAD_ID) {
...
export function deleteParticipantHistory(global, chatId, peerId, actions) {
    const byId = selectChatMessages(global, chatId);
...
export function deleteThread(global, chatId, threadId, actions) {
    const byId = selectChatMessages(global, chatId);
    if (!byId) {
        return;
    }
...
export function deleteMessages(global, chatId, ids, actions) {
    // Channel update
    if (chatId) {
        const chat = selectChat(global, chatId);
        if (!chat)
...
function deleteScheduledMessages(chatId, ids, actions, global) {
    ...
    setTimeout(() => {
        ...
        global = replaceThreadParam(global, chatId, MAIN_THREAD_ID, 'scheduledIds', Object.keys(scheduledMessages || {}).map(Number));
        setGlobal(global);
    }, isAnimatingAsSnap ? SNAP_ANIMATION_DELAY : ANIMATION_DELAY);
}
