...
import { addBlockedUser, addChats, addStoriesForPeer, addUsers, removeBlockedUser, removePeerStory, setConfirmPaymentUrl, setPaymentStep, updateLastReadStoryForPeer, updatePeerStory, updatePeersWithStories, updatePoll, updateStealthMode, updateThreadInfos, } from '../../reducers';
import { updateTabState } from '../../reducers/tabs';
import { selectPeer, selectPeerStories, selectPeerStory, selectTabState, } from '../../selectors';
addActionHandler('apiUpdate', (global, actions, update) => {
    switch (update['@type']) {
        case 'updateEntities': {
            const { users, chats, threadInfos, polls, } = update;
            if (users)
                global = addUsers(global, users);
            if (chats)
                global = addChats(global, chats);
            if (threadInfos)
                global = updateThreadInfos(global, threadInfos);
            if (polls) {
                polls.forEach((poll) => {
        ...
        case 'updateMoveStickerSetToTop': {
            ...
            if (!oldOrder?.some((id) => id === update.id))
                return global;
        ...
        case 'newMessage': {
            ...
            if (!update.message.isOutgoing && update.message.chatId !== SERVICE_NOTIFICATIONS_USER_ID)
                return undefined;
