...
function loadWindowUrl(window, url, hash) {
...
export function setupElectronActionHandlers() {
  ipcMain.handle(ElectronAction.OPEN_NEW_WINDOW, (_, url) => {
    createWindow(url);
  });
  ipcMain.handle(ElectronAction.SET_WINDOW_TITLE, (_, newTitle) => {
  ...
  ipcMain.handle(ElectronAction.SET_WINDOW_BUTTONS_POSITION, (_, position) => {
    if (!IS_MAC_OS) {
      return;
    }
    getCurrentWindow()?.setWindowButtonPosition(
      WINDOW_BUTTONS_POSITION[position]
    );
  });
  ipcMain.handle(
    ElectronAction.SET_IS_AUTO_UPDATE_ENABLED,
    async (_, isAutoUpdateEnabled) => {
      if (IS_PREVIEW) {
        return;
      }
  ...
  ipcMain.handle(
    ElectronAction.SET_IS_TRAY_ICON_ENABLED,
    (_, isTrayIconEnabled) => {
  ...
  ipcMain.handle(ElectronAction.RESTORE_LOCAL_STORAGE, () =>
    restoreLocalStorage()
  );
}
