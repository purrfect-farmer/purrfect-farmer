...
import { bufferXor, generateKeyDataFromNonce, generateRandomBytes, getByteArray, modExp, readBigIntFromBuffer, readBufferFromBigInt, sha1, sha256, toSignedLittleBuffer, } from '../Helpers';
const RETRIES = 20;
export async function doAuthentication(sender, log) {
    ...
    const serverDhParams = await sender.send(new Api.ReqDHParams({
        nonce: resPQ.nonce,
        serverNonce: resPQ.serverNonce,
        p: pBuffer,
        q: qBuffer,
    ...
    const { key, iv } = await generateKeyDataFromNonce(resPQ.serverNonce, newNonce);
    ...
    const dhGen = await sender.send(new Api.SetClientDHParams({
        nonce: resPQ.nonce,
        serverNonce: resPQ.serverNonce,
