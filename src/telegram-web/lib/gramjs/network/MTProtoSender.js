...
import { BadMessageError, InvalidBufferError, SecurityError, TypeNotFoundError, } from '../errors/Common';
...
export default class MTProtoSender {
    ...
    authKey;
    _state;
    _getShouldDebugExportedSenders;
    _handlers;
    _onConnectionBreak;
    userDisconnected;
    isConnecting = false;
    ...
    */
    ...
    _handleStateForgotten(message) {
        this._sendQueue.append(new RequestState(new Api.MsgsStateInfo({
    ...
    async _reconnect() {
        ...
        try {
            ...
            if (currentConnection)
                this._disconnect(currentConnection);
            if (currentFallbackConnection)
                this._disconnect(currentFallbackConnection);
        }
        catch (err) {
            this._log.warn(err);
        }
