...
export default function createMockedDialog(id, mockData) {
    const dialog = mockData.dialogs.active.find((d) => d.id === id)
        || mockData.dialogs.archived.find((d) => d.id === id);
    if (!dialog)
        throw Error('No such dialog');
    const { unreadMentionsCount = 0, unreadReactionsCount = 0, readInboxMaxId = 0, readOutboxMaxId = 0, unreadCount = 0, topMessage = 0, } = dialog;
