...
import { FloodTestPhoneWaitError, FloodWaitError, MsgWaitError, NetworkMigrateError, PhoneMigrateError, RPCError, ServerError, TimedOutError, UserMigrateError, } from '../errors';
import { Logger } from '../extensions';
import { ConnectionTCPObfuscated, HttpConnection, MTProtoSender, UpdateConnectionState, } from '../network';
import { Api } from '../tl';
...
class TelegramClient {
    ...
    defaultDcId;
    _useIPV6;
    _shouldForceHttpTransport;
    _shouldAllowHttpTransport;
    ...
    _requestRetries;
    _connectionRetries;
    _connectionRetriesToFallback;
    _retryDelay;
    _retryMainConnectionDelay;
    _timeout;
    _autoReconnect;
    _config;
    ...
    _destroyed = false;
    _connectedDeferred = new Deferred();
    pingCallback;
    _initWith;
    isPremium = false;
    ...
    async connect() {
        ...
        if (mediaSender)
            this.releaseExportedSender(mediaSender);
    }
    async _initSession() {
    ...
    downloadStickerSetThumb(stickerSet) {
        ...
        if (!stickerSet.thumbDocumentId) {
            return this.downloadFile(new Api.InputStickerSetThumb({
                stickerset: new Api.InputStickerSetID({
                    id: stickerSet.id,
                    accessHash: stickerSet.accessHash,
                }),
                thumbVersion,
            }), { dcId: stickerSet.thumbDcId }); // Sticker thumb cannot be larger than 2GB, right?
        }
        ...
        for (attempt = 0; attempt < this._requestRetries; attempt++) {
            ...
            try {
                ...
                if (e instanceof ServerError
                    || (e instanceof RPCError && (e.errorMessage === 'RPC_CALL_FAIL'
    ...
    async getMe() {
        try {
            return (await this.invoke(new Api.users.GetUsers({
