 ...
 */
export async function updateTwoFaSettings(client, { isCheckPassword, currentPassword, newPassword, hint = '', email, emailCodeCallback, onEmailCodeError, }) {
...
export async function getTmpPassword(client, currentPassword, ttl = 60) {
    const pwd = await client.invoke(new Api.account.GetPassword());
    if (!pwd) {
        return undefined;
    }
...
export async function getCurrentPassword(client, currentPassword) {
    const pwd = await client.invoke(new Api.account.GetPassword());
    if (!pwd) {
        return undefined;
    }
