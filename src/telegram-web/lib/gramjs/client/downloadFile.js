...
export async function downloadFile(client, inputLocation, fileParams, shouldDebugExportedSenders) {
    ...
    for (let i = 0; i < SENDER_RETRIES; i++) {
        try {
            ...
            if (err instanceof RPCError && (err.errorMessage.startsWith('SESSION_REVOKED')
                || err.errorMessage.startsWith('CONNECTION_NOT_INITED')) && i < SENDER_RETRIES - 1) {
...
async function downloadFile2(client, inputLocation, fileParams, shouldDebugExportedSenders) {
    ...
    const { fileSize, dcId, progressCallback, isPriority, start = 0, } = fileParams;
