...
function generateSchema(input, output, reducedMethods) {
  let apiTl = fs.readFileSync(path.resolve(DIR_NAME, input), "utf-8");
  if (reducedMethods) {
    apiTl = stripTl(apiTl);
    const methodList = JSON.parse(
      fs.readFileSync(path.resolve(DIR_NAME, reducedMethods), "utf-8")
    );
...
function main() {
  ...
  generateSchema(
    "./static/api.tl",
    "./apiTl.js",
    !FULL_SCHEMA ? "./static/api.json" : undefined
  );
  generateSchema(
    "./static/schema.tl",
    "./schemaTl.js",
    !FULL_SCHEMA ? "./static/schema.json" : undefined
  );
}
function stripTl(tl) {
  return tl
