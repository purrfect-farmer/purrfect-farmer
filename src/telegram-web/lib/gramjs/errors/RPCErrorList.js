...
import { BadRequestError, FloodError, InvalidDCError, RPCError, TimedOutError, } from './RPCBaseErrors';
...
export class MsgWaitError extends FloodError {
    constructor(args) {
        super(`Message failed to be sent.${RPCError._fmtRequest(args.request)}`, args.request);
...
export class EmailUnconfirmedError extends BadRequestError {
    ...
    constructor(args) {
        ...
        super(`Email unconfirmed, the length of the code must be ${codeLength}${RPCError._fmtRequest(args.request)}`, args.request, 400);
...
export const rpcErrorRe = new Map([
