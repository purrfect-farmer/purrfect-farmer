...
function render($element, parentEl) {
  ...
  if (import.meta.env.VITE_TW_APP_ENV === "perf") {
...
function renderWithVirtual(
  ...
  const isCurrentFragment =
    !isCurrentComponent && $current?.type === VirtualType.Fragment;
...
function mountChildren(parentEl, $element, currentContext, options) {
  ...
  if ($element.type === VirtualType.Fragment && children.length === 0) {
    ...
    insertBefore(
      options.fragment || parentEl,
      fragmentEl.placeholderTarget,
      options.nextSibling
    );
    return;
  }
...
function unmountRealTree($element) {
  if ($element.type === VirtualType.Component) {
    ...
    if (fragment.placeholderTarget && fragment.children.length === 0) {
      fragment.placeholderTarget.parentNode?.removeChild(
        fragment.placeholderTarget
      );
...
function getNextSibling($current) {
  if (
    ...
    $current.type === VirtualType.Fragment
  ) {
...
function renderChildren(
  ...
  if (
    ...
    if (
      currentFragment.children.length === 0 &&
      newFragment.children.length > 0 &&
      currentFragment.placeholderTarget
    ) {
      nextSibling = currentFragment.placeholderTarget.nextSibling || undefined;
      // Remove the placeholder as we're adding real content
      currentFragment.placeholderTarget.parentNode?.removeChild(
        currentFragment.placeholderTarget
      );
    ...
    if (
      ...
      const lastCurrentChild =
        currentFragment.children[currentFragment.children.length - 1];
      ...
      insertBefore(
        currentEl,
        newFragment.placeholderTarget,
        siblingAfterFragment
      );
    }
  }
  ...
  const fragmentNextSibling =
    fragment &&
    (nextSibling ||
      (lastCurrentChild ? getNextSibling(lastCurrentChild) : undefined));
  for (let i = 0; i < maxLength; i++) {
    const $renderedChild = renderWithVirtual(
      ...
      i >= currentChildrenLength
        ? { fragment, namespace }
        : { nextSibling, forceMoveToEnd, namespace }
    );
...
function processControlled(tag, props) {
  ...
  const { value, checked, onInput, onChange, onBlur } = props;
  ...
  props.onInput = (e) => {
    ...
    if (value !== undefined && value !== e.currentTarget.value) {
      ...
      if (
        ...
        e.currentTarget.dataset[SELECTION_STATE_ATTRIBUTE] =
  ...
  if (key === "className") {
    ...
    if (inputEl.value !== value) {
      ...
      if (selectionStateJson) {
        const { selectionStart, selectionEnd, isCaretAtEnd } =
...
export function addExtraClass(element, className) {
...
export function removeExtraClass(element, className) {
...
export function toggleExtraClass(element, className, force) {
  if (force === true) {
