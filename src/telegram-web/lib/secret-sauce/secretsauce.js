...
import buildSdp from './buildSdp';
import { black, silence } from './blacksilence';
import { getAmplitude, IS_ECHO_CANCELLATION_SUPPORTED, IS_NOISE_SUPPRESSION_SUPPORTED, THRESHOLD, } from './utils';
import Deferred from "../../util/Deferred";
import safePlay from "../../util/safePlay";
const DEFAULT_MID = 3;
let state;
export async function getDevices(streamType, isInput = true) {
...
export function getUserStreams(userId) {
...
export function setVolume(userId, volume) {
    ...
    if (!participantFunctions)
        return;
    participantFunctions.setVolume?.(volume);
}
export function isStreamEnabled(streamType, userId) {
...
export async function toggleStream(streamType, value = undefined) {
    ...
    try {
        ...
        if (streamType === 'presentation' && !value)
...
function analyzeAmplitudes() {
    if (!state || !state.participantFunctions)
        return;
...
export async function handleUpdateGroupCallParticipants(updatedParticipants) {
    if (!state) {
        return;
    }
    const { participants, conference, connection, myId, } = state;
...
// }
export async function handleUpdateGroupCallConnection(data, isPresentation) {
    if (!state) {
        return;
    }
...
function initializeConnection(streams, resolve, isPresentation = false) {
    ...
    connection.onnegotiationneeded = async () => {
        ...
        if (isPresentation) {
            if (videoSsrc)
                ssrcs.push(videoSsrc);
            if (audioSsrc)
                ssrcs.push(audioSsrc);
        }
        else {
            if (audioSsrc)
                ssrcs.push(audioSsrc);
...
export async function startSharingScreen() {
    if (!state) {
        return undefined;
    }
    try {
...
export function joinGroupCall(myId, audioContext, audioElement, onUpdate) {
    if (state) {
