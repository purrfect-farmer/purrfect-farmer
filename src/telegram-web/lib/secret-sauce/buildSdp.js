...
export default (conference, isAnswer = false, isPresentation = false, isP2p = false) => {
    ...
    const { sessionId, ssrcs, audioExtensions, videoExtensions, audioPayloadTypes, videoPayloadTypes, transport: { ufrag, pwd, fingerprints, candidates, }, } = conference;
    // Header
    add('v=0'); // version
    ...
    const addPayloadType = (payloadType) => {
        const { channels, id, name, clockrate, parameters, } = payloadType;
