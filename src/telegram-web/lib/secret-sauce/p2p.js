...
import { fromTelegramSource, IS_ECHO_CANCELLATION_SUPPORTED, IS_NOISE_SUPPRESSION_SUPPORTED, p2pPayloadTypeToConference, } from './utils';
...
export async function toggleStreamP2p(streamType, value = undefined) {
    if (!state)
        return;
...
export async function joinPhoneCall(connections, emitSignalingData, isOutgoing, shouldStartVideo, isP2p, onUpdate) {
...
function sendInitialSetup(sdp) {
    ...
    if (!sdp.ssrc || !sdp['ssrc-groups'] || !sdp['ssrc-groups'][0] || !sdp['ssrc-groups'][1])
        return;
    emitSignalingData({
        '@type': 'InitialSetup',
...
export async function processSignalingMessage(message) {
    ...
    switch (message['@type']) {
        ...
        case 'InitialSetup': {
            const { connection, isOutgoing, } = state;
            if (!connection)
                return;
            const newConference = {
                transport: {
            ...
            if (!isOutgoing) {
                ...
                sendInitialSetup(parseSdp(connection.localDescription, true));
            }
...
async function createOffer(conn, params) {
    ...
    sendInitialSetup(parseSdp(conn.localDescription, true));
}
