...
export default function useTopOverscroll(containerRef, onOverscroll, onReset, isDisabled) {
    const overscrollTriggerRef = useRef();
    ...
    const enableOverscrollTrigger = useLastCallback((noScrollInertiaStop = false) => {
        if (isTriggerEnabledRef.current)
            return;
        if (!overscrollTriggerRef.current || !containerRef.current)
            return;
    ...
    const handleScroll = useLastCallback(() => {
        ...
        if (isMovingUp && ((lastIsOnTopRef.current && lastEventDelay > INERTIA_THRESHOLD)
    ...
    return !IS_IOS && !isDisabled ? (<div ref={overscrollTriggerRef} className="overscroll-trigger" key="overscroll-trigger"/>) : undefined;
}
