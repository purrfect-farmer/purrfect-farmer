...
const useChatContextActions = ({ chat, user, folderId, isPinned, isMuted, canChangeFolder, isSavedDialog, currentUserId, isPreview, topics, handleDelete, handleMute, handleChatFolderChange, handleReport, }, isInSearch = false) => {
    const lang = useLang();
    const { isSelf } = user || {};
    ...
    return useMemo(() => {
        ...
        const { toggleChatPinned, toggleSavedDialogPinned, updateChatMutedState, toggleChatArchived, markChatMessagesRead, markChatUnread, openChatInNewTab, } = getActions();
        ...
        if (isSavedDialog) {
            return compact([actionOpenInNewTab, actionPin, actionDelete]);
        }
        ...
        const actionMaskAsRead = (chat.unreadCount || chat.hasUnreadMark || Object.values(topics || {}).some(({ unreadCount }) => unreadCount)) ? {
