...
export default function useCollapsibleLines(ref, maxLinesBeforeCollapse, cutoutRef, isDisabled) {
    const isFirstRenderRef = useRef(true);
    const cutoutHeightRef = useRef();
    const [isCollapsible, setIsCollapsible] = useState(!isDisabled);
    const [isCollapsed, setIsCollapsed] = useState(isCollapsible);
    useLayoutEffect(() => {
        const element = (cutoutRef || ref).current;
        if (isDisabled || !element || isFirstRenderRef.current)
            return;
        requestMutation(() => {
    ...
    const debouncedRecalcTextLines = useDebouncedCallback(() => requestMeasure(recalculateTextLines), [recalculateTextLines], WINDOW_RESIZE_LINE_RECALC_DEBOUNCE);
    useLayoutEffect(() => {
