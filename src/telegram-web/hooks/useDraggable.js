...
export default function useDraggable(ref, dragHandleElementRef, isDragEnabled = true, originalSize, isFullscreen = false, minimumSize = { width: 0, height: 0 }, cachedPosition) {
    const [elementCurrentPosition, setElementCurrentPosition] = useState(cachedPosition);
    ...
    const [hitResizeHandle, setHitResizeHandle] = useState(undefined);
    ...
    const updateCurrentPosition = useLastCallback((position) => {
        if (!isFullscreen)
            setElementCurrentPosition({ x: position.x, y: position.y });
    });
    ...
    const handleStartDrag = useLastCallback((event) => {
    ...
    function getResizeHandleFromTarget(targetElement) {
        ...
        if (!closest(RESIZE_HANDLE_SELECTOR))
            return undefined;
    ...
    const handleStartResize = useLastCallback((event) => {
        if (event instanceof MouseEvent && event.button !== 0) {
            return;
        }
        const targetElement = event.target;
    ...
    const ensurePositionInVisibleArea = (x, y) => {
        ...
        if (newY < 0)
            newY = 0;
        if (newX + componentWidth > visibleAreaWidth)
            newX = visibleAreaWidth - componentWidth;
        if (newY + componentHeight > visibleAreaHeight)
            newY = visibleAreaHeight - componentHeight;
    ...
    const adjustPositionWithinBounds = useLastCallback(() => {
        ...
        if (!elementCurrentSize || !position)
            return;
        const newPosition = ensurePositionInVisibleArea(position.x, position.y);
        updateCurrentPosition(newPosition);
    });
    const ensureSizeInVisibleArea = useLastCallback((sizeForCheck) => {
    ...
    const handleDrag = useLastCallback((event) => {
        ...
        if (elementCurrentSize)
            setElementCurrentPosition(ensurePositionInVisibleArea(newX, newY));
    });
    const handleResize = useLastCallback((event) => {
        if (!isResizing || !element || hitResizeHandle === undefined)
            return;
        const { x, y } = getPointerPosition(event);
    ...
    const style = buildStyle(`left: ${actualPosition.x}px;`, `top: ${actualPosition.y}px;`, !isFullscreen && `max-width: ${elementCurrentSize.width}px;`, !isFullscreen && `max-height: ${elementCurrentSize.height}px;`, 'position: fixed;', (isDragging || isResizing || isWindowsResizing) && 'transition: none !important;', cursorStyle);
    return {
