...
export default function useMultiaccountInfo(currentUser) {
    ...
    const updateAvatar = useLastCallback(async (url, abortSignal) => {
    ...
    useEffect(() => {
        if (!avatarUrl || !isUpdater || !isSynced)
            return undefined;
    ...
    useEffect(() => {
        function handleStorageChange(e) {
            if (!e.key?.startsWith(SESSION_ACCOUNT_PREFIX))
                return;
            refresh();
        }
