...
const useShowTransitionDeprecated = (isOpen = false, onCloseTransitionEnd, noFirstOpenTransition = false, className = 'fast', noCloseTransition = false, closeDuration = CLOSE_DURATION, noOpenTransition = false) => {
    ...
    const transitionClassNames = buildClassName(className && 'opacity-transition', className, shouldHaveOpenClassName && 'open', !shouldHaveOpenClassName && 'not-open', shouldRender && 'shown', !shouldRender && 'not-shown', isClosing && 'closing');
    return {
        shouldRender,
