...
import { getAudioHasCover, getChatAvatarHash, getChatTitle, getMediaHash, getMessageContent, } from '../global/helpers';
...
const useMessageMediaMetadata = (message, sender, chat) => {
    ...
    const { audio, voice } = message ? getMessageContent(message) : {};
...
function makeGoodArtwork(url, size) {
    if (!url)
        return Promise.resolve(telegramLogoPath);
    if (!size)
        return resizeImage(url, MINIMAL_SIZE, MINIMAL_SIZE);
...
function getCoverSize(audio, voice, url) {
    if (!url)
        return LOGO_DIMENSIONS;
    if (audio) {
        if (!audio.thumbnailSizes?.length)
            return undefined;
