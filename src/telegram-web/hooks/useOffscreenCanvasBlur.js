...
export default function useOffscreenCanvasBlur(thumbData, // data URI or blob URL
isDisabled = false) {
    const canvasRef = useRef();
    const workerIndex = useMemo(() => cycleRestrict(MAX_WORKERS, ++lastWorkerIndex), []);
    const offscreenRef = useRef();
    const blurThumb = useLastCallback(async (canvas, uri) => {
    ...
    useLayoutEffect(() => {
        if (!thumbData || isDisabled || offscreenRef.current)
            return;
