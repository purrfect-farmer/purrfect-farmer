...
export default function useMenuPosition(isOpen, containerRef, bubbleRef, options) {
...
function applyStaticOptions(containerRef, bubbleRef, { positionX = 'left', positionY = 'top', transformOriginX, transformOriginY, style, bubbleStyle, }) {
...
function processDynamically(containerRef, bubbleRef, { anchor, getRootElement, getMenuElement, getTriggerElement, getLayout, withMaxHeight, }) {
    ...
    const { extraPaddingX = 0, extraTopPadding = 0, extraMarginTop = 0, topShiftY = 0, menuElMinWidth = 0, deltaX = 0, shouldAvoidNegativePosition = false, withPortal = false, isDense = false, } = getLayout?.() || {};
