...
import { getMediaFileSize, getMediaFormat, getMediaHash, getMediaThumbUri, getPhotoFullDimensions, getProfilePhotoMediaHash, getVideoDimensions, getVideoProfilePhotoMediaHash, isDocumentPhoto, isDocumentVideo, } from '../../../global/helpers';
...
export const useMediaProps = ({ media, isAvatar, origin, delay, }) => {
    ...
    const getMediaOrAvatarHash = useMemo(() => (isFull) => {
        if (!media)
            return undefined;
    ...
    const pictogramBlobUrl = useMedia(media
        // Only use pictogram if it's already loaded
        && (isFromSharedMedia || isFromSearch || isDocument)
        && getMediaHash(media, 'pictogram'), undefined, ApiMediaFormat.BlobUrl, delay);
    const previewMediaHash = getMediaOrAvatarHash();
    const previewBlobUrl = useMedia(previewMediaHash, undefined, ApiMediaFormat.BlobUrl, delay);
    const { mediaData: fullMediaBlobUrl, loadProgress, } = useMediaWithLoadProgress(getMediaOrAvatarHash(true), undefined, media && getMediaFormat(media, 'full'), delay);
