...
let cancelAnimation;
const SeekLine = ({ duration, bufferedRanges, isReady, posterSize, playbackRate, url, isActive, isPlaying, isPreviewDisabled, onSeek, onSeekStart, }) => {
    ...
    const setPreview = useLastCallback((time) => {
        time = Math.floor(time);
        setPreviewTime(time);
        renderVideoPreview(time);
    });
    useEffect(() => {
        if (isPreviewDisabled || !url || !isReady)
            return undefined;
    ...
    useEffect(() => {
        ...
        const getPreviewProps = (e) => {
            ...
            if (isPreviewDisabled)
                return [t, 0];
            if (!seekerSize.width)
                seekerSize = seeker.getBoundingClientRect();
            const preview = previewRef.current;
            const o = clamp(pageX - seekerSize.left - preview.clientWidth / 2, -4, seekerSize.width - preview.clientWidth + 4);
            return [t, o];
        };
        const stopAnimation = () => {
            if (cancelAnimation)
    ...
    return (<div className={styles.container} ref={seekerRef}>
      {!isPreviewDisabled && (<ShowTransition isOpen isHidden={!isPreviewVisible} className={styles.preview} style={`width: ${previewSize.width}px; height: ${previewSize.height}px`} ref={previewRef}>
      ...
      <div className={styles.track}>
        {bufferedRanges.map(({ start, end, }) => (<div key={`${start}-${end}`} className={styles.buffered} 
        // @ts-ignore
        style={`left: ${start * 100}%; right: ${100 - end * 100}%`}/>))}
      </div>
      <div className={styles.track}>
        <div ref={progressRef} className={buildClassName(styles.played, isSeeking && styles.seeking)}/>
      </div>
    </div>);
};
