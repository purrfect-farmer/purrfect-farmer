...
const SenderInfo = ({ owner, item, }) => {
    ...
    const subtitle = useMemo(() => {
        ...
        const parts = [];
        if (avatar) {
            const chat = !isUserId(avatarOwner.id) ? avatarOwner : undefined;
            const isChannel = chat && isChatChannel(chat);
            const isGroup = chat && isChatGroup(chat);
            parts.push(lang(isPersonalAvatar ? 'lng_mediaview_profile_photo_by_you'
    ...
    return (<div className="SenderInfo" onClick={handleFocusMessage}>
...
export default withGlobal((global, { item }) => {
