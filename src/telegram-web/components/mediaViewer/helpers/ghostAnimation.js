...
import { calculateDimensions, getMediaViewerAvailableDimensions, MEDIA_VIEWER_MEDIA_QUERY, REM, } from '../../common/helpers/mediaDimensions';
const ANIMATION_DURATION = 200;
export function animateOpening(hasFooter, origin, bestImageData, dimensions, isVideo, message, mediaIndex) {
    const { mediaEl: fromImage } = getNodes(origin, message, mediaIndex);
    if (!fromImage) {
        return;
    }
    const { width: windowWidth } = windowSize.get();
    const { width: availableWidth, height: availableHeight, } = getMediaViewerAvailableDimensions(hasFooter, isVideo);
    const { width: toWidth, height: toHeight } = calculateDimensions(availableWidth, availableHeight, dimensions.width, dimensions.height);
    const toLeft = (windowWidth - toWidth) / 2;
    const toTop = getTopOffset(hasFooter) + (availableHeight - toHeight) / 2;
    let { top: fromTop, left: fromLeft, width: fromWidth, height: fromHeight, } = fromImage.getBoundingClientRect();
    if ([
...
export function animateClosing(origin, bestImageData, message, mediaIndex) {
    ...
    const { top: fromTop, left: fromLeft, width: fromWidth, height: fromHeight, } = fromImage.getBoundingClientRect();
...
function createGhost(source, origin) {
    ...
    if (origin === MediaViewerOrigin.ProfileAvatar || origin === MediaViewerOrigin.SettingsAvatar) {
        ...
        const profileInfo = document.querySelector(origin === MediaViewerOrigin.ProfileAvatar
...
function getNodes(origin, message, index) {
    ...
    const mediaEls = mediaSelector
        ? container?.querySelectorAll(mediaSelector) : undefined;
    return {
        container,
        mediaEl: mediaEls?.[0],
    };
