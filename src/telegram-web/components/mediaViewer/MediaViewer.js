...
import { getChatMediaMessageIds, getMessagePaidMedia, isChatAdmin, } from '../../global/helpers';
import { selectChatMessage, selectChatMessages, selectChatScheduledMessages, selectCurrentChatMediaSearch, selectCurrentSharedMediaSearch, selectIsChatWithSelf, selectListedIds, selectOutlyingListByMessageId, selectPeer, selectPeerPhotos, selectPerformanceSettingsValue, selectScheduledMessage, selectSponsoredMessage, selectTabState, } from '../../global/selectors';
...
import getViewableMedia, { getMediaViewerItem } from './helpers/getViewableMedia';
...
const MediaViewer = ({ chatId, threadId, messageId, message, collectedMessageIds, isChatWithSelf, canUpdateMedia, origin, avatar, avatarOwner, profilePhotos, chatMessages, sponsoredMessage, standaloneMedia, mediaIndex, withAnimation, isHidden, shouldSkipHistoryAnimations, withDynamicLoading, isLoadingMoreMedia, isSynced, }) => {
    const { openMediaViewer, closeMediaViewer, openForwardMenu, focusMessage, toggleChatInfo, searchChatMediaMessages, loadMoreProfilePhotos, clickSponsored, openUrl, } = getActions();
    ...
    const prevSenderId = usePreviousDeprecated(senderId);
    ...
    const { isVideo, isPhoto, bestImageData, bestData, dimensions, isGif, isFromSharedMedia, } = useMediaProps({
        media, isAvatar: Boolean(avatarOwner), origin, delay: isGhostAnimation && ANIMATION_DURATION,
    });
    const canReportAvatar = (() => {
        if (isChatWithSelf)
            return false;
        if (currentItem?.type !== 'avatar' || !avatarOwner)
            return false;
        const info = currentItem.profilePhotos;
        if (media === info.personalPhoto)
            return false;
        return true;
    })();
    ...
    const prevMessage = usePreviousDeprecated(message);
    ...
    const handleFooterClick = useLastCallback((e) => {
        if (e.target instanceof HTMLElement && e.target.closest('a'))
            return; // Prevent closing on timestamp click
        handleClose();
        if (!chatId || !messageId)
            return;
        if (isMobile) {
    ...
    const handleSponsoredClick = useLastCallback((isFromMedia) => {
        if (!sponsoredMessage || !chatId)
            return;
    ...
    const loadMoreItemsIfNeeded = useLastCallback((item) => {
        if (!item || isLoadingMoreMedia)
            return;
        if (item.type === 'avatar') {
    ...
    const getNextItem = useLastCallback((from, direction) => {
        if (direction === 0 || isSingle)
            return undefined;
    ...
    const openMediaViewerItem = useLastCallback((item) => {
        if (!item) {
            handleClose();
            return;
        }
    ...
    return (<ShowTransition id="MediaViewer" isOpen={isOpen} isHidden={isHidden} shouldAnimateFirstRender noCloseTransition={shouldSkipHistoryAnimations}>
      <div className="media-viewer-head" dir={lang.isRtl ? 'rtl' : undefined} ref={headerRef}>
        {isMobile && (<Button className="media-viewer-close" round size="smaller" color="translucent-white" ariaLabel={lang('Close')} onClick={handleClose}>
            <Icon name="close"/>
          </Button>)}
        <Transition activeKey={animationKey.current} name={headerAnimation}>
          <SenderInfo key={media?.id} item={currentItem}/>
        </Transition>
        <MediaViewerActions mediaData={bestData} isVideo={isVideo} item={currentItem} canUpdateMedia={canUpdateMedia} canReportAvatar={canReportAvatar} onBeforeDelete={handleBeforeDelete} onReportAvatar={openReportAvatarModal} onCloseMediaViewer={handleClose} onForward={handleForward}/>
        <ReportAvatarModal isOpen={isReportAvatarModalOpen} onClose={closeReportAvatarModal} photo={avatar} peerId={avatarOwner?.id}/>
      </div>
      <MediaViewerSlides item={currentItem} loadMoreItemsIfNeeded={loadMoreItemsIfNeeded} isLoadingMoreMedia={isLoadingMoreMedia} isSynced={isSynced} getNextItem={getNextItem} chatId={chatId} isPhoto={isPhoto} isGif={isGif} threadId={threadId} avatarOwnerId={avatarOwner?.id} origin={origin} isOpen={isOpen} hasFooter={hasFooter} isVideo={isVideo} withAnimation={withAnimation} onClose={handleClose} selectItem={openMediaViewerItem} isHidden={isHidden} onFooterClick={handleFooterClick} handleSponsoredClick={handleSponsoredClick}/>
    </ShowTransition>);
};
export default memo(withGlobal((global) => {
    const { mediaViewer, shouldSkipHistoryAnimations } = selectTabState(global);
    const { chatId, threadId, messageId, origin, isHidden, withDynamicLoading, standaloneMedia, mediaIndex, isAvatarView, isSponsoredMessage, } = mediaViewer;
    ...
    if (isAvatarView) {
        ...
        if (peer) {
            canUpdateMedia = isUserId(peer.id) ? peer.id === currentUserId : isChatAdmin(peer);
        }
