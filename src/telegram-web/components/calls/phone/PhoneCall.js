...
import { memo, useCallback, useEffect, useMemo, useRef, } from '../../../lib/teact/teact';
import { getActions, withGlobal } from '../../../global';
import { getStreams, IS_SCREENSHARE_SUPPORTED, switchCameraInputP2p, toggleStreamP2p, } from '../../../lib/secret-sauce';
import { selectTabState } from '../../../global/selectors';
import { selectPhoneCallUser } from '../../../global/selectors/calls';
import { IS_ANDROID, IS_IOS, IS_REQUEST_FULLSCREEN_SUPPORTED, } from '../../../util/browser/windowEnvironment';
...
const PhoneCall = ({ user, isOutgoing, phoneCall, isCallPanelVisible, }) => {
    const lang = useOldLang();
    const { hangUp, requestMasterAndAcceptCall, playGroupCallSound, toggleGroupCallPanel, connectToActivePhoneCall, } = getActions();
    ...
    useEffect(() => {
        if (!IS_REQUEST_FULLSCREEN_SUPPORTED)
            return undefined;
    ...
    return (<Modal isOpen={phoneCall && phoneCall?.state !== 'discarded' && !isCallPanelVisible} onClose={handleClose} className={buildClassName(styles.root, isMobile && styles.singleColumn)} dialogRef={containerRef}>
      <Avatar peer={user} size="jumbo" className={hasVideo || hasPresentation ? styles.blurred : ''}/>
      ...
      <video className={buildClassName(styles.secondVideo, !isHidingPresentation && hasOwnPresentation && styles.visible, isFullscreen && styles.fullscreen)} muted autoPlay playsInline srcObject={streams?.ownPresentation}/>
      <video className={buildClassName(styles.secondVideo, !isHidingVideo && hasOwnVideo && styles.visible, isFullscreen && styles.fullscreen)} muted autoPlay playsInline srcObject={streams?.ownVideo}/>
      <div className={styles.header}>
        {IS_REQUEST_FULLSCREEN_SUPPORTED && (<Button round size="smaller" color="translucent" onClick={handleToggleFullscreen} ariaLabel={lang(isFullscreen ? 'AccExitFullscreen' : 'AccSwitchToFullscreen')}>
            <Icon name={isFullscreen ? 'smallscreen' : 'fullscreen'}/>
          </Button>)}

        <Button round size="smaller" color="translucent" onClick={handleClose} className={styles.closeButton}>
          <Icon name="close"/>
        </Button>
      </div>
      <div className={buildClassName(styles.emojisBackdrop, isEmojiOpen && styles.open)} onClick={!isEmojiOpen ? openEmoji : closeEmoji}>
      ...
      <div className={styles.buttons}>
        <PhoneCallButton onClick={handleToggleAudio} icon="microphone" isDisabled={!isActive} isActive={hasOwnAudio} label={lang(hasOwnAudio ? 'lng_call_mute_audio' : 'lng_call_unmute_audio')}/>
        <PhoneCallButton onClick={handleToggleVideo} icon="video" isDisabled={!isActive} isActive={hasOwnVideo} label={lang(hasOwnVideo ? 'lng_call_stop_video' : 'lng_call_start_video')}/>
        {hasOwnVideo && (IS_ANDROID || IS_IOS) && (<PhoneCallButton onClick={handleFlipCamera} customIcon={(<AnimatedIcon tgsUrl={LOCAL_TGS_URLS.CameraFlip} playSegment={!isFlipping ? [0, 1] : [0, 10]} size={32}/>)} isDisabled={!isActive} label={lang('VoipFlip')}/>)}
        {IS_SCREENSHARE_SUPPORTED && (<PhoneCallButton onClick={handleTogglePresentation} icon="share-screen" isDisabled={!isActive} isActive={hasOwnPresentation} label={lang('lng_call_screencast')}/>)}
        {isIncomingRequested && (<PhoneCallButton onClick={requestMasterAndAcceptCall} icon="phone-discard" isDisabled={isDiscarded} label={lang('lng_call_accept')} className={styles.accept} iconClassName={styles.acceptIcon}/>)}
        <PhoneCallButton onClick={handleHangUp} icon="phone-discard" isDisabled={isDiscarded} label={lang(isIncomingRequested ? 'lng_call_decline' : 'lng_call_end_call')} className={styles.leave}/>
      </div>
    </Modal>);
};
