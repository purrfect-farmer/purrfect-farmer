...
const RatePhoneCallModal = ({ isOpen, }) => {
    ...
    return (<Modal title={lang('lng_call_rate_label')} className="narrow" onClose={closeCallRatingModal} isOpen={isOpen}>
      <div className={styles.stars}>
        {new Array(5).fill(undefined).map((_, i) => {
            const isFilled = rating !== undefined && rating >= i;
            return (<Icon name={isFilled ? 'favorite-filled' : 'favorite'} className={buildClassName(isFilled && styles.isFilled, styles.star)} onClick={handleClickStar(i)}/>);
        })}
      </div>
      <InputText ref={inputRef} placeholder={lang('lng_call_rate_comment')} className={buildClassName(styles.comment, rating !== 4 && rating !== undefined && styles.visible)}/>
