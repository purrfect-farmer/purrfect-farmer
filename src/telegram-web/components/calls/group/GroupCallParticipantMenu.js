...
const GroupCallParticipantMenu = ({ participant, onCloseAnimationEnd, onClose, isDropdownOpen, isAdmin, menuRef, ...menuPositionOptions }) => {
    const { toggleGroupCallMute, setGroupCallParticipantVolume, toggleGroupCallPanel, openChat, requestToSpeak, } = getActions();
    const lang = useOldLang();
    const [isDeleteUserModalOpen, openDeleteUserModal, closeDeleteUserModal] = useFlag();
    const id = participant?.id;
    const { isMutedByMe, isMuted, isSelf, canSelfUnmute, } = participant || {};
    const isRaiseHand = Boolean(participant?.raiseHandRating);
    const shouldRaiseHand = !canSelfUnmute && isMuted;
    const [localVolume, setLocalVolume] = useState(isMutedByMe ? VOLUME_ZERO : ((participant?.volume || GROUP_CALL_DEFAULT_VOLUME) / GROUP_CALL_VOLUME_MULTIPLIER));
    ...
    const handleSetLocalVolume = useLastCallback((volume) => {
    ...
    const handleRemove = useLastCallback((e) => {
        e.stopPropagation();
        openDeleteUserModal();
        onClose();
    });
    const handleCancelRequestToSpeak = useLastCallback((e) => {
        e.stopPropagation();
        requestToSpeak({
            value: false,
        });
    ...
    const handleMute = useLastCallback((e) => {
        e.stopPropagation();
        onClose();
        if (!isAdmin) {
    ...
    const handleOpenProfile = useLastCallback((e) => {
        e.stopPropagation();
    ...
    return (<div>
      <Menu isOpen={isDropdownOpen} ref={menuRef} withPortal onClose={onClose} onCloseAnimationEnd={onCloseAnimationEnd} className="participant-menu with-menu-transitions" {...menuPositionOptions}>
        {!isSelf && !shouldRaiseHand && (<div className="group">
            <div className={buildClassName('volume-control', localVolume < VOLUME_LOW && 'low', localVolume >= VOLUME_LOW && localVolume < VOLUME_MEDIUM && 'medium', localVolume >= VOLUME_MEDIUM && localVolume < VOLUME_NORMAL && 'normal', localVolume >= VOLUME_NORMAL && 'high')}>
              <input type="range" min="0" max="200" value={localVolume} onChange={handleChangeVolume}/>
              <div className="info">
                <AnimatedIcon tgsUrl={LOCAL_TGS_URLS.Speaker} play={shouldPlay ? speakerIconPlaySegment.toString() : false} playSegment={speakerIconPlaySegment} size={SPEAKER_ICON_SIZE}/>
                <span>
                  {localVolume}
                  %
                </span>
        ...
        <div className="group">
          {(isRaiseHand && isSelf) && (<MenuItem icon="stop-raising-hand" onClick={handleCancelRequestToSpeak}>
              {lang('VoipGroupCancelRaiseHand')}
            </MenuItem>)}
          {!isSelf && <MenuItem icon="user" onClick={handleOpenProfile}>{lang('VoipGroupOpenProfile')}</MenuItem>}
          {!isSelf && (
        // TODO cross mic
        <MenuItem icon={isMuted ? (isAdmin && shouldRaiseHand ? 'allow-speak' : 'microphone-alt') : 'microphone-alt'} onClick={handleMute}>
              {isAdmin
      ...
      {!isSelf && isAdmin && (<DeleteMemberModal isOpen={isDeleteUserModalOpen} userId={id} onClose={closeDeleteUserModal}/>)}
    </div>);
};
