...
const ICON_SIZE = 36;
const MicrophoneButton = ({ className, noAudioStream, canSelfUnmute, isMuted, connectionState, }) => {
    const { toggleGroupCallMute, requestToSpeak, playGroupCallSound, } = getActions();
    ...
    return (<Button round size="default" className={buildClassName(styles.root, !isConnecting && noAudioStream && styles.canUnmute, !isConnecting && shouldRaiseHand && styles.mutedByAdmin, className)} onMouseDown={handleMouseDownMute} onMouseUp={handleMouseUpMute} ariaLabel={lang(isMuted ? 'VoipUnmute' : 'VoipMute')} disabled={isConnecting}>
      <AnimatedIcon tgsUrl={LOCAL_TGS_URLS[animatedIconName]} size={ICON_SIZE} play={playSegment.toString()} playSegment={playSegment} className={styles.icon} forceAlways/>
...
export default memo(withGlobal((global) => {
    ...
    const { raiseHandRating, hasAudioStream, canSelfUnmute, isMuted, } = meParticipant || {};
    return {
