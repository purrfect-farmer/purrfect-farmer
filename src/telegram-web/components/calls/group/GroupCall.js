...
import { selectCanInviteToActiveGroupCall, selectGroupCall, selectGroupCallParticipant, selectIsAdminInActiveGroupCall, } from '../../../global/selectors/calls';
...
const GroupCall = ({ groupCallId, isCallPanelVisible, connectionState, participantsCount, title, meParticipant, isAdmin, participants, canInvite, }) => {
    const { toggleGroupCallVideo, toggleGroupCallPresentation, leaveGroupCall, toggleGroupCallPanel, connectToActiveGroupCall, playGroupCallSound, createGroupCallInviteLink, } = getActions();
    ...
    const { videoLayout, panelOffset, } = useGroupCallVideoLayout({
    ...
    const handleSetPinnedVideo = useLastCallback((video) => {
    ...
    return (<Modal isOpen={isOpen} onClose={toggleGroupCallPanel} className={buildClassName(styles.root, (isFullscreen || isMobile) && styles.fullscreen, isAppFullscreen && styles.appFullscreen, isLandscapeLayout && styles.landscape, !hasVideoParticipants && styles.noVideoParticipants, !isLandscapeLayout && styles.portrait, !isSidebarOpen && isLandscapeWithVideos && styles.noSidebar)} dialogRef={containerRef} onCloseAnimationEnd={handleCloseAnimationEnd}>
      {isLandscapeWithVideos && (<div className={styles.videos}>
          <div className={styles.videosHeader}>
            {!isMobile && (<Button round size="smaller" color="translucent" onClick={handleToggleFullscreen} className={buildClassName(styles.headerButton, styles.firstButton)} ariaLabel={lang(isFullscreen ? 'AccExitFullscreen' : 'AccSwitchToFullscreen')}>
            ...
            {isLandscapeWithVideos && !isSidebarOpen && (<Button round size="smaller" color="translucent" className={buildClassName(styles.headerButton, styles.videosHeaderLastButton)} onClick={handleToggleSidebar} ariaLabel={lang('AccDescrExpandPanel')}>
                <Icon name="sidebar"/>
              </Button>)}
          </div>

          <div className={styles.videosContent} ref={primaryVideoContainerRef}/>
        </div>)}
      ...
      <div className={styles.panelWrapper} ref={panelRef}>
        <div className={buildClassName(styles.panel, 'custom-scroll')}>
          ...
          <div className={buildClassName(styles.panelHeader, hasScrolled && styles.scrolled)}>
            {!isLandscapeWithVideos && !isMobile && (<Button round size="smaller" color="translucent" ripple={!isMobile} className={buildClassName(styles.firstButton, styles.headerButton)} onClick={handleToggleFullscreen} ariaLabel={lang('AccSwitchToFullscreen')}>
                <Icon name="fullscreen"/>
              </Button>)}

            {isMobile && (<Button round size="smaller" color="translucent" onClick={handleToggleGroupCallPanel} className={buildClassName(styles.headerButton, styles.firstButton)} ariaLabel={lang('Close')}>
                <Icon name="close"/>
              </Button>)}

            {isLandscapeWithVideos && (<Button round size="smaller" ripple={!isMobile} className={buildClassName(styles.firstButton, styles.headerButton)} color="translucent" onClick={handleToggleSidebar} ariaLabel={lang('AccDescrCollapsePanel')}>
                <Icon name="sidebar"/>
              </Button>)}
            ...
            <div className={styles.panelHeaderText}>
              ...
              {!isLandscapeWithVideos && (<span className={styles.subtitle}>
                  {membersString}
                </span>)}
            </div>

            {!isLandscapeWithVideos && canInvite && (<Button round size="smaller" ripple={!isMobile} className={buildClassName(styles.lastButton, styles.headerButton)} color="translucent" onClick={handleInviteMember} ariaLabel={lang('VoipGroupInviteMember')}>
                <Icon name="add-user"/>
              </Button>)}
          </div>

          <div className={styles.participants}>
            <div className={styles.participantVideos} ref={secondaryVideoContainerRef} style={`height: ${panelOffset}px;`}>
              {videoLayout.map((layout) => {
            const participant = participants[layout.participantId];
            if (!layout.isRemounted || !participant) {
                return (<div teactOrderKey={layout.orderKey} key={`${layout.participantId}_${layout.type}`}/>);
            }
            return (<GroupCallParticipantVideo teactOrderKey={layout.orderKey} key={`${layout.participantId}_${layout.type}`} layout={layout} canPin={canPinVideo} setPinned={handleSetPinnedVideo} pinnedVideo={pinnedVideo} participant={participant}/>);
        })}
            </div>
            <GroupCallParticipantList panelOffset={panelOffset} isLandscape={isLandscapeWithVideos}/>
          </div>
        </div>

        <FloatingActionButton key="add-participant" isShown={isLandscapeWithVideos && canInvite} onClick={handleInviteMember} className={styles.addParticipantButton} ariaLabel={lang('VoipGroupInviteMember')}>
      ...
      <div className={styles.mainVideoContainer}>
        {videoLayout.map((layout) => {
            ...
            return (<GroupCallParticipantVideo teactOrderKey={layout.orderKey} key={`${layout.participantId}_${layout.type}`} layout={layout} canPin={canPinVideo} setPinned={handleSetPinnedVideo} pinnedVideo={pinnedVideo} participant={participant} className={styles.video}/>);
        })}
      </div>

      <div className={styles.actions}>
        <Button round size="default" ripple className={buildClassName(styles.actionButton, !hasAudioStream && styles.muted, canRequestToSpeak && styles.canRequestToSpeak)} onClick={handleClickVideo} ariaLabel={lang(hasVideo ? 'VoipStopVideo' : 'VoipStartVideo')} disabled={isConnecting}>
          <Icon name={!hasVideo ? 'video-stop' : 'video'}/>
        </Button>

        <Button round size="default" ripple className={buildClassName(styles.actionButton, !hasAudioStream && styles.muted, canRequestToSpeak && styles.canRequestToSpeak)} onClick={handleToggleGroupCallPresentation} ariaLabel={lang(hasPresentation ? 'lng_group_call_screen_share_stop' : 'lng_group_call_tooltip_screen')} disabled={isConnecting || !IS_SCREENSHARE_SUPPORTED}>
          <Icon name={!hasPresentation ? 'share-screen-stop' : 'share-screen'}/>
        </Button>

        <MicrophoneButton className={styles.actionButton}/>

        <Button round size="default" ripple className={buildClassName(styles.actionButton, !hasAudioStream && styles.muted, canRequestToSpeak && styles.canRequestToSpeak)} ariaLabel={lang('lng_group_call_settings')} disabled>
          <Icon name="settings-filled"/>
        </Button>

        <Button round size="default" ripple className={buildClassName(styles.actionButton, styles.destructive)} onClick={handleLeaveGroupCall} ariaLabel={lang('lng_group_call_leave')}>
          <Icon name="close"/>
        </Button>
      </div>

      <Modal isOpen={isConfirmLeaveModalOpen} onClose={handleCloseConfirmLeaveModal} className="error" title={lang(isEndGroupCallModal ? 'VoipGroupEndAlertTitle' : 'VoipGroupLeaveAlertTitle')}>
        <p>{lang(isEndGroupCallModal ? 'VoipGroupEndAlertText' : 'VoipGroupLeaveAlertText')}</p>
        {!isEndGroupCallModal && (<Checkbox className="dialog-checkbox" label={lang('VoipGroupEndChat')} checked={shouldEndGroupCall} onCheck={setShouldEndGroupCall}/>)}
...
export default memo(withGlobal((global, { groupCallId }) => {
    const { connectionState, title, participants, participantsCount, chatId, } = selectGroupCall(global, groupCallId) || {};
