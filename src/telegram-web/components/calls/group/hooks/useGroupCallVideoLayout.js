...
export default function useGroupCallVideoLayout({ primaryContainerRef, secondaryContainerRef, videoParticipants, isLandscapeLayout, pinnedVideo, }) {
    ...
    const removedVideoParticipants = useMemo(() => {
        return prevVideoParticipants?.filter(({ id, type }) => !videoParticipants.some((p) => p.id === id && p.type === type));
    ...
    const recalculateLayout = useLastCallback(() => {
        ...
        const { x: secondaryInitialX, y: secondaryInitialY, width: secondaryContainerWidth, } = secondaryContainer.getBoundingClientRect();
        const layout = [];
        if (pinnedVideo !== undefined || !primaryContainer || !isLandscapeLayout) {
            ...
            if (pinnedVideo && primaryContainer && isLandscapeLayout) {
                ...
                if (pinnedSkipIndex !== -1) {
                    const { x: initialX, y: initialY, width: containerWidth, height: containerHeight, } = primaryContainer.getBoundingClientRect();
            ...
            for (let i = 0; i < secondaryVideosCounts; i++) {
                ...
                const y = (isRemounted ? 0 : secondaryInitialY) + (isFirstBig && i !== 0 ? (PREFERRED_PANEL_VIDEO_HEIGHT + PADDING_VERTICAL
                    + (Math.floor(realIndex / columns) - 1) * (height + PADDING_VERTICAL)) : (Math.floor(realIndex / columns) * (height + PADDING_VERTICAL)));
                layout.push({
                    x,
                    y,
                    width,
