...
const GroupCallParticipantVideo = ({ layout, pinnedVideo, setPinned, canPin, className, participant, user, chat, }) => {
    const lang = useOldLang();
    const thumbnailRef = useRef();
    const videoRef = useRef();
    const videoFallbackRef = useRef();
    const { x, y, width, height, noAnimate, isRemoved, type, } = layout;
    ...
    const { x: prevX, y: prevY, width: prevWidth, height: prevHeight, } = prevLayoutRef.current || {};
    ...
    const { isContextMenuOpen, contextMenuAnchor, handleContextMenu, handleContextMenuClose, handleContextMenuHide, } = useContextMenuHandlers(ref, isSelf);
    const getTriggerElement = useCallback(() => ref.current, []);
    const getRootElement = useCallback(() => ref.current.closest('.custom-scroll, .no-scrollbar'), []);
    const getMenuElement = useCallback(() => menuRef.current, []);
    const getLayout = useCallback(() => ({ withPortal: true }), []);
    ...
    return (<div className={buildClassName(styles.wrapper, (isHidden || isRemoved) && styles.hidden, noAnimate && styles.noAnimate, className, isPinned && styles.pinned)} style={`--x: ${currentX}px; --y: ${currentY}px; --width: ${currentWidth}px; --height: ${currentHeight}px;`} ref={ref} onContextMenu={handleContextMenu} onDoubleClick={canPin ? handleClickPin : undefined}>
      <div className={buildClassName(styles.root, isSpeaking && styles.speaking)}>
        {isLoading && (<Skeleton className={buildClassName(styles.video, styles.loader)}/>)}
        {stream && (<video className={buildClassName(styles.video, shouldFlipVideo && styles.flipped)} muted autoPlay playsInline srcObject={stream} ref={videoRef} onCanPlay={handleCanPlay}/>)}
        <canvas className={buildClassName(styles.videoFallback, shouldFlipVideo && styles.flipped)} ref={videoFallbackRef}/>
        <div className={styles.thumbnailWrapper}>
          <canvas className={buildClassName(styles.thumbnail, shouldFlipVideo && styles.flipped)} ref={thumbnailRef}/>
        </div>
        {canPin && (<Button round size="smaller" ripple color="translucent" className={styles.pinButton} ariaLabel={lang(isPinned ? 'lng_group_call_context_unpin_camera' : 'lng_group_call_context_pin_camera')} onClick={handleClickPin}>
      ...
      <GroupCallParticipantMenu participant={participant} isDropdownOpen={isContextMenuOpen} anchor={contextMenuAnchor} getTriggerElement={getTriggerElement} getRootElement={getRootElement} getMenuElement={getMenuElement} getLayout={getLayout} onClose={handleContextMenuClose} onCloseAnimationEnd={handleContextMenuHide} menuRef={menuRef}/>
    </div>);
};
export default memo(withGlobal((global, { participant }) => {
    return {
