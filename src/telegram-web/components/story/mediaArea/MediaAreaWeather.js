...
const MediaAreaWeather = ({ mediaArea, className, style, restrictedEmoji, isPreview, }) => {
    const ref = useRef();
    ...
    const updateCustomSize = useLastCallback((isImmediate) => {
        if (!ref.current)
            return undefined;
    ...
    return (<div ref={ref} className={buildClassName(styles.weather, className)} style={buildStyle(style, `--custom-background-color: ${backgroundColor}`, `color: ${textColor}`)}>
      <div className={styles.weatherInner}>
        {restrictedEmoji && (<CustomEmoji key={restrictedEmoji.id} documentId={restrictedEmoji.id} size={customEmojiSize} noPlay={isPreview} withTranslucentThumb forceAlways/>)}
