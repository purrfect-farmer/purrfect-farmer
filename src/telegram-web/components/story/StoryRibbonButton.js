...
function StoryRibbonButton({ peer, isArchived }) {
    const { openChat, openChatWithInfo, openStoryViewer, toggleStoriesHidden, } = getActions();
    ...
    return (<div ref={ref} role="button" data-peer-id={peer.id} tabIndex={0} className={styles.peer} onMouseDown={handleMouseDown} onClick={handleClick} onContextMenu={handleContextMenu}>
      <Avatar peer={peer} withStory storyViewerOrigin={StoryViewerOrigin.StoryRibbon} storyViewerMode="full"/>
      <div className={buildClassName(styles.name, peer.hasUnreadStories && styles.name_hasUnreadStory)}>
        {isSelf ? lang('MyStory') : getPeerTitle(lang, peer)}
      </div>
      {contextMenuAnchor !== undefined && (<Menu isOpen={isContextMenuOpen} anchor={contextMenuAnchor} getTriggerElement={getTriggerElement} getRootElement={getRootElement} getMenuElement={getMenuElement} getLayout={getLayout} className={buildClassName('story-peer-context-menu', styles.contextMenu)} autoClose withPortal onClose={handleContextMenuClose} onCloseAnimationEnd={handleContextMenuHide}>
          {isSelf ? (<>
              ...
              {!isChannel && (<MenuItem onClick={handleOpenChat} icon="message">
                  {lang('SendMessageTitle')}
                </MenuItem>)}
              {isChannel ? (<MenuItem onClick={handleOpenProfile} icon="channel">
                  {lang('ChatList.ContextOpenChannel')}
                </MenuItem>) : (<MenuItem onClick={handleOpenProfile} icon="user">
                  {lang('StoryList.Context.ViewProfile')}
                </MenuItem>)}
              <MenuItem onClick={handleArchivePeer} icon={isArchived ? 'unarchive' : 'archive'}>
