...
function StorySlides({ peerIds, currentPeerId, currentStoryId, isOpen, isSinglePeer, isSingleStory, isPrivate, isArchive, byPeerId, isDeleteModalOpen, onDelete, onClose, onReport, }) {
    ...
    useEffect(() => {
        return () => {
            if (!currentStoryId || !currentPeerId)
                return;
            stopActiveReaction({
    ...
    useEffect(() => {
        ...
        return captureEvents(containerRef.current, {
            ...
            onDrag: (event, captureEvent, { dragOffsetX, dragOffsetY, }) => {
                if (isReleasedRef.current)
                    return;
    ...
    const handleTransitionEnd = useLastCallback((event) => {
        // It is `target` that is needed here, not `currentTarget`
        const target = event.target;
        if (!target || !target.classList.contains(styles.activeSlide))
            return;
    ...
    if (isMobile) {
        ...
        <div className={styles.mobileSlide} ref={(ref) => setRef(ref, renderingPeerId)}>
          <Story peerId={renderingPeerId} storyId={renderingStoryId} onDelete={onDelete} dimensions={slideSizes.activeSlide} isPrivateStories={renderingIsPrivate} isArchivedStories={renderingIsArchive} isDeleteModalOpen={isDeleteModalOpen} isSingleStory={isSingleStory} getIsAnimating={getIsAnimating} onClose={onClose} onReport={onReport}/>
        </div>
      </div>);
    }
    function renderStoryPreview(peerId, index, position) {
        const style = buildStyle(`width: ${slideSizes.slide.width}px`, `height: ${slideSizes.slide.height}px`);
        const className = buildClassName(styles.slide, styles.slidePreview, `slide-${position}`);
        return (<div key={peerId} ref={(ref) => setRef(ref, peerId)} className={className} style={style}>
        <StoryPreview peer={selectPeer(getGlobal(), peerId)} peerStories={byPeerId?.[peerId]}/>
      </div>);
    }
    function renderStory(peerId) {
        const style = isMobile ? undefined : buildStyle(`width: ${slideSizes.activeSlide.width}px`, `--slide-media-height: ${slideSizes.activeSlide.height}px`);
        return (<div key={peerId} ref={(ref) => setRef(ref, peerId)} className={buildClassName(styles.slide, styles.activeSlide)} style={style}>
        <Story peerId={peerId} storyId={renderingStoryId} onDelete={onDelete} dimensions={slideSizes.activeSlide} isPrivateStories={renderingIsPrivate} isArchivedStories={renderingIsArchive} isDeleteModalOpen={isDeleteModalOpen} isSingleStory={isSingleStory} getIsAnimating={getIsAnimating} onClose={onClose} onReport={onReport}/>
      </div>);
    }
    return (<div className={styles.wrapper} ref={containerRef} style={`--story-viewer-scale: ${slideSizes.scale}`} onTransitionEnd={handleTransitionEnd}>
      <div className={styles.fullSize} onClick={onClose}/>
      {renderingPeerIds.length > 1 && (<div className={styles.backdropNonInteractive} style={`height: ${slideSizes.slide.height}px`}/>)}
...
export default memo(withGlobal((global) => {
    const { storyViewer: { peerId: currentPeerId, storyId: currentStoryId, isSinglePeer, isSingleStory, isPrivate, isArchive, storyList, }, } = selectTabState(global);
