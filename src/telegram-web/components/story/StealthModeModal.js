...
const StealthModeModal = ({ isOpen, stealthMode, isCurrentUserPremium }) => {
    const { toggleStealthModal, activateStealthMode, showNotification, openPremiumModal, } = getActions();
    ...
    return (<Modal className="component-theme-dark" contentClassName={styles.root} isOpen={isOpen} isSlim onClose={handleClose}>
      ...
      <ListItem className={buildClassName(styles.listItem, 'smaller-icon')} multiline inactive leftElement={<Icon name="stealth-past" className={styles.icon}/>}>
      ...
      <ListItem className={buildClassName(styles.listItem, 'smaller-icon')} multiline inactive leftElement={<Icon name="stealth-future" className={styles.icon} aria-hidden/>}>
      ...
      <Button className={styles.button} size="smaller" disabled={isOnCooldown} isShiny={!isCurrentUserPremium} withPremiumGradient={!isCurrentUserPremium} onClick={handleActivate}>
