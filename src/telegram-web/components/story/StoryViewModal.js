...
import { STORY_MIN_REACTIONS_SORT, STORY_VIEWS_MIN_CONTACTS_FILTER, STORY_VIEWS_MIN_SEARCH, } from '../../config';
import { selectIsCurrentUserPremium, selectPeerStory, selectTabState, } from '../../global/selectors';
...
function StoryViewModal({ story, viewersExpirePeriod, views, nextOffset, isLoading, isCurrentUserPremium, }) {
    const { loadStoryViewList, closeStoryViewModal, clearStoryViews, } = getActions();
    ...
    const handleLoadMore = useLastCallback(() => {
        if (!story?.id || nextOffset === undefined)
            return;
        loadStoryViewList({
            peerId: story.peerId,
            storyId: story.id,
    ...
    const MoreMenuButton = useMemo(() => {
        return ({ onTrigger, isOpen: isMenuOpen }) => (<Button fluid size="tiny" color="translucent" className={buildClassName(!isMenuOpen && 'active', styles.sortButton, styles.topButton)} faded={isMenuOpen} onClick={onTrigger} ariaLabel={lang('SortBy')}>
    ...
    return (<Modal isOpen={isOpen} onClose={handleClose} className="component-theme-dark" contentClassName={styles.viewsList} isSlim>
      {hasHeader && (<div className={styles.header}>
          {shouldShowJustContacts && (<div className={styles.contactFilter}>
              <Button className={buildClassName(!areJustContacts && styles.selected, styles.topButton)} size="tiny" color="translucent-white" fluid onClick={unmarkJustContacts}>
                {lang('AllViewers')}
              </Button>
              <Button className={buildClassName(areJustContacts && styles.selected, styles.topButton)} size="tiny" color="translucent-white" fluid onClick={markJustContacts}>
                {lang('Contacts')}
              </Button>
            </div>)}
          {shouldShowSortByReactions && (<DropdownMenu className={styles.sort} trigger={MoreMenuButton} positionX="right">
              <MenuItem icon="heart-outline" onClick={markReactionsFirst}>
                {lang('SortByReactions')}
                {areReactionsFirst && (<Icon name="check" className={styles.check}/>)}
              </MenuItem>
          ...
          {shouldShowSearch && (<SearchInput className={styles.search} value={query} onChange={setQuery}/>)}
        </div>)}
      <div className={buildClassName(styles.content, !isAtBeginning && styles.topScrolled, 'custom-scroll')} onScroll={handleScroll}>
        {isExpired && !isLoading && !query && Boolean(!sortedViews?.length) && (<div className={buildClassName(styles.info, styles.centeredInfo)}>
            {renderText(lang(isCurrentUserPremium ? 'ServerErrorViewers' : 'ExpiredViewsStub'), ['simple_markdown', 'emoji'])}
          </div>)}
        {!isLoading && Boolean(query.length) && !sortedViews?.length && (<div className={styles.info}>
            {lang('Story.ViewList.EmptyTextSearch')}
          </div>)}
        <InfiniteScroll items={sortedViews} onLoadMore={handleLoadMore}>
          {sortedViews?.map((view) => {
            const additionalKeyId = view.type === 'forward' ? view.messageId
                : view.type === 'repost' ? view.storyId : 'user';
            return (<StoryView key={`${view.peerId}-${view.date}-${additionalKeyId}`} storyView={view}/>);
        })}
          {isLoading && Array.from({ length: placeholderCount }).map((_, i) => (<ListItem key={`placeholder-${i}`} className="chat-item-clickable contact-list-item scroll-item small-icon" disabled>
              <PlaceholderChatInfo />
            </ListItem>))}
          {notAllAvailable && (<div key="not-all-available" className={buildClassName(styles.info, styles.bottomInfo)}>
              {lang('Story.ViewList.NotFullyRecorded')}
            </div>)}
        </InfiniteScroll>
      </div>
      <div className={buildClassName(styles.footer, 'dialog-buttons')}>
        <Button className={buildClassName('confirm-dialog-button', styles.close)} isText onClick={handleClose}>
          {lang('Close')}
        </Button>
      </div>
    </Modal>);
...
export default memo(withGlobal((global) => {
    ...
    const { storyId, views, nextOffset, isLoading, } = viewModal || {};
