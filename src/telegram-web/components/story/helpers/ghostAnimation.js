...
export function animateOpening(userId, origin, thumb, bestImageData, dimensions) {
...
export function animateClosing(userId, origin, bestImageData) {
    const { mediaEl: toImage } = getNodes(origin, userId);
    const fromImage = document.getElementById('StoryViewer').querySelector(`.${styles.mobileSlide} .${styles.media}, .${styles.activeSlide} .${styles.media}`);
