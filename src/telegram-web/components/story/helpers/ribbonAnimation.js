...
export function animateOpening(isArchived) {
    cancelDelayedCallbacks();
    const { container, toggler, leftMainHeader, ribbonPeers, toggleAvatars, } = getHTMLElements(isArchived);
    ...
    Array.from(ribbonPeers).reverse().forEach((peer, index, { length }) => {
        ...
        if (!toggleAvatar)
            return;
        let { top: fromTop, left: fromLeft, width: fromWidth, } = toggleAvatar.getBoundingClientRect();
        const { left: toLeft, width: toWidth, } = peer.getBoundingClientRect();
...
export function animateClosing(isArchived) {
    cancelDelayedCallbacks();
    const { container, toggler, toggleAvatars, ribbonPeers, leftMainHeader, } = getHTMLElements(isArchived);
    if (!toggler || !toggleAvatars || !ribbonPeers || !container || !leftMainHeader) {
        return;
    }
    ...
    Array.from(ribbonPeers).reverse().forEach((peer, index, { length }) => {
        ...
        if (!toggleAvatar)
            return;
        const { top: fromTop, left: fromLeft, width: fromWidth, } = peer.getBoundingClientRect();
        let { left: toLeft, width: toWidth, top: toTop, } = toggleAvatar.getBoundingClientRect();
