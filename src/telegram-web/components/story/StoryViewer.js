import { beginHeavyAnimation, memo, useCallback, useEffect, useState, } from '../../lib/teact/teact';
import { getActions, withGlobal } from '../../global';
import { ANIMATION_END_DELAY } from '../../config';
import { selectIsStoryViewerOpen, selectPeerStory, selectPerformanceSettingsValue, selectTabState, } from '../../global/selectors';
...
function StoryViewer({ isOpen, peerId, storyId, story, origin, shouldSkipHistoryAnimations, withAnimation, isPrivacyModalOpen, }) {
    const { closeStoryViewer, closeStoryPrivacyEditor, reportStory } = getActions();
    const lang = useOldLang();
    const [storyToDelete, setStoryToDelete] = useState(undefined);
    ...
    const handleOpenDeleteModal = useLastCallback((s) => {
        setStoryToDelete(s);
        openDeleteModal();
    });
    ...
    return (<ShowTransition id="StoryViewer" className={styles.root} isOpen={isOpen} shouldAnimateFirstRender noCloseTransition={shouldSkipHistoryAnimations}>
      <div className={styles.backdrop} onClick={handleClose}/>
      <Button className={styles.close} round size="smaller" color="translucent-white" ariaLabel={lang('Close')} onClick={handleClose}>
        <Icon name="close"/>
      </Button>

      <StorySlides isOpen={isOpen} isDeleteModalOpen={isDeleteModalOpen} onReport={openMessageReport} onClose={handleClose} onDelete={handleOpenDeleteModal}/>

      <StoryDeleteConfirmModal isOpen={isDeleteModalOpen} story={storyToDelete} onClose={handleCloseDeleteModal}/>
      <StoryViewModal />
      <StealthModeModal />
...
export default memo(withGlobal((global) => {
    const { shouldSkipHistoryAnimations, storyViewer: { storyId, peerId, isPrivacyModalOpen, origin, }, } = selectTabState(global);
