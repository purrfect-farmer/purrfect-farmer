...
const BULLET = '\u2022';
const StoryView = ({ storyView, peer, availableReactions, }) => {
    const { openChat, closeStoryViewer, unblockUser, blockUser, deleteContact, updateStoryView, focusMessage, openStoryViewer, closeStoryViewModal, } = getActions();
    ...
    const statusIcon = storyView.type === 'user' ? 'message-read'
    ...
    return (<ListItem key={storyView.peerId} className={buildClassName('chat-item-clickable small-icon', styles.opacityFadeIn, (storyView.isUserBlocked || storyView.areStoriesBlocked) && styles.blocked)} onClick={handleClick} rightElement={storyView.type === 'user' && storyView.reaction ? (<ReactionStaticEmoji reaction={storyView.reaction} className={styles.viewReaction} size={DEFAULT_REACTION_SIZE} availableReactions={availableReactions} withIconHeart/>) : undefined} contextActions={contextActions} withPortalForMenu menuBubbleClassName={styles.menuBubble}>
      {isUserId(storyView.peerId) ? (<PrivateChatInfo className={buildClassName(shouldColorStatus && styles.withColoredStatus)} userId={storyView.peerId} noStatusOrTyping status={status} statusIcon={statusIcon} withStory forceShowSelf/>) : (<GroupChatInfo className={buildClassName(shouldColorStatus && styles.withColoredStatus)} chatId={storyView.peerId} status={status} statusIcon={statusIcon} withStory/>)}
    </ListItem>);
};
export default memo(withGlobal((global, { storyView }) => {
