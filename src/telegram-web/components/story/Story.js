...
import { selectChat, selectIsCurrentUserFrozen, selectIsCurrentUserPremium, selectPeer, selectPeerPaidMessagesStars, selectPeerStory, selectPerformanceSettingsValue, selectTabState, selectUser, selectUserFullInfo, } from '../../global/selectors';
...
function Story({ peerId, storyId, peer, forwardSender, fromPeer, isMuted, isArchivedStories, isPrivateStories, story, orderedIds, isSingleStory, dimensions, shouldForcePause, storyChangelogUserId, viewersExpirePeriod, isChatExist, arePeerSettingsLoaded, getIsAnimating, isCurrentUserPremium, stealthMode, withHeaderAnimation, onDelete, onClose, onReport, paidMessagesStars, isAccountFrozen, }) {
    const { viewStory, setStoryViewerMuted, openPreviousStory, openNextStory, loadPeerSkippedStories, openForwardMenu, copyStoryLink, toggleStoryInProfile, openChat, showNotification, openStoryPrivacyEditor, loadPeerSettings, fetchChat, loadStoryViews, toggleStealthModal, } = getActions();
    ...
    const { isDeletedStory, hasText, hasForwardInfo, thumbnail, previewBlobUrl, isVideo, noSound, fullMediaData, altMediaHash, altMediaData, hasFullData, hasThumb, canDownload, downloadMediaData, } = useStoryProps(story, isCurrentUserPremium, isDropdownMenuOpen);
    const isLoadedStory = story && 'content' in story;
    const isChangelog = peerId === storyChangelogUserId;
    const isUserStory = isUserId(peerId);
    const isChatStory = !isUserStory;
    const isChannelStory = isChatStory && isChatChannel(peer);
    const isOut = isLoadedStory && story.isOut;
    const canPinToProfile = useCurrentOrPrev(isOut ? !story.isInProfile : undefined, true);
    const canUnpinFromProfile = useCurrentOrPrev(isOut ? story.isInProfile : undefined, true);
    const areViewsExpired = Boolean(isOut && (story.date + viewersExpirePeriod) < getServerTime());
    const forwardSenderTitle = forwardSender ? getPeerTitle(oldLang, forwardSender)
        : (isLoadedStory && story.forwardInfo?.fromName);
    const canCopyLink = Boolean(isLoadedStory
        && story.isPublic
        && !isChangelog
        && peer?.hasUsername);
    const canShare = Boolean(isLoadedStory
        && story.isPublic
        && !story.noForwards
        && !isChangelog
        && !isCaptionExpanded);
    const canPlayStory = Boolean(hasFullData && !shouldForcePause && isAppFocused && !isComposerHasFocus && !isCaptionExpanded
    ...
    const { shouldRender: shouldRenderSkeleton, transitionClassNames: skeletonTransitionClassNames, } = useShowTransitionDeprecated(!hasFullData);
    const { transitionClassNames: mediaTransitionClassNames, } = useShowTransitionDeprecated(Boolean(fullMediaData));
    ...
    const { shouldRender: shouldRenderComposer, transitionClassNames: composerAppearanceAnimationClassNames, } = useShowTransitionDeprecated(shouldShowComposer);
    const { shouldRender: shouldRenderCaptionBackdrop, transitionClassNames: captionBackdropTransitionClassNames, } = useShowTransitionDeprecated(hasText && isCaptionExpanded);
    const { transitionClassNames: appearanceAnimationClassNames } = useShowTransitionDeprecated(true);
    const { shouldRender: shouldRenderCaption, transitionClassNames: captionAppearanceAnimationClassNames, } = useShowTransitionDeprecated(hasText || hasForwardInfo);
    ...
    const { onMouseDown: handleLongPressMouseDown, onMouseUp: handleLongPressMouseUp, onMouseLeave: handleLongPressMouseLeave, onTouchStart: handleLongPressTouchStart, onTouchEnd: handleLongPressTouchEnd, } = useLongPress({
        onStart: handleLongPressStart,
        onEnd: handleLongPressEnd,
    });
    const isUnsupported = useUnsupportedMedia(videoRef, undefined, !isVideo || !fullMediaData || isStreamingSupported);
    const hasAllData = fullMediaData && (!altMediaHash || altMediaData);
    // Play story after media has been downloaded
    useEffect(() => {
        if (hasAllData && !isUnsupported)
            handlePlayStory();
    ...
    useEffect(() => {
        if (!isLoadedStory || isDeletedStory || areViewsExpired)
            return;
        if (!isOut && !isChannelStory)
            return;
    ...
    useEffect(() => {
        if (shouldForcePause || !isAppFocused || isComposerHasFocus
    ...
    useEffectWithPrevDeps(([prevIsComposerHasFocus, prevIsCaptionExpanded, prevShouldForcePause, prevIsAppFocused, prevIsPausedByLongPress,]) => {
        if (!isPausedBySpacebar || isCaptionExpanded || isComposerHasFocus
            || shouldForcePause || !isAppFocused || isPausedByLongPress) {
            return;
        }
        if (prevIsCaptionExpanded !== isCaptionExpanded
    ...
    const handleVideoStoryTimeUpdate = useLastCallback((e) => {
    ...
    const MenuButton = useMemo(() => {
        return ({ onTrigger, isOpen }) => {
            return (<Button round ripple={!isMobile} size="tiny" color="translucent-white" onClick={onTrigger} className={buildClassName(styles.button, isOpen && 'active')} ariaLabel={oldLang('AccDescrOpenMenu2')}>
          <Icon name="more"/>
        </Button>);
        };
    }, [isMobile, oldLang]);
    function renderStoriesTabs() {
        return (<div className={styles.storyIndicators}>
        {(isSingleStory ? [storyId] : orderedIds ?? []).map((id) => (<StoryProgress key={`progress-${id}`} isActive={id === story?.id} isVideo={isVideo} isViewed={Boolean(story?.id && ((isPrivateStories || isArchivedStories) ? id > story?.id : id < story?.id))} isPaused={!isStoryPlaying} duration={duration} onImageComplete={handleOpenNextStory}/>))}
      </div>);
    }
    function renderStoryPrivacyButton() {
        ...
        return (<div className={buildClassName(styles.visibilityButton, isOut && styles.visibilityButtonSelf)} onClick={isOut ? handleInfoPrivacyEdit : handleInfoPrivacyClick} style={`--color-from: ${gradient[privacyIcon][0]}; --color-to: ${gradient[privacyIcon][1]}`}>
        <Icon name={privacyIcon}/>
        {isOut && <Icon name="next"/>}
      </div>);
    }
    function renderSenderInfo() {
        return (<div className={styles.senderInfo}>
        <Avatar peer={peer} size="tiny" onClick={handleOpenChat}/>
        <div className={styles.senderMeta}>
          <span onClick={handleOpenChat} className={styles.senderName}>
            {renderText(getPeerTitle(oldLang, peer) || '')}
          </span>
          <div className={styles.storyMetaRow}>
            {forwardSenderTitle && (<span className={buildClassName(styles.storyMeta, styles.forwardHeader, forwardSender && styles.clickable)} onClick={forwardSender ? handleForwardPeerClick : undefined}>
                <Icon name="loop"/>
                <span className={styles.headerTitle}>
                  {renderText(forwardSenderTitle)}
                </span>
              </span>)}
            {fromPeer && (<span className={buildClassName(styles.storyMeta, styles.fromPeer)} onClick={handleFromPeerClick}>
                <Avatar peer={fromPeer} size="micro"/>
                <span className={styles.headerTitle}>
                  {renderText(getPeerTitle(oldLang, fromPeer) || '')}
                </span>
              </span>)}
            {story && 'date' in story && (<span className={styles.storyMeta}>{formatRelativePastTime(oldLang, serverTime, story.date)}</span>)}
            {isLoadedStory && story.isEdited && (<span className={styles.storyMeta}>{oldLang('Story.HeaderEdited')}</span>)}
          </div>
        </div>
      </div>);
    }
    function renderSender() {
        ...
        <div className={styles.actions}>
          ...
          {isVideo && (<Button className={styles.button} round ripple={!isMobile} size="tiny" color="translucent-white" disabled={!hasFullData} onClick={handleVolumeMuted} ariaLabel={oldLang('Volume')}>
              <Icon name={(isMuted || noSound) ? 'speaker-muted-story' : 'speaker-story'}/>
            </Button>)}
          <DropdownMenu className={styles.buttonMenu} trigger={MenuButton} positionX="right" onOpen={handleDropdownMenuOpen} onClose={handleDropdownMenuClose}>
            {canCopyLink && <MenuItem icon="copy" onClick={handleCopyStoryLink}>{oldLang('CopyLink')}</MenuItem>}
            {canPinToProfile && (<MenuItem icon="save-story" onClick={handlePinClick}>
                {oldLang(isUserStory ? 'StorySave' : 'SaveToPosts')}
              </MenuItem>)}
            {canUnpinFromProfile && (<MenuItem icon="delete" onClick={handleUnpinClick}>
                {oldLang(isUserStory ? 'ArchiveStory' : 'RemoveFromPosts')}
              </MenuItem>)}
            {canDownload && (<MenuItem icon="download" disabled={!downloadMediaData} onClick={handleDownload}>
                {oldLang('lng_media_download')}
              </MenuItem>)}
            {!isOut && isUserStory && (<MenuItem icon="eye-crossed-outline" onClick={handleOpenStealthModal}>
                {oldLang('StealthMode')}
              </MenuItem>)}
            {!isOut && <MenuItem icon="flag" onClick={handleReportStoryClick}>{oldLang('lng_report_story')}</MenuItem>}
            {isOut && (<MenuItem icon="delete" destructive onClick={handleDeleteStoryClick}>
                {oldLang('Delete')}
              </MenuItem>)}
          </DropdownMenu>
          <Button className={buildClassName(styles.button, styles.closeButton)} round size="tiny" color="translucent-white" ariaLabel={oldLang('Close')} onClick={onClose}>
            <Icon name="close"/>
          </Button>
        </div>
      </div>);
    ...
    return (<div className={buildClassName(styles.slideInner, 'component-theme-dark')} onMouseDown={handleLongPressMouseDown} onMouseUp={handleLongPressMouseUp} onMouseLeave={handleLongPressMouseLeave} onTouchStart={handleLongPressTouchStart} onTouchEnd={handleLongPressTouchEnd}>
      <div className={buildClassName(styles.storyHeader, appearanceAnimationClassNames)}>
        {renderStoriesTabs()}
        {renderSender()}
      </div>

      <div className={styles.mediaWrapper} style={`width: ${dimensions.width}px; height: ${dimensions.height}px`}>
        <canvas ref={thumbRef} className={styles.thumbnail}/>
        {previewBlobUrl && (<img key={`preview-${storyId}`} src={previewBlobUrl} draggable={false} alt="" className={buildClassName(styles.media, styles.mediaPreview, previewTransitionClassNames)}/>)}
        {shouldRenderSkeleton && (<Skeleton className={buildClassName(skeletonTransitionClassNames, styles.fullSize)}/>)}
        {!isVideo && fullMediaData && (<img src={fullMediaData} alt="" className={buildClassName(styles.media, mediaTransitionClassNames)} draggable={false}/>)}
        {isVideo && fullMediaData && (<OptimizedVideo ref={videoRef} key={`video-${storyId}`} className={buildClassName(styles.media, mediaTransitionClassNames)} canPlay={isStoryPlaybackRequested} muted={isMuted} width={BASE_STORY_WIDTH} height={BASE_STORY_HEIGHT} draggable={false} playsInline disablePictureInPicture isPriority onPause={unmarkStoryPlaying} onWaiting={unmarkStoryPlaying} disableRemotePlayback onTimeUpdate={handleVideoStoryTimeUpdate}>
        ...
        {!isPausedByLongPress && !isComposerHasFocus && (<>
            <button type="button" className={buildClassName(styles.navigate, styles.prev)} onClick={handleOpenPrevStory} aria-label={oldLang('Previous')}/>
            <button type="button" className={buildClassName(styles.navigate, styles.next)} onClick={handleOpenNextStory} aria-label={oldLang('Next')}/>
          </>)}
        {isLoadedStory && fullMediaData && (<MediaAreaOverlay key={`area-overlay-${storyId}-${peerId}`} story={story} isActive isStoryPlaying={isDropdownMenuOpen}/>)}
        {!isMobile && (<div className={styles.content}>
            <div className={styles.contentInner}>
              <Avatar peer={peer} withStory storyViewerMode="disabled"/>
              <div className={styles.name}>{renderText(getPeerTitle(oldLang, peer) || '')}</div>
            </div>
          </div>)}
      </div>

      {shouldShowFooter && (<StoryFooter story={story} className={appearanceAnimationClassNames}/>)}
      {shouldRenderCaptionBackdrop && (<div tabIndex={0} role="button" className={buildClassName(styles.captionBackdrop, captionBackdropTransitionClassNames)} onClick={() => foldCaption()} aria-label={oldLang('Close')}/>)}
      {hasText && <div className={buildClassName(styles.captionGradient, captionAppearanceAnimationClassNames)}/>}
      {shouldRenderCaption && (<StoryCaption key={`caption-${storyId}-${peerId}`} story={story} isExpanded={isCaptionExpanded} onExpand={expandCaption} onFold={foldCaption} className={captionAppearanceAnimationClassNames}/>)}
      {shouldRenderComposer && (<Composer type="story" chatId={peerId} threadId={MAIN_THREAD_ID} storyId={storyId} isReady={!isOut} messageListType="thread" isMobile={getIsMobile()} editableInputCssSelector={EDITABLE_STORY_INPUT_CSS_SELECTOR} editableInputId={EDITABLE_STORY_INPUT_ID} inputId="story-input-text" className={buildClassName(styles.composer, composerAppearanceAnimationClassNames)} inputPlaceholder={inputPlaceholder} onForward={canShare ? handleForwardClick : undefined} onFocus={markComposerHasFocus} onBlur={unmarkComposerHasFocus}/>)}
    </div>);
}
export default memo(withGlobal((global, { peerId, storyId, isDeleteModalOpen, }) => {
    const { appConfig } = global;
    const user = selectUser(global, peerId);
    const chat = selectChat(global, peerId);
    const userFullInfo = selectUserFullInfo(global, peerId);
    const tabState = selectTabState(global);
    const { storyViewer: { isMuted, viewModal, isPrivacyModalOpen, isStealthModalOpen, storyList, }, forwardMessages: { storyId: forwardedStoryId }, premiumModal, safeLinkModalUrl, mapModal, reportModal, giftInfoModal, isPaymentMessageConfirmDialogOpen, } = tabState;
    const { isOpen: isPremiumModalOpen } = premiumModal || {};
    const story = selectPeerStory(global, peerId, storyId);
    const isLoadedStory = story && 'content' in story;
    const shouldForcePause = Boolean(isPaymentMessageConfirmDialogOpen
    ...
    const forwardSenderId = forwardInfo?.fromPeerId
        || mediaAreas?.find((area) => area.type === 'channelPost')?.channelId;
