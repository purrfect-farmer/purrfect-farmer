...
function StorySettings({ isOpen, story, visibility, contactListIds, usersById, currentUserId, onClose, }) {
    ...
    const [activeKey, setActiveKey] = useState(Screens.privacy);
    const [editingBlockingCategory, setEditingBlockingCategory] = useState('everybody');
    ...
    function renderContent(isActive) {
        switch (activeKey) {
            ...
            case Screens.closeFriends:
                return (<CloseFriends key="close-friends" isActive={isActive} contactListIds={contactListIds} currentUserId={currentUserId} usersById={usersById} onClose={handleCloseButtonClick}/>);
            case Screens.denyList:
                return (<AllowDenyList key="deny-list" id="deny-list" contactListIds={contactListIds} currentUserId={currentUserId} selectedIds={selectedBlockedIds} onSelect={handleDenyUserIdsChange}/>);
            case Screens.allowList:
                return (<AllowDenyList key="allow-list" id="allow-list" contactListIds={contactListIds} lockedIds={lockedIds} currentUserId={currentUserId} selectedIds={privacy?.allowUserIds} onSelect={handleAllowUserIdsChange}/>);
        }
    }
    function renderPrivacyList() {
        ...
        <div className={styles.section}>
          ...
          <div className={styles.list}>
            {OPTIONS.map((option) => (<label key={option.value} className={buildClassName(styles.option, option.value === privacy?.visibility && styles.checked)}>
                <input type="radio" name="story_privacy" className={styles.input} value={option.value} checked={option.value === privacy?.visibility} onChange={() => handleVisibilityChange(option.value)} teactExperimentControlled/>
                <span className={styles.icon} style={`--color-from: ${option.color[0]}; --color-to: ${option.color[1]}`}>
                  <Icon name={option.icon}/>
                </span>
                <div className={styles.optionContent}>
                  <span className={buildClassName(styles.option_name)}>{lang(option.name)}</span>
                  {option.actions && (<div tabIndex={0} role="button" className={styles.action} aria-label={lang('Edit')} onClick={(e) => { handleActionClick(e, option.actions); }}>
        ...
        <div className={styles.section}>
          <ListItem ripple onClick={handleIsPinnedToggle}>
            ...
            <Switcher id="group-notifications" label={lang('StoryKeep')} checked={isPinned} inactive/>
          </ListItem>
        </div>
    ...
    return (<Modal isOpen={isOpenModal} className={buildClassName(styles.modal, 'component-theme-dark')} onClose={closeModal} noBackdrop onCloseAnimationEnd={onClose}>
      <div className={styles.header}>
        <Button className={buildClassName(styles.closeButton, 'close-button')} round color="translucent" size="smaller" onClick={handleCloseButtonClick} ariaLabel={isBackButton ? lang('Common.Back') : lang('Common.Close')}>
      ...
      <Transition activeKey={activeKey} name="slideFade" slideClassName="ChatOrUserPicker_slide" className={styles.content}>
        {renderContent}
      </Transition>
    </Modal>);
}
export default memo(withGlobal((global) => {
    const { storyViewer: { storyId, peerId, }, } = selectTabState(global);
