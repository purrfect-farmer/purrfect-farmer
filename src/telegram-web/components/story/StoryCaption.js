...
function StoryCaption({ story, isExpanded, className, onExpand, onFold, }) {
    ...
    const { shouldRender: shouldRenderShowMore, transitionClassNames } = useShowTransitionDeprecated(canExpand, undefined, true, 'slow', true);
    ...
    const fullClassName = buildClassName(styles.captionContent, isInExpandedState && styles.expanded, shouldRenderShowMore && styles.withShowMore);
    return (<div className={buildClassName(styles.caption, className)}>
      <div ref={contentRef} className={fullClassName} role={canExpand ? 'button' : undefined} onClick={canExpand ? onExpand : onFold}>
        <div ref={ref} className={buildClassName(styles.captionInner, 'allow-selection', 'custom-scroll')}>
          {renderingStory?.forwardInfo && (<EmbeddedStoryForward forwardInfo={renderingStory.forwardInfo} className={styles.forwardInfo}/>)}
          {renderingStory && (<div ref={textRef} className={styles.captionText}>
              <MessageText messageOrStory={renderingStory} withTranslucentThumbs forcePlayback/>
            </div>)}
        </div>
      </div>
      {shouldRenderShowMore && (<div ref={showMoreButtonRef} className={buildClassName(styles.captionShowMore, transitionClassNames)} onClick={onExpand}>
