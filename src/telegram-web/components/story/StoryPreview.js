...
function StoryPreview({ peer, peerStories, lastViewedId, storyIdsForViewer, origin, }) {
    ...
    const story = useMemo(() => {
        ...
        const { orderedIds, lastReadId, byId, } = peerStories;
    ...
    return (<div className={styles.slideInner} onClick={() => { openStoryViewer({ peerId: story.peerId, storyId: story.id, origin }); }}>
...
export default memo(withGlobal((global, { peer }) => {
    const { storyViewer: { lastViewedByPeerId: lastViewedByPeerIds, origin, storyList, }, } = selectTabState(global);
    return {
