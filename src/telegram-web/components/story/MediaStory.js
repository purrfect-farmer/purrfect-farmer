...
function MediaStory({ story, isProtected, isArchive, isPinned, canPin, }) {
    const { openStoryViewer, loadPeerSkippedStories, toggleStoryInProfile, toggleStoryPinnedToTop, showNotification, } = getActions();
    ...
    const { isContextMenuOpen, contextMenuAnchor, handleBeforeContextMenu, handleContextMenu, handleContextMenuClose, handleContextMenuHide, } = useContextMenuHandlers(containerRef, !isOwn);
    ...
    const handleUnarchiveClick = useLastCallback((e) => {
        stopEvent(e);
    ...
    const handleArchiveClick = useLastCallback((e) => {
        stopEvent(e);
    ...
    return (<div ref={containerRef} className={buildClassName(styles.root, 'scroll-item')} onMouseDown={handleMouseDown} onClick={handleClick} onContextMenu={handleContextMenu}>
      {isDeleted && (<span>
      ...
      {isFullyLoaded && Boolean(story.views?.viewsCount) && (<span className={buildClassName(styles.overlayIcon, styles.viewsCount)}>
          <Icon name="eye"/>
          {story.views.viewsCount}
        </span>)}
      {duration && <span className={buildClassName(styles.overlayIcon, styles.duration)}>{duration}</span>}
      <div className={styles.wrapper}>
        {thumbUrl && (<img src={thumbUrl} alt="" className={styles.media} draggable={false}/>)}
        {isFullyLoaded && <MediaAreaOverlay story={story}/>}
        {isProtected && <span className="protector"/>}
      </div>
      {contextMenuAnchor !== undefined && (<Menu isOpen={isContextMenuOpen} anchor={contextMenuAnchor} getTriggerElement={getTriggerElement} getRootElement={getRootElement} getMenuElement={getMenuElement} getLayout={getLayout} className={buildClassName(styles.contextMenu, 'story-context-menu')} autoClose onClose={handleContextMenuClose} onCloseAnimationEnd={handleContextMenuHide} withPortal>
          {isArchive && (<MenuItem icon="archive" onClick={handleUnarchiveClick}>
              {lang('StoryList.SaveToProfile')}
            </MenuItem>)}
          {!isArchive && (<MenuItem icon="archive" onClick={handleArchiveClick}>
              {lang('Story.Context.RemoveFromProfile')}
            </MenuItem>)}
          {!isArchive && !isPinned && canPin && (<MenuItem icon="pin" onClick={handleTogglePinned}>
              {lang('StoryList.ItemAction.Pin')}
            </MenuItem>)}
          {!isArchive && isPinned && (<MenuItem icon="unpin" onClick={handleTogglePinned}>
              {lang('StoryList.ItemAction.Unpin')}
            </MenuItem>)}
        </Menu>)}
    </div>);
}
export default memo(withGlobal((global, { story }) => {
