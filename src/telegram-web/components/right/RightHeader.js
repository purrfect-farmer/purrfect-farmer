...
import { getCanAddContact, getCanManageTopic, isChatChannel, isUserBot, } from '../../global/helpers';
import { selectCanManage, selectCanUseGiftProfileAdminFilter, selectCanUseGiftProfileFilter, selectChat, selectChatFullInfo, selectCurrentGifSearch, selectCurrentStickerSearch, selectIsChatWithSelf, selectTabState, selectTopic, selectUser, } from '../../global/selectors';
...
const RightHeader = ({ chatId, threadId, isColumnOpen, isProfile, isManagement, isStatistics, isMessageStatistics, isStoryStatistics, isMonetizationStatistics, isBoostStatistics, isStickerSearch, isGifSearch, isPollResults, isCreatingTopic, isEditingTopic, isAddingChatMembers, profileState, managementScreen, canAddContact, userId, isSelf, canManage, isChannel, stickerSearchQuery, gifSearchQuery, isEditingInvite, canViewStatistics, currentInviteInfo, shouldSkipHistoryAnimations, isBot, isInsideTopic, canEditTopic, isSavedMessages, canEditBot, giftProfileFilter, canUseGiftFilter, canUseGiftAdminFilter, onClose, onScreenSelect, }) => {
    const { setStickerSearchQuery, setGifSearchQuery, toggleManagement, openAddContactDialog, toggleStatistics, setEditingExportedInvite, deleteExportedChatInvite, openEditTopicPanel, updateGiftProfileFilter, } = getActions();
    const [isDeleteDialogOpen, openDeleteDialog, closeDeleteDialog] = useFlag();
    const { isMobile } = useAppLayout();
    const { sortType: giftsSortType, shouldIncludeUnlimited: shouldIncludeUnlimitedGifts, shouldIncludeLimited: shouldIncludeLimitedGifts, shouldIncludeUnique: shouldIncludeUniqueGifts, shouldIncludeDisplayed: shouldIncludeDisplayedGifts, shouldIncludeHidden: shouldIncludeHiddenGifts, } = giftProfileFilter;
    ...
    const handleStickerSearchQueryChange = useLastCallback((query) => {
        setStickerSearchQuery({ query });
    });
    const handleGifSearchQueryChange = useLastCallback((query) => {
    ...
    const PrimaryLinkMenuButton = useMemo(() => {
        return ({ onTrigger, isOpen }) => (<Button round ripple={!isMobile} size="smaller" color="translucent" className={isOpen ? 'active' : ''} onClick={onTrigger} ariaLabel={lang('AccDescrOpenMenu2')}>
        <Icon name="more"/>
      </Button>);
    }, [isMobile, lang]);
    function renderHeaderContent() {
        ...
        switch (renderingContentKey) {
            ...
            <section className="tools">
              {currentInviteInfo && !currentInviteInfo.isRevoked && (<Button round color="translucent" size="smaller" ariaLabel={oldLang('Edit')} onClick={handleEditInviteClick}>
                  <Icon name="edit"/>
                </Button>)}
              {currentInviteInfo && currentInviteInfo.isRevoked && (<>
                  <Button round color="danger" size="smaller" ariaLabel={oldLang('Delete')} onClick={openDeleteDialog}>
                    <Icon name="delete"/>
                  </Button>
                  <ConfirmDialog isOpen={isDeleteDialogOpen} onClose={closeDeleteDialog} title={oldLang('DeleteLink')} text={oldLang('DeleteLinkHelp')} confirmIsDestructive confirmLabel={oldLang('Delete')} confirmHandler={handleDeleteInviteClick}/>
                </>)}
            </section>
          </>);
            ...
            case HeaderContent.StickerSearch:
                return (<SearchInput value={stickerSearchQuery} placeholder={oldLang('SearchStickersHint')} autoFocusSearch onChange={handleStickerSearchQueryChange}/>);
            case HeaderContent.GifSearch:
                return (<SearchInput value={gifSearchQuery} placeholder={oldLang('SearchGifsTitle')} autoFocusSearch onChange={handleGifSearchQueryChange}/>);
            ...
            {canUseGiftFilter && chatId && (<section className="tools">
                <DropdownMenu trigger={PrimaryLinkMenuButton} positionX="right" autoClose={false}>
                  <MenuItem icon={giftsSortType === 'byDate' ? 'cash-circle' : 'calendar-filter'} onClick={() => updateGiftProfileFilter({ peerId: chatId, filter: { sortType: giftsSortType === 'byDate' ? 'byValue' : 'byDate' } })}>
                    {lang(giftsSortType === 'byDate' ? 'GiftSortByValue' : 'GiftSortByDate')}
                  </MenuItem>

                  <MenuSeparator />

                  <MenuItem icon={shouldIncludeUnlimitedGifts ? 'check' : 'placeholder'} onClick={() => updateGiftProfileFilter({ peerId: chatId, filter: { shouldIncludeUnlimited: !shouldIncludeUnlimitedGifts } })}>
                    {lang('GiftFilterUnlimited')}
                  </MenuItem>

                  <MenuItem icon={shouldIncludeLimitedGifts ? 'check' : 'placeholder'} onClick={() => updateGiftProfileFilter({ peerId: chatId, filter: { shouldIncludeLimited: !shouldIncludeLimitedGifts } })}>
                    {lang('GiftFilterLimited')}
                  </MenuItem>

                  <MenuItem icon={shouldIncludeUniqueGifts ? 'check' : 'placeholder'} onClick={() => updateGiftProfileFilter({ peerId: chatId, filter: { shouldIncludeUnique: !shouldIncludeUniqueGifts } })}>
                    {lang('GiftFilterUnique')}
                  </MenuItem>

                  {canUseGiftAdminFilter && (<>
                      <MenuSeparator />
                      <MenuItem icon={shouldIncludeDisplayedGifts ? 'check' : 'placeholder'} onClick={() => updateGiftProfileFilter({ peerId: chatId, filter: { shouldIncludeDisplayed: !shouldIncludeDisplayedGifts } })}>
                        {lang('GiftFilterDisplayed')}
                      </MenuItem>

                      <MenuItem icon={shouldIncludeHiddenGifts ? 'check' : 'placeholder'} onClick={() => updateGiftProfileFilter({ peerId: chatId, filter: { shouldIncludeHidden: !shouldIncludeHiddenGifts } })}>
            ...
            <section className="tools">
              {canAddContact && (<Button round color="translucent" size="smaller" ariaLabel={oldLang('AddContact')} onClick={handleAddContact}>
                  <Icon name="add-user"/>
                </Button>)}
              {canManage && !isInsideTopic && (<Button round color="translucent" size="smaller" ariaLabel={oldLang('Edit')} onClick={handleToggleManagement}>
                  <Icon name="edit"/>
                </Button>)}
              {canEditBot && (<Button round color="translucent" size="smaller" ariaLabel={oldLang('Edit')} onClick={handleToggleManagement}>
                  <Icon name="edit"/>
                </Button>)}
              {canEditTopic && (<Button round color="translucent" size="smaller" ariaLabel={oldLang('EditTopic')} onClick={toggleEditTopic}>
                  <Icon name="edit"/>
                </Button>)}
              {canViewStatistics && (<Button round color="translucent" size="smaller" ariaLabel={oldLang('Statistics')} onClick={handleToggleStatistics}>
                  <Icon name="stats"/>
                </Button>)}
            </section>
          </>);
    ...
    const isBackButton = isMobile || (!isSavedMessages && (contentKey === HeaderContent.SharedMedia
        ...
        || isManagement));
    const buttonClassName = buildClassName('animated-close-icon', isBackButton && 'state-back', (shouldSkipTransition || shouldSkipHistoryAnimations) && 'no-transition');
    const headerRef = useRef();
    useElectronDrag(headerRef);
    return (<div className="RightHeader" ref={headerRef}>
      <Button className="close-button" round color="translucent" size="smaller" onClick={handleClose} ariaLabel={isBackButton ? oldLang('Common.Back') : oldLang('Common.Close')}>
        <div className={buttonClassName}/>
      </Button>
      <Transition name={(shouldSkipTransition || shouldSkipHistoryAnimations) ? 'none' : 'slideFade'} activeKey={renderingContentKey}>
        {renderHeaderContent()}
      </Transition>
    </div>);
};
export default withGlobal((global, { chatId, isProfile, isManagement, threadId, }) => {
