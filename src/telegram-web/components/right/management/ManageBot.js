...
import { getChatAvatarHash, getMainUsername, getUserFirstOrLastName, } from '../../../global/helpers';
import { selectBot, selectTabState, selectUserFullInfo, } from '../../../global/selectors';
...
const ManageBot = ({ userId, user, progress, onClose, currentBio, isActive, maxBioLength, }) => {
    const { setBotInfo, uploadProfilePhoto, uploadContactProfilePhoto, startBotFatherConversation, } = getActions();
    ...
    const handleNameChange = useLastCallback((e) => {
    ...
    const handleBioChange = useLastCallback((e) => {
        setBio(e.target.value);
        markFieldTouched();
    });
    const handlePhotoChange = useLastCallback((newPhoto) => {
        setPhoto(newPhoto);
        markAvatarTouched();
    });
    ...
    const handleSelectAvatar = useLastCallback((file) => {
        markAvatarTouched();
    ...
    return (<div className="Management">
      <div className="custom-scroll">
        <div className="section">
          <AvatarEditable currentAvatarBlobUrl={currentAvatarBlobUrl} onChange={handlePhotoChange} title={lang('ChatSetPhotoOrVideo')} disabled={isLoading}/>
          <InputText id="user-name" label={lang('PaymentCheckoutName')} onChange={handleNameChange} value={name} error={error === ERROR_NAME_MISSING ? error : undefined} teactExperimentControlled/>
          <TextArea value={bio} onChange={handleBioChange} label={lang('DescriptionPlaceholder')} disabled={isLoading} maxLength={maxBioLength} maxLengthIndicator={maxBioLength ? (maxBioLength - bio.length).toString() : undefined}/>
        </div>
        <div className="section">
      ...
      <FloatingActionButton isShown={isFieldTouched || isAvatarTouched} onClick={handleProfileSave} disabled={isLoading} ariaLabel={lang('Save')}>
