...
import { MAX_UNIQUE_REACTIONS, } from '../../../config';
...
const ManageReactions = ({ availableReactions, enabledReactions, chat, isActive, onClose, maxUniqueReactions, reactionsLimit, isChannel, }) => {
    ...
    const availableActiveReactions = useMemo(() => availableReactions?.filter(({ isInactive }) => !isInactive), [availableReactions]);
    useEffect(() => {
        ...
        if (localEnabledReactions?.type === 'some' && enabledReactions?.type === 'some') {
            ...
            if (localAllowedReactions.length !== enabledAllowedReactions.length
                || localAllowedReactions.reverse().some((localReaction) => !enabledAllowedReactions.find((enabledReaction) => isSameReaction(localReaction, enabledReaction)))) {
                setIsTouched(true);
                return;
            }
        }
    ...
    const handleReactionsOptionChange = useCallback((value) => {
        if (value === 'all') {
    ...
    const handleReactionChange = useCallback((e) => {
        if (!chat || !availableActiveReactions)
            return;
        const { name, checked } = e.currentTarget;
        if (localEnabledReactions?.type === 'some') {
            const reaction = { emoticon: name };
            if (checked) {
                setLocalEnabledReactions({
                    type: 'some',
    ...
    const handleReactionsLimitChange = useCallback((value) => {
        setLocalReactionsLimit(value);
    }, []);
    const renderReactionsMaxCountValue = useCallback((value) => {
    ...
    return (<div className="Management">
      <div className="panel-content custom-scroll">
        {Boolean(localReactionsLimit && shouldShowReactionsLimit) && (<div className="section">
            <h3 className="section-heading">
              {lang('MaximumReactionsHeader')}
            </h3>
            <RangeSlider min={1} max={maxUniqueReactions} value={localReactionsLimit} onChange={handleReactionsLimitChange} renderValue={renderReactionsMaxCountValue} isCenteredLayout/>
        ...
        <div className="section">
          ...
          <RadioGroup selected={localEnabledReactions?.type || 'none'} name="reactions" options={reactionsOptions} onChange={handleReactionsOptionChange}/>
          <p className="section-info section-info_push">
        ...
        {localEnabledReactions?.type === 'some' && (<div className="section section-with-fab">
            <h3 className="section-heading">
              {lang('OnlyAllowThisReactions')}
            </h3>
            {availableActiveReactions?.map(({ reaction, title }) => (<div className="ListItem">
                <Checkbox name={reaction.emoticon} checked={localEnabledReactions?.allowed.some((r) => isSameReaction(reaction, r))} label={(<div className="Reaction">
                      <ReactionStaticEmoji reaction={reaction} availableReactions={availableReactions}/>
                      {title}
                    </div>)} withIcon onChange={handleReactionChange}/>
              </div>))}
          </div>)}
      </div>

      <FloatingActionButton isShown={isTouched} onClick={handleSaveReactions} ariaLabel={lang('Save')} disabled={isLoading}>
