...
const BoostStatistics = ({ boostStatistics, isGiveawayAvailable, chatId, giveawayBoostsPerPremium, isChannel, }) => {
    const { openChat, loadMoreBoosters, closeBoostStatistics, openGiveawayModal, showNotification, } = getActions();
    ...
    const { currentLevel, hasNextLevel, boosts, levelProgress, remainingBoosts, } = useMemo(() => {
        if (!status) {
            return {
                currentLevel: 0,
                hasNextLevel: false,
    ...
    const renderBoostIcon = useLastCallback((multiplier) => (<div className={styles.quantity}>
    ...
    const renderBoostTypeIcon = useLastCallback((boost) => {
        ...
        <div className={buildClassName(styles.floatingBadge, !boost.isFromGiveaway && styles.floatingBadgeWarning, styles.floatingBadgeButtonColor, styles.floatingBadgeButton)}>
    ...
    const handleBoosterClick = useLastCallback((userId) => {
        if (!userId) {
            showNotification({
    ...
    const renderBoostList = useLastCallback((boost) => {
        ...
        return (<ListItem className={buildClassName(styles.boostInfo, 'chat-item-clickable')} onClick={() => handleBoosterClick(boost.userId)}>
        <PrivateChatInfo className={styles.user} userId={boost.userId} customPeer={customPeer} status={lang('BoostExpireOn', formatDateAtTime(lang, boost.expires * 1000))} noEmojiStatus forceShowSelf noFake noVerified iconElement={boost.multiplier ? renderBoostIcon(boost.multiplier) : undefined} rightElement={renderBoostTypeIcon(boost)}/>
      </ListItem>);
    });
    ...
    const launchPrepaidGiveawayHandler = useLastCallback((prepaidGiveaway) => {
    ...
    return (<div className={buildClassName(styles.root, 'panel-content custom-scroll')}>
      {!isLoaded && <Loading />}
      {isLoaded && statsOverview && (<>
          <div className={styles.section}>
            <PremiumProgress leftText={lang('BoostsLevel', currentLevel)} rightText={hasNextLevel ? lang('BoostsLevel', currentLevel + 1) : undefined} progress={levelProgress} floatingBadgeText={formatInteger(boosts)} floatingBadgeIcon="boost"/>
            <StatisticsOverview className={styles.stats} statistics={statsOverview} type="boost"/>
          </div>
          {statsOverview.prepaidGiveaways && (<div className={styles.section}>
              ...
              {statsOverview?.prepaidGiveaways?.map((prepaidGiveaway) => {
                    ...
                    return (<ListItem key={prepaidGiveaway.id} className="chat-item-clickable" onClick={() => launchPrepaidGiveawayHandler(prepaidGiveaway)}>
                    <div className={buildClassName(styles.status, 'status-clickable')}>
                      <div>
                        {isStarsGiveaway
                            ? (<img src={GiftStar} className={styles.giveawayIcon} alt={lang('GiftStar')}/>) : (<img src={GIVEAWAY_IMG_LIST[prepaidGiveaway.months] || GIVEAWAY_IMG_LIST[3]} className={styles.giveawayIcon} alt={lang('Giveaway')}/>)}
                      </div>
                      ...
                      <div className={styles.quantity}>
                        <div className={buildClassName(styles.floatingBadge, styles.floatingBadgeButtonColor, styles.floatingBadgeButton)}>
                          <Icon name="boost" className={styles.floatingBadgeIcon}/>
          ...
          <div className={styles.section}>
            {shouldDisplayGiftList ? (<div className={buildClassName(styles.boostSection, styles.content)}>
                <Transition ref={transitionRef} name={lang.isRtl ? 'slideOptimizedRtl' : 'slideOptimized'} activeKey={activeKey} renderCount={tabs.length} shouldRestoreHeight>
                  {renderContent()}
                </Transition>
                ...
                {!boostStatistics?.boosts?.list?.length && (<div className={styles.noResults}>
                    {lang(isChannel ? 'NoBoostersHint' : 'NoBoostersGroupHint')}
                  </div>)}
                {boostStatistics?.boosts?.list?.map((boost) => renderBoostList(boost))}
              </>)}
            {Boolean(boostersToLoadCount) && (<ListItem key="load-more" className={styles.showMore} disabled={boostStatistics?.isLoadingBoosters} onClick={handleLoadMore}>
                {boostStatistics?.isLoadingBoosters ? (<Spinner className={styles.loadMoreSpinner}/>) : (<Icon name="down" className={styles.down}/>)}
                {lang('ShowVotes', boostersToLoadCount, 'i')}
              </ListItem>)}
          </div>
          <LinkField className={styles.section} link={status.boostUrl} withShare title={lang('LinkForBoosting')}/>
          {isGiveawayAvailable && (<div className={styles.section}>
              <ListItem key="load-more" icon="gift" onClick={handleGiveawayClick} className={styles.giveawayButton}>
