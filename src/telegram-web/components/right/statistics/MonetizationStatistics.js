...
const MonetizationStatistics = ({ chatId, dcId, statistics, isCreator, isChannelRevenueWithdrawalEnabled, hasPassword, }) => {
    ...
    function renderAvailableReward() {
        ...
        <span className={styles.integer}>
          ≈ $
          {integerUsdPart}
          {decimalUsdPart ? (<span className={styles.decimalUsdPart}>
              .
              {decimalUsdPart}
            </span>) : undefined}
        </span>
    ...
    const topText = useMemo(() => {
        ...
        return lang('ChannelEarnAbout', {
            link: (<Link isPrimary onClick={openAboutMonetizationModal}>
            {linkText}
            <Icon name="next"/>
          </Link>),
        }, {
    ...
    const rewardsText = useMemo(() => {
        ...
        return lang('MonetizationBalanceZeroInfo', {
            link: (<SafeLink url={oldLang('MonetizationProceedsInfoLink')} text={linkText}>
            {linkText}
            <Icon name="next"/>
          </SafeLink>),
        }, {
    ...
    return (<div className={buildClassName(styles.root, 'custom-scroll', isReady && styles.ready)}>
      ...
      <StatisticsOverview statistics={statistics} isToncoin type="monetization" title={oldLang('MonetizationOverview')} subtitle={<div className={styles.textBottom}>{oldLang('MonetizationProceedsTONInfo')}</div>}/>

      {!loadedCharts.current.length && <Loading />}

      <div ref={containerRef} className={styles.section}>
        {MONETIZATION_GRAPHS.filter(Boolean).map((graph) => (<div key={graph} className={buildClassName(styles.graph, styles.hidden)}/>))}
      </div>
      ...
      <div className={styles.section}>
        ...
        <Button size="smaller" type="button" onClick={verificationMonetizationHandler} disabled={!canWithdraw}>
      ...
      <AboutMonetizationModal isOpen={isAboutMonetizationModalOpen} onClose={closeAboutMonetizationModal}/>
      <ConfirmDialog isOnlyConfirm isOpen={isConfirmPasswordDialogOpen} onClose={closeConfirmPasswordDialogOpen} confirmHandler={closeConfirmPasswordDialogOpen} confirmLabel={lang('OK')}>
...
export default memo(withGlobal((global) => {
    ...
    const { settings: { byKey: { hasPassword, }, }, } = global;
