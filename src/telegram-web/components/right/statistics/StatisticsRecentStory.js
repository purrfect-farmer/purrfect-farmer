...
function StatisticsRecentStory({ chat, story, postStatistic }) {
    ...
    return (<div className={buildClassName(styles.root, styles.withImage)} onClick={handleClick}>
...
function renderSummary(lang, chat, blobUrl) {
    return (<span>
      {blobUrl ? (<span className={styles.imageContainer}>
          <img src={blobUrl} alt="" draggable={false} className={buildClassName(styles.image, styles.circle, styles.withStoryCircle)}/>
        </span>) : (<Avatar peer={chat} size="small" className={styles.image} withStorySolid forceUnreadStorySolid/>)}
