...
const TestOrdered = () => {
    const [items, setItems] = useState({ a: 1, b: 5, c: 10 });
    const [value, setValue] = useState();
    const [isDesc, setIsDesc] = useState(false);
    const insertData = (newValue) => {
        const key = `key${Math.random()}`;
        setItems((currentItems) => ({ ...currentItems, [key]: newValue }));
        setValue(undefined);
    };
    ...
    return (<div id="mainContainer">
      <div>
        <p>
          <label>
            <input type="checkbox" checked={isDesc} onChange={handleDescChange}/>
            {' '}
            — DESC
          </label>
        </p>
      ...
      <ul teactFastList>
        {sortedItems.map(([key, itemValue]) => (<MyComponent teactOrderKey={itemValue} key={key} value={itemValue} onChange={(newValue) => updateData(key, Number(newValue))} onDelete={() => deleteData(key)}/>))}
      </ul>
    </div>);
};
function MyComponent({ value, onChange, onDelete, }) {
