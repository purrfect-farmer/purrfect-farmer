...
import { DARK_THEME_BG_COLOR, INACTIVE_MARKER, LIGHT_THEME_BG_COLOR, PAGE_TITLE, } from '../config';
...
const App = ({ authState, isScreenLocked, hasPasscode, isInactiveAuth, hasWebAuthTokenFailed, isTestServer, theme, }) => {
    ...
    return (<UiLoader page={page} isMobile={isMobile}>
      <Transition name="fade" activeKey={activeKey} shouldCleanup className={buildClassName('full-height', (activeKey === AppScreens.auth || prevActiveKey === AppScreens.auth) && 'is-auth')} renderCount={TRANSITION_RENDER_COUNT}>
        {renderContent}
      </Transition>
