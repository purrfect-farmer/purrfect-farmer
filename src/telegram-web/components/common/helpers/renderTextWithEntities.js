...
export function renderTextWithEntities({ text, entities, highlight, emojiSize, shouldRenderAsHtml, containerId, asPreview, isProtected, observeIntersectionForLoading, observeIntersectionForPlaying, withTranslucentThumbs, sharedCanvasRef, sharedCanvasHqRef, cacheBuster, forcePlayback, noCustomEmojiPlayback, isInSelectMode, chatId, messageId, threadId, maxTimestamp, }) {
    ...
    function renderEntity(textPartStart, textPartEnd, organizedEntity, isLastEntity) {
        ...
        if (nestedEntities.length) {
            ...
            nestedEntities.forEach((nestedEntity, nestedEntityIndex) => {
                const { renderResult: nestedResult, entityEndIndex: nestedEntityEndIndex, } = renderEntity(nestedIndex, entityEndIndex, nestedEntity, nestedEntityIndex === nestedEntities.length - 1);
    ...
    organizedEntities.forEach((entity, arrayIndex) => {
        const { renderResult, entityEndIndex } = renderEntity(index, text.length, entity, arrayIndex === organizedEntities.length - 1);
        result.push(...renderResult);
        index = entityEndIndex;
    });
    return result;
}
export function getTextWithEntitiesAsHtml(formattedText) {
...
function renderMessagePart({ content, highlight, focusedQuote, emojiSize, shouldRenderAsHtml, asPreview, }) {
...
export function insertTextEntity(entities, newEntity) {
...
function processEntity({ entity, entityContent, nestedEntityContent, highlight, containerId, asPreview, isProtected, observeIntersectionForLoading, observeIntersectionForPlaying, withTranslucentThumbs, emojiSize, sharedCanvasRef, sharedCanvasHqRef, cacheBuster, forcePlayback, noCustomEmojiPlayback, isInSelectMode, chatId, messageId, threadId, maxTimestamp, }) {
    ...
    if (asPreview) {
        ...
        if (entity.type === ApiMessageEntityTypes.CustomEmoji) {
            return (<CustomEmoji key={cacheBuster ? `${cacheBuster}-${entity.offset}` : undefined} documentId={entity.documentId} size={emojiSize} isSelectable withSharedAnimation sharedCanvasRef={sharedCanvasRef} sharedCanvasHqRef={sharedCanvasHqRef} observeIntersectionForLoading={observeIntersectionForLoading} observeIntersectionForPlaying={observeIntersectionForPlaying} withTranslucentThumb={withTranslucentThumbs} forceAlways={forcePlayback} noPlay={noCustomEmojiPlayback}/>);
        }
        return text;
    }
    switch (entity.type) {
        ...
        case ApiMessageEntityTypes.Blockquote:
            return (<Blockquote canBeCollapsible={entity.canCollapse} isToggleDisabled={isInSelectMode}>
          {renderNestedMessagePart()}
        </Blockquote>);
        case ApiMessageEntityTypes.BotCommand:
            return (<a onClick={handleBotCommandClick} className="text-entity-link" dir="auto" data-entity-type={entity.type}>
          {renderNestedMessagePart()}
        </a>);
        case ApiMessageEntityTypes.Hashtag: {
            const [tag, username] = entityContent.split('@');
            return (<a onClick={() => handleHashtagClick(tag, username)} className="text-entity-link" dir="auto" data-entity-type={entity.type}>
          {renderNestedMessagePart()}
        </a>);
        }
        ...
        case ApiMessageEntityTypes.Code:
            return (<code className={buildClassName('text-entity-code', 'clickable')} onClick={handleCodeClick} role="textbox" tabIndex={0} data-entity-type={entity.type}>
          {renderNestedMessagePart()}
        </code>);
        case ApiMessageEntityTypes.Email:
            return (<a href={`mailto:${entityText}`} target="_blank" rel="noopener noreferrer" className="text-entity-link" dir="auto" data-entity-type={entity.type}>
          {renderNestedMessagePart()}
        </a>);
        case ApiMessageEntityTypes.Italic:
            return <em data-entity-type={entity.type}>{renderNestedMessagePart()}</em>;
        case ApiMessageEntityTypes.MentionName:
            return (<MentionLink userId={entity.userId}>
          {renderNestedMessagePart()}
        </MentionLink>);
        case ApiMessageEntityTypes.Mention:
            return (<MentionLink username={entityText}>
          {renderNestedMessagePart()}
        </MentionLink>);
        case ApiMessageEntityTypes.Phone:
            return (<a href={`tel:${entityText}`} className="text-entity-link" dir="auto" data-entity-type={entity.type}>
          {renderNestedMessagePart()}
        </a>);
        ...
        case ApiMessageEntityTypes.Url:
            return (<SafeLink url={getLinkUrl(entityText, entity)} text={entityText}>
          {renderNestedMessagePart()}
        </SafeLink>);
        ...
        case ApiMessageEntityTypes.Timestamp:
            ...
            return (<a onClick={() => handleTimecodeClick(chatId, messageId, threadId, entity.timestamp)} className="text-entity-link" dir="auto" data-entity-type={entity.type}>
          {renderNestedMessagePart()}
        </a>);
        ...
        case ApiMessageEntityTypes.QuoteFocus:
            return (<span className="matching-text-highlight is-quote">{renderNestedMessagePart()}</span>);
        default:
            return renderNestedMessagePart();
    }
}
function processEntityAsHtml(entity, entityContent, nestedEntityContent) {
    ...
    const content = entity.type === ApiMessageEntityTypes.Pre ? entityContent.trimEnd() : entityContent;
    ...
    switch (entity.type) {
        ...
        case ApiMessageEntityTypes.Pre:
            ...
            return `\`\`\`${renderText(entity.language || '', ['escape_html'])[0]}<br/>${renderedContent}<br/>\`\`\`<br/>`;
        case ApiMessageEntityTypes.Strike:
...
function handleTimecodeClick(chatId, messageId, threadId, timestamp) {
