...
const PeerPicker = ({ className, categories, itemIds, categoryPlaceholderKey, filterValue, filterPlaceholder, notFoundText, searchInputId, itemClassName, isLoading, noScrollRestore, isSearchable, lockedUnselectedSubtitle, forceShowSelf, isViewOnly, itemInputType, withStatus, withPeerTypes, withPeerUsernames, withDefaultPadding, onFilterChange, onDisabledClick, onLoadMore, ...optionalProps }) => {
    ...
    const handleItemClick = useLastCallback((id) => {
        if (lockedSelectedIdsSet.has(id)) {
    ...
    const renderItem = useCallback((id, isCategory) => {
        ...
        function getSubtitle() {
            if (isAlwaysUnselected)
                return [lockedUnselectedSubtitle];
            if (!peer)
                return undefined;
        ...
        return (<PickerItem key={id} className={buildClassName(ITEM_CLASS_NAME, itemClassName)} title={<FullNameTitle peer={peerOrCategory}/>} avatarElement={(<Avatar peer={peer || category} isSavedMessages={isSelf} size="medium"/>)} subtitle={subtitle} subtitleClassName={subtitleClassName} disabled={isLocked} inactive={isViewOnly} ripple inputElement={getInputElement()} inputPosition="end" onClick={() => handleItemClick(id)} onDisabledClick={onDisabledClick && (() => onDisabledClick(id, isAlwaysSelected))}/>);
    }, [
    ...
    return (<div className={buildClassName(styles.container, className)}>
      {isSearchable && (<div className={buildClassName(styles.header, 'custom-scroll')} dir={lang.isRtl ? 'rtl' : undefined}>
          {selectedCategories?.map((category) => (<PeerChip className={styles.peerChip} customPeer={categoriesByType[category]} onClick={handleItemClick} clickArg={category} canClose/>))}
          {lockedSelectedIds?.map((id, i) => (<PeerChip className={styles.peerChip} peerId={id} isMinimized={shouldMinimize && i < selectedIds.length - ALWAYS_FULL_ITEMS_COUNT} forceShowSelf={forceShowSelf} onClick={handleItemClick} clickArg={id}/>))}
          {unlockedSelectedIds.map((id, i) => (<PeerChip className={styles.peerChip} peerId={id} isMinimized={shouldMinimize && i + (lockedSelectedIds?.length || 0) < selectedIds.length - ALWAYS_FULL_ITEMS_COUNT} canClose onClick={handleItemClick} clickArg={id}/>))}
          <InputText id={searchInputId} ref={inputRef} value={filterValue} onChange={handleFilterChange} placeholder={filterPlaceholder || lang('SelectChat')}/>
        </div>)}

      {viewportIds?.length ? (<InfiniteScroll className={buildClassName(styles.pickerList, withDefaultPadding && styles.padded, 'custom-scroll')} items={viewportIds} itemSelector={`.${ITEM_CLASS_NAME}`} beforeChildren={beforeChildren} onLoadMore={getMore} noScrollRestore={noScrollRestore}>
          {viewportIds.map((id) => renderItem(id))}
        </InfiniteScroll>) : !isLoading && viewportIds && !viewportIds.length ? (<p className={styles.noResults}>{notFoundText || 'Sorry, nothing found.'}</p>) : (<Loading />)}
    </div>);
};
