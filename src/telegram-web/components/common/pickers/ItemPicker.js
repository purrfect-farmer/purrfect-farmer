...
const ItemPicker = ({ className, isSearchable, searchInputId, items, filterValue, notFoundText, isLoading, noScrollRestore, filterPlaceholder, isViewOnly, itemInputType, itemClassName, withDefaultPadding, forceRenderAllItems, onFilterChange, onDisabledClick, onLoadMore, ...optionalProps }) => {
    ...
    const handleItemClick = useLastCallback((value) => {
        if (allowMultiple) {
    ...
    const [viewportValuesList, getMore] = useInfiniteScroll(onLoadMore, sortedItemValuesList, Boolean(forceRenderAllItems || filterValue));
    ...
    const renderItem = useCallback((value) => {
        ...
        return (<PickerItem key={value} className={buildClassName(ITEM_CLASS_NAME, itemClassName)} title={label} subtitle={subLabel} disabled={isLocked} inactive={isViewOnly} ripple inputElement={getInputElement()} onClick={() => handleItemClick(value)} onDisabledClick={onDisabledClick && (() => onDisabledClick(value, isAlwaysSelected))}/>);
    }, [
    ...
    return (<div className={buildClassName(styles.container, className)}>
      {isSearchable && (<div className={buildClassName(styles.header, 'custom-scroll')} dir={lang.isRtl ? 'rtl' : undefined}>
          <InputText id={searchInputId} ref={inputRef} value={filterValue} onChange={handleFilterChange} placeholder={filterPlaceholder || lang('Search')}/>
        </div>)}

      {viewportValuesList?.length ? (<InfiniteScroll className={buildClassName(styles.pickerList, withDefaultPadding && styles.padded, 'custom-scroll')} items={viewportValuesList} itemSelector={`.${ITEM_CLASS_NAME}`} onLoadMore={getMore} noScrollRestore={noScrollRestore}>
          {viewportValuesList.map((value) => renderItem(value))}
        </InfiniteScroll>) : !isLoading && viewportValuesList && !viewportValuesList.length ? (<p className={styles.noResults}>{notFoundText || lang('SearchEmptyViewTitle')}</p>) : (<Loading />)}
    </div>);
};
