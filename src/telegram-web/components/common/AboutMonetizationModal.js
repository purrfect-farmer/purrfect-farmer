...
const AboutMonetizationModal = ({ isOpen, onClose, }) => {
    const oldLang = useOldLang();
    const lang = useLang();
    const blockchainText = useMemo(() => {
        const linkText = oldLang('LearnMore');
        return lang('ChannelEarnLearnCoinAbout', {
            link: (<SafeLink url={oldLang('MonetizationInfoTONLink')} text={linkText}>
            {linkText}
            <Icon name="next"/>
          </SafeLink>),
        }, {
    ...
    const modalData = useMemo(() => {
        if (!isOpen)
            return undefined;
        const header = (<h3 className={styles.title}>{oldLang('lng_channel_earn_learn_title')}</h3>);
        const listItemData = [
    ...
    return (<TableAboutModal isOpen={isOpen} listItemData={modalData.listItemData} headerIconName="cash-circle" withSeparator header={modalData.header} footer={modalData.footer} buttonText={oldLang('RevenueSharingAdsUnderstood')} onClose={onClose}/>);
};
