...
import { getHasAdminRight, getPrivateChatUserId, getUserFirstOrLastName, isChatBasicGroup, isChatChannel, isChatSuperGroup, isSystemBot, } from '../../global/helpers';
import { getPeerTitle } from '../../global/helpers/peers';
import { getSendersFromSelectedMessages, selectBot, selectCanDeleteSelectedMessages, selectChat, selectChatFullInfo, selectIsChatWithBot, selectSenderFromMessage, selectTabState, selectUser, } from '../../global/selectors';
...
const DeleteMessageModal = ({ isOpen, chat, isChannel, isSuperGroup, isSchedule, currentUserId, messageIds, isCreator, canDeleteForAll, contactName, willDeleteForCurrentUserOnly, willDeleteForAll, chatBot, adminMembersById, canBanUsers, linkedChatId, onConfirm, }) => {
    const { closeDeleteMessageModal, deleteMessages, reportChannelSpam, deleteChatMember, deleteScheduledMessages, exitMessageSelectMode, updateChatMemberBannedRights, deleteParticipantHistory, } = getActions();
    const prevIsOpen = usePreviousDeprecated(isOpen);
    const oldLang = useOldLang();
    const lang = useLang();
    const { permissions, havePermissionChanged, handlePermissionChange, resetPermissions, } = useManagePermissions(chat?.defaultBannedRights);
    const [peerIdsToDeleteAll, setPeerIdsToDeleteAll] = useState([]);
    const [peerIdsToBan, setPeerIdsToBan] = useState([]);
    const [peerIdsToReportSpam, setPeerIdsToReportSpam] = useState([]);
    ...
    const peerList = useMemo(() => {
        ...
        return senderArray ? unique(senderArray)
            .filter((peer) => (peer?.id !== chat?.id
            && peer?.id !== linkedChatId
            && peer?.id !== chat?.linkedMonoforumId)) : MEMO_EMPTY_ARRAY;
    ...
    const peerListToDeleteAll = useMemo(() => {
        return peerList.filter((peer) => (peer.id !== linkedChatId
    ...
    const peerListToReportSpam = useMemo(() => {
        return peerList.filter((peer) => (peer.id !== currentUserId
            && peer.id !== linkedChatId
            && peer.id !== chat?.linkedMonoforumId));
    }, [peerList, currentUserId, linkedChatId, chat?.linkedMonoforumId]);
    ...
    const ACTION_DELETE_OPTION = useMemo(() => {
        return [
            {
    ...
    const ACTION_BAN_OPTION = useMemo(() => {
        return [
            {
    ...
    const filterMessageIdByPeerId = useLastCallback((peerIds, selectedMessageIdList) => {
        if (!chat)
            return MEMO_EMPTY_ARRAY;
        const global = getGlobal();
    ...
    const handleReportSpam = useLastCallback((userMessagesMap) => {
    ...
    const handleDeleteMessages = useLastCallback((filteredMessageIdList) => {
        deleteMessages({ messageIds: filteredMessageIdList, shouldDeleteForAll: true });
    });
    const handleDeleteAllPeerMessages = useLastCallback((peerIdList) => {
        if (!chat)
            return;
    ...
    const handleDeleteMember = useLastCallback((filteredUserIdList) => {
        filteredUserIdList.forEach((userId) => {
            deleteChatMember({ chatId: chat.id, userId });
        });
    });
    const handleUpdateChatMemberBannedRights = useLastCallback((filteredUserIdList) => {
        filteredUserIdList.forEach((userId) => {
    ...
    const handleDeleteMessageList = useLastCallback(() => {
        ...
        if (isSchedule) {
            ...
            if (peerIdsToReportSpam?.length) {
                ...
                const messageList = messageIds.reduce((acc, msgId) => {
    ...
    function renderHeader() {
        return (<div className={shouldShowOption && styles.container} dir={oldLang.isRtl ? 'rtl' : undefined}>
        {shouldShowOption && (<AvatarList size="small" peers={peerList}/>)}
    ...
    function renderAdditionalActionOptions() {
        ...
        <CheckboxGroup options={ACTION_SPAM_OPTION} onChange={setPeerIdsToReportSpam} selected={peerIdsToReportSpam} nestedCheckbox={messageIds && peerList.length >= 2}/>
        {peerListToDeleteAll?.length > 0 && (<CheckboxGroup options={ACTION_DELETE_OPTION} onChange={setPeerIdsToDeleteAll} selected={peerIdsToDeleteAll} nestedCheckbox={messageIds && peerList.length >= 2}/>)}
        {peerListToBan?.length > 0 && (<CheckboxGroup options={ACTION_BAN_OPTION} onChange={setPeerIdsToBan} selected={peerIdsToBan} nestedCheckbox={messageIds && peerList.length >= 2}/>)}
      </div>);
    }
    function renderPartiallyRestrictedUser() {
        return (<div className={buildClassName(styles.restrictionContainer, isAdditionalOptionsVisible && styles.restrictionContainerOpen)}>
        <h3 className={buildClassName(styles.actionTitle, styles.restrictionTitle)}>
          {oldLang('UserRestrictionsCanDoUsers', peerList.length)}
        </h3>
        <PermissionCheckboxList withCheckbox chatId={chat?.id} isMediaDropdownOpen={isMediaDropdownOpen} setIsMediaDropdownOpen={setIsMediaDropdownOpen} handlePermissionChange={handlePermissionChange} permissions={permissions} className={buildClassName(styles.dropdownList, isMediaDropdownOpen && styles.dropdownListOpen)}/>
      </div>);
    }
    return (<Modal isOpen={isOpen} onClose={onCloseHandler} onEnter={canDeleteForAll ? undefined : handleDeleteMessageList} className={styles.root}>
      <div className={styles.main}>
        ...
        {shouldShowOption && (<>
            ...
            {peerIdsToBan?.length && canBanUsers ? (<ListItem narrow buttonClassName={styles.button} onClick={toggleAdditionalOptions}>
                {oldLang(isAdditionalOptionsVisible ? 'DeleteToggleBanUsers' : 'DeleteToggleRestrictUsers')}
                <Icon name={isAdditionalOptionsVisible ? 'up' : 'down'} className={buildClassName(styles.button, 'ml-2')}/>
              </ListItem>) : setIsAdditionalOptionsVisible(false)}
          </>)}
        {(canDeleteForAll || chatBot || !shouldShowOption) && (<>
            ...
            {willDeleteForCurrentUserOnly && (<p>{oldLang('lng_delete_for_me_chat_hint', 1, 'i')}</p>)}
            {willDeleteForAll && (<p>{oldLang('lng_delete_for_everyone_hint', 1, 'i')}</p>)}
          </>)}
        {canDeleteForAll && (<Checkbox className="dialog-checkbox" label={contactName ? renderText(oldLang('DeleteMessagesOptionAlso', contactName))
                : oldLang('Conversation.DeleteMessagesForEveryone')} checked={shouldDeleteForAll} onCheck={setShouldDeleteForAll}/>)}
        <div className={buildClassName('dialog-buttons', isMediaDropdownOpen ? styles.dialogButtons : styles.proceedButtons)}>
...
export default memo(withGlobal((global) => {
    const { deleteMessageModal, } = selectTabState(global);
