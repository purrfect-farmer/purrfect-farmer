...
const GiftMenuItems = ({ peerId, canManage, gift: typeGift, currentUserEmojiStatus, collectibleEmojiStatuses, }) => {
    const { showNotification, openChatWithDraft, openGiftTransferModal, openGiftResalePriceComposerModal, openGiftStatusInfoModal, setEmojiStatus, toggleSavedGiftPinned, changeGiftVisibility, updateStarGiftPrice, closeGiftInfoModal, } = getActions();
    const lang = useLang();
    ...
    const handleTransfer = useLastCallback(() => {
        if (!savedGift || savedGift?.gift.type !== 'starGiftUnique')
            return;
    ...
    const handleUnsell = useLastCallback(() => {
        if (!savedGift || savedGift.gift.type !== 'starGiftUnique' || !savedGift.inputGift)
            return;
        closeGiftInfoModal();
    ...
    const handleWear = useLastCallback(() => {
        if (gift?.type !== 'starGiftUnique' || !userCollectibleStatus)
            return;
    ...
    return (<>
      {hasPinOptions && (<MenuItem icon={savedGift.isPinned ? 'unpin' : 'pin'} onClick={handleTogglePin}>
      ...
      {canManage && isGiftUnique && (<MenuItem icon="diamond" onClick={handleTransfer}>
          {lang('GiftInfoTransfer')}
        </MenuItem>)}
      {canManage && isGiftUnique && !giftResalePrice && (<MenuItem icon="sell-outline" onClick={handleSell}>
          {lang('Sell')}
        </MenuItem>)}
      {canManage && isGiftUnique && Boolean(giftResalePrice) && (<MenuItem icon="unlist-outline" onClick={handleUnsell}>
          {lang('GiftInfoUnlist')}
        </MenuItem>)}
      {canManage && savedGift && (<MenuItem icon={savedGift.isUnsaved ? 'eye-outline' : 'eye-crossed-outline'} onClick={handleTriggerVisibility}>
          {lang(savedGift.isUnsaved ? 'GiftActionShow' : 'GiftActionHide')}
        </MenuItem>)}
      {canWear && (<MenuItem icon="crown-wear-outline" onClick={handleWear}>
          {lang('GiftInfoWear')}
        </MenuItem>)}
      {canTakeOff && (<MenuItem icon="crown-take-off-outline" onClick={handleTakeOff}>
