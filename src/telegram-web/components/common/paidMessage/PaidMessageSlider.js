...
const PaidMessageSlider = ({ min = 0, max, value, disabled, readOnly, bold, className, defaultValue, onChange, canChangeChargeForMessages, }) => {
    const lang = useLang();
    ...
    return (<div className={mainClassName}>
      ...
      <div className="slider-main">
        <div className="slider-fill-track" style={`width: ${(getProgress(points, value) / points.length) * 100}%`}/>
        <input min={0} max={points.length} defaultValue={getProgress(points, defaultValue)} step="any" type="range" className="RangeSlider__input" onChange={handleChange}/>
      </div>
    </div>);
};
