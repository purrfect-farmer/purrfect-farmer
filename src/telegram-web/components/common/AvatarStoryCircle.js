...
function AvatarStoryCircle({ size, className, peerStories, storyIds, lastReadId, withExtraGap, appTheme, }) {
    const ref = useRef();
    ...
    return (<canvas ref={ref} className={buildClassName('story-circle', className)} style={`max-width: ${adaptedSize}px; max-height: ${adaptedSize}px;`}/>);
}
...
export function drawGradientCircle({ canvas, size, color, segmentsCount, readSegmentsCount = 0, withExtraGap = false, readSegmentColor, dpr, }) {
