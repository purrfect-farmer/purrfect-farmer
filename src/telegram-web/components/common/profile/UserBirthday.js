...
const UserBirthday = ({ user, birthday, isPremiumPurchaseBlocked, birthdayNumbers, animatedEmojiEffects, isInSettings, }) => {
    const { openGiftModal, requestConfetti } = getActions();
    const ref = useRef();
    const animationPlayedRef = useRef(false);
    const [isPlayingAnimation, playAnimation, stopAnimation] = useFlag();
    const lang = useOldLang();
    const { formattedDate, isToday, age, } = useMemo(() => {
    ...
    useEffect(() => {
        if (isPlayingAnimation) {
            ...
            if (!column)
                return;
            requestMeasure(() => {
                const { top, left, width, height, } = column.getBoundingClientRect();
                requestConfetti({
                    top,
                    left,
                    width,
    ...
    return (<div className={styles.root}>
      <ListItem icon="calendar" secondaryIcon={canGiftPremium ? 'gift' : undefined} secondaryIconClassName={styles.giftIcon} multiline narrow ref={ref} ripple={!isStatic} onClick={handleClick} isStatic={isStatic} onSecondaryIconClick={handleOpenGiftModal}>
      ...
      {isPlayingAnimation && IS_OFFSET_PATH_SUPPORTED && numbersForAge?.map((sticker, index) => (<div className={buildClassName(styles.number, index > 0 && styles.shiftOrigin)} style={`--digit-offset: ${index}`}>
          <StickerView containerRef={ref} sticker={sticker} size={NUMBER_STICKER_SIZE} forceAlways/>
        </div>))}
      {isPlayingAnimation && effectSticker && (<div className={styles.effect}>
          <StickerView containerRef={ref} sticker={effectSticker} size={EFFECT_SIZE} shouldLoop forceAlways/>
        </div>)}
    </div>);
};
