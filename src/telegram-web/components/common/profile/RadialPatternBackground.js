...
const MIN_SIZE = 250;
const RadialPatternBackground = ({ backgroundColors, patternColor, patternIcon, clearBottomSector, className, }) => {
    ...
    const draw = useLastCallback(() => {
        ...
        if (!width || !height)
            return;
        ctx.save();
        patternPositions.forEach(({ x, y, sizeFactor, }) => {
    ...
    return (<div ref={containerRef} className={buildClassName(styles.root, className)} style={buildStyle(`--_bg-1: ${backgroundColors[0]}`, `--_bg-2: ${backgroundColors[1] || backgroundColors[0]}`)}>
