...
import { getUtcOffset, getWeekStart, shiftTimeRanges, splitDays, } from '../../../util/dates/workHours';
...
const BusinessHours = ({ businessHours, }) => {
    ...
    const workHours = useMemo(() => {
        ...
        DAYS.forEach((day) => {
            ...
            result[day] = segments.map(({ startMinute, endMinute }) => {
                if (endMinute - startMinute === 24 * 60)
                    return lang('BusinessHoursDayFullOpened');
    ...
    const isBusinessOpen = useMemo(() => {
        ...
        return localTimeHours.some(({ startMinute, endMinute }) => (startMinute <= minutesSinceWeekStart && minutesSinceWeekStart <= endMinute));
    ...
    const handleTriggerOffset = useLastCallback((e) => {
        e.stopPropagation();
        if (isMyTime) {
            showInLocalTime();
        }
    ...
    useEffect(() => {
        if (!isExpanded)
            return;
        const slide = document.querySelector(`.${ACTIVE_SLIDE_CLASS_NAME} > .${styles.timetable}`);
        if (!slide)
            return;
    ...
    const handleAnimationStart = useLastCallback(() => {
        const slide = document.querySelector(`.${TO_SLIDE_CLASS_NAME} > .${styles.timetable}`);
        requestMeasure(() => {
            const height = slide.offsetHeight;
            requestMutation(() => {
                transitionRef.current.style.height = `${height}px`;
    ...
    return (<ListItem icon="clock" iconClassName={styles.icon} multiline className={styles.root} isStatic={isExpanded} ripple narrow withColorTransition onClick={handleClick}>
      ...
      {isExpanded && (<div className={styles.bottom}>
          {Boolean(timezoneMinuteDifference) && (<div className={styles.offsetTrigger} role="button" tabIndex={0} onMouseDown={!IS_TOUCH_ENV ? handleTriggerOffset : undefined} onClick={IS_TOUCH_ENV ? handleTriggerOffset : undefined}>
              {lang(isMyTime ? 'BusinessHoursProfileSwitchMy' : 'BusinessHoursProfileSwitchLocal')}
            </div>)}
          <Transition className={styles.transition} ref={transitionRef} name="fade" activeKey={Number(isMyTime)} onStart={handleAnimationStart}>
