...
export function useStickerPickerObservers(containerRef, headerRef, idPrefix, isHidden) {
    const stickerSetIntersectionsRef = useRef([]);
    const [activeSetIndex, setActiveSetIndex] = useState(0);
    const { observe: observeIntersectionForSet, freeze: freezeForSet, unfreeze: unfreezeForSet, } = useIntersectionObserver({
    ...
    const { observe: observeIntersectionForShowingItems, freeze: freezeForShowingItems, unfreeze: unfreezeForShowingItems, } = useIntersectionObserver({
        rootRef: containerRef,
        throttleMs: STICKER_INTERSECTION_THROTTLE,
        margin: STICKER_INTERSECTION_MARGIN,
    });
    const { observe: observeIntersectionForPlayingItems, } = useIntersectionObserver({
        rootRef: containerRef,
        throttleMs: STICKER_INTERSECTION_THROTTLE,
        margin: STICKER_INTERSECTION_MARGIN,
    });
    ...
    const selectStickerSet = useLastCallback((index) => {
