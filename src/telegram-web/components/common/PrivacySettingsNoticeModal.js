...
const PrivacySettingsNoticeModal = ({ isOpen, isReadDate, user }) => {
    const lang = useOldLang();
    const { updateGlobalPrivacySettings, openPremiumModal, closePrivacySettingsNoticeModal, showNotification, setPrivacyVisibility, loadUser, } = getActions();
    ...
    return (<Modal isSlim isOpen={isOpen} onClose={handleClose}>
      <div className={styles.container} dir={lang.isRtl ? 'rtl' : undefined}>
        <Button className={styles.closeButton} color="translucent" round size="smaller" onClick={handleClose} ariaLabel="Close">
          <Icon name="close"/>
        </Button>
        <AnimatedIconWithPreview tgsUrl={isReadDate ? LOCAL_TGS_URLS.ReadTime : LOCAL_TGS_URLS.LastSeen} size={84} className={styles.icon} nonInteractive noLoop/>
        ...
        <Button size="smaller" onClick={isReadDate ? handleShowReadTime : handleShowLastSeen} className={styles.button}>
