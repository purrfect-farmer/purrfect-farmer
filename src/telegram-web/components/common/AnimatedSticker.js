import { getIsHeavyAnimating, memo, useEffect, useRef, useState, useUnmountCleanup, } from '../../lib/teact/teact';
...
const AnimatedSticker = ({ ref, renderId, className, style, tgsUrl, play, playSegment, speed, noLoop, size, quality, isLowPriority, color, forceAlways, forceOnHeavyAnimation, sharedCanvas, sharedCanvasCoords, onClick, onLoad, onEnded, onLoop, }) => {
    ...
    const [canInitialize, markCanInitialize, unmarkCanInitialize,] = useFlag(!getIsHeavyAnimating() || shouldForceOnHeavyAnimation);
    useHeavyAnimation(unmarkCanInitialize, markCanInitialize, shouldForceOnHeavyAnimation);
    useEffect(() => {
        if (shouldForceOnHeavyAnimation)
            markCanInitialize();
    ...
    const init = useLastCallback(() => {
        ...
        const newAnimation = getRLottie().init(tgsUrl, container, renderId || generateUniqueId(), {
            size,
            noLoop,
            quality,
            isLowPriority,
    ...
    return (<div ref={containerRef} className={buildClassName('AnimatedSticker', className)} style={buildStyle(size !== undefined && `width: ${size}px; height: ${size}px;`, onClick && !IS_ELECTRON && 'cursor: pointer', colorFilter, style)} onClick={onClick}/>);
};
