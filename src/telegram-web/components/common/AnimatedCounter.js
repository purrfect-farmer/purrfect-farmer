...
const AnimatedCounter = ({ text, className, isDisabled, ref, }) => {
    const { isRtl } = useLang();
    const prevText = usePreviousDeprecated(text);
    const forceUpdate = useForceUpdate();
    const shouldAnimate = scheduleAnimation(!isDisabled && selectCanAnimateInterface(getGlobal()) && prevText !== undefined && prevText !== text);
    ...
    return (<span ref={ref} className={buildClassName(className, !isDisabled && styles.root)} dir={isRtl ? 'rtl' : undefined}>
      {characters}
    </span>);
};
export default memo(AnimatedCounter);
function scheduleAnimation(condition) {
    if (!condition || scheduledAnimationsCounter >= MAX_SIMULTANEOUS_ANIMATIONS)
        return false;
...
function renderAnimatedCharacters(text, prevText) {
    ...
    for (let i = 0; i <= textLength; i++) {
        ...
        if (prevText && prevTextCharIndex >= 0 && text[charIndex] !== prevText[prevTextCharIndex]) {
            elements.unshift(<div className={styles.characterContainer}>
