...
const EmbeddedStoryForward = ({ className, forwardInfo, sender, story, }) => {
    ...
    function renderTextContent() {
        if (story && 'content' in story && story.content.text) {
            return (<p className="embedded-text-wrapper">
    ...
    function renderSender() {
        ...
        const icon = !isUserId(sender.id) ? 'channel-filled' : 'user-filled';
        return (<>
        {icon && <Icon name={icon} className="embedded-chat-icon"/>}
        {senderTitle && renderText(senderTitle)}
      </>);
    }
    return (<PeerColorWrapper ref={ref} className={buildClassName('EmbeddedMessage', className, getPeerColorClass(sender, true, true))} dir={lang.isRtl ? 'rtl' : undefined} onClick={handleClick} onMouseDown={handleMouseDown}>
      {sender?.color?.backgroundEmojiId && (<EmojiIconBackground emojiDocumentId={sender.color.backgroundEmojiId} className="EmbeddedMessage--background-icons"/>)}
...
export default memo(withGlobal((global, { forwardInfo }) => {
