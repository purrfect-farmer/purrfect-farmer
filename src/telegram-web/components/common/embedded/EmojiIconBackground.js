...
const EmojiIconBackground = ({ emojiDocumentId, className, }) => {
    ...
    const updateCanvas = useLastCallback(() => {
        ...
        if (!canvas || !emojiImage || !customColor)
            return;
        const context = canvas.getContext('2d');
        const { width, height } = canvas;
        context.clearRect(0, 0, width, height);
        ICON_POSITIONS.forEach(({ inline, block, opacity, scale, }) => {
    ...
    const updateCanvasSize = useThrottleForHeavyAnimation((parentWidth, parentHeight) => {
        requestMutation(() => {
            const canvas = canvasRef.current;
            if (!canvas)
                return;
    ...
    const handleResize = useThrottleForHeavyAnimation((entry) => {
    ...
    return (<div className={buildClassName(styles.root, className)} ref={containerRef}>
