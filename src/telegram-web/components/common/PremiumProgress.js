...
const PremiumProgress = ({ leftText, rightText, floatingBadgeText, floatingBadgeIcon, progress, isPrimary, className, }) => {
    const lang = useOldLang();
    const floatingBadgeRef = useRef();
    ...
    return (<div ref={parentContainerRef} className={buildClassName(styles.root, hasFloatingBadge && styles.withBadge, isPrimary && styles.primary, className)} style={buildStyle(progress !== undefined && `--progress: ${progress}`, tailPosition !== undefined && `--tail-position: ${tailPosition}`, `--shift-x: ${shiftX}`)}>
      {hasFloatingBadge && (<div className={styles.badgeContainer}>
          <div className={styles.floatingBadgeWrapper}>
            <div className={styles.floatingBadge} ref={floatingBadgeRef}>
              {floatingBadgeIcon && <Icon name={floatingBadgeIcon} className={styles.floatingBadgeIcon}/>}
              {floatingBadgeText && (<div className={styles.floatingBadgeValue} dir={lang.isRtl ? 'rtl' : undefined}>
                  {floatingBadgeText}
                </div>)}
            </div>
