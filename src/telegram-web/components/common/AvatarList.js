...
const AvatarList = ({ peers, size, className, limit = DEFAULT_LIMIT, badgeText, }) => {
    const lang = useOldLang();
    const pxSize = typeof size === 'number' ? size : AVATAR_SIZES[size];
    const renderingBadgeText = useMemo(() => {
        if (badgeText)
            return badgeText;
        if (!peers?.length || peers.length <= limit)
            return undefined;
        return `+${peers.length - limit}`;
    }, [badgeText, peers, limit]);
    return (<div className={buildClassName(className, styles.root)} style={`--_size: ${pxSize}px;`} dir={lang.isRtl ? 'rtl' : 'ltr'}>
      {peers?.slice(0, limit).map((peer) => <Avatar size={size} peer={peer} className={styles.avatar}/>)}
      {renderingBadgeText && (<div className={styles.badge}>
          {renderingBadgeText}
        </div>)}
    </div>);
};
