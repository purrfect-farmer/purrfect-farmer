...
const BotAppResults = ({ searchQuery, isLoading, foundIds, recentBotIds, }) => {
    const { searchPopularBotApps, openChatWithInfo, } = getActions();
    ...
    const handleChatClick = useLastCallback((id) => {
    ...
    return (<div ref={containerRef} className="LeftSearch--content">
      <InfiniteScroll className="search-content custom-scroll" items={canRenderContents ? filteredFoundIds : undefined} onLoadMore={handleLoadMore} noFastList>
        {!canRenderContents && <Loading />}
        {canRenderContents && !filteredFoundIds?.length && (<NothingFound text={lang('ChatList.Search.NoResults')} description={lang('ChatList.Search.NoResultsDescription')}/>)}
        {canRenderContents && !searchQuery && Boolean(recentBotIds?.length) && (<div className="search-section">
            <h3 className="section-heading">
              {recentBotIds.length > LESS_LIST_ITEMS_AMOUNT && (<Link className="Link" onClick={handleToggleShowMoreMine}>
            ...
            {recentBotIds.map((id, index) => {
                ...
                return (<LeftSearchResultChat chatId={id} onClick={handleChatClick} withOpenAppButton/>);
            })}
          </div>)}
        {canRenderContents && Boolean(filteredFoundIds?.length) && (<div className="search-section">
