import { useCallback, useLayoutEffect, useRef, } from '../../../../lib/teact/teact';
import { ANIMATION_END_DELAY, CHAT_HEIGHT_PX } from '../../../../config';
import { requestMutation } from '../../../../lib/fasterdom/fasterdom';
import { getMessageIsSpoiler, getMessageMediaHash, getMessageMediaThumbDataUri, getMessageRoundVideo, getMessageSticker, getMessageVideo, } from '../../../../global/helpers';
...
export default function useChatListEntry({ chat, topics, lastMessage, statefulMediaContent, chatId, typingStatus, draft, lastMessageTopic, lastMessageSender, observeIntersection, animationType, orderDiff, withInterfaceAnimations, isTopic, isSavedDialog, isPreview, }) {
    ...
    const renderLastMessageOrTyping = useCallback(() => {
        ...
        if (canDisplayDraft) {
            return (<p className="last-message" dir={oldLang.isRtl ? 'auto' : 'ltr'}>
        ...
        const senderName = lastMessageSender
            ...
            : undefined;
        return (<p className="last-message shared-canvas-container" dir={oldLang.isRtl ? 'auto' : 'ltr'}>
        {senderName && (<>
    ...
    function renderSubtitle() {
        if (chat?.isForum && !isTopic) {
            return (<ChatForumLastMessage chat={chat} renderLastMessage={renderLastMessageOrTyping} observeIntersection={observeIntersection} topics={topics}/>);
        }
...
function renderSummary(message, observeIntersection, blobUrl, isRoundVideo) {
    const messageSummary = (<MessageSummary message={message} noEmoji={Boolean(blobUrl)} observeIntersectionForLoading={observeIntersection} inChatList/>);
    if (!blobUrl) {
        return messageSummary;
    }
    const isSpoiler = getMessageIsSpoiler(message);
    return (<span className="media-preview">
      <img src={blobUrl} alt="" className={buildClassName('media-preview--image', isRoundVideo && 'round', isSpoiler && 'media-preview-spoiler')} draggable={false}/>
