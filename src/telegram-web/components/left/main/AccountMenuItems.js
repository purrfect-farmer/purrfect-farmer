...
const AccountMenuItems = ({ currentUser, totalLimit, onSelectCurrent, }) => {
    ...
    return (<>
      {Object.entries(accounts || {})
            ...
            return (<>
              <MenuItem className="account-menu-item" customIcon={(<Avatar size="mini" className="account-avatar" peer={mockUser} previewUrl={account.avatarUri}/>)} onClick={account.userId === currentUser.id ? onSelectCurrent : undefined} href={account.userId !== currentUser.id ? getAccountSlotUrl(Number(slot)) : undefined}>
      ...
      {newAccountUrl && (<MenuItem icon="add" rel="noopener" // Allow referrer to be passed
         href={!shouldShowLimit ? newAccountUrl : undefined} onClick={shouldShowLimit ? handleLimitClick : undefined}>
          {lang('MenuAddAccount')}
        </MenuItem>)}
    </>);
};
