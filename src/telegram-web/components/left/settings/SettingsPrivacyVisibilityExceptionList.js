...
import { isChatChannel, isDeletedUser, } from '../../../global/helpers';
...
const SettingsPrivacyVisibilityExceptionList = ({ isAllowList, withPremiumCategory, withMiniAppsCategory, screen, isActive, currentUserId, settings, usersOnly = false, onReset, }) => {
    const { setPrivacySettings } = getActions();
    const oldLang = useOldLang();
    const lang = useLang();
    const customPeerBots = useMemo(() => {
        return {
            isCustomPeer: true,
            type: 'bots',
    ...
    const [newSelectedCategoryTypes, setNewSelectedCategoryTypes] = useState(selectedCategoryTypes);
    ...
    function getCustomCategory() {
        if (withPremiumCategory)
            return PREMIUM_CATEGORY;
        if (withMiniAppsCategory && isAllowList)
            return miniAppsCategory;
        return undefined;
    }
    return (<div className="NewChat-inner step-1">
      <PeerPicker categories={getCustomCategory()} itemIds={displayedIds || []} selectedIds={newSelectedContactIds} selectedCategories={newSelectedCategoryTypes} filterValue={searchQuery} filterPlaceholder={isAllowList ? oldLang('AlwaysAllowPlaceholder') : oldLang('NeverAllowPlaceholder')} categoryPlaceholderKey="PrivacyUserTypes" searchInputId="new-group-picker-search" isSearchable onSelectedIdsChange={handleSelectedContactIdsChange} onSelectedCategoriesChange={handleSelectedCategoriesChange} onFilterChange={setSearchQuery} allowMultiple itemInputType="checkbox" withDefaultPadding withStatus/>

      <FloatingActionButton isShown={isSubmitShown} onClick={handleSubmit} ariaLabel={isAllowList ? oldLang('AlwaysAllow') : oldLang('NeverAllow')}>
        <Icon name="check"/>
