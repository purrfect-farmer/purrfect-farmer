...
import { selectIsCurrentUserPremium, selectNewNoncontactPeersRequirePremium, selectNonContactPeersPaidStars, } from '../../../global/selectors';
...
function PrivacyMessages({ isActive, canLimitNewMessagesWithoutPremium, canChargeForMessages, shouldNewNonContactPeersRequirePremium, shouldChargeForMessages, nonContactPeersPaidStars, isCurrentUserPremium, noPaidReactionsForUsersCount, onReset, privacy, }) {
    ...
    const [chargeForMessages, setChargeForMessages] = useState(nonContactPeersPaidStars);
    const [hasShownNotification, setHasShownNotification] = useState(false);
    const selectedValue = useMemo(() => {
        if (shouldChargeForMessages)
            return 'charge_for_messages';
        if (shouldNewNonContactPeersRequirePremium)
            return 'contacts_and_premium';
        return 'everybody';
    }, [shouldChargeForMessages, shouldNewNonContactPeersRequirePremium]);
    useEffectWithPrevDeps(([prevSelectedValue]) => {
        if (!hasShownNotification && prevSelectedValue !== undefined
    ...
    const options = useMemo(() => {
        return [
            { value: 'everybody', label: oldLang('P2PEverybody') },
            {
                ...
                label: canChangeForContactsAndPremium ? (oldLang('PrivacyMessagesContactsAndPremium')) : (<PrivacyLockedOption label={oldLang('PrivacyMessagesContactsAndPremium')} isChecked={selectedValue === 'contacts_and_premium'}/>),
                hidden: !canChangeForContactsAndPremium,
                isCanCheckedInDisabled: true,
            },
            {
                value: 'charge_for_messages',
                label: canChangeChargeForMessages ? (lang('PrivacyChargeForMessages')) : (<PrivacyLockedOption label={lang('PrivacyChargeForMessages')} isChecked={selectedValue === 'charge_for_messages'}/>),
                hidden: !canChangeChargeForMessages,
                isCanCheckedInDisabled: true,
            },
        ];
    }, [oldLang, lang, canChangeForContactsAndPremium, canChangeChargeForMessages, selectedValue]);
    const handleChange = useLastCallback((privacyValue) => {
    ...
    const updateGlobalPrivacySettingsWithDebounced = useDebouncedCallback((value) => {
        updateGlobalPrivacySettings({
            nonContactPeersPaidStars: value,
        });
    }, [updateGlobalPrivacySettings], 300, true);
    const handleChargeForMessagesChange = useCallback((value) => {
    ...
    function renderSectionNoPaidMessagesForUsers() {
        ...
        <ListItem narrow icon="delete-user" onClick={() => {
    ...
    const privacyDescription = useMemo(() => {
        if (shouldChargeForMessages)
            return lang('PrivacyDescriptionChargeForMessages');
    ...
    return (<>
      <div className="settings-item">
        ...
        <RadioGroup name="privacy-messages" options={options} onChange={handleChange} selected={selectedValue}/>
        <p className="settings-item-description-larger" dir={oldLang.isRtl ? 'rtl' : undefined}>
          {privacyDescription}
        </p>
      </div>
      {selectedValue === 'charge_for_messages' && (<div className="settings-item fluid-container">
          <PaidMessagePrice canChangeChargeForMessages={canChangeChargeForMessages} chargeForMessages={chargeForMessages} onChange={handleChargeForMessagesChange}/>
        </div>)}
