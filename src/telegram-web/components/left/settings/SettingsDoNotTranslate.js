...
const SUPPORTED_LANGUAGES = SUPPORTED_TRANSLATION_LANGUAGES.filter((lang) => (LOCAL_SUPPORTED_DETECTION_LANGUAGES.includes(lang)));
const SettingsDoNotTranslate = ({ isActive, doNotTranslate, onReset, }) => {
    ...
    useEffectWithPrevDeps(([prevIsActive, prevLanguage]) => {
        if (prevIsActive === isActive && prevLanguage?.find((option) => option === language))
            return;
    ...
    const handleChange = useLastCallback((newSelectedIds) => {
    ...
    return (<div className={buildClassName(styles.root, 'settings-content infinite-scroll')}>
      <div className={buildClassName(styles.item)}>
        <ItemPicker className={styles.picker} items={displayedOptionList} selectedValues={displayedOptions} onSelectedValuesChange={handleChange} filterValue={searchQuery} onFilterChange={setSearchQuery} isSearchable allowMultiple withDefaultPadding itemInputType="checkbox" searchInputId="lang-picker-search"/>
      </div>
    </div>);
};
