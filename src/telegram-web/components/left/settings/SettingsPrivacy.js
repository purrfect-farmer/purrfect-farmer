...
const SettingsPrivacy = ({
  ...
  const handleArchiveAndMuteChange = useCallback(
    (isEnabled) => {
      updateGlobalPrivacySettings({
  ...
  const handleChatInTitleChange = useCallback((isChecked) => {
    setSharedSettingOption({
      canDisplayChatInTitle: isChecked,
    });
  }, []);
  const handleUpdateContentSettings = useCallback(
    (isChecked) => {
  ...
  function getVisibilityValue(setting) {
    ...
    const blockCount =
      setting.blockChatIds.length + setting.blockUserIds.length;
    const allowCount =
      setting.allowChatIds.length + setting.allowUserIds.length;
    const total = [];
    if (blockCount) total.push(`-${blockCount}`);
    if (allowCount && !isVisibilityEverybody) total.push(`+${allowCount}`);
    const botPrivacyString = shouldShowBotsString
      ? lang("PrivacyValueBots")
      : "";
    const totalString = lang.conjunction(total);
    const exceptionString = [botPrivacyString, totalString]
      .filter(Boolean)
      .join(" ");
    ...
    <div className="settings-content custom-scroll">
      <div className="settings-item">
        <ListItem
          ...
          narrow
          onClick={() =>
            openSettingsScreen({ screen: SettingsScreens.PrivacyBlockedUsers })
          }
        >
        ...
        {webAuthCount > 0 && (
          <ListItem
            ...
            narrow
            onClick={() =>
              openSettingsScreen({ screen: SettingsScreens.ActiveWebsites })
            }
          >
      ...
      <div className="settings-item">
        ...
        <ListItem
          ...
          allowDisabledClick
          rightElement={
            isCurrentUserPremium && <StarIcon size="big" type="premium" />
          }
          className="no-icon"
          onClick={() =>
        ...
        >
          <div className="multiline-item">
            <span className="title">
              {oldLang("PrivacyVoiceMessagesTitle")}
            </span>
            <span className="subtitle" dir="auto">
      ...
      <div className="settings-item">
        ...
        <ListItem narrow onClick={handleOpenDeleteAccountModal}>
