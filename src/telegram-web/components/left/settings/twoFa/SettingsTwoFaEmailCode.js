...
const ICON_SIZE = 160;
const SettingsTwoFaEmailCode = ({ animatedEmoji, codeLength, isLoading, error, clearError, onSubmit, isActive, onReset, recoveryEmail, }) => {
    ...
    return (<div className="settings-content two-fa custom-scroll">
      <div className="settings-content-header no-border">
        ...
        {recoveryEmail && (<p className="settings-item-description mb-3" dir="auto">
            {lang('TwoStepAuth.ConfirmEmailDescription', recoveryEmail)}
          </p>)}
      </div>

      <div className="settings-item settings-group">
        <InputText value={value} ref={inputRef} inputMode="decimal" label={lang('YourEmailCode')} error={error} onChange={handleInputChange}/>
        {isLoading && <Loading />}
      </div>
    </div>);
};
