...
import { ANIMATION_LEVEL_CUSTOM, ANIMATION_LEVEL_MAX, ANIMATION_LEVEL_MED, ANIMATION_LEVEL_MIN, } from '../../../config';
...
function SettingsPerformance({ isActive, performanceSettings, onReset, }) {
    const { setSharedSettingOption, updatePerformanceSettings, } = getActions();
    useHistoryBack({
        isActive,
        onBack: onReset,
    });
    const lang = useLang();
    const [sectionExpandedStates, setSectionExpandedStates] = useState({});
    ...
    const handleToggleSection = useCallback((e, index) => {
        e.preventDefault();
    ...
    const handleAnimationLevelChange = useCallback((newLevel) => {
    ...
    const handlePropertyGroupChange = useCallback((e) => {
    ...
    const handlePropertyChange = useCallback((e) => {
        const { name, checked } = e.target;
        updatePerformanceSettings({ [name]: checked });
    }, []);
    return (<div className="settings-content custom-scroll">
      <div className="settings-item">
        ...
        <RangeSlider options={animationLevelOptions} value={animationLevelState === ANIMATION_LEVEL_CUSTOM ? ANIMATION_LEVEL_MED : animationLevelState} onChange={handleAnimationLevelChange}/>
      </div>
      ...
      <div className="settings-item-simple settings-item__with-shifted-dropdown">
        ...
        {PERFORMANCE_OPTIONS.map(([sectionName, options], index) => {
            return (<div key={sectionName} className="settings-dropdown-section">
              <div className="ListItem with-checkbox">
                <Checkbox name={sectionName} value={index.toString()} checked={sectionCheckedStates[index]} label={lang(sectionName)} rightIcon={sectionExpandedStates[index] ? 'up' : 'down'} onChange={handlePropertyGroupChange} onClickLabel={handleToggleSection}/>
              </div>
              {Boolean(sectionExpandedStates[index]) && (<div className="DropdownList DropdownList--open">
                  {options.map(({ key, label, disabled }) => {
                        if (key === 'snapEffect' && !IS_SNAP_EFFECT_SUPPORTED)
                            return undefined;
                        return (<Checkbox key={key} name={key} checked={performanceSettings[key]} label={lang(label)} disabled={disabled} onChange={handlePropertyChange}/>);
                    })}
                </div>)}
            </div>);
        })}
