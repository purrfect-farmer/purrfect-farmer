...
const SettingsHeader = ({ currentScreen, editedFolderId, onReset, }) => {
    const { signOut, openDeleteChatFolderModal, openSettingsScreen, } = getActions();
    ...
    const SettingsMenuButton = useMemo(() => {
        return ({ onTrigger, isOpen }) => (<Button round ripple={!isMobile} size="smaller" color="translucent" className={isOpen ? 'active' : ''} onClick={onTrigger} ariaLabel="More actions">
        <Icon name="more"/>
      </Button>);
    }, [isMobile]);
    ...
    function renderHeaderContent() {
        switch (currentScreen) {
            ...
            {Boolean(editedFolderId) && (<DropdownMenu className="settings-more-menu" trigger={SettingsMenuButton} positionX="right">
            ...
            case SettingsScreens.FoldersExcludedChatsFromChatList:
                return (<h3>
            {oldLang(currentScreen === SettingsScreens.FoldersIncludedChats
            ...
            <Button round ripple={!isMobile} size="smaller" color="translucent" onClick={() => openSettingsScreen({ screen: SettingsScreens.EditProfile })} ariaLabel={oldLang('lng_settings_information')}>
              <Icon name="edit"/>
            </Button>
            <DropdownMenu className="settings-more-menu" trigger={SettingsMenuButton} positionX="right">
    ...
    return (<div className="left-header">
      <Button round size="smaller" color="translucent" onClick={onReset} ariaLabel={oldLang('AccDescrGoBack')}>
        <Icon name="arrow-left"/>
      </Button>
      {renderHeaderContent()}
      <ConfirmDialog isOpen={isSignOutDialogOpen} onClose={closeSignOutConfirmation} text={oldLang('lng_sure_logout')} confirmLabel={oldLang('AccountSettings.Logout')} confirmHandler={handleSignOutMessage} confirmIsDestructive/>
    </div>);
};
