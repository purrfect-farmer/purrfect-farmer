...
const SettingsPrivacyVisibility = ({ screen, isActive, primaryPrivacy, secondaryPrivacy, currentUserId, hasCurrentUserFullInfo, currentUserFallbackPhoto, isPremiumRequired, onReset, shouldDisplayGiftsButton, isCurrentUserPremium, }) => {
    ...
    return (<div className="settings-content custom-scroll">
      {screen === SettingsScreens.PrivacyGifts && (<div className="settings-item">
          <ListItem onClick={handleShowGiftIconInChats}>
            <span>{lang('PrivacyDisplayGiftsButton')}</span>
            <Switcher id="gift" disabled={!isCurrentUserPremium} label={shouldDisplayGiftsButton ? lang('HideGiftsButton') : lang('DisplayGiftsButton')} checked={shouldDisplayGiftsButton}/>
          </ListItem>
      ...
      <PrivacySubsection screen={screen} privacy={primaryPrivacy} isPremiumRequired={isPremiumRequired}/>
      {screen === SettingsScreens.PrivacyProfilePhoto && primaryPrivacy?.visibility !== 'everybody' && (<SettingsPrivacyPublicProfilePhoto currentUserId={currentUserId} hasCurrentUserFullInfo={hasCurrentUserFullInfo} currentUserFallbackPhoto={currentUserFallbackPhoto}/>)}
      {screen === SettingsScreens.PrivacyLastSeen && (<SettingsPrivacyLastSeen visibility={primaryPrivacy?.visibility}/>)}
      {screen === SettingsScreens.PrivacyGifts && (<SettingsAcceptedGift />)}
      {Boolean(secondaryScreen) && (<PrivacySubsection screen={secondaryScreen} privacy={secondaryPrivacy}/>)}
    </div>);
};
function PrivacySubsection({ screen, privacy, isPremiumRequired, }) {
    ...
    const visibilityOptions = useMemo(() => {
        ...
        const options = [
            ...
            {
                value: 'contacts',
                label: isPremiumRequired ? (<PrivacyLockedOption label={oldLang('P2PContacts')}/>) : (oldLang('P2PContacts')),
        ...
        if (hasNobody) {
            options.push({
                value: 'nobody',
                label: isPremiumRequired ? (<PrivacyLockedOption label={oldLang('P2PNobody')}/>) : (oldLang('P2PNobody')),
                hidden: isPremiumRequired,
            });
        }
        return options;
    ...
    const prepareSubtitle = useLastCallback((userIds, chatIds, shouldAllowPremium, botsPrivacy) => {
    ...
    return (<>
      <div className="settings-item">
        ...
        <RadioGroup name={`visibility-${privacyKey}`} options={visibilityOptions} onChange={handleVisibilityChange} selected={privacy?.visibility}/>
        {descriptionText && (<p className="settings-item-description-larger" dir={oldLang.isRtl ? 'rtl' : undefined}>{descriptionText}</p>)}
      </div>
      {!isPremiumRequired && (primaryExceptionLists.shouldShowAllowed || primaryExceptionLists.shouldShowDenied) && (<div className="settings-item">
          <h4 className="settings-item-header" dir={oldLang.isRtl ? 'rtl' : undefined}>
            {oldLang('PrivacyExceptions')}
          </h4>
          {primaryExceptionLists.shouldShowAllowed && (<ListItem narrow icon="add-user" onClick={() => {
          ...
          {primaryExceptionLists.shouldShowDenied && (<ListItem narrow icon="delete-user" onClick={() => {
...
export default memo(withGlobal((global, { screen }) => {
    let primaryPrivacy;
    let secondaryPrivacy;
    const { currentUserId, settings: { privacy, byKey: { shouldDisplayGiftsButton, }, }, } = global;
