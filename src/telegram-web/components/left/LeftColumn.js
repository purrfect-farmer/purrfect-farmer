...
import { selectCurrentChat, selectIsCurrentUserFrozen, selectIsForumPanelOpen, selectTabState, } from '../../global/selectors';
import { IS_APP, IS_FIREFOX, IS_MAC_OS, IS_TOUCH_ENV, LAYERS_ANIMATION_NAME, } from '../../util/browser/windowEnvironment';
...
function LeftColumn({ ref, contentKey, settingsScreen, searchQuery, searchDate, isFirstChatFolderActive, shouldSkipHistoryAnimations, currentUserId, hasPasscode, nextFoldersAction, isChatOpen, isAppUpdateAvailable, isElectronUpdateAvailable, isForumPanelOpen, forumPanelChatId, isClosingSearch, archiveSettings, isArchivedStoryRibbonShown, isAccountFrozen, }) {
    const { setGlobalSearchQuery, setGlobalSearchClosing, setGlobalSearchChatId, resetChatCreation, setGlobalSearchDate, loadPasswordInfo, clearTwoFaError, openChat, openLeftColumnContent, openSettingsScreen, } = getActions();
    ...
    const handleSearchQuery = useLastCallback((query) => {
    ...
    const handleHotkeySearch = useLastCallback((e) => {
    ...
    const handleHotkeySavedMessages = useLastCallback((e) => {
        e.preventDefault();
        openChat({ id: currentUserId, shouldReplaceHistory: true });
    });
    const handleArchivedChats = useLastCallback((e) => {
        e.preventDefault();
        openLeftColumnContent({ contentKey: LeftColumnContent.Archived });
    });
    const handleHotkeySettings = useLastCallback((e) => {
        e.preventDefault();
    ...
    function renderContent(isActive) {
        switch (contentType) {
            case ContentType.Archived:
                return (<ArchivedChats isActive={isActive} onReset={handleReset} onTopicSearch={handleTopicSearch} foldersDispatch={foldersDispatch} isForumPanelOpen={isForumPanelOpen} archiveSettings={archiveSettings} isStoryRibbonShown={isArchivedStoryRibbonShown}/>);
            case ContentType.Settings:
                return (<Settings isActive={isActive} currentScreen={settingsScreen} foldersState={foldersState} foldersDispatch={foldersDispatch} shouldSkipTransition={shouldSkipHistoryAnimations} onReset={handleReset}/>);
            case ContentType.NewChannel:
                return (<NewChat key={lastResetTime} isActive={isActive} isChannel content={contentKey} onReset={handleReset}/>);
            case ContentType.NewGroup:
                return (<NewChat key={lastResetTime} isActive={isActive} content={contentKey} onReset={handleReset}/>);
            default:
                return (<LeftMain content={contentKey} isClosingSearch={isClosingSearch} searchQuery={searchQuery} searchDate={searchDate} contactsFilter={contactsFilter} foldersDispatch={foldersDispatch} onSearchQuery={handleSearchQuery} onReset={handleReset} shouldSkipTransition={shouldSkipHistoryAnimations} isAppUpdateAvailable={isAppUpdateAvailable} isElectronUpdateAvailable={isElectronUpdateAvailable} isForumPanelOpen={isForumPanelOpen} onTopicSearch={handleTopicSearch} isAccountFrozen={isAccountFrozen}/>);
        }
    }
    return (<Transition ref={ref} name={shouldSkipHistoryAnimations ? 'none' : LAYERS_ANIMATION_NAME} renderCount={RENDER_COUNT} activeKey={contentType} shouldCleanup cleanupExceptionKey={ContentType.Main} shouldWrap wrapExceptionKey={ContentType.Main} id="LeftColumn" withSwipeControl>
      {renderContent}
    </Transition>);
}
export default memo(withGlobal((global) => {
    const tabState = selectTabState(global);
    const { globalSearch: { query, minDate, }, shouldSkipHistoryAnimations, activeChatFolder, leftColumn, nextFoldersAction, storyViewer: { isArchivedRibbonShown, }, } = tabState;
    const { currentUserId, passcode: { hasPasscode, }, isAppUpdateAvailable, isElectronUpdateAvailable, archiveSettings, } = global;
