...
const PremiumFeatureModal = ({ promo, initialSection, isPremium, limits, premiumPromoOrder, subscriptionOption, onBack, onClickSubscribe, }) => {
    ...
    function handleScroll(e) {
        ...
        requestMutation(() => {
            ...
            if (prevElement)
                toggleExtraClass(prevElement, 'reverse', false);
            if (reverseElement)
                toggleExtraClass(reverseElement, 'reverse', true);
        });
        if (isScrolling)
            return;
    ...
    useEffect(() => {
        ...
        if (!scrollContainer || (prevInitialSection === initialSection))
            return;
    ...
    const handleSelectSlide = useLastCallback(async (index) => {
        const scrollContainer = scrollContainerRef.current;
        if (!scrollContainer)
            return;
        setCurrentSlideIndex(index);
    ...
    return (<div className={styles.root}>
      <Button round size="smaller" className={buildClassName(styles.backButton, hasHeaderBackdrop && styles.whiteBackButton)} color={hasHeaderBackdrop ? 'translucent-white' : 'translucent'} onClick={onBack} ariaLabel={oldLang('Back')}>
      ...
      <div className={buildClassName(styles.content, 'no-scrollbar')} onScroll={handleScroll} ref={scrollContainerRef}>
        ...
        {filteredSections.map((section, index) => {
            if (section === 'double_limits') {
                return (<div className={buildClassName(styles.slide, styles.limits)}>
                ...
                <div className={buildClassName(styles.limitsContent, 'custom-scroll')} onScroll={handleLimitsScroll}>
                  {PREMIUM_LIMITS_ORDER.map((limit, i) => {
                        ...
                        return (<PremiumLimitPreview title={oldLang(LIMITS_TITLES[limit])} description={oldLang(LIMITS_DESCRIPTIONS[limit], premiumLimit)} leftValue={defaultLimit} rightValue={premiumLimit} colorStepProgress={i / (PREMIUM_LIMITS_ORDER.length - 1)}/>);
                    })}
                </div>
              </div>);
            }
            ...
            if (section === 'stories') {
                return (<div className={buildClassName(styles.slide, styles.stories)}>
            ...
            return (<div className={styles.slide}>
              <div className={styles.frame}>
                <PremiumFeaturePreviewVideo isActive={currentSlideIndex === index} videoId={promo.videos[i].id} videoThumbnail={promo.videos[i].thumbnail} isDown={PREMIUM_BOTTOM_VIDEOS.includes(section)} index={index} isReverseAnimation={index === reverseAnimationSlideIndex}/>
              </div>
              <h1 className={styles.title}>
                {shouldUseNewLang
                    ? lang(PREMIUM_FEATURE_TITLES[promo.videoSections[i]], undefined, { withNodes: true, renderTextFilters: ['br'] })
                    : oldLang(PREMIUM_FEATURE_TITLES[promo.videoSections[i]])}
              </h1>
              <div className={styles.description}>
                {renderText(shouldUseNewLang
                    ? lang(PREMIUM_FEATURE_DESCRIPTIONS[promo.videoSections[i]], undefined, { withNodes: true, renderTextFilters: ['br'] })
      ...
      <div className={buildClassName(styles.footer, (isScrolledToBottom || currentSlideIndex !== 0) && styles.noFooterBorder)}>
        <SliderDots length={PREMIUM_FEATURE_SECTIONS.length} active={currentSlideIndex} onSelectSlide={handleSelectSlide}/>
        {Boolean(subscriptionButtonText) && (<Button className={buildClassName(styles.button)} isShiny={!isPremium} withPremiumGradient={!isPremium} onClick={isPremium ? onBack : handleClick}>
