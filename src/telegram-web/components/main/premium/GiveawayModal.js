...
import { GIVEAWAY_BOOST_PER_PREMIUM, GIVEAWAY_MAX_ADDITIONAL_CHANNELS, GIVEAWAY_MAX_ADDITIONAL_COUNTRIES, GIVEAWAY_MAX_ADDITIONAL_USERS, STARS_CURRENCY_CODE, } from '../../../config';
...
const GiveawayModal = ({ chatId, gifts, isOpen, isChannel, selectedMemberList, selectedChannelList, giveawayBoostPerPremiumLimit = GIVEAWAY_BOOST_PER_PREMIUM, countryList, prepaidGiveaway, countrySelectionLimit = GIVEAWAY_MAX_ADDITIONAL_COUNTRIES, userSelectionLimit = GIVEAWAY_MAX_ADDITIONAL_USERS, isStarsGiftEnabled, starsGiftOptions, }) => {
    const dialogRef = useRef();
    const { closeGiveawayModal, openInvoice, openPremiumModal, launchPrepaidGiveaway, launchPrepaidStarsGiveaway, } = getActions();
    ...
    const [selectedRandomUserCount, setSelectedRandomUserCount] = useState(DEFAULT_BOOST_COUNT);
    const [selectedGiveawayOption, setGiveawayOption] = useState(TYPE_OPTIONS[0].value);
    const [selectedStarOption, setSelectedStarOption] = useState();
    const [selectedSubscriberOption, setSelectedSubscriberOption] = useState('all');
    const [selectedMonthOption, setSelectedMonthOption] = useState();
    const [selectedUserIds, setSelectedUserIds] = useState([]);
    const [selectedChannelIds, setSelectedChannelIds] = useState([]);
    const [selectedCountryIds, setSelectedCountryIds] = useState([]);
    const [shouldShowWinners, setShouldShowWinners] = useState(false);
    const [shouldShowPrizes, setShouldShowPrizes] = useState(false);
    const [prizeDescription, setPrizeDescription] = useState(undefined);
    const [dataPrepaidGiveaway, setDataPrepaidGiveaway] = useState(undefined);
    const [dataStarsPrepaidGiveaway, setDataStarsPrepaidGiveaway,] = useState(undefined);
    ...
    const handlePrizeDescriptionChange = useLastCallback((e) => {
    ...
    const handleChangeTypeOption = useLastCallback((value) => {
        setGiveawayOption(value);
        setSelectedUserIds([]);
        setSelectedRandomUserCount(DEFAULT_BOOST_COUNT);
    });
    const handleExpireDateChange = useLastCallback((date) => {
        setCustomExpireDate(date.getTime());
        closeCalendar();
    });
    const handleSetCountriesListChange = useLastCallback((value) => {
        setSelectedCountryIds(value);
    });
    const handleSelectedUserIdsChange = useLastCallback((newSelectedIds) => {
        setSelectedUserIds(newSelectedIds);
        if (!newSelectedIds.length) {
            setGiveawayOption('premium_giveaway');
        }
    });
    const handleSelectedChannelIdsChange = useLastCallback((newSelectedIds) => {
        setSelectedChannelIds(newSelectedIds);
    });
    const handleShouldShowWinnersChange = useLastCallback((e) => {
        setShouldShowWinners(e.target.checked);
    });
    const handleShouldShowPrizesChange = useLastCallback((e) => {
    ...
    function renderTypeOptions() {
        ...
        {TYPE_OPTIONS.map((option) => {
                return (<GiveawayTypeOption key={option.name} name={option.name} text={option.text} option={option.value} img={option.img} onChange={handleChangeTypeOption} checked={selectedGiveawayOption === option.value} isLink={option.isLink} userNames={userNames} selectedMemberIds={selectedUserIds} onClickAction={option.onClickAction}/>);
            })}
      </div>);
    }
    function renderSubscribersOptions() {
        return (<div className={styles.options}>
        <RadioGroup name="subscribers" options={SUBSCRIBER_OPTIONS} selected={selectedSubscriberOption} onChange={handleChangeSubscriberOption} onClickAction={onClickActionHandler} subLabelClassName={styles.subLabelClassName} isLink/>
      </div>);
    }
    function renderSubscriptionOptions() {
        return (<div className={styles.options}>
        {filteredGifts?.map((gift) => (<PremiumSubscriptionOption isGiveaway key={gift.months} option={gift} fullMonthlyAmount={fullMonthlyAmount} checked={gift.months === selectedMonthOption} onChange={setSelectedMonthOption}/>))}
      </div>);
    }
    function renderPremiumFeaturesLink() {
        ...
        return (<p className={styles.premiumFeatures}>
        {parts[1]}
    ...
    function renderStarOptionList() {
        return (<StarTopupOptionList className={styles.starOptions} options={starsGiftOptions} selectedStarCount={selectedRandomUserCount} selectedStarOption={selectedStarOption} onClick={handleStarClick}/>);
    }
    function renderGiveawayOptionList() {
        ...
        <div className={styles.section}>
          ...
          <ListItem inactive className="chat-item-clickable contact-list-item">
            <GroupChatInfo chatId={chatId} status={lang(isChannel ? 'BoostingChannelWillReceiveBoost'
                : 'BoostingGroupWillReceiveBoost', boostQuantity || boostStarsQuantity, 'i')}/>
          </ListItem>

          {selectedChannelIds?.map((channelId) => {
                return (<ListItem ripple key={channelId} className="chat-item-clickable contact-list-item" onClick={() => deleteParticipantsHandler(channelId)} rightElement={(<Icon name="close" className={styles.removeChannel}/>)}>
                <GroupChatInfo chatId={channelId.toString()}/>
              </ListItem>);
            })}

          {selectedChannelIds.length < MAX_ADDITIONAL_CHANNELS && (<ListItem icon="add" ripple onClick={openChannelPickerModal} className={styles.addButton} iconClassName={styles.addChannel}>
        ...
        <div className={styles.section}>
          <div className={styles.checkboxSection}>
            ...
            <Switcher label={lang('BoostingGiveawayAdditionalPrizes')} checked={shouldShowPrizes} onChange={handleShouldShowPrizesChange}/>
          </div>

          {shouldShowPrizes && (<div className={styles.prizesSection}>
              <h2 className={styles.title}>
                {selectedRandomUserCount}
              </h2>
              <InputText className={styles.prizesInput} value={prizeDescription} onChange={handlePrizeDescriptionChange} label={lang('BoostingGiveawayEnterYourPrize')}/>
            </div>)}
        </div>

        {shouldShowPrizes ? (!isStarsGiveaway && !isStarsPrepaidGiveaway ? (<div className={styles.subscription}>
              {prizeDescription?.length ? renderText(lang('BoostingGiveawayAdditionPrizeCountNameHint', dataPrepaidGiveaway
                    ? [dataPrepaidGiveaway.quantity, prizeDescription, monthQuantity]
                    : [selectedUserCount, prizeDescription, monthQuantity], undefined, selectedMonthOption), ['simple_markdown']) : renderText(lang('BoostingGiveawayAdditionPrizeCountHint', dataPrepaidGiveaway
                    ? [dataPrepaidGiveaway.quantity, monthQuantity]
                    : [selectedUserCount, monthQuantity], undefined, selectedMonthOption), ['simple_markdown'])}
            </div>) : undefined) : (<div className={styles.subscription}>
        ...
        <div className={styles.section}>
          <div className={styles.checkboxSection}>
            ...
            <Switcher label={lang('BoostingGiveawayAdditionalPrizes')} checked={shouldShowWinners} onChange={handleShouldShowWinnersChange}/>
          </div>
        </div>
        ...
        <div className={buildClassName(styles.section, (dataPrepaidGiveaway || dataStarsPrepaidGiveaway || isStarsGiveaway) && styles.subscriptionFooter)}>
          <h2 className={styles.giveawayTitle}>
            {lang('BoostingDateWhenGiveawayEnds')}
          </h2>

          <Button ariaLabel={lang('BoostGift.DateEnds')} className={buildClassName(styles.dateButton, 'expire-limit')} isText onClick={openCalendar}>
    ...
    return (<Modal className={styles.root} onClose={handleClose} isOpen={isOpen} dialogRef={dialogRef} onEnter={(dataPrepaidGiveaway || dataStarsPrepaidGiveaway) ? openConfirmModal : handleClick}>
      <div className={buildClassName(styles.main, 'custom-scroll')} onScroll={handleScroll}>
        <Button round size="smaller" className={styles.closeButton} color="translucent" onClick={handleClose} ariaLabel={lang('Close')}>
          <Icon name="close"/>
        </Button>
        ...
        {(dataPrepaidGiveaway || dataStarsPrepaidGiveaway) ? (<div className={styles.status}>
            <div>
              {dataStarsPrepaidGiveaway ? (<img className={styles.prepaidImg} src={GiftStar} alt=""/>) : (<img className={styles.prepaidImg} src={GIVEAWAY_IMG_LIST[dataPrepaidGiveaway.months] || GIVEAWAY_IMG_LIST[3]} alt=""/>)}
            </div>
          ...
          </div>) : (<div className={buildClassName(styles.section, styles.types)}>
            {renderTypeOptions()}
          </div>)}
        ...
        {isPremiumGiveaway && !selectedUserIds?.length && (<>
            {!dataPrepaidGiveaway && !dataStarsPrepaidGiveaway && (<>
                <div className={styles.section}>
                  ...
                  <RangeSliderWithMarks rangeCount={selectedUserCount} marks={userCountOptions} onChange={handleRandomUserCountChange}/>
                </div>
        ...
        {isStarsGiveaway && (<>
            {!dataStarsPrepaidGiveaway && !dataPrepaidGiveaway && (<>
                ...
                <div className={styles.section}>
                  ...
                  <RangeSliderWithMarks rangeCount={selectedRandomUserCount} marks={winnerCountOptions} onChange={handleWinnerCountChange}/>
        ...
        {selectedGiveawayOption && (<div className={styles.footer}>
            <Button className={styles.button} onClick={(dataPrepaidGiveaway || dataStarsPrepaidGiveaway) ? openConfirmModal : handleClick}>
              {lang('BoostingStartGiveaway')}
            </Button>
          </div>)}
      </div>
      <CalendarModal isOpen={isCalendarOpened} isFutureMode withTimePicker onClose={closeCalendar} onSubmit={handleExpireDateChange} selectedAt={customExpireDate} submitButtonLabel={lang('Save')}/>
      <CountryPickerModal isOpen={isCountryPickerModalOpen} onClose={closeCountryPickerModal} countryList={countryList} onSubmit={handleSetCountriesListChange} selectionLimit={countrySelectionLimit}/>
      <GiveawayUserPickerModal isOpen={isUserPickerModalOpen} onClose={closeUserPickerModal} onSelectedIdsConfirmed={handleSelectedUserIdsChange} initialSelectedIds={selectedUserIds} selectionLimit={userSelectionLimit} giveawayChatId={chatId}/>
      <GiveawayChannelPickerModal isOpen={isChannelPickerModalOpen} onClose={closeChannelPickerModal} initialSelectedIds={selectedChannelIds} onSelectedIdsConfirmed={handleSelectedChannelIdsChange} selectionLimit={GIVEAWAY_MAX_ADDITIONAL_CHANNELS} giveawayChatId={chatId}/>
      <ConfirmDialog title={lang('BoostingStartGiveawayConfirmTitle')} text={lang('BoostingStartGiveawayConfirmText')} confirmLabel={lang('Start')} isOpen={isConfirmModalOpen} onClose={closeConfirmModal} confirmHandler={confirmLaunchPrepaidGiveaway}/>
    </Modal>);
};
export default memo(withGlobal((global) => {
    const { giveawayModal, } = selectTabState(global);
