...
const PremiumFeaturePreviewVideo = ({ currentUser, }) => {
    ...
    return (<div className={styles.root}>
      ...
      <div className={buildClassName(styles.features, !isAtBeginning && styles.scrolled, 'custom-scroll')} onScroll={handleScroll}>
        {STORY_FEATURE_ORDER.map((section, index) => {
            return (<PremiumFeatureItem key={section} title={lang(STORY_FEATURE_TITLES[section])} text={lang(STORY_FEATURE_DESCRIPTIONS[section])} icon={STORY_FEATURE_ICONS[section]} isFontIcon index={index} count={STORY_FEATURE_ORDER.length} section={section}/>);
        })}
