...
const ForwardRecipientPicker = ({ isOpen, currentUserId, isManyMessages, isStory, isForwarding, }) => {
    const { openChatOrTopicWithReplyInDraft, setForwardChatOrTopic, exitForwardMode, forwardToSavedMessages, forwardStory, showNotification, } = getActions();
    ...
    const handleSelectRecipient = useCallback((recipientId, threadId) => {
            ...
            if (isUserId(recipientId)) {
                showNotification({
                    message: isSelf
                        ...
                        : lang('StorySharedTo', getUserFirstOrLastName(selectUser(global, recipientId))),
                });
            }
            else {
        ...
        if (isSelf) {
            const message = lang(isManyMessages
    ...
    return (<RecipientPicker isOpen={isOpen} className={renderingIsStory ? 'component-theme-dark' : undefined} searchPlaceholder={lang(isForwarding ? 'ForwardTo' : 'ReplyToDialog')} onSelectRecipient={handleSelectRecipient} onClose={handleClose} onCloseAnimationEnd={unmarkIsShown} isForwarding={isForwarding}/>);
};
