...
const BotTrustModal = ({ bot, type, shouldRequestWriteAccess }) => {
    ...
    return (<ConfirmDialog isOpen={Boolean(bot)} onClose={handleBotTrustDecline} title={title} confirmHandler={handleBotTrustAccept}>
      {renderText(text, ['simple_markdown'])}
      {shouldRequestWriteAccess && (<Checkbox className="dialog-checkbox" checked={isWriteAllowed} label={renderText(lang('WebApp.AddToAttachmentAllowMessages', bot?.firstName), ['simple_markdown'])} onCheck={setIsWriteAllowed}/>)}
    </ConfirmDialog>);
};
