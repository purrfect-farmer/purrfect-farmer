...
const ConfettiContainer = ({ confetti }) => {
    ...
    const { lastConfettiTime, top, width, left, height, style = 'poppers', } = confetti || {};
    const generateConfetti = useLastCallback((w, h, amount = defaultConfettiAmount) => {
        for (let i = 0; i < amount; i++) {
            const { position, velocity, } = generateRandomPositionData(style, w, h, i);
    ...
    const updateCanvas = useLastCallback(() => {
        ...
        confettiRef.current.forEach((c, i) => {
            const { pos, velocity, size, color, flicker, flickerFrequency, rotation, lastDrawnAt, frameCount, } = c;
    ...
    const containerStyle = buildStyle(Boolean(top) && `top: ${top}px`, Boolean(left) && `left: ${left}px`, Boolean(width) && `width: ${width}px`, Boolean(height) && `height: ${height}px`);
    return (<div id="Confetti" className={styles.root} style={containerStyle}>
      <canvas ref={canvasRef} className={styles.canvas} width={windowSize.width} height={windowSize.height}/>
    </div>);
};
export default memo(withGlobal((global) => pick(selectTabState(global), ['confetti']))(ConfettiContainer));
