...
const SnapEffectContainer = () => {
    return (<div className={styles.root} id={SNAP_EFFECT_CONTAINER_ID}/>);
};
...
export function animateSnap(element) {
    ...
    const component = (<svg className={styles.ghost} width={width} height={height} style={`left: ${x}px; top: ${y}px;`} viewBox={`0 0 ${width} ${height}`}>
      <defs>
    ...
    const svg = jsxToHtml(component)[0];
...
function createFilter(id, smallestSide, baseSeed = 42) {
    return (<filter xmlns={SVG_NAMESPACE} id={id} x="-150%" y="-150%" width="400%" height="400%" color-interpolation-filters="sRGB">
      ...
      <feTurbulence type="fractalNoise" baseFrequency="0.015" numOctaves="1" result="displacementNoise1" seed={baseSeed + 1}/>
      <feTurbulence type="fractalNoise" baseFrequency="1" numOctaves="2" result="displacementNoise2" seed={baseSeed + 2}/>
