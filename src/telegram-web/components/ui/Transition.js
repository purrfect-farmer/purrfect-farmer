import { beginHeavyAnimation, useEffect, useLayoutEffect, useRef, } from '../../lib/teact/teact';
...
const DISABLEABLE_ANIMATIONS = new Set([
...
function Transition({ ref, activeKey, nextKey, name, direction = 'auto', renderCount, shouldRestoreHeight, shouldCleanup, cleanupExceptionKey, cleanupOnlyKey, shouldWrap, wrapExceptionKey, id, className, slideClassName, withSwipeControl, isBlockingAnimation, onStart, onStop, children, }) {
    ...
    useEffect(() => {
        ...
        const container = containerRef.current;
        const activeElement = container.querySelector(`:scope > .${CLASSES.active}`)
            || container.querySelector(`:scope > .${CLASSES.from}`);
        if (!activeElement) {
            return;
        }
    ...
    return (<div ref={containerRef} id={id} className={buildClassName('Transition', className)} teactFastList={asFastList}>
      {contents}
    </div>);
}
