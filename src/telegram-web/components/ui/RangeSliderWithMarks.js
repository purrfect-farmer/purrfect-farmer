...
const RangeSliderWithMarks = ({ marks, onChange, rangeCount }) => {
    ...
    return (<div className={styles.dotWrapper}>
      <form>
        <div className={styles.sliderContainer}>
          <div className={styles.tickMarks}>
            {marks.map((mark, index) => {
            const isFilled = index <= rangeCountIndex;
            return (<div key={mark} className={buildClassName(styles.tick, isFilled ? styles.filled : styles.tickUnfilled)}/>);
        })}
          </div>
          <div className={styles.marksContainer}>
            {marks.map((mark) => (<div key={mark} className={buildClassName(styles.mark, rangeCount === mark && styles.active)}>
                {mark}
              </div>))}
          </div>
          <input ref={sliderRef} type="range" className={styles.slider} min="0" max={marks.length - 1} value={rangeValue} onChange={handleChange} step="1"/>
        </div>
      </form>
    </div>);
};
