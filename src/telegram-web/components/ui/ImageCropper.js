...
const ImageCropper = ({ onChange, image, maxOutputSize, minOutputSize, }) => {
    ...
    const scaleFactor = image
        ? Math.max(previewContainerSize / image.width, previewContainerSize / image.height) : 1;
    ...
    const moveDrag = useLastCallback((e) => {
    ...
    const handleZoomChange = useLastCallback((newZoom) => {
    ...
    const handleCrop = () => {
        ...
        canvas.toBlob((blob) => {
            if (blob)
                onChange(blobToFile(blob, 'avatar.jpg'));
        }, 'image/jpeg');
    };
    if (!image)
        return undefined;
    ...
    const backgroundImageStyle = buildStyle(`width: ${previewImageSize.width}px`, `height: ${previewImageSize.height}px`, `left: ${imageLeft}px`, `top: ${imageTop}px`);
    const foregroundImageStyle = buildStyle(`width: ${previewImageSize.width}px`, `height: ${previewImageSize.height}px`, `left: ${imageLeft - CROP_AREA_INSET}px`, `top: ${imageTop - CROP_AREA_INSET}px`);
    return (<div>
      <div className={styles.previewContainer} style={buildStyle(`width: ${previewContainerSize}px`, `height: ${previewContainerSize}px`)}>
        <img src={image.src} className={styles.backgroundImage} style={backgroundImageStyle} draggable={false} onMouseDown={startDrag} onTouchStart={startDrag} alt="" role="presentation"/>
        <div className={styles.previewMask}/>
        <div className={styles.cropArea}>
          <img src={image.src} className={styles.foregroundImage} style={foregroundImageStyle} draggable={false} alt="" role="presentation"/>
        </div>
      </div>
      <div className={styles.bottomControls}>
        <RangeSlider className={styles.zoomSlider} min={MIN_ZOOM} max={MAX_ZOOM} value={zoom} onChange={handleZoomChange}/>
