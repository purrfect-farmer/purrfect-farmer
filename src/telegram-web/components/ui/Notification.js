...
const Notification = ({ notification, }) => {
    ...
    const lang = useLang();
    const { localId, message, action, actionText, cacheBreaker, className, disableClickDismiss, dismissAction, duration = DEFAULT_DURATION, icon, shouldUseCustomIcon, customEmojiIconId, shouldShowTimer, title, containerSelector, } = notification;
    ...
    const closeAndDismiss = useLastCallback((force) => {
        if (!force && disableClickDismiss)
            return;
        setIsOpen(false);
    ...
    const renderedIcon = useMemo(() => {
        if (customEmojiIconId) {
            return (<CustomEmoji className="notification-emoji-icon" forceAlways size={CUSTOM_EMOJI_SIZE} documentId={customEmojiIconId}/>);
        }
        if (shouldUseCustomIcon) {
            if (icon === 'star') {
                return (<StarIcon type="gold" className={buildClassName('notification-icon')} size="adaptive"/>);
            }
        }
        return <Icon name={icon || 'info-filled'} className="notification-icon"/>;
    }, [customEmojiIconId, icon, shouldUseCustomIcon]);
    return (<Portal className="Notification-container" containerSelector={containerSelector}>
      <div className={buildClassName('Notification', transitionClassNames, className)} onClick={handleClick} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
        {renderedIcon}
        <div className="content">
          {Boolean(renderedTitle) && (<div className="notification-title">{renderedTitle}</div>)}
          {renderedMessage}
        </div>
        {Boolean(renderedActionText) && (<Button color="translucent-white" onClick={handleActionClick} className="notification-button">
            {renderedActionText}
          </Button>)}
        {shouldShowTimer && (<RoundTimer className="notification-timer" key={cacheBreaker} duration={Math.ceil(duration / 1000)}/>)}
      </div>
    </Portal>);
};
