...
import { getBotCoverMediaHash, getChatTitle, getPhotoFullDimensions, getVideoDimensions, getVideoMediaHash, isChatWithVerificationCodesBot, } from '../../global/helpers';
import { selectBot, selectChat, selectPeer, selectUserCommonChats, selectUserFullInfo, } from '../../global/selectors';
...
const MessageListAccountInfo = ({ chat, chatId, botInfo, isLoadingFullUser, phoneCodeList, commonChats, userFullInfo, hasMessages, }) => {
    const { loadCommonChats, openChatWithInfo } = getActions();
    const oldLang = useOldLang();
    const lang = useLang();
    const { phoneCountry, registrationMonth, nameChangeDate, photoChangeDate, } = userFullInfo?.settings || {};
    useEffect(() => {
        loadCommonChats({ userId: chatId });
    }, [chatId]);
    const country = useMemo(() => {
        if (!phoneCodeList || !phoneCountry)
            return undefined;
    ...
    const securityNameInfo = nameChangeDate && chat ? (<div className="local-action-message" key="security-name-message">
      <span>
    ...
    const securityPhotoInfo = photoChangeDate && chat ? (<div className="local-action-message" key="security-photo-message">
      <span>
    ...
    return (<div className={buildClassName(styles.root, 'empty')}>
      {isLoadingFullUser && isChatInfoEmpty && <span>{oldLang('Loading')}</span>}
      {(isBotInfoEmpty && isChatInfoEmpty) && !isLoadingFullUser && !hasMessages && (<span>{oldLang('NoMessages')}</span>)}
      {botInfo && (<div className={buildClassName(styles.chatInfo, styles.botBackground)} style={buildStyle(width ? `width: ${width}px` : undefined)}>
          {botInfoPhotoUrl && (<img className={styles.media} src={botInfoPhotoUrl} width={width} height={height} alt="Bot info"/>)}
          {botInfoGifUrl && (<OptimizedVideo canPlay className={styles.media} src={botInfoGifUrl} loop disablePictureInPicture muted playsInline style={buildStyle(Boolean(width) && `width: ${width}px`, Boolean(height) && `height: ${height}px`)}/>)}
          {botInfoDimensions && !botInfoPhotoUrl && !botInfoGifUrl && (<Skeleton className={styles.media} width={width} height={height} forceAspectRatio/>)}
          {isVerifyCodes && (<div className={styles.botInfoDescription}>
              {oldLang('VerifyChatInfo')}
            </div>)}
          {!isVerifyCodes && botInfo.description && (<div className={styles.botInfoDescription}>
              <p className={styles.botInfoTitle}>{oldLang('BotInfoTitle')}</p>
              {renderText(botInfo.description, ['br', 'emoji', 'links'])}
            </div>)}
        </div>)}
      {!isChatInfoEmpty && chat && (<div className={buildClassName(styles.chatInfo, styles.chatBackground)}>
          ...
          {!chat?.isVerified && (<div className={buildClassName(styles.chatNotVerified, styles.textColor)}>
