...
import { getIsSavedDialog, getReactionKey, isSameReaction, isSystemBot, } from '../../../global/helpers';
import { selectChat, selectChatMessage, selectCurrentMessageList, selectCurrentMiddleSearch, selectForwardedSender, selectIsChatWithSelf, selectIsCurrentUserPremium, selectMonoforumChannel, selectSender, selectTabState, } from '../../../global/selectors';
...
const MiddleSearch = ({ isActive, chat, monoforumChat, threadId, requestedQuery, savedTags, savedTag, totalCount, lastSearchQuery, foundIds, isHistoryCalendarOpen, isCurrentUserPremium, isSavedMessages, fetchingQuery, isHashtagQuery, searchType = 'chat', currentUserId, }) => {
    const { updateMiddleSearch, resetMiddleSearch, performMiddleSearch, focusMessage, closeMiddleSearch, openHistoryCalendar, openPremiumModal, loadSavedReactionTags, } = getActions();
    const ref = useRef();
    ...
    const handleClickOutside = useLastCallback((event) => {
        if (maybeLongPressActiveRef.current)
            return;
    ...
    const hasResultsDropdown = isActive && (isViewAsList || !isMobile) && (isFocused || isNonFocusedDropdownForced)
        && Boolean(hasResultsContainer || hasResultsPlaceholder || savedTags);
    ...
    useEffectWithPrevDeps(([prevIsCalendarOpen]) => {
        if (!isActive || prevIsCalendarOpen === isHistoryCalendarOpen)
            return;
    ...
    const handleSearch = useLastCallback(() => {
        ...
        runDebouncedForSearch(() => {
            if (shouldCancelSearchRef.current)
                return;
            performMiddleSearch({ chatId, threadId, query });
        });
    });
    const handleQueryChange = useLastCallback((newQuery) => {
    ...
    const handleMessageClick = useLastCallback((message) => {
    ...
    const updateSearchParams = useLastCallback((update) => {
        updateMiddleSearch({ chatId: chat.id, threadId, update });
        handleSearch();
    });
    const activateSearchTag = useLastCallback((tag) => {
    ...
    const handleChangeSearchType = useLastCallback((type) => {
    ...
    function renderTypeTag(type, isForTag) {
        ...
        switch (type) {
            case 'chat':
                return (<PeerChip className={buildClassName(styles.searchType, isSelected && styles.selectedType)} peerId={chat?.id} title={oldLang('SearchThisChat')} clickArg="chat" onClick={isForTag ? handleDeleteTag : handleChangeSearchType}/>);
            case 'myChats':
                return (<PeerChip className={buildClassName(styles.searchType, isSelected && styles.selectedType)} peerId={currentUserId} forceShowSelf title={oldLang('SearchMyMessages')} clickArg="myChats" onClick={isForTag ? handleDeleteTag : handleChangeSearchType}/>);
            case 'channels':
                return (<PeerChip className={buildClassName(styles.searchType, isSelected && styles.selectedType)} customPeer={CHANNELS_PEER} title={oldLang('SearchPublicPosts')} clickArg="channels" onClick={isForTag ? handleDeleteTag : handleChangeSearchType}/>);
        }
        return undefined;
    }
    function renderDropdown() {
        return (<div className={buildClassName(styles.dropdown, !hasResultsDropdown && styles.dropdownHidden)}>
        {!isMobile && <div className={styles.separator}/>}
        {hasSavedTags && !isHashtagQuery && (<div className={buildClassName(styles.savedTags, !isMobile && styles.wrap, 'no-scrollbar')}>
            {savedTagsArray.map((tag) => {
                    const isChosen = isSameReaction(tag.reaction, savedTag);
                    return (<SavedTagButton containerId="local-search" key={getReactionKey(tag.reaction)} reaction={tag.reaction} tag={tag} withCount isDisabled={areSavedTagsDisabled} isChosen={isChosen} onClick={isChosen ? removeSearchSavedTag : activateSearchTag}/>);
                })}
          </div>)}
        {isHashtagQuery && (<div className={buildClassName(styles.searchTypes, 'no-scrollbar')}>
            {renderTypeTag('chat')}
            {renderTypeTag('myChats')}
            {renderTypeTag('channels')}
          </div>)}
        {hasResultsContainer && (<InfiniteScroll ref={containerRef} className={buildClassName(styles.results, 'custom-scroll')} items={viewportResults} itemSelector={`.${RESULT_ITEM_CLASS_NAME}`} preloadBackwards={0} onLoadMore={getMore} onKeyDown={handleKeyDown}>
            {areResultsEmpty && (<span key="nothing" className={styles.placeholder}>
                {oldLang('NoResultFoundFor', query)}
              </span>)}
            {isOnlyHash && (<span key="enterhash" className={styles.placeholder}>
                {oldLang('HashtagSearchPlaceholder')}
              </span>)}
            {viewportResults?.map(({ message, senderPeer, messageChat, searchResultKey, }, i) => (<MiddleSearchResult key={searchResultKey} teactOrderKey={-message.date} className={RESULT_ITEM_CLASS_NAME} query={query} message={message} senderPeer={senderPeer} messageChat={messageChat} shouldShowChat={isHashtagQuery} isActive={focusedIndex - viewportOffset === i} onClick={handleMessageClick}/>))}
          </InfiniteScroll>)}
      </div>);
    }
    return (<div id="MiddleSearch" className={buildClassName(styles.root, isActive && styles.active, !isActive && isFullyHidden && 'visually-hidden', // `display: none` would prevent focus on iOS
        isFocused && styles.focused, isMobile && styles.mobile)} ref={ref}>
      <div className={styles.header}>
        {!isMobile && (<Avatar className={styles.avatar} peer={monoforumChat || chat} size="medium" isSavedMessages={isSavedMessages}/>)}
        <SearchInput ref={inputRef} value={query} className={buildClassName(styles.input, hasResultsDropdown && styles.withDropdown, hasResultsDropdown && !isMobile && styles.adaptSearchBorders)} canClose={!isMobile} isLoading={isLoading} resultsItemSelector={`.${styles.results} .${RESULT_ITEM_CLASS_NAME}`} hasUpButton={hasNavigationButtons && !isMobile} hasDownButton={hasNavigationButtons && !isMobile} placeholder={isHashtagQuery ? oldLang('SearchHashtagsHint') : oldLang('Search')} teactExperimentControlled onChange={handleQueryChange} onStartBackspace={handleDeleteTag} onReset={handleReset} withBackIcon={isMobile} onFocus={markFocused} focused={isFocused} onUpClick={canFocusOlder ? handleFocusOlder : undefined} onDownClick={canFocusNewer ? handleFocusNewer : undefined}>
          <div className={styles.searchTags}>
            {savedTag && (<SavedTagButton containerId="local-search-tags" className={styles.savedSearchTag} reaction={savedTag} tag={savedTags[getReactionKey(savedTag)]} onClick={removeSearchSavedTag}/>)}
            {isHashtagQuery && <div className={styles.hash}>#</div>}
          </div>
          {!isMobile && renderDropdown()}
        </SearchInput>
        {!isMobile && (<div className={styles.icons}>
            <Button round size="smaller" color="translucent" onClick={() => openHistoryCalendar({ selectedAt: getDayStartAt(Date.now()) })} ariaLabel={oldLang('JumpToDate')}>
              <Icon name="calendar"/>
            </Button>
          </div>)}
      </div>
      ...
      {isMobile && (<div className={styles.footer}>
          ...
          <div className={styles.counter}>
            {hasQueryData && (foundIds?.length ? (oldLang('Of', [Math.max(focusedIndex + 1, 1), totalCount])) : foundIds && !foundIds.length && (oldLang('NoResult')))}
          </div>
          {searchType === 'chat' && Boolean(foundIds?.length) && (<Button className={styles.viewStyle} size="smaller" isText fluid noForcedUpperCase onClick={handleTriggerViewStyle}>
              {isViewAsList ? oldLang('SearchAsChat') : oldLang('SearchAsList')}
            </Button>)}
          {hasNavigationButtons && !hasResultsDropdown && (<div className={styles.mobileNavigation}>
              <Button className={buildClassName(styles.navigationButton, !canFocusOlder && styles.navigationDisabled)} round size="smaller" color="secondary" onClick={handleFocusOlder} nonInteractive={!canFocusOlder} ariaLabel={lang('AriaSearchOlderResult')}>
                <Icon name="up"/>
              </Button>
              <Button className={buildClassName(styles.navigationButton, !canFocusNewer && styles.navigationDisabled)} round size="smaller" color="secondary" onClick={handleFocusNewer} nonInteractive={!canFocusNewer} ariaLabel={lang('AriaSearchNewerResult')}>
                <Icon name="down"/>
              </Button>
            </div>)}
        </div>)}
...
export default memo(withGlobal((global) => {
    ...
    const { requestedQuery, savedTag, results, fetchingQuery, isHashtag, type, } = selectCurrentMiddleSearch(global) || {};
