...
const ReactionPickerLimited = ({ loadAndPlay, enabledReactions, availableReactions, topReactions, selectedReactionIds, isWithPaidReaction, message, reactionsLimit, onReactionSelect, onReactionContext, }) => {
    ...
    const allAvailableReactions = useMemo(() => {
        ...
        if (enabledReactions.type === 'all') {
            const reactionsToSort = (availableReactions || []).map(({ reaction }) => reaction);
            if (isWithPaidReaction) {
                reactionsToSort.unshift({ type: 'paid' });
            }
            return sortReactions(reactionsToSort, topReactions);
        }
        const reactionsToSort = enabledReactions.allowed.slice();
        if (isWithPaidReaction) {
            reactionsToSort.unshift({ type: 'paid' });
        }
        return sortReactions(reactionsToSort, topReactions);
    ...
    return (<div className={styles.root} style={`height: ${pickerHeight}px`}>
      <div className={buildClassName(styles.wrapper, isTouchScreen ? 'no-scrollbar' : 'custom-scroll')}>
        <div className="symbol-set-container shared-canvas-container">
          ...
          {allAvailableReactions.map((reaction) => {
            ...
            return (<ReactionEmoji key={reactionId} reaction={reaction} isSelected={isSelected} loadAndPlay={loadAndPlay} availableReactions={availableReactions} onClick={onReactionSelect} onContextMenu={onReactionContext} sharedCanvasRef={sharedCanvasRef} sharedCanvasHqRef={sharedCanvasHqRef}/>);
        })}
        </div>
      </div>
    </div>);
