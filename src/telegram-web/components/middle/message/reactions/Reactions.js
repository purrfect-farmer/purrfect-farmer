...
const Reactions = ({ message, threadId, isOutside, maxWidth, metaChildren, observeIntersection, noRecentReactors, isCurrentUserPremium, tags, isAccountFrozen, }) => {
    const { toggleReaction, addLocalPaidReaction, updateMiddleSearch, performMiddleSearch, openPremiumModal, resetLocalPaidReactions, showNotification, openFrozenAccountModal, } = getActions();
    ...
    const totalCount = useMemo(() => (results.reduce((acc, reaction) => acc + reaction.count, 0)), [results]);
    ...
    const handleClick = useLastCallback((reaction) => {
        if (isAccountFrozen) {
    ...
    const handlePaidClick = useLastCallback((count) => {
        if (isAccountFrozen) {
            openFrozenAccountModal();
            return;
        }
    ...
    const handleRemoveReaction = useLastCallback((reaction) => {
        if (isAccountFrozen) {
            openFrozenAccountModal();
            return;
        }
    ...
    return (<div className={buildClassName('Reactions', isOutside && 'is-outside', withServiceReactions && 'is-service')} style={maxWidth ? `max-width: ${maxWidth}px` : undefined} dir={lang.isRtl ? 'rtl' : 'ltr'}>
      {props.map(({ reaction, recentReactors, messageKey, reactionKey, isChosen, tag, }) => (areTags ? (<SavedTagButton key={reactionKey} className="message-reaction" chosenClassName="chosen" containerId={messageKey} isOwnMessage={message.isOutgoing} isChosen={isChosen} reaction={reaction.reaction} tag={tag} withContextMenu={isCurrentUserPremium} onClick={handleClick} onRemove={handleRemoveReaction} observeIntersection={observeIntersection}/>) : (<ReactionButton key={reactionKey} chatId={message.chatId} messageId={message.id} className="message-reaction" chosenClassName="chosen" containerId={messageKey} isOwnMessage={message.isOutgoing} recentReactors={recentReactors} isOutside={isOutside} reaction={reaction} onClick={handleClick} onPaidClick={handlePaidClick} observeIntersection={observeIntersection}/>)))}
      {metaChildren}
    </div>);
};
