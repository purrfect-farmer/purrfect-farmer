...
import { selectChat, selectChatFullInfo, selectIsCurrentUserPremium, selectSimilarChannelIds, } from '../../../global/selectors';
...
const SimilarChannels = ({ chatId, similarChannelIds, isExpanded, count, isCurrentUserPremium, channelJoinInfo, }) => {
    ...
    const shouldRenderChannels = Boolean(!shouldRenderSkeleton
    ...
    useEffect(() => {
        if (!channelJoinInfo?.joinedDate || isExpanded)
            return;
    ...
    return (<div className={buildClassName(styles.root)}>
      {shouldRenderSkeleton && <Skeleton className={styles.skeleton}/>}
      {shouldRenderChannels && (<div className={buildClassName(isShowing && styles.isAppearing, isHiding && styles.isHiding)}>
          <div className={styles.notch}>
            <svg width="19" height="7" viewBox="0 0 19 7" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path className={styles.notchPath} fill-rule="evenodd" clip-rule="evenodd" d="M19 7C16.8992 7 13.59 3.88897 11.5003 1.67424C10.7648 0.894688 10.397 0.50491 10.0434 0.385149C9.70568 0.270811 9.4225 0.270474 9.08456 0.38401C8.73059 0.50293 8.36133 0.892443 7.62279 1.67147C5.52303 3.88637 2.18302 7 0 7L19 7Z" fill="white"/>
            </svg>
          </div>
          <div className={styles.inner}>
            <div className={styles.header}>
              <span className={styles.title}>{lang('SimilarChannels')}</span>
              <Button className={styles.close} color="translucent" onClick={handleToggle}>
                <Icon name="close"/>
              </Button>
            </div>
            <div ref={ref} className={buildClassName(styles.channelList, 'no-scrollbar')}>
              {firstSimilarChannels?.map((channel, i) => {
                return i === SHOW_CHANNELS_NUMBER - 1 ? (<MoreChannels channel={channel} chatId={chatId} channelsCount={count - SHOW_CHANNELS_NUMBER + 1} isCurrentUserPremium={isCurrentUserPremium}/>) : (<SimilarChannel channel={channel}/>);
            })}
            </div>
          </div>
        </div>)}
...
function SimilarChannel({ channel }) {
    const { openChat } = getActions();
    const color = useAverageColor(channel, DEFAULT_BADGE_COLOR);
    const lang = useLang();
    return (<div className={styles.item} onClick={() => openChat({ id: channel.id })}>
...
function MoreChannels({ channel, chatId, channelsCount, isCurrentUserPremium, }) {
    ...
    return (<div className={buildClassName(styles.item, styles.lastItem)} onClick={() => handleClickMore()}>
      <Avatar className={styles.avatar} key={channel.id} size="large" peer={channel}/>
