...
import { selectCanPlayAnimatedEmojis, selectPeer, selectSender, selectUser, } from '../../../../global/selectors';
...
const StarGiftAction = ({ action, message, canPlayAnimatedEmojis, sender, recipient, starGiftMaxConvertPeriod, onClick, observeIntersectionForLoading, observeIntersectionForPlaying, }) => {
    const ref = useRef();
    ...
    const giftDescription = useMemo(() => {
        ...
        if (action.alreadyPaidUpgradeStars) {
            return translateWithYou(lang, 'ActionStarGiftUpgradeText', !isOutgoing || isSelf, { peer: peerLink });
        }
        if (action.isConverted) {
            return translateWithYou(lang, 'ActionStarGiftConvertedText', !isOutgoing || isSelf, { peer: peerLink, amount: starsAmount });
        }
        if (starGiftMaxConvertPeriod && getServerTime() < message.date + starGiftMaxConvertPeriod) {
            return translateWithYou(lang, 'ActionStarGiftConvertText', !isOutgoing || isSelf, { peer: peerLink, amount: starsAmount });
        }
        if (isChannel) {
            return lang('ActionStarGiftChannelText', { amount: starsAmount }, { withNodes: true });
        }
        return translateWithYou(lang, 'ActionStarGiftNoConvertText', !isOutgoing || isSelf, { peer: peerLink });
    }, [
        action, fallbackPeerTitle, isChannel, isOutgoing, lang, message.date, peer?.id, peerTitle, starGiftMaxConvertPeriod,
        isSelf,
    ]);
    return (<div ref={ref} className={buildClassName(styles.contentBox, styles.starGift)} tabIndex={0} role="button" onClick={onClick}>
      ...
      {Boolean(action.gift.availabilityTotal) && (<GiftRibbon color={backgroundColor || 'blue'} text={lang('ActionStarGiftLimitedRibbon', {
...
export default memo(withGlobal((global, { message, action }) => {
