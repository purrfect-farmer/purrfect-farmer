...
const SuggestedPhotoAction = ({ message, action, peer, observeIntersection, }) => {
    ...
    const handleSetSuggestedAvatar = useLastCallback((file) => {
    ...
    return (<div className={styles.contentBox} tabIndex={0} role="button" onClick={handleViewSuggestedAvatar}>
      <Avatar className={styles.suggestedAvatar} photo={action.photo} loopIndefinitely withVideo observeIntersection={observeIntersection} size="jumbo"/>
      ...
      <CropModal file={cropModalBlob} onClose={handleCloseCropModal} onChange={handleSetSuggestedAvatar}/>
      <ConfirmDialog isOpen={isVideoModalOpen} title={lang('ActionSuggestedVideoTitle')} confirmHandler={handleSetVideo} onClose={closeVideoModal} text={lang('ActionSuggestedVideoText')}/>
    </div>);
};
