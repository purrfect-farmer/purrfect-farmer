...
import { selectPeer, selectPeerStories, selectPeerStory, selectUser, } from '../../../global/selectors';
...
function StoryMention({ message, story, peer, isUnread, targetUser, }) {
    ...
    return (<div className={buildClassName('action-message-story-mention', isUnread && 'is-unread', isLoaded && 'with-preview')} tabIndex={0} role="button" onClick={isLoaded ? handleClick : undefined}>
      {isLoaded && (<span className="story-media-wrapper">
          {thumbUrl && (<img src={thumbUrl} alt="" className="story-media"/>)}
        </span>)}
