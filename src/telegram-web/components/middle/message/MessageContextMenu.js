...
import { getUserFullName, groupStatefulContent, } from '../../../global/helpers';
...
const MessageContextMenu = ({ isReactionPickerOpen, availableReactions, topReactions, defaultTagReactions, isOpen, message, poll, story, isPrivate, isCurrentUserPremium, enabledReactions, isWithPaidReaction, reactionsLimit, anchor, targetHref, canSendNow, canReschedule, canBuyPremium, canReply, canQuote, canEdit, canAppendTodoList, noReplies, canPin, canUnpin, canDelete, canForward, canReport, canFaveSticker, canUnfaveSticker, canCopy, canCopyLink, canSelect, canDownload, canSaveGif, canRevote, canClosePoll, canTranslate, canShowOriginal, canSelectLanguage, isDownloading, repliesThreadInfo, canShowSeenBy, canShowReactionsCount, canShowReactionList, seenByRecentPeers, hasCustomEmoji, customEmojiSets, canPlayAnimatedEmojis, isInSavedMessages, shouldRenderShowWhen, canLoadReadDate, onReply, onOpenThread, onEdit, onAppendTodoList, onPin, onUnpin, onForward, onDelete, onFaveSticker, onReport, onUnfaveSticker, onSelect, onSend, onReschedule, onClose, onCloseAnimationEnd, onCopyLink, onCopyNumber, onDownload, onSaveGif, onCancelVote, onClosePoll, onShowSeenBy, onShowReactors, onToggleReaction, onSendPaidReaction, onShowPaidReactionModal, onCopyMessages, onReactionPickerOpen, onTranslate, onShowOriginal, onSelectLanguage, userFullName, canGift, }) => {
    const { showNotification, openStickerSet, openCustomEmojiSets, loadStickers, openGiftModal, } = getActions();
    ...
    const copyOptions = getMessageCopyOptions(message, groupStatefulContent({ poll, story }), targetHref, canCopy, handleAfterCopy, canCopyLink ? onCopyLink : undefined, onCopyMessages, onCopyNumber);
    ...
    const handleOpenMessageReactionPicker = useLastCallback((position) => {
        onReactionPickerOpen(position);
        hideItems();
    });
    return (<Menu ref={menuRef} isOpen={isOpen} anchor={anchor} getTriggerElement={getTriggerElement} getRootElement={getRootElement} getMenuElement={getMenuElement} getLayout={getLayout} withMaxHeight className={buildClassName('MessageContextMenu', 'fluid', withReactions && 'with-reactions')} onClose={onClose} onCloseAnimationEnd={onCloseAnimationEnd}>
      {withReactions && (<ReactionSelector enabledReactions={enabledReactions} topReactions={topReactions} allAvailableReactions={availableReactions} defaultTagReactions={defaultTagReactions} currentReactions={message.reactions?.results} reactionsLimit={reactionsLimit} onToggleReaction={onToggleReaction} onSendPaidReaction={onSendPaidReaction} onShowPaidReactionModal={onShowPaidReactionModal} isWithPaidReaction={isWithPaidReaction} isPrivate={isPrivate} isReady={isReady} canBuyPremium={canBuyPremium} isCurrentUserPremium={isCurrentUserPremium} isInSavedMessages={isInSavedMessages} canPlayAnimatedEmojis={canPlayAnimatedEmojis} onShowMore={handleOpenMessageReactionPicker} onClose={onClose} className={buildClassName(areItemsHidden && 'ReactionSelector-hidden')}/>)}

      <div ref={scrollableRef} className={buildClassName('MessageContextMenu_items scrollable-content custom-scroll', areItemsHidden && 'MessageContextMenu_items-hidden')} dir={oldLang.isRtl ? 'rtl' : undefined}>
        ...
        {canReschedule && (<MenuItem icon="schedule" onClick={onReschedule}>{oldLang('MessageScheduleEditTime')}</MenuItem>)}
        {canReply && (<MenuItem icon="reply" onClick={onReply}>
            {oldLang(canQuote ? 'lng_context_quote_and_reply' : 'Reply')}
          </MenuItem>)}
        {!noReplies && Boolean(repliesThreadInfo?.messagesCount) && (<MenuItem icon="replies" onClick={onOpenThread}>
        ...
        {canAppendTodoList && (<MenuItem icon="add" onClick={onAppendTodoList}>
            {lang('MenuButtonAppendTodoList')}
          </MenuItem>)}
        {canFaveSticker && (<MenuItem icon="favorite" onClick={onFaveSticker}>{oldLang('AddToFavorites')}</MenuItem>)}
        {canUnfaveSticker && (<MenuItem icon="favorite" onClick={onUnfaveSticker}>{oldLang('Stickers.RemoveFromFavorites')}</MenuItem>)}
        {canTranslate && <MenuItem icon="language" onClick={onTranslate}>{oldLang('TranslateMessage')}</MenuItem>}
        {canShowOriginal && (<MenuItem icon="language" onClick={onShowOriginal}>
            {oldLang('ShowOriginalButton')}
          </MenuItem>)}
        {canSelectLanguage && (<MenuItem icon="web" onClick={onSelectLanguage}>{oldLang('lng_settings_change_lang')}</MenuItem>)}
        {copyOptions.map((option) => (<MenuItem key={option.label} icon={option.icon} onClick={option.handler} withPreventDefaultOnMouseDown>
        ...
        {canDownload && (<MenuItem icon="download" onClick={onDownload}>
        ...
        {hasCustomEmoji && (<>
            ...
            {customEmojiSets && customEmojiSets.length === 1 && (<MenuItem withWrap onClick={handleOpenCustomEmojiSets} className="menu-custom-emoji-sets">
                {renderText(oldLang('MessageContainsEmojiPack', customEmojiSets[0].title), ['simple_markdown', 'emoji'])}
              </MenuItem>)}
            {customEmojiSets && customEmojiSets.length > 1 && (<MenuItem withWrap onClick={handleOpenCustomEmojiSets} className="menu-custom-emoji-sets">
        ...
        {(canShowSeenBy || canShowReactionsCount) && (<>
            ...
            <MenuItem icon={canShowReactionsCount ? 'heart-outline' : 'group'} onClick={canShowReactionsCount ? onShowReactors : onShowSeenBy} disabled={!canShowReactionsCount && !seenByDatesCount}>
              <span className="MessageContextMenu--seen-by-label-wrapper">
                <span className="MessageContextMenu--seen-by-label" dir={oldLang.isRtl ? 'rtl' : undefined}>
                  {canShowReactionsCount && message.reactors?.count ? (canShowSeenBy && seenByDatesCount
                ? oldLang('Chat.OutgoingContextMixedReactionCount', [message.reactors.count, seenByDatesCount])
                : oldLang('Chat.ContextReactionCount', message.reactors.count, 'i')) : (seenByDatesCount === 1 && seenByRecentPeers
                ? renderText(isUserId(seenByRecentPeers[0].id)
                    ? getUserFullName(seenByRecentPeers[0])
                    : seenByRecentPeers[0].title) : (seenByDatesCount
        ...
        {(canLoadReadDate || shouldRenderShowWhen || isEdited) && (<MenuSeparator size={hasCustomEmoji ? 'thin' : 'thick'}/>)}
        {(canLoadReadDate || shouldRenderShowWhen) && (<ReadTimeMenuItem canLoadReadDate={canLoadReadDate} shouldRenderShowWhen={shouldRenderShowWhen} message={message} closeContextMenu={onClose}/>)}
        {isEdited && (<LastEditTimeMenuItem message={message}/>)}
      </div>
    </Menu>);
};
