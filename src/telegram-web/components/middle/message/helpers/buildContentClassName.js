...
export function buildContentClassName(message, album, { poll, hasSubheader, isCustomShape, isLastInGroup, asForwarded, hasThread, forceSenderName, hasCommentCounter, hasCommentButton, hasActionButton, hasReactions, isGeoLiveActive, withVoiceTranscription, peerColorClass, hasOutsideReactions, } = {}) {
    ...
    const { photo = paidMediaPhoto, video = paidMediaVideo, audio, voice, document, webPage, contact, location, invoice, storyData, giveaway, giveawayResults, } = content;
    ...
    const hasFooter = (() => {
        if (isInvertedMedia && isInvertibleMedia) {
            if (hasReactions && !hasOutsideReactions)
                return true;
            if (hasFactCheck)
                return true;
            if (webPage && hasText)
