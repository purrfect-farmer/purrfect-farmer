...
function getRatios(messages, isSingleMessage, isMobile) {
    ...
    const allMedia = (isSingleMessage
        ? messages[0].content.paidMedia.extendedMedia.map((media) => ('mediaType' in media ? media : (media.photo || media.video)))
        : messages.map((message) => (getMessageContent(message).photo || getMessageContent(message).video))).filter(Boolean);
...
export function calculateAlbumLayout(isOwn, noAvatars, album, isMobile) {
    const spacing = 2;
...
function layoutWithComplexLayouter({ ratios: originalRatios, averageRatio, maxWidth, minWidth, spacing, maxHeight = (4 * maxWidth) / 3, }) {
