...
const InlineButtons = ({ message, onClick }) => {
    ...
    return (<div className="InlineButtons">
      {message.inlineButtons.map((row, i) => (<div className="row">
          {row.map((button, j) => (<Button size="tiny" ripple disabled={button.type === 'unsupported'} onClick={() => onClick({ chatId: message.chatId, messageId: message.id, button })}>
