...
import { getChatTitle, getUserFullName, isOwnMessage, } from '../../../global/helpers';
import { isApiPeerChat } from '../../../global/helpers/peers';
import { selectCanPlayAnimatedEmojis, selectChat, selectForwardedSender, selectGiftStickerForDuration, } from '../../../global/selectors';
...
const Giveaway = ({ chat, sender, message, canPlayAnimatedEmojis, giftSticker, }) => {
    ...
    const { months, untilDate, prizeDescription, stars, } = (giveaway || giveawayResults);
    ...
    const countryList = useMemo(() => {
        ...
        return giveaway?.countries?.map((countryCode) => (`${isoToEmoji(countryCode)}${NBSP}${translatedNames.of(countryCode)}`)).join(', ');
    }, [giveaway, isResults, lang.code]);
    const handlePeerClick = useLastCallback((channelId) => {
        openChat({ id: channelId });
    });
    const handleShowInfoClick = useLastCallback(async () => {
        if (isLoadingInfo.current)
            return;
    ...
    function renderGiveawayDescription(media) {
        ...
        <div className={styles.section}>
          ...
          <p className={styles.description}>
            {message?.content?.giveaway?.stars ? (<>
                {renderText(lang('Chat.Giveaway.Message.Stars.PrizeText', lang('Stars', message?.content?.giveaway?.stars)), ['simple_markdown'])}
                <br />
        ...
        <div className={styles.section}>
          ...
          <div className={styles.peers}>
            {channelIds.map((peerId) => (<PeerChip peerId={peerId} forceShowSelf withPeerColors={!isOwn} className={styles.peer} clickArg={peerId} onClick={handlePeerClick}/>))}
          </div>
          {countryList && (<span>{renderText(lang('Chat.Giveaway.Message.CountriesFrom', countryList))}</span>)}
        </div>
        <div className={styles.section}>
          <strong className={styles.title}>
    ...
    function renderGiveawayResultsDescription(media) {
        ...
        <div className={styles.section}>
          ...
          <div className={styles.peers}>
            {winnerIds.map((peerId) => (<PeerChip peerId={peerId} forceShowSelf withPeerColors={!isOwn} className={styles.peer} clickArg={peerId} onClick={handlePeerClick}/>))}
          </div>
        </div>
        <div className={styles.section}>
          <p className={styles.description}>
    ...
    function renderGiveawayInfo() {
        ...
        return (<>
        {result && (<p className={styles.result}>
    ...
    return (<div className={styles.root}>
      <div className={buildClassName(styles.sticker, isResults && styles.resultSticker)}>
        {isResults ? (<AnimatedIconWithPreview size={RESULT_STICKER_SIZE} tgsUrl={LOCAL_TGS_URLS.PartyPopper} nonInteractive noLoop/>) : (<AnimatedIconFromSticker sticker={giftSticker} play={canPlayAnimatedEmojis && hasEnded} noLoop nonInteractive size={GIFT_STICKER_SIZE}/>)}
        <span className={styles.count}>
          {`x${quantity}`}
        </span>
      </div>
      {isResults ? renderGiveawayResultsDescription(giveawayResults) : renderGiveawayDescription(giveaway)}
      <Button className={styles.button} color="adaptive" size="smaller" onClick={handleShowInfoClick}>
        {lang('BoostingHowItWork')}
      </Button>
      <ConfirmDialog isOpen={Boolean(giveawayInfo)} isOnlyConfirm title={giveawayInfoTitle} confirmHandler={handleCloseInfo} onClose={handleCloseInfo}>
        {renderGiveawayInfo()}
      </ConfirmDialog>
    </div>);
};
