...
const MAX_LINES = 4;
const FactCheck = ({ factCheck, isToggleDisabled }) => {
    const lang = useOldLang();
    const ref = useRef();
    const cutoutRef = useRef();
    const { isCollapsed, isCollapsible, setIsCollapsed, } = useCollapsibleLines(ref, MAX_LINES, cutoutRef);
    const countryLocalized = useMemo(() => {
        if (!factCheck.countryCode || !lang.code)
            return undefined;
    ...
    return (<PeerColorWrapper peerColor={COLOR} className={styles.root} onClick={canExpand ? handleExpand : undefined}>
      <div ref={cutoutRef} className={buildClassName(styles.cutoutWrapper, isCollapsed && styles.collapsed)}>
