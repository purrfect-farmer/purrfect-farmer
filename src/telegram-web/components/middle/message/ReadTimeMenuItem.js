...
function ReadTimeMenuItem({ message, shouldRenderShowWhen, canLoadReadDate, closeContextMenu, }) {
    ...
    return (<MenuItem icon="message-read" className={styles.item}>
      <Transition name="fade" activeKey={shouldRenderSkeleton ? 1 : 2} className={styles.transition}>
        {shouldRenderSkeleton ? <Skeleton className={styles.skeleton}/> : (<>
            ...
            {!readDate && shouldRenderShowWhen && (<div>
                {lang('PmRead')}
