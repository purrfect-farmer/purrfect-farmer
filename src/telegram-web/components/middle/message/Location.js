...
import { buildStaticMapHash, getMessageLocation, isGeoLiveExpired, } from '../../../global/helpers';
import buildClassName from '../../../util/buildClassName';
import { formatCountdownShort, formatLocationLastUpdate } from '../../../util/dates/dateFormat';
import { getMetersPerPixel, getVenueColor, getVenueIconUrl, } from '../../../util/map';
...
const Location = ({ message, peer, }) => {
    ...
    const updateCountdown = useLastCallback((countdownEl) => {
        ...
        if (mediaType === 'geoLive') {
          ...
          {!isExpired && (<div className="geo-countdown" ref={countdownRef}>
              ...
              <svg width="32px" height="32px">
                <circle cx="16" cy="16" r={TIMER_RADIUS} className="geo-countdown-progress" transform="rotate(-90, 16, 16)" stroke-dasharray={TIMER_CIRCUMFERENCE} stroke-dashoffset="0"/>
              </svg>
            </div>)}
        </div>);
        }
    ...
    function renderMap() {
        ...
        return (<img className="full-media map" src={mapBlobUrl} alt="Location on a map" draggable={false} style={`width: ${DEFAULT_MAP_CONFIG.width}px; height: ${DEFAULT_MAP_CONFIG.height}px;`}/>);
    }
    function renderPin() {
        const pinClassName = buildClassName('pin', mediaType, isExpired && 'expired');
        if (mediaType === 'geoLive') {
            return (<div className={pinClassName}>
          <PinSvg />
          <Avatar peer={peer} className="location-avatar"/>
          {location.heading !== undefined && (<div className="direction" style={`--direction: ${location.heading}deg`}/>)}
        </div>);
        }
        if (mediaType === 'venue') {
            ...
            if (iconSrc) {
                return (<div className={pinClassName} style={`--pin-color: ${color}`}>
            <PinSvg />
            <img src={iconSrc} draggable={false} className="venue-icon" alt=""/>
          </div>);
            }
        }
        return (<img className={pinClassName} draggable={false} src={mapPin} alt=""/>);
    }
    function renderOverlay() {
        if (!mapBlobUrl)
            return undefined;
        return (<>
        {Boolean(accuracyRadiusPx) && !isExpired && (<div className="location-accuracy" style={`width: ${accuracyRadiusPx * 2}px; height: ${accuracyRadiusPx * 2}px`}/>)}
        {renderPin()}
      </>);
    }
    return (<div ref={ref} className="Location media-inner interactive" onClick={handleClick}>
...
function PinSvg() {
    return (<svg className="round-pin" style="enable-background:new 0 0 64 64" viewBox="0 0 64 64">
