...
import { getMainUsername, getMessageInvoice, getMessageText, isChatChannel, } from '../../../global/helpers';
import { getMessageContent } from '../../../global/helpers';
import { getPeerTitle } from '../../../global/helpers/peers';
import { getMessageReplyInfo } from '../../../global/helpers/replies';
import { selectChat, selectChatMessage, selectPeer, selectSender, selectThreadIdFromMessage, selectTopic, } from '../../../global/selectors';
...
import { getCallMessageKey, getPinnedMediaValue, renderMessageLink, renderPeerLink, translateWithYou, } from './helpers/messageActions';
...
const ActionMessageText = ({ message, currentUserId, sender, chat, replyMessage, isInsideTopic, asPreview, }) => {
    const { openThread, openTelegramLink, openUrl, } = getActions();
        ...
        switch (action.type) {
            case 'pinMessage': {
                if (replyMessage) {
                    ...
                    if (formattedText) {
                        const textLink = renderMessageLink(replyMessage, renderTextWithEntities({
                            text: formattedText.text,
                            entities: formattedText.entities,
                            asPreview: true,
                        }), asPreview);
                        return translateWithYou(lang, 'ActionPinnedText', isOutgoing, { text: textLink, from: senderLink });
                    }
                    ...
                    if (mediaValue) {
                        ...
                        return translateWithYou(lang, 'ActionPinnedMedia', isOutgoing, { from: senderLink, media: messageLink });
                    }
                }
                return translateWithYou(lang, 'ActionPinnedNotFound', isOutgoing, { from: senderLink });
            }
            case 'gameScore': {
                const { score } = action;
                ...
                if (gameLink) {
                    return translateWithYou(lang, 'ActionGameScore', isOutgoing, { from: senderLink, count: score, game: gameLink }, { pluralValue: score });
                }
                return translateWithYou(lang, 'ActionGameScoreNoGame', isOutgoing, { from: senderLink, count: score }, { pluralValue: score });
            }
            ...
            case 'chatEditTitle': {
                const { title } = action;
                if (isChannel)
                    return lang('ActionChangedTitleChannel', { title });
            ...
            case 'topicCreate': {
                ...
                const topicLink = (<Link className={styles.topicLink} onClick={() => openThread({ chatId, threadId: topicId })}>
            ...
            case 'topicEdit': {
                const { iconEmojiId, isClosed, isHidden, title, } = action;
                const topicId = selectThreadIdFromMessage(global, message);
            ...
            {iconEmojiId && iconEmojiId !== DEFAULT_TOPIC_ICON_ID
                        ...
                        : (<TopicDefaultIcon topicId={topicId} title={title || currentTopic?.title || lang('ActionTopicPlaceholder')} iconColor={currentTopic?.iconColor}/>)}
            {topicId !== GENERAL_TOPIC_ID && NBSP}
            {renderText(title || currentTopic?.title || lang('ActionTopicPlaceholder'))}
          </Link>);
                const topicPlaceholderLink = (<Link className={styles.topicLink} onClick={() => openThread({ chatId, threadId: topicId })}>
            {lang('ActionTopicPlaceholder')}
                ...
                if (isClosed !== undefined) {
                    ...
                    return lang(isClosed ? 'ActionTopicClosed' : 'ActionTopicReopened', { from: senderLink, topic: topicLink }, { withNodes: true });
                }
                if (isHidden !== undefined) {
                    if (isInsideTopic) {
                        return lang(isHidden ? 'ActionTopicHiddenInside' : 'ActionTopicUnhiddenInside');
                    }
                    return lang(isHidden ? 'ActionTopicHidden' : 'ActionTopicUnhidden', { topic: topicLink }, { withNodes: true });
                }
                if (title && iconEmojiId) {
                    return lang('ActionTopicIconAndRenamed', {
                        from: senderLink,
                ...
                if (title === undefined) {
                    if (!iconEmojiId || iconEmojiId === DEFAULT_TOPIC_ICON_ID) {
                        return lang('ActionTopicIconRemoved', { from: senderLink, link: topicPlaceholderLink }, { withNodes: true });
                    }
                    return lang('ActionTopicIconChanged', {
                        from: senderLink,
                        link: topicPlaceholderLink,
            ...
            case 'boostApply':
                return translateWithYou(lang, 'ActionBoostApply', isOutgoing, { from: senderLink, count: action.boosts }, { pluralValue: action.boosts });
            case 'chatAddUser': {
                ...
                const userLinks = users.map((user) => (renderPeerLink(user.id, getPeerTitle(lang, user) || userFallbackText, asPreview)));
                return translateWithYou(lang, 'ActionAddUsersMany', isOutgoing, { from: senderLink, users: conjuctionWithNodes(lang, userLinks) });
            }
            case 'chatDeleteUser': {
                const { userId } = action;
                if (sender?.id === userId) {
            ...
            case 'botAllowed': {
                const { app, domain, isAttachMenu, isFromRequest, } = action;
                if (isAttachMenu)
                    return lang('ActionAttachMenuBotAllowed');
                if (isFromRequest)
                    return lang('ActionWebappBotAllowed');
                if (app) {
            ...
            case 'giveawayLaunch': {
                const { stars } = action;
                if (stars) {
                    return lang(isChannel ? 'ActionGiveawayStarsStarted' : 'ActionGiveawayStarsStartedGroup', { from: senderLink, amount: renderStrong(formatStarsAsText(lang, stars)) }, { withNodes: true });
                }
                return lang(isChannel ? 'ActionGiveawayStarted' : 'ActionGiveawayStartedGroup', { from: senderLink }, { withNodes: true });
            }
            case 'giveawayResults': {
                const { winnersCount, isStars, unclaimedCount } = action;
                if (!winnersCount)
                    return lang('ActionGiveawayResultsNone');
                if (unclaimedCount) {
                    return lang(isStars ? 'ActionGiveawayResultsStarsSome' : 'ActionGiveawayResultsSome');
                }
                return lang(isStars ? 'ActionGiveawayResultsStars' : 'ActionGiveawayResults', { count: winnersCount }, { pluralValue: winnersCount });
            }
            case 'giftStars':
            case 'giftPremium': {
                const { amount, currency, cryptoAmount, cryptoCurrency, } = action;
                ...
                return translateWithYou(lang, 'ActionGiftTextCost', isOutgoing, { from: senderLink, cost: renderStrong(cost) });
            }
            case 'prizeStars':
            case 'giftCode': {
            ...
            case 'groupCall': {
                ...
                if (durationText) {
                    ...
                    return lang('ActionGroupCallFinishedGroup', { from: senderLink, duration: durationText }, { withNodes: true });
                }
                if (isChannel)
            ...
            case 'inviteToGroupCall': {
                ...
                const users = userIds.map((userId) => selectPeer(global, userId)).filter(Boolean);
                if (!users.length) {
                    return translateWithYou(lang, 'ActionVideoInvited', isOutgoing, { from: senderLink, user: userFallbackText });
                }
                const userLinks = users.map((user) => (renderPeerLink(user.id, getPeerTitle(lang, user) || userFallbackText, asPreview)));
                return translateWithYou(lang, 'ActionVideoInvitedMany', isOutgoing, { from: senderLink, users: conjuctionWithNodes(lang, userLinks) });
            }
            case 'paymentSent': {
                const { currency, totalAmount, isRecurringInit, isRecurringUsed, } = action;
                ...
                if (isRecurringInit) {
                    return lang('ActionPaymentInitRecurringFor', { amount: cost, user: chatLink, invoice: renderMessageLink(replyMessage, invoiceTitle, asPreview) }, { withNodes: true });
                }
                return lang('ActionPaymentDoneFor', { amount: cost, user: chatLink, invoice: renderMessageLink(replyMessage, invoiceTitle, asPreview) }, { withNodes: true });
            }
            ...
            case 'starGift': {
                const { gift, alreadyPaidUpgradeStars, peerId, savedId, fromId, } = action;
                ...
                if (isToChannel) {
                    ...
                    return translateWithYou(lang, 'ActionStarGiftSentChannel', isYou, { user: fromLink, channel: channelLink, cost });
                }
                if (isServiceNotificationsChat) {
            ...
            case 'starGiftUnique': {
                const { isTransferred, isUpgrade, savedId, peerId, fromId, resaleStars, gift, } = action;
                const isToChannel = Boolean(peerId && savedId);
                const fromPeer = fromId ? selectPeer(global, fromId) : sender;
                const fromTitle = (fromPeer && getPeerTitle(lang, fromPeer)) || userFallbackText;
                const fromLink = renderPeerLink(fromPeer?.id, fromTitle, asPreview);
                if (resaleStars) {
                    return lang(isOutgoing
                ...
                if (isToChannel) {
                    ...
                    const channelLink = renderPeerLink(peerId, channelTitle, asPreview);
                    if (isUpgrade) {
                        return translateWithYou(lang, 'ActionStarGiftUpgradedChannel', isYou, { user: fromLink, channel: channelLink });
                    }
                    if (isTransferred) {
                        if (isAnonymous) {
                            return lang('ActionStarGiftTransferredUnknownChannel', { channel: channelLink }, { withNodes: true });
                        }
                        return translateWithYou(lang, 'ActionStarGiftTransferredChannel', isYou, { user: fromLink, channel: channelLink });
                    }
                }
                if (isSavedMessages) {
                    if (isUpgrade)
            ...
            case 'suggestProfilePhoto': {
                ...
                const actionPeerLink = renderPeerLink(actionPeer.id, getPeerTitle(lang, actionPeer) || userFallbackText, asPreview);
            ...
            case 'expired': {
                ...
                if (isVoice)
                    return lang('ActionExpiredVoice');
                if (isRoundVideo)
                    return lang('ActionExpiredVideo');
                return lang(UNSUPPORTED_LANG_KEY);
            }
            case 'historyClear':
            ...
            case 'paidMessagesPrice': {
                ...
                if (stars === 0) {
                    if (isChannel) {
                        return lang(isAllowedInChannel ? 'ActionMessageChannelFree' : 'ActionMessageChannelDisabled', { peer: chatLink }, { withNodes: true });
                    }
            ...
            case 'todoCompletions': {
                ...
                if (incompletedItem) {
                    ...
                    const incompletedTaskLink = renderMessageLink(replyMessage, renderTextWithEntities({
                ...
                if (completedItem) {
                    ...
                    const completedTaskLink = renderMessageLink(replyMessage, renderTextWithEntities({
                ...
                if (completedIds) {
                    ...
                    const completedLink = renderMessageLink(replyMessage, renderTextWithEntities({
                        text: completedText,
                        asPreview: true,
                    }), asPreview, { noEllipsis: true });
                    return translateWithYou(lang, 'MessageActionTodoCompletionsAsDone', isOutgoing, {
                        peer: senderLink,
                        task: completedLink,
                    });
                ...
                const incompletedLink = renderMessageLink(replyMessage, renderTextWithEntities({
            ...
            case 'todoAppendTasks': {
                ...
                const listLink = renderMessageLink(replyMessage, renderTextWithEntities({
                    text: listTitle,
                    asPreview: true,
                }), asPreview);
                if (items.length === 1) {
                    const taskTitle = items[0].title;
                    const taskLink = renderMessageLink(replyMessage, renderTextWithEntities({
                        text: taskTitle.text,
                ...
                const tasksLink = renderMessageLink(replyMessage, renderTextWithEntities({
                    text: tasksText,
                    asPreview: true,
                }), asPreview, { noEllipsis: true });
