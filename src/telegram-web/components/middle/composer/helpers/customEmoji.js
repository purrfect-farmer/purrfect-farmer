...
export function buildCustomEmojiHtml(emoji) {
    const [isPlaceholder, src, uniqueId] = getInputCustomEmojiParams(emoji);
    const className = buildClassName('custom-emoji', 'emoji', 'emoji-small', isPlaceholder && 'placeholder', emoji.shouldUseTextColor && 'colorable');
    return `<img
    class="${className}"
    draggable="false"
    alt="${emoji.emoji}"
...
export function buildCustomEmojiHtmlFromEntity(rawText, entity) {
    ...
    const className = buildClassName('custom-emoji', 'emoji', 'emoji-small', isPlaceholder && 'placeholder', customEmoji?.shouldUseTextColor && 'colorable');
    return `<img
    class="${className}"
    draggable="false"
    alt="${rawText}"
...
export function getCustomEmojiSize(maxEmojisInLine) {
    if (!maxEmojisInLine)
        return undefined;
    // Should be the same as in _message-content.scss
    if (maxEmojisInLine > EMOJI_SIZES)
        return 2.25 * REM;
    if (maxEmojisInLine === 1)
        return 7 * REM;
