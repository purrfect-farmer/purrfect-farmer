...
import { TODO_ITEM_LENGTH_LIMIT, TODO_ITEMS_LIMIT, TODO_TITLE_LENGTH_LIMIT, } from '../../../config';
...
const ToDoListModal = ({ modal, maxItemsCount = TODO_ITEMS_LIMIT, maxTitleLength = TODO_TITLE_LENGTH_LIMIT, maxItemLength = TODO_ITEM_LENGTH_LIMIT, editingMessage, onSend, onClear, }) => {
    const { editTodo, closeTodoListModal, appendTodoList } = getActions();
    const titleInputRef = useRef();
    const itemsListRef = useRef();
    const [title, setTitle] = useState('');
    const [items, setItems] = useState([{ id: generateUniqueNumberId(), text: '' }]);
    ...
    const addNewItem = useLastCallback((newItems) => {
    ...
    const handleCreate = useLastCallback(() => {
        ...
        if (!titleTrimmed || todoItems.length === 0) {
            ...
            if (todoItems.length) {
                const itemsTrimmed = items.map((o) => ({ ...o, text: o.text.trim().substring(0, maxItemLength) }))
    ...
    const updateItem = useLastCallback((index, text) => {
    ...
    const removeItem = useLastCallback((index) => {
        const newItems = [...items];
    ...
    const handleIsOthersCanAppendChange = useLastCallback((e) => {
        setIsOthersCanAppend(e.target.checked);
    });
    const handleIsOthersCanCompleteChange = useLastCallback((e) => {
    ...
    const getItemsError = useLastCallback((index) => {
    ...
    function renderItems() {
        ...
        return tasksToRender.map((item, index) => {
          ...
          <InputText maxLength={MAX_OPTION_LENGTH} label={index !== tasksToRender.length - 1 || tasksToRender.length === maxItemsCount
                    ? lang('TitleTask')
                    : lang('TitleAddTask')} error={getItemsError(stateIndex)} value={item.text} disabled={item.isDisabled} onChange={(e) => updateItem(stateIndex, e.currentTarget.value)} onKeyPress={handleKeyPress}/>
          {index !== tasksToRender.length - 1 && !item.isDisabled && (<Button className="item-remove-button" round color="translucent" size="smaller" ariaLabel={lang('Delete')} onClick={() => removeItem(stateIndex)}>
              <Icon name="close"/>
            </Button>)}
        </div>);
        });
    }
    return (<Modal isOpen={isOpen} onClose={onClear} header={renderHeader()} className="ToDoListModal">
      {!isAddTaskMode && (<InputText ref={titleInputRef} label={lang('InputTitle')} value={title} error={getTitleError()} onChange={handleTitleChange} onKeyPress={handleKeyPress}/>)}
      {isAddTaskMode && (<div className="readonly-title">
          {title}
        </div>)}
      ...
      {!isAddTaskMode && (<div className="options-footer">
          <div className="dialog-checkbox-group">
            <Checkbox label={lang('AllowOthersAddTasks')} checked={isOthersCanAppend} onChange={handleIsOthersCanAppendChange}/>
            <Checkbox label={lang('AllowOthersMarkAsDone')} checked={isOthersCanComplete} onChange={handleIsOthersCanCompleteChange}/>
          </div>
        </div>)}
    </Modal>);
};
