import { useCallback, useEffect, useLayoutEffect, useRef, useState, useUnmountCleanup, } from '../../../lib/teact/teact';
...
export default function useHeaderPane({ ref: providedRef, isOpen, isDisabled, withResizeObserver, onStateChange, }) {
    ...
    const handleUpdate = useCallback(() => {
        ...
        if (isDisabled || !element || !shouldRender)
            return;
        if (!isOpen) {
            onStateChange?.({
                element,
...
export function applyAnimationState(list, noTransition = false) {
