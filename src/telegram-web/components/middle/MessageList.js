...
const MessageList = ({
    ...
    messageIds.forEach((id, index, arr) => {
      ...
      const { shouldAppendJoinMessage, shouldAppendJoinMessageAfterCurrent } =
        (() => {
          ...
          if (
            prevMessage &&
            prevMessage.date < channelJoinInfo.joinedDate &&
            channelJoinInfo.joinedDate <= message.date
          ) {
            return {
          ...
          if (
            index === arr.length - 1 &&
            message.date < channelJoinInfo.joinedDate
          ) {
            return {
              shouldAppendJoinMessage: true,
      ...
      if (shouldAppendJoinMessage) {
        const lastMessageId = shouldAppendJoinMessageAfterCurrent
          ? message.id
          : prevMessage?.id || message.id - 1;
        listedMessages.push({
  ...
  useInterval(() => {
    if (!messageIds || !messagesById || type === "scheduled" || isAccountFrozen)
      return;
    if (!isChannelChat && !isGroupChat) return;
    const ids = messageIds.filter((id) => {
      const message = messagesById[id];
      return (
        message && message.reactions?.results.length && !message.content.action
      );
    });
  ...
  useInterval(() => {
    ...
    const storyDataList = messageIds
      .map((id) => messagesById[id]?.content.storyData)
      .filter(Boolean);
  ...
  useInterval(
    () => {
      ...
      const ids = messageIds.filter(
        (id) =>
          selectThreadInfo(global, chatId, id)?.isCommentsInfo ||
  ...
  useInterval(() => {
    ...
    const ids = messageIds.filter(
      (id) => messagesById[id]?.factCheck?.shouldFetch
    );
    if (!ids.length) return;
  ...
  useEffect(() => {
    ...
    const isLastMessageLocal =
      messageIds && isLocalMessageId(messageIds[messageIds.length - 1]);
  ...
  useLayoutEffectWithPrevDeps(
    ([prevMessageIds, prevIsViewportNewest]) => {
      ...
      listItemElementsRef.current = Array.from(
        container.querySelectorAll(".message-list-item")
      );
      ...
      requestForcedReflow(() => {
        ...
        if (isAtBottom && isResized) {
          ...
          newScrollTop = Math.min(
            getOffsetToContainer(unreadDivider, container).top -
              UNREAD_DIVIDER_TOP,
  ...
  const noAvatars = Boolean(
    !withUsers || (isChannelChat && !isChannelWithAvatars)
  );
...
export default memo(
  withGlobal((global, { chatId, threadId, type }) => {
    ...
    const lastMessage = selectChatLastMessage(
      global,
      chatId,
    ...
    const areAdsEnabled =
      ...
      selectUserFullInfo(global, currentUserId)?.areAdsEnabled;
    ...
    const canTranslate =
      ...
      !chatFullInfo?.isTranslationDisabled;
    ...
    return {
      ...
      hasLinkedChat: chatFullInfo
        ? Boolean(chatFullInfo.linkedChatId)
        : undefined,
