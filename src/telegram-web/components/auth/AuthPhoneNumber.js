...
const AuthPhoneNumber = ({ connectionState, authState, authPhoneNumber, authIsLoading, authIsLoadingQrCode, authErrorKey, authRememberMe, authNearestCountry, phoneCodeList, language, isTestServer, }) => {
    const { setAuthPhoneNumber, setAuthRememberMe, loadNearestCountry, loadCountryList, clearAuthErrorKey, goToAuthQrCode, setSharedSettingOption, } = getActions();
    const lang = useLang();
    ...
    const phoneNumberSlots = useMemo(() => (Object.entries(accountsInfo)
        .filter(([, info]) => info.isTest === isTestServer)
        .reduce((acc, [key, { phone }]) => {
        if (phone)
            acc[phone] = Number(key);
        return acc;
    ...
    const parseFullNumber = useLastCallback((newFullNumber) => {
    ...
    const handleCountryChange = useLastCallback((value) => {
        setCountry(value);
        setPhoneNumber('');
    });
    const handlePhoneNumberChange = useLastCallback((e) => {
        if (authErrorKey) {
            clearAuthErrorKey();
        }
        ...
        setLastSelection(selectionStart && selectionEnd && selectionEnd < value.length
            ? [selectionStart, selectionEnd]
            : undefined);
        setIsTouched(true);
        const shouldFixSafariAutoComplete = (IS_SAFARI && country && fullNumber !== undefined
    ...
    const handleKeepSessionChange = useLastCallback((e) => {
    ...
    return (<div id="auth-phone-number-form" className="custom-scroll">
      {hasActiveAccount && (<Button size="smaller" round color="translucent" className="auth-close" onClick={handleBackNavigation}>
          <Icon name="close"/>
        </Button>)}
      <div className="auth-form">
        ...
        <form className="form" action="" onSubmit={handleSubmit}>
          <CountryCodeInput id="sign-in-phone-code" value={country} isLoading={!authNearestCountry && !country} onChange={handleCountryChange}/>
          <InputText ref={inputRef} id="sign-in-phone-number" label={lang('LoginPhonePlaceholder')} value={fullNumber} error={authErrorKey && lang.withRegular(authErrorKey)} inputMode="tel" onChange={handlePhoneNumberChange} onPaste={IS_SAFARI ? handlePaste : undefined}/>
          <Checkbox id="sign-in-keep-session" label={lang('AuthKeepSignedIn')} checked={Boolean(authRememberMe)} onChange={handleKeepSessionChange}/>
          {canSubmit && (isAuthReady ? (<Button size="smaller" type="submit" ripple isLoading={authIsLoading}>{lang('LoginNext')}</Button>) : (<Loading />))}
          {isAuthReady && (<Button size="smaller" isText ripple isLoading={authIsLoadingQrCode} onClick={handleGoToAuthQrCode}>
              {lang('LoginQRLogin')}
            </Button>)}
          {suggestedLanguage && suggestedLanguage !== language && continueText && (<Button size="smaller" isText isLoading={isLoading} onClick={handleLangChange}>{continueText}</Button>)}
        </form>
      </div>
    </div>);
};
export default memo(withGlobal((global) => {
    const { sharedState: { settings: { language } }, countryList: { phoneCodes: phoneCodeList }, config, } = global;
    return {
        ...pick(global, [
            'connectionState',
            'authState',
