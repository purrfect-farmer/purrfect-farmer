...
const AuthCode = ({ connectionState, authState, authQrCode, language, }) => {
    ...
    return (<div id="auth-qr-form" className="custom-scroll">
      {hasActiveAccount && (<Button size="smaller" round color="translucent" className="auth-close" onClick={handleBackNavigation}>
          <Icon name="close"/>
        </Button>)}
      <div className="auth-form qr">
        <div className="qr-outer">
          <div className={buildClassName('qr-inner', transitionClassNames)} key="qr-inner">
            <div key="qr-container" className="qr-container" ref={qrCodeRef} style={`width: ${QR_SIZE}px; height: ${QR_SIZE}px`}/>
            <AnimatedIcon tgsUrl={LOCAL_TGS_URLS.QrPlane} size={QR_PLANE_SIZE} className="qr-plane" nonInteractive noLoop={false}/>
          </div>
        ...
        {isAuthReady && (<Button size="smaller" isText onClick={handleReturnToAuthPhoneNumber}>{lang('LoginQRCancel')}</Button>)}
