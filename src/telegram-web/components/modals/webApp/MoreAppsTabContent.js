...
const MoreAppsTabContent = ({ foundIds, recentBotIds, }) => {
    const oldLang = useOldLang();
    const lang = useLang();
    const [shouldShowMoreMine, setShouldShowMoreMine] = useState(false);
    const { searchPopularBotApps, } = getActions();
    ...
    return (<InfiniteScroll className={buildClassName(styles.root, 'custom-scroll')} items={filteredFoundIds} onLoadMore={handleLoadMore} itemSelector=".PopularAppGridItem" noFastList preloadBackwards={POPULAR_APPS_SLICE}>
      <SearchInput className={styles.search} value={searchQuery} onChange={setSearchQuery} onReset={handleSearchInputReset} placeholder={lang('SearchApps')}/>
      {recentBotIds && !searchQuery && (<div className={styles.section}>
          ...
          <div className={styles.sectionContent}>
            {recentBotIds.map((id, index) => {
                ...
                return (<WebAppGridItem chatId={id}/>);
            })}
          </div>
        </div>)}
      <div className={styles.section}>
        ...
        <div className={styles.sectionContent}>
          {filteredFoundIds && filteredFoundIds.map((id) => {
            return (<WebAppGridItem chatId={id} isPopularApp={!searchQuery}/>);
        })}
        </div>
      </div>
    </InfiniteScroll>);
