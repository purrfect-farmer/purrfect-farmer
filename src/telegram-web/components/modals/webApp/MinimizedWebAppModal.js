...
const MinimizedWebAppModal = ({ activeTabBot, isMinimizedState, openedWebApps, }) => {
    const { changeWebAppModalState, closeWebAppModal, } = getActions();
    ...
    return (<div ref={ref} className={buildClassName(styles.root)}>
      <Button className={styles.button} round color="translucent" size="tiny" ariaLabel={oldLang('Close')} onClick={handleCloseClick}>
        <Icon className={styles.icon} name="close"/>
      </Button>
      <AvatarList className={styles.avatars} size="mini" peers={peers}/>
      {renderTitle()}
      <Button className={buildClassName(styles.windowStateButton, 'no-drag')} round color="translucent" size="tiny" onClick={handleExpandClick}>
