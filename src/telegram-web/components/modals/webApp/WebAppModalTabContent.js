...
import buildStyle from "../../../util/buildStyle.js";
import download from "../../../util/download";
import {
  extractCurrentThemeParams,
  validateHexColor,
} from "../../../util/themeStyle";
...
import useFullscreen, {
  checkIfFullscreen,
} from "../../../hooks/window/useFullscreen";
...
const WebAppModalTabContent = ({
  ...
  const [popupParameters, setPopupParameters] = useState();
  const [isRequestingPhone, setIsRequestingPhone] = useState(false);
  const [isRequestingWriteAccess, setIsRequestingWriteAccess] = useState(false);
  const [requestedFileDownload, setRequestedFileDownload] = useState();
  const [bottomBarColor, setBottomBarColor] = useState();
  const { unlockPopupsAt, handlePopupOpened, handlePopupClosed } =
    usePopupLimit(POPUP_SEQUENTIAL_LIMIT, POPUP_RESET_DELAY);
  const containerRef = useRef();
  const headerButtonRef = useRef();
  const headerButtonCaptionRef = useRef();
  ...
  const [, setFullscreen, exitFullscreen] = useFullscreen(
    fullscreenElementRef,
    exitFullScreenCallback
  );
  const activeWebApp = modal?.activeWebAppKey
    ? modal.openedWebApps[modal.activeWebAppKey]
    : undefined;
  const { appName: activeWebAppName, backgroundColor } = activeWebApp || {};
  const { url, buttonText, isBackButtonVisible } = webApp || {};
  const { placeholderPath } = botAppSettings || {};
  ...
  const updateCurrentWebApp = useLastCallback((updatedPartialWebApp) => {
    if (!webAppKey) return;
  ...
  const [backgroundColorFromEvent, setBackgroundColorFromEvent] = useState();
  const backgroundColorFromSettings =
    theme === "light"
      ? botAppSettings?.backgroundColor
      : botAppSettings?.backgroundDarkColor;
  useEffect(() => {
    const color =
      backgroundColorFromEvent ||
  ...
  const [headerColorFromEvent, setHeaderColorFromEvent] = useState();
  const headerColorFromSettings =
    theme === "light"
      ? botAppSettings?.headerColor
      : botAppSettings?.headerDarkColor;
  useEffect(() => {
    const color =
      headerColorFromEvent || headerColorFromSettings || themeHeaderColor;
  ...
  const isMainButtonVisible =
    isLoaded && mainButton?.isVisible && mainButton.text.trim().length > 0;
  const isSecondaryButtonVisible =
    isLoaded &&
    secondaryButton?.isVisible &&
    secondaryButton.text.trim().length > 0;
  ...
  const handleAppPopupClose = useLastCallback((buttonId) => {
  ...
  const sendFullScreenChangedCallback = useLastCallback((value) => {
  ...
  const visibilityChangedCallBack = useLastCallback((visibility) => {
    sendEvent({
  ...
  const handleRejectFileDownload = useLastCallback((shouldCloseActive) => {
  ...
  function handleEvent(event) {
    ...
    if (eventType === "web_app_set_background_color") {
      setBackgroundColorFromEvent(
        validateHexColor(eventData.color) ? eventData.color : undefined
      );
    }
    if (eventType === "web_app_set_header_color") {
      const key = eventData.color_key;
      setHeaderColorFromEvent(
        eventData.color || (key ? themeParams[key] : undefined)
      );
    }
    ...
    if (eventType === "web_app_setup_main_button") {
      ...
      setMainButton({
        isVisible:
          eventData.is_visible && Boolean(eventData.text?.trim().length),
    ...
    if (eventType === "web_app_open_popup") {
      if (
        popupParameters ||
        !eventData.message.trim().length ||
        !eventData.buttons?.length ||
        eventData.buttons.length > 3 ||
        isRequestingPhone ||
    ...
    if (eventType === "web_app_switch_inline_query") {
      const filter = eventData.chat_types
        ?.map(convertToApiChatType)
        .filter(Boolean);
  ...
  const mainButtonCurrentTextColor = useCurrentOrPrev(
    mainButton?.textColor,
    true
  );
  const mainButtonCurrentIsActive = useCurrentOrPrev(
    mainButton && Boolean(mainButton.isActive),
    true
  );
  const mainButtonCurrentText = useCurrentOrPrev(mainButton?.text, true);
  const secondaryButtonCurrentPosition = useCurrentOrPrev(
    secondaryButton?.position,
    true
  );
  const secondaryButtonCurrentColor = useCurrentOrPrev(
    secondaryButton?.color,
    true
  );
  const secondaryButtonCurrentTextColor = useCurrentOrPrev(
    secondaryButton?.textColor,
    true
  );
  const secondaryButtonCurrentIsActive = useCurrentOrPrev(
    secondaryButton && Boolean(secondaryButton.isActive),
    true
  );
  const secondaryButtonCurrentText = useCurrentOrPrev(
    secondaryButton?.text,
    true
  );
  const [shouldDecreaseWebFrameSize, setShouldDecreaseWebFrameSize] =
    useState(false);
  const [shouldHideMainButton, setShouldHideMainButton] = useState(true);
  const [shouldHideSecondaryButton, setShouldHideSecondaryButton] =
    useState(true);
  const [shouldShowMainButton, setShouldShowMainButton] = useState(false);
  const [shouldShowSecondaryButton, setShouldShowSecondaryButton] =
    useState(false);
  const [shouldShowAppNameInFullscreen, setShouldShowAppNameInFullscreen] =
    useState(false);
  ...
  const isVerticalLayout =
    secondaryButtonCurrentPosition === "top" ||
    secondaryButtonCurrentPosition === "bottom";
  const isHorizontalLayout = !isVerticalLayout;
  const rowsCount =
    isVerticalLayout && shouldShowMainButton && shouldShowSecondaryButton
      ? 2
      : shouldShowMainButton || shouldShowSecondaryButton
      ? 1
      : 0;
  const hideDirection =
    isHorizontalLayout && !shouldHideMainButton && !shouldHideSecondaryButton
      ? "horizontal"
      : "vertical";
  const mainButtonChangeTimeout = useRef();
  const mainButtonFastTimeout = useRef();
  const secondaryButtonChangeTimeout = useRef();
  const secondaryButtonFastTimeout = useRef();
  const appNameDisplayTimeout = useRef();
  useEffect(() => {
  ...
  useEffect(() => {
    if (mainButtonChangeTimeout.current)
      clearTimeout(mainButtonChangeTimeout.current);
    if (mainButtonFastTimeout.current)
      clearTimeout(mainButtonFastTimeout.current);
  ...
  useEffect(() => {
    if (secondaryButtonChangeTimeout.current)
      clearTimeout(secondaryButtonChangeTimeout.current);
    if (secondaryButtonFastTimeout.current)
      clearTimeout(secondaryButtonFastTimeout.current);
  ...
  const handleShowContextMenu = useLastCallback((e) => {
  ...
  const backButtonCaption = shouldShowAppNameInFullscreen
    ? activeWebAppName
  ...
  function renderFullscreenBackButtonCaption() {
    return (
      ...
      >
        <Transition activeKey={getBackButtonActiveKey()} name="slideFade">
          <div
    ...
    return (
      <div className={styles.headerPanel}>
        ...
        <div className={styles.headerSplitButton}>
          <div
            className={buildClassName(styles.headerButton, styles.left)}
            tabIndex={0}
            role="button"
            aria-label={lang("WebAppCollapse")}
            onClick={handleCollapseClick}
          >
            <Icon name="down" className={styles.icon} />
          </div>
          <div
            className={buildClassName(styles.headerButton, styles.right)}
            tabIndex={0}
            role="button"
            aria-haspopup="menu"
          ...
          >
            <Icon
              name="more"
              className={buildClassName(styles.icon, styles.moreIcon)}
            />
          </div>
        </div>
      </div>
  ...
  return (
    ...
    >
      {isFullscreen &&
        getIsWebAppsFullscreenSupported() &&
        renderFullscreenHeaderPanel()}
      ...
      {!isMinimizedState && (
        ...
        >
          <Button
            ...
            disabled={
              !secondaryButtonCurrentIsActive &&
          ...
          <Button
            ...
            disabled={
              !mainButtonCurrentIsActive && !mainButton?.isProgressVisible
            }
          ...
          >
            ...
            {mainButton?.isProgressVisible && (
              <Spinner className={styles.mainButtonSpinner} color="white" />
            )}
          </Button>
        </div>
      )}
      {popupParameters && (
        <Modal
          ...
          className={buildClassName(
            styles.webAppPopup,
            !popupParameters.title?.trim().length && styles.withoutTitle
          )}
        >
      ...
      <ConfirmDialog
        ...
        textParts={renderText(oldLang("BotRemoveFromMenu", bot?.firstName), [
          "simple_markdown",
        ])}
...
export default memo(
  withGlobal((global, { modal }) => {
    const activeWebApp = modal?.activeWebAppKey
      ? selectWebApp(global, modal.activeWebAppKey)
      : undefined;
    const { botId: activeBotId } = activeWebApp || {};
    const modalState = modal?.modalState;
    const attachBot = activeBotId
      ? global.attachMenu.bots[activeBotId]
      : undefined;
    const bot = activeBotId ? selectUser(global, activeBotId) : undefined;
    const userFullInfo = activeBotId
      ? selectUserFullInfo(global, activeBotId)
      : undefined;
    ...
    const { isPaymentModalOpen, status: regularPaymentStatus } =
      selectTabState(global).payment;
    const { status: starsPaymentStatus, inputInvoice: starsInputInvoice } =
      selectTabState(global).starsPayment;
    const botAppPermissions = bot
      ? selectBotAppPermissions(global, bot.id)
      : undefined;
