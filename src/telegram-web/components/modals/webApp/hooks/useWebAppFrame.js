...
const useWebAppFrame = (ref, isOpen, isFullscreen, isSimpleView, onEvent, webApp, onLoad) => {
    const { showNotification, setWebAppPaymentSlug, openInvoice, closeWebApp, openSuggestedStatusModal, updateWebApp, } = getActions();
    const isReloadSupported = useRef(false);
    const reloadTimeout = useRef();
    const ignoreEventsRef = useRef(false);
    const lastFrameSizeRef = useRef();
    ...
    const sendFullScreenChanged = useCallback((value) => {
        sendEvent({
    ...
    const forceReloadFrame = useLastCallback((url) => {
        if (!ref.current)
            return;
        const frame = ref.current;
    ...
    const sendViewport = useCallback((isNonStable) => {
        if (!ref.current) {
            return;
        }
    ...
    const sendCustomStyle = useCallback((style) => {
        sendEvent({
    ...
    const handleMessage = useCallback((event) => {
        ...
        try {
            ...
            if (eventType === 'web_app_data_send') {
                if (!isSimpleView)
                    return; // Allowed only in simple view
    ...
    useEffect(() => {
        ...
        if (lastFrameSizeRef.current && lastFrameSizeRef.current.width === width
            && lastFrameSizeRef.current.height === height && !lastFrameSizeRef.current.isResizing)
            return;
        lastFrameSizeRef.current = { width, height, isResizing };
        sendViewport(isResizing);
    }, [sendViewport, sendSafeArea, windowSize]);
    useEffect(() => {
        if (!webApp?.plannedEvents?.length)
            return;
