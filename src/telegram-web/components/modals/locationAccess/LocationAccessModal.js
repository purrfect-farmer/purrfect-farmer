...
const LocationAccessModal = ({ modal, currentUser, }) => {
    const { closeLocationAccessModal, toggleUserLocationPermission, sendWebAppEvent, } = getActions();
    ...
    const renderInfo = useLastCallback(() => {
        ...
        <Avatar size="large" peer={currentUser}/>
        <Icon name="next" className={styles.arrow}/>
        <Avatar size="large" peer={modal.bot}/>
      </div>);
    });
    ...
    return (<Modal className={buildClassName('confirm')} isOpen={isOpen} onClose={onCloseHandler}>
      {renderInfo()}
      <div>
        {renderStatusText()}
