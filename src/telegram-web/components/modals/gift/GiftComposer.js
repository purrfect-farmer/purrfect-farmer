...
import { selectPeer, selectPeerPaidMessagesStars, selectTabState, selectTheme, selectThemeValues, selectUserFullInfo, } from '../../../global/selectors';
...
function GiftComposer({ gift, giftByStars, peerId, peer, captionLimit, theme, isBackgroundBlurred, patternColor, backgroundColor, customBackground, currentUserId, isPaymentFormLoading, starBalance, paidMessagesStars, areUniqueStarGiftsDisallowed, shouldDisallowLimitedStarGifts, }) {
    const { sendStarGift, sendPremiumGiftByStars, openInvoice, openGiftUpgradeModal, openStarsBalanceModal, } = getActions();
    const lang = useLang();
    ...
    const [shouldPayByStars, setShouldPayByStars] = useState(false);
    ...
    const handleGiftMessageChange = useLastCallback((e) => {
    ...
    const toggleShouldPayByStars = useLastCallback(() => {
        if (hasPremiumByStars)
            setShouldPayByStars(!shouldPayByStars);
    });
    ...
    function renderOptionsSection() {
        ...
        {!paidMessagesStars && (<TextArea className={styles.messageInput} onChange={handleGiftMessageChange} value={giftMessage} label={lang('GiftMessagePlaceholder')} maxLength={captionLimit} maxLengthIndicator={symbolsLeft && symbolsLeft < LIMIT_DISPLAY_THRESHOLD ? symbolsLeft.toString() : undefined}/>)}

        {canUseStarsPayment && (<ListItem className={styles.switcher} narrow ripple onClick={toggleShouldPayByStars}>
            <span>
              {lang('GiftPremiumPayWithStars', {
                    stars: formatStarsAsIcon(lang, giftByStars.amount, { className: styles.switcherStarIcon }),
                }, { withNodes: true })}
            </span>
            <Switcher checked={shouldPayByStars} onChange={toggleShouldPayByStars} label={lang('GiftPremiumPayWithStarsAcc')}/>
          </ListItem>)}

        {hasPremiumByStars && starBalance && (<div className={styles.description}>
        ...
        {isStarGift && Boolean(gift.upgradeStars) && !areUniqueStarGiftsDisallowed && (<ListItem className={styles.switcher} narrow ripple onClick={handleShouldPayForUpgradeChange} disabled={shouldDisallowLimitedStarGifts}>
            <span>
              {lang('GiftMakeUnique', {
                    stars: formatStarsAsIcon(lang, gift.upgradeStars, { className: styles.switcherStarIcon }),
                }, { withNodes: true })}
            </span>
            <Switcher checked={shouldPayForUpgrade} onChange={handleShouldPayForUpgradeChange} label={lang('GiftMakeUniqueAcc')}/>
          </ListItem>)}
        {isStarGift && Boolean(gift.upgradeStars) && !areUniqueStarGiftsDisallowed && (<div className={styles.description}>
            {isPeerUser
        ...
        {isStarGift && (<ListItem className={styles.switcher} narrow ripple onClick={handleShouldHideNameChange}>
            <span>{lang('GiftHideMyName')}</span>
            <Switcher checked={shouldHideName} onChange={handleShouldHideNameChange} label={lang('GiftHideMyName')}/>
          </ListItem>)}
        {isStarGift && (<div className={styles.description}>
    ...
    function renderFooter() {
        ...
        {isStarGift && Boolean(gift.availabilityRemains) && (<PremiumProgress isPrimary progress={gift.availabilityRemains / gift.availabilityTotal} rightText={lang('GiftSoldCount', {
                    count: gift.availabilityTotal - gift.availabilityRemains,
                })} leftText={lang('GiftLeftCount', { count: gift.availabilityRemains })} className={styles.limited}/>)}
        <Button className={styles.mainButton} size="smaller" onClick={handleMainButtonClick} isLoading={isPaymentFormLoading}>
          {lang('GiftSend', {
                amount,
            }, {
                withNodes: true,
    ...
    const bgClassName = buildClassName(styles.background, styles.withTransition, customBackground && styles.customBgImage, backgroundColor && styles.customBgColor, customBackground && isBackgroundBlurred && styles.blurred);
    if ((!isStarGift && !isPremiumGift) || !localMessage)
        return;
    return (<div className={buildClassName(styles.root, 'custom-scroll')}>
      <div className={buildClassName(styles.actionMessageView, 'MessageList')} 
    // @ts-ignore -- FIXME: Find a way to disable interactions but keep a11y
    inert style={buildStyle(`--pattern-color: ${patternColor}`, backgroundColor && `--theme-background-color: ${backgroundColor}`)}>
        <div className={bgClassName} style={customBackgroundValue ? `--custom-background: ${customBackgroundValue}` : undefined}/>
        <ActionMessage key={isStarGift ? gift.id : isPremiumGift ? gift.months : undefined} message={localMessage} threadId={MAIN_THREAD_ID} appearanceOrder={0}/>
      </div>
...
export default memo(withGlobal((global, { peerId }) => {
    ...
    const { stars, } = global;
    const { isBlurred: isBackgroundBlurred, patternColor, background: customBackground, backgroundColor, } = selectThemeValues(global, theme) || {};
