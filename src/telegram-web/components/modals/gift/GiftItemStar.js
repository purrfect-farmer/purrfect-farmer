...
function GiftItemStar({ gift, observeIntersection, onClick, isResale, }) {
    ...
    const giftRibbon = useMemo(() => {
        if (isGiftUnique) {
            ...
            return (<GiftRibbon color={[backdrop.centerColor, backdrop.edgeColor]} text={lang('GiftSavedNumber', { number: giftNumber })}/>);
        }
        if (isResale) {
    ...
    return (<div ref={ref} className={buildClassName(styles.container, styles.starGift, 'starGiftItem')} tabIndex={0} role="button" onClick={handleGiftClick}>
      ...
      <Button className={styles.buy} nonInteractive size="tiny" color={isGiftUnique ? 'bluredStarsBadge' : 'stars'} withSparkleEffect={isVisible} pill fluid>
