...
const GiftModal = ({ modal, starGiftsById, starGiftIdsByCategory, starBalance, peer, isSelf, disallowedGifts, resaleGiftsCount, areResaleGiftsLoading, }) => {
    const { closeGiftModal, openGiftInfoModal, resetResaleGifts, loadResaleGifts, } = getActions();
    const dialogRef = useRef();
    const transitionRef = useRef();
    const giftHeaderRef = useRef();
    ...
    const [selectedResellGift, setSelectedResellGift] = useState();
    ...
    const areAllGiftsDisallowed = useMemo(() => {
        ...
        const { shouldDisallowPremiumGifts, ...disallowedGiftTypes } = disallowedGifts;
    ...
    const giftsByStars = useMemo(() => {
        ...
        filteredGifts.forEach((gift) => {
            const giftByStars = allGifts?.find((starsGift) => starsGift.currency === STARS_CURRENCY_CODE
    ...
    const { observe: observeIntersection, } = useIntersectionObserver({ rootRef: scrollerRef, throttleMs: INTERSECTION_THROTTLE, isDisabled: !isOpen });
    ...
    const giftPremiumDescription = lang('GiftPremiumDescription', {
        ...
        link: (<SafeLink text={lang('GiftPremiumDescriptionLinkCaption')} url={lang('GiftPremiumDescriptionLink')}/>),
    }, { withNodes: true });
    ...
    function renderGiftPremiumHeader() {
        return (<h2 className={buildClassName(styles.headerText, styles.center)}>
        {lang('GiftPremiumHeader')}
      </h2>);
    }
    function renderGiftPremiumDescription() {
        return (<p className={buildClassName(styles.description, styles.center)}>
        {giftPremiumDescription}
      </p>);
    }
    function renderStarGiftsHeader() {
        return (<h2 ref={giftHeaderRef} className={buildClassName(styles.headerText, styles.center)}>
        {lang(isSelf ? 'StarsGiftHeaderSelf' : 'StarsGiftHeader')}
      </h2>);
    }
    function renderStarGiftsDescription() {
        return (<p className={buildClassName(styles.description, styles.starGiftsDescription, styles.center)}>
        {starGiftDescription}
      </p>);
    }
    const handleGiftClick = useLastCallback((gift, target) => {
        if (target === 'resell') {
            if (!('id' in gift)) {
                return;
            }
    ...
    function renderStarGifts() {
        ...
        return (<div className={styles.starGiftsContainer}>
        {starGiftsById && filteredGiftIds?.flatMap((giftId) => {
                ...
                const elements = [
                    <GiftItemStar key={giftId} gift={gift} observeIntersection={observeIntersection} isResale={shouldShowResale && !shouldDuplicateAsResale} onClick={handleGiftClick}/>,
                ];
                if (shouldDuplicateAsResale) {
                    elements.push(<GiftItemStar key={`resale_${giftId}`} isResale gift={gift} observeIntersection={observeIntersection} onClick={handleGiftClick}/>);
                }
                return elements;
            })}
      </div>);
    ...
    function renderPremiumGifts() {
        return (<div className={styles.premiumGiftsGallery}>
        {filteredGifts?.map((gift) => {
                return (<GiftItemPremium option={gift} optionByStars={giftsByStars.get(gift)} baseMonthAmount={baseGift ? Math.floor(baseGift.amount / baseGift.months) : undefined} onClick={handleGiftClick}/>);
            })}
      </div>);
    }
    const onCategoryChanged = useLastCallback((category) => {
    ...
    function renderMainScreen() {
        return (<div ref={scrollerRef} className={buildClassName(styles.main, 'custom-scroll')} onScroll={handleScroll}>
        ...
        {!areAllGiftsDisallowed && (<>
            ...
            <StarGiftCategoryList areLimitedStarGiftsDisallowed={areLimitedStarGiftsDisallowed} onCategoryChanged={onCategoryChanged}/>
            <Transition name="zoomFade" activeKey={getCategoryKey(selectedCategory)} className={styles.starGiftsTransition}>
              {renderStarGifts()}
            </Transition>
          </>)}
      </div>);
    ...
    const buttonClassName = buildClassName('animated-close-icon', isBackButton && 'state-back');
    function renderHeader() {
        if (!shouldShowHeader)
            return undefined;
        if (isResaleScreen) {
            const isFirstLoading = areResaleGiftsLoading && !resaleGiftsCount;
            return (<div className={styles.resaleHeaderContentContainer}>
        ...
        return (<h2 className={styles.commonHeaderText}>
        {lang(isHeaderForStarGifts ? (isSelf ? 'StarsGiftHeaderSelf' : 'StarsGiftHeader') : 'GiftPremiumHeader')}
      </h2>);
    }
    return (<Modal dialogRef={dialogRef} onClose={handleCloseModal} isOpen={isOpen} isSlim contentClassName={styles.content} className={buildClassName(styles.modalDialog, styles.root)} isLowStackPriority>
      <Button className={styles.closeButton} round color="translucent" size="smaller" onClick={handleCloseButtonClick} ariaLabel={isBackButton ? oldLang('Common.Back') : oldLang('Common.Close')}>
        <div className={buttonClassName}/>
      </Button>
      <BalanceBlock className={styles.balance} balance={starBalance} withAddButton/>
      <div className={buildClassName(styles.header, isResaleScreen && styles.resaleHeader, !shouldShowHeader && styles.hiddenHeader)}>
        <Transition name="slideVerticalFade" activeKey={!shouldShowHeader ? 0 : isResaleScreen ? 1 : isHeaderForStarGifts ? 2 : 3} slideClassName={styles.headerSlide}>
          {renderHeader()}
        </Transition>
      </div>
      <Transition ref={transitionRef} className={styles.transition} name="pushSlide" activeKey={isGiftScreen ? 1 : isResaleScreen ? 2 : 0}>
        {!isGiftScreen && !isResaleScreen && renderMainScreen()}
        {isResaleScreen && selectedResellGift
            && (<GiftModalResaleScreen onGiftClick={handleGiftClick}/>)}
        {isGiftScreen && renderingModal?.forPeerId && (<GiftSendingOptions gift={selectedGift} giftByStars={giftsByStars.get(selectedGift)} peerId={renderingModal.forPeerId}/>)}
      </Transition>
    </Modal>);
};
export default memo(withGlobal((global, { modal }) => {
    const { starGifts, stars, currentUserId, } = global;
