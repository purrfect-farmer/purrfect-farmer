...
import TableInfoModal from '../../common/TableInfoModal';
import UniqueGiftHeader from '../UniqueGiftHeader';
import styles from './GiftInfoModal.module.scss';
const STICKER_SIZE = 120;
const GiftInfoModal = ({ modal, fromPeer, targetPeer, currentUserId, starGiftMaxConvertPeriod, hasAdminRights, currentUserEmojiStatus, collectibleEmojiStatuses, tonExplorerUrl, currentUser, recipientPeer, }) => {
    const { closeGiftInfoModal, changeGiftVisibility, convertGiftToStars, openChatWithInfo, focusMessage, openGiftUpgradeModal, showNotification, buyStarGift, closeGiftModal, } = getActions();
    ...
    const canConvertDifference = (savedGift && starGiftMaxConvertPeriod && (savedGift.date + starGiftMaxConvertPeriod - getServerTime())) || 0;
    ...
    const handleFocusUpgraded = useLastCallback(() => {
        ...
        if (!savedGift?.upgradeMsgId || !giftChat)
            return;
    ...
    const handleBuyGift = useLastCallback(() => {
        if (gift?.type !== 'starGiftUnique' || !gift.resellPriceInStars)
            return;
    ...
    const handleConfirmBuyGift = useLastCallback(() => {
        ...
        if (!peer || gift?.type !== 'starGiftUnique' || !gift.resellPriceInStars)
            return;
        closeConfirmModal();
        closeGiftModal();
    ...
    const SettingsMenuButton = useMemo(() => {
        return ({ onTrigger }) => (<div className={buildClassName(styles.headerButton, styles.left)} tabIndex={0} role="button" aria-haspopup="menu" aria-label={lang('AriaMoreButton')} onClick={onTrigger}>
        <Icon name="more" className={styles.icon}/>
      </div>);
    }, [lang]);
    const renderFooterButton = useLastCallback(() => {
        if (canBuyGift) {
            return (<Button noForcedUpperCase size="smaller" onClick={handleBuyGift}>
          {lang('ButtonBuyGift', {
        ...
        if (canFocusUpgrade) {
            return (<Button size="smaller" onClick={handleFocusUpgraded}>
          {lang('GiftInfoViewUpgraded')}
        </Button>);
        }
        if (canManage && savedGift?.alreadyPaidUpgradeStars && !savedGift.upgradeMsgId) {
            return (<Button size="smaller" isShiny onClick={handleOpenUpgradeModal}>
          {lang('GiftInfoUpgradeForFree')}
        </Button>);
        }
        return (<Button size="smaller" onClick={handleClose}>
        {lang('OK')}
      </Button>);
    });
    ...
    const modalData = useMemo(() => {
        ...
        const { fromId, isNameHidden, starsToConvert, isUnsaved, isConverted, upgradeMsgId, } = savedGift || {};
        ...
        const description = (() => {
            ...
            if (isTargetChat)
                return undefined;
            if (savedGift.upgradeMsgId)
                return lang('GiftInfoDescriptionUpgraded');
        ...
        function getTitle() {
            if (isGiftUnique)
                return gift.title;
            if (!savedGift)
                return lang('GiftInfoSoldOutTitle');
            return canManage ? lang('GiftInfoReceived') : lang('GiftInfoTitle');
        }
        const uniqueGiftContextMenu = (<DropdownMenu className="with-menu-transitions" trigger={SettingsMenuButton} positionX="right">
        <GiftMenuItems peerId={renderingModal.peerId} gift={typeGift} canManage={canManage} collectibleEmojiStatuses={collectibleEmojiStatuses} currentUserEmojiStatus={currentUserEmojiStatus}/>
      </DropdownMenu>);
        const uniqueGiftModalHeader = (<div className={styles.modalHeader}>
        {Boolean(canManage && resellPriceInStars) && (<div className={styles.giftResalePriceContainer}>
        ...
        <div className={styles.headerSplitButton}>
          ...
          <div className={buildClassName(styles.headerButton, styles.right)} tabIndex={0} role="button" aria-haspopup="menu" aria-label={lang('Close')} onClick={handleClose}>
            <Icon name="close" className={buildClassName(styles.icon, styles.moreIcon)}/>
          </div>
        </div>
      </div>);
        const uniqueGiftHeader = isGiftUnique && (<div className={buildClassName(styles.header, styles.uniqueGift)}>
        <UniqueGiftHeader backdropAttribute={giftAttributes.backdrop} patternAttribute={giftAttributes.pattern} modelAttribute={giftAttributes.model} title={gift.title} subtitle={lang('GiftInfoCollectible', { number: gift.number })}/>
      </div>);
        const regularHeader = (<div className={styles.header}>
        <AnimatedIconFromSticker className={styles.giftSticker} sticker={giftSticker} size={STICKER_SIZE}/>
        <h1 className={styles.title}>
          {getTitle()}
        </h1>
        {Boolean(description) && (<p className={buildClassName(styles.description, !savedGift && gift?.type === 'starGift' && styles.soldOut)}>
            {description}
          </p>)}
      </div>);
        const tableData = [];
        if (gift.type === 'starGift') {
          ...
          {canManage && hasConvertOption && Boolean(starsToConvert) && (<BadgeButton onClick={openConvertConfirm}>
        ...
        if (isGiftUnique) {
            ...
            const { model, backdrop, pattern, originalDetails, } = giftAttributes || {};
            if (ownerAddress) {
                tableData.push([
                    lang('GiftInfoOwner'),
                    <span className={styles.ownerAddress} onClick={() => {
            ...
            if (originalDetails) {
                const { date, recipientId, message, senderId, } = originalDetails;
                const global = getGlobal(); // Peer titles do not need to be reactive
                const openChat = (id) => {
                    openChatWithInfo({ id });
                    closeGiftInfoModal();
                ...
                const recipientLink = (<Link onClick={() => openChat(recipientId)} isPrimary>
            {getPeerTitle(lang, recipient)}
                ...
                if (!sender || senderId === recipientId) {
                    ...
                    const senderLink = (<Link onClick={() => openChat(sender.id)} isPrimary>
        ...
        {(canManage || tonLink || canBuyGift) && (<div className={styles.footerDescription}>
            {tonLink && (<div>
            ...
            {canManage && (<div>
                {lang(`GiftInfo${isTargetChat ? 'Channel' : ''}${isUnsaved ? 'Hidden' : 'Saved'}`, {
                        link: (<Link isPrimary onClick={handleTriggerVisibility}>
            ...
            {!canBuyGift && isVisibleForMe && (<div>
                {lang('GiftInfoSenderHidden')}
              </div>)}
            {canBuyGift && giftOwnerTitle && (<div>
    ...
    return (<>
      <TableInfoModal isOpen={isOpen} modalHeader={modalData?.modalHeader} header={modalData?.header} hasBackdrop={isGiftUnique} tableData={modalData?.tableData} footer={modalData?.footer} className={styles.modal} onClose={handleClose} withBalanceBar={Boolean(canBuyGift)} isLowStackPriority/>
      {uniqueGift && currentUser && Boolean(resellPriceInStars) && (<ConfirmDialog isOpen={isConfirmModalOpen} noDefaultTitle onClose={closeConfirmModal} confirmLabel={lang('ButtonBuyGift', {
                stars: formatStarsAsIcon(lang, resellPriceInStars, { asFont: true }),
            }, { withNodes: true })} confirmHandler={handleConfirmBuyGift}>

          <GiftTransferPreview peer={recipientPeer || currentUser} gift={uniqueGift}/>
          {!recipientPeer
                && (<p>
      ...
      {savedGift && (<ConfirmDialog isOpen={isConvertConfirmOpen} onClose={closeConvertConfirm} confirmHandler={handleConvertToStars} title={lang('GiftInfoConvertTitle')}>
          <div>
