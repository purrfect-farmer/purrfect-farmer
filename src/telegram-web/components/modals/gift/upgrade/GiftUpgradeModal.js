...
const GiftUpgradeModal = ({ modal, recipient }) => {
    ...
    useEffect(() => {
        ...
        if (!attributes)
            return;
        const patternStickers = attributes.filter((attr) => attr.type === 'pattern')
            .map((attr) => attr.sticker);
        const modelStickers = attributes.filter((attr) => attr.type === 'model')
            .map((attr) => attr.sticker);
    ...
    const modalData = useMemo(() => {
        ...
        const header = (<UniqueGiftHeader modelAttribute={previewAttributes.model} backdropAttribute={previewAttributes.backdrop} patternAttribute={previewAttributes.pattern} title={lang('GiftUpgradeTitle')} subtitle={subtitle}/>);
        const footer = (<div className={styles.footer}>
        {!gift && (<Button className={styles.footerButton} size="smaller" onClick={handleClose}>
            {lang('OK')}
          </Button>)}
        {gift && (<>
            <Checkbox label={lang('GiftUpgradeKeepDetails')} onCheck={setShouldKeepOriginalDetails} checked={shouldKeepOriginalDetails}/>
            <Button className={styles.footerButton} size="smaller" isShiny onClick={handleUpgrade}>
              {gift.alreadyPaidUpgradeStars
                    ...
                    : lang('GiftUpgradeButton', {
                        amount: formatStarsAsIcon(lang, gift.gift.upgradeStars, { asFont: true }),
                    }, { withNodes: true })}
            </Button>
          </>)}
      </div>);
    ...
    return (<TableAboutModal isOpen={isOpen} header={modalData?.header} footer={modalData?.footer} listItemData={modalData?.listItemData} hasBackdrop onClose={handleClose}/>);
};
