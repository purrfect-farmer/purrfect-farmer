...
const GiftResaleFilters = ({ attributes, counters, filter, dialogRef, }) => {
    const lang = useLang();
    const { updateResaleGiftsFilter, } = getActions();
    ...
    const { isContextMenuOpen: isSortContextMenuOpen, contextMenuAnchor: sortContextMenuAnchor, handleContextMenu: handleSortContextMenu, handleContextMenuClose: handleSortContextMenuClose, handleContextMenuHide: handleSortContextMenuHide, } = useContextMenuHandlers(dialogRef);
    const getSortMenuElement = useLastCallback(() => sortMenuRef.current);
    // Model Menu
    const modelMenuRef = useRef();
    const { isContextMenuOpen: isModelContextMenuOpen, contextMenuAnchor: modelContextMenuAnchor, handleContextMenu: handleModelContextMenu, handleContextMenuClose: handleModelContextMenuClose, handleContextMenuHide: handleModelContextMenuHide, } = useContextMenuHandlers(dialogRef);
    const getModelMenuElement = useLastCallback(() => modelMenuRef.current);
    // Backdrop Menu
    const backdropMenuRef = useRef();
    const { isContextMenuOpen: isBackdropContextMenuOpen, contextMenuAnchor: backdropContextMenuAnchor, handleContextMenu: handleBackdropContextMenu, handleContextMenuClose: handleBackdropContextMenuClose, handleContextMenuHide: handleBackdropContextMenuHide, } = useContextMenuHandlers(dialogRef);
    const getBackdropMenuElement = useLastCallback(() => backdropMenuRef.current);
    // Pattern Menu
    const patternMenuRef = useRef();
    const { isContextMenuOpen: isPatternContextMenuOpen, contextMenuAnchor: patternContextMenuAnchor, handleContextMenu: handlePatternContextMenu, handleContextMenuClose: handlePatternContextMenuClose, handleContextMenuHide: handlePatternContextMenuHide, } = useContextMenuHandlers(dialogRef);
    ...
    const SortMenuButton = useMemo(() => {
        ...
        return ({ onTrigger, isOpen: isMenuOpen }) => (<div className={styles.item} onClick={onTrigger}>
         ...
         <Icon name="dropdown-arrows" className={styles.itemIcon}/>
       </div>);
    }, [lang, filter]);
    ...
    const handleSortMenuItemClick = useLastCallback((type) => {
    ...
    const handleModelMenuItemClick = useLastCallback((attribute) => {
        if (!counters)
            return;
    ...
    const handlePatternMenuItemClick = useLastCallback((attribute) => {
        if (!counters)
            return;
    ...
    const handleBackdropMenuItemClick = useLastCallback((attribute) => {
        if (!counters)
            return;
    ...
    function renderSortMenuItems() {
        ...
        <MenuItem icon="sort-by-price" onClick={() => { handleSortMenuItemClick('byPrice'); }}>
          ...
          <Icon className={styles.menuItemIcon} name={filter?.sortType === 'byPrice' ? 'check' : 'placeholder'}/>
        </MenuItem>
        <MenuItem icon="sort-by-date" onClick={() => { handleSortMenuItemClick('byDate'); }}>
          <div className={styles.menuItemText}>
            {lang('GiftSortByDate')}
          </div>
          <Icon className={styles.menuItemIcon} name={filter?.sortType === 'byDate' ? 'check' : 'placeholder'}/>
        ...
        <MenuItem icon="sort-by-number" onClick={() => { handleSortMenuItemClick('byNumber'); }}>
          ...
          <Icon className={styles.menuItemIcon} name={filter?.sortType === 'byNumber' ? 'check' : 'placeholder'}/>
        </MenuItem>
      </>);
    }
    function renderSortMenu() {
        return (<Menu isOpen={isSortContextMenuOpen} anchor={sortContextMenuAnchor} ref={sortMenuRef} className={buildClassName(styles.menu, styles.left, 'with-menu-transitions')} getMenuElement={getSortMenuElement} autoClose onClose={handleSortContextMenuClose} onCloseAnimationEnd={handleSortContextMenuHide} positionX="left">
        {renderSortMenuItems()}
      </Menu>);
    }
    ...
    const handleSearchInputClick = useLastCallback((e) => {
        e.stopPropagation();
        e.preventDefault();
    });
    const modelMenuItemsContainerRef = useRef();
    ...
    function renderModelMenuItems() {
        ...
        return (<div className={styles.menuContentContainer} ref={modelMenuItemsContainerRef}>
        <SearchInput onClick={handleSearchInputClick} className={styles.search} value={searchModelQuery} onChange={setSearchModelQuery} onReset={handleSearchModelInputReset} placeholder={lang('Search')}/>
        ...
        {models.map((model) => {
                ...
                return (<MenuItem key={model.name} onClick={() => {
                        handleModelMenuItemClick(model);
                    }}>
              <ResaleGiftMenuAttributeSticker className={styles.sticker} sticker={model.sticker} type="model" observeIntersectionForLoading={observe} observeIntersectionForPlaying={observe}/>
              <div className={styles.menuItemStickerText}>
                {model.name}
              </div>
              <Icon className={styles.menuItemIcon} name={isSelected ? 'check' : 'placeholder'}/>
            </MenuItem>);
            })}
      </div>);
    }
    function renderModelMenu() {
        return (<Menu isOpen={isModelContextMenuOpen} anchor={modelContextMenuAnchor} ref={modelMenuRef} className={buildClassName(styles.menu, styles.left, 'with-menu-transitions')} getMenuElement={getModelMenuElement} autoClose onClose={handleModelContextMenuClose} onCloseAnimationEnd={handleModelContextMenuHide}>
    ...
    function renderBackdropMenuItems() {
        ...
        const isSelectedAll = selectedAttributes.length === 0;
        return (<div className={styles.menuContentContainer}>
        <SearchInput onClick={handleSearchInputClick} className={styles.search} value={searchBackdropQuery} onChange={setSearchBackdropQuery} onReset={handleSearchBackdropInputReset} placeholder={lang('Search')}/>
        ...
        {backdrops.map((backdrop) => {
                ...
                return (<MenuItem key={backdrop.name} onClick={() => {
                        handleBackdropMenuItemClick(backdrop);
                    }}>
              <RadialPatternBackground className={styles.backdrop} backgroundColors={[backdrop.centerColor, backdrop.edgeColor]} patternColor={backdrop.patternColor}/>
    ...
    function renderBackdropMenu() {
        return (<Menu isOpen={isBackdropContextMenuOpen} anchor={backdropContextMenuAnchor} ref={backdropMenuRef} className={buildClassName(styles.menu, styles.right, 'with-menu-transitions')} getMenuElement={getBackdropMenuElement} autoClose onClose={handleBackdropContextMenuClose} onCloseAnimationEnd={handleBackdropContextMenuHide} positionX="right">
    ...
    function renderPatternMenuItems() {
        ...
        const isSelectedAll = selectedAttributes.length === 0;
        return (<div className={styles.menuContentContainer}>
        <SearchInput onClick={handleSearchInputClick} className={styles.search} value={searchPatternQuery} onChange={setSearchPatternQuery} onReset={handleSearchPatternInputReset} placeholder={lang('Search')}/>
        ...
        {patterns.map((pattern) => {
                ...
                return (<MenuItem key={pattern.name} onClick={() => {
                        handlePatternMenuItemClick(pattern);
                    }}>

              <ResaleGiftMenuAttributeSticker className={styles.sticker} sticker={pattern.sticker} type="pattern"/>
    ...
    function renderPatternMenu() {
        return (<Menu isOpen={isPatternContextMenuOpen} anchor={patternContextMenuAnchor} ref={patternMenuRef} className={buildClassName(styles.menu, styles.right, 'with-menu-transitions')} getMenuElement={getPatternMenuElement} autoClose onClose={handlePatternContextMenuClose} onCloseAnimationEnd={handlePatternContextMenuHide}>
    ...
    return (<div className={styles.root}>
      ...
      <div className={styles.buttonsContainer}>
        <SortMenuButton onTrigger={handleSortContextMenu} isOpen={isSortContextMenuOpen}/>
        <ModelMenuButton onTrigger={handleModelContextMenu} isOpen={isModelContextMenuOpen}/>
        <BackdropMenuButton onTrigger={handleBackdropContextMenu} isOpen={isBackdropContextMenuOpen}/>
        <PatternMenuButton onTrigger={handlePatternContextMenu} isOpen={isPatternContextMenuOpen}/>
      </div>
    </div>);
};
