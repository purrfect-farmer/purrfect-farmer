...
const GiftResalePriceComposerModal = ({ modal, starsStargiftResaleCommissionPermille, starsStargiftResaleAmountMin, starsStargiftResaleAmountMax, starsUsdWithdrawRate, }) => {
    const { closeGiftResalePriceComposerModal, closeGiftInfoModal, updateStarGiftPrice, showNotification, } = getActions();
    ...
    const handleSellGift = useLastCallback(() => {
        if (!savedGift || savedGift.gift.type !== 'starGiftUnique' || !savedGift.inputGift || !price)
            return;
        closeGiftResalePriceComposerModal();
        closeGiftInfoModal();
    ...
    return (<Modal isOpen={isOpen} title={lang('GiftSellTitle')} hasCloseButton isSlim onClose={handleClose}>
      <div className={styles.inputPrice}>
        <InputText label={lang('InputPlaceholderGiftResalePrice')} onChange={handleChangePrice} value={price?.toString()} inputMode="numeric" tabIndex={0} teactExperimentControlled/>
      </div>
      ...
      <div className={styles.descriptionContainer}>
        ...
        {isPriceCorrect && Boolean(starsUsdWithdrawRate) && (<span className={styles.descriptionPrice}>
            {`≈ ${formatCurrencyAsString(price * starsUsdWithdrawRate, 'USD', lang.code)}`}
          </span>)}
      </div>
