...
const GiftTransferModal = ({ modal, contactIds, currentUserId, }) => {
    ...
    const handleCategoryChange = useLastCallback((category) => {
    ...
    const displayIds = useMemo(() => {
        if (isLoading)
            return MEMO_EMPTY_ARRAY;
        const global = getGlobal();
        return sortChatIds((foundIds || []).filter((peerId) => (peerId !== currentUserId && selectCanGift(global, peerId))), false);
    ...
    const handleTransfer = useLastCallback(() => {
        if (!renderingModal?.gift.inputGift)
            return;
        transferGift({
    ...
    return (<PickerModal isOpen={isOpen} onClose={closeGiftTransferModal} title={lang('GiftTransferTitle')} hasCloseButton shouldAdaptToSearch withFixedHeight ignoreFreeze>
      <PeerPicker itemIds={displayIds} categories={categories} onSelectedCategoryChange={handleCategoryChange} withDefaultPadding withPeerUsernames isSearchable noScrollRestore isLoading={isLoading} filterValue={searchQuery} filterPlaceholder={lang('Search')} onFilterChange={setSearchQuery} onSelectedIdChange={setSelectedId}/>
      {giftAttributes && (<ConfirmDialog isOpen={Boolean(selectedId)} noDefaultTitle onClose={closeConfirmModal} confirmLabel={renderingModal?.gift.transferStars
                ? lang('GiftTransferConfirmButton', { amount: formatStarsAsIcon(lang, renderingModal.gift.transferStars, { asFont: true }) }, { withNodes: true }) : lang('GiftTransferConfirmButtonFree')} confirmHandler={handleTransfer}>
          {renderingSelectedPeer && (<GiftTransferPreview peer={renderingSelectedPeer} gift={uniqueGift}/>)}
          <p>
