...
const GiftStatusInfoModal = ({ modal, currentUser, isCurrentUserPremium, }) => {
    const { closeGiftStatusInfoModal, setEmojiStatus, } = getActions();
    const lang = useLang();
    ...
    const radialPatternBackdrop = useMemo(() => {
        if (!emojiStatus || !isOpen)
            return undefined;
        const backdropColors = [emojiStatus.centerColor, emojiStatus.edgeColor];
        const patternColor = emojiStatus.patternColor;
        return (<RadialPatternBackground className={styles.radialPattern} backgroundColors={backdropColors} patternColor={patternColor} patternIcon={patternIcon.customEmoji}/>);
    ...
    const header = useMemo(() => {
        ...
        <div className={buildClassName(styles.profileBlock)} style={buildStyle(subtitleColor && `color: ${subtitleColor}`)}>

          {radialPatternBackdrop}
          <Avatar peer={mockPeerWithStatus} size="jumbo" className={styles.avatar}/>
          <FullNameTitle peer={mockPeerWithStatus} className={styles.userTitle} withEmojiStatus noFake noVerified statusSparklesColor={subtitleColor}/>
    ...
    const footer = useMemo(() => {
        ...
        <Button size="smaller" onClick={onWearClick}>
