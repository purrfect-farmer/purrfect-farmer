...
const ChatInviteModal = ({ modal }) => {
    ...
    const { about, isBroadcast, participantIds, participantsCount, photo, isRequestNeeded, } = inviteInfo || {};
    ...
    return (<Modal isOpen={Boolean(modal)} contentClassName={styles.content} isSlim onClose={handleClose} onEnter={handleAccept}>
      ...
      {participants && (<div ref={participantsRef} className={buildClassName(styles.participants, 'no-scrollbar')}>
          {participants.map((participant) => (<PeerBadge className={styles.participant} peer={participant} text={getUserFullName(participant)}/>))}
        </div>)}
