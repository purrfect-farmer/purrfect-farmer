...
const CollectibleInfoModal = ({ modal, phoneCodeList, }) => {
    const { closeCollectibleInfoModal, openChat, openUrl, showNotification, } = getActions();
    ...
    return (<Modal isOpen={Boolean(modal)} isSlim contentClassName={styles.content} onClose={closeCollectibleInfoModal}>
      <Button round color="translucent" size="smaller" className={styles.closeButton} ariaLabel={lang('Close')} onClick={handleClose}>
        <Icon name="close"/>
      </Button>
      <div className={styles.icon}>
        <AnimatedIconWithPreview tgsUrl={isUsername ? LOCAL_TGS_URLS.Mention : LOCAL_TGS_URLS.Fragment} size={TOP_ICON_SIZE}/>
      </div>
      <h3 className={styles.title}>
        {title && renderText(title, ['simple_markdown'])}
      </h3>
      <PeerChip className={styles.chip} peerId={modal?.peerId} forceShowSelf clickArg={modal?.peerId} onClick={handleOpenChat}/>
