...
const GiftCodeModal = ({ currentUserId, modal, messageSender, }) => {
    const { closeGiftCodeModal, applyGiftCode, focusMessage, } = getActions();
    ...
    const handleOpenGiveaway = useLastCallback(() => {
        if (!modal || !modal.info.giveawayMessageId)
            return;
        focusMessage({
    ...
    const modalData = useMemo(() => {
        ...
        const footer = (<span className={styles.centered}>
        {renderText(info.usedAt ? lang('BoostingUsedLinkDate', formatDateTimeToString(info.usedAt * 1000, lang.code, true))
                : lang('BoostingSendLinkToAnyone'), ['simple_markdown'])}
      </span>);
        return {
            header,
            tableData,
    ...
    if (!modalData)
        return undefined;
    return (<TableInfoModal isOpen={isOpen} title={lang('lng_gift_link_title')} tableData={modalData.tableData} header={modalData.header} footer={modalData.footer} buttonText={canUse ? lang('BoostingUseLink') : lang('Close')} onButtonClick={handleButtonClick} onClose={closeGiftCodeModal}/>);
};
