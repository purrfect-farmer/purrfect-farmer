...
const ReportAdModal = ({ modal, }) => {
    const { reportSponsored, closeReportAdModal, openPreviousReportAdModal, } = getActions();
    ...
    const header = useMemo(() => {
        ...
        <Button round color="translucent" size="smaller" ariaLabel={lang(renderingDepth ? 'Back' : 'Close')} onClick={handleBackClick}>
    ...
    return (<Modal isOpen={isOpen} hasCloseButton className={styles.root} header={header} onClose={closeReportAdModal}>
      <Transition name="slide" className={styles.transition} ref={transitionRef} activeKey={renderingDepth} onStart={handleAnimationStart}>
        <div className={styles.slide}>
          ...
          {renderingSection?.options.map((option) => (<ListItem narrow secondaryIcon="next" className={styles.option} buttonClassName={styles.optionButton} clickArg={option.option} onClick={handleOptionClick}>
