...
const SuggestedStatusModal = ({ modal, currentUser, bot }) => {
    ...
    return (<Modal isOpen={isOpen} contentClassName={styles.content} hasAbsoluteCloseButton isSlim onClose={handleClose}>
      {renderingModal && (<CustomEmoji className={styles.topEmoji} documentId={renderingModal.customEmojiId} size={CUSTOM_EMOJI_SIZE} loopLimit={1} forceAlways/>)}
      <div>
        <h3 className={styles.title}>{lang('BotSuggestedStatusTitle')}</h3>
        <p className={styles.description}>{description}</p>
      </div>
      {mockPeerWithStatus && (<PeerChip mockPeer={mockPeerWithStatus} withEmojiStatus/>)}
