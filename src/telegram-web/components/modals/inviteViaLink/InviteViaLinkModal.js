...
const InviteViaLinkModal = ({ modal, chat, }) => {
    ...
    const premiumSectionText = useMemo(() => {
        if (!invitableWithPremiumIds?.length || !topListPeers?.length)
            return undefined;
    ...
    return (<Modal isOpen={Boolean(userIds && chat)} contentClassName={styles.content} onClose={handleClose} isSlim>
      <Button round color="translucent" size="smaller" className={styles.closeButton} ariaLabel={lang('Close')} onClick={handleClose}>
        <Icon name="close"/>
      </Button>
      {premiumSectionText && (<>
          <AvatarList className={styles.avatarList} peers={topListPeers} size="large"/>
          ...
          <Button withPremiumGradient isShiny size="smaller" onClick={handleOpenPremiumModal}>
            {lang('InvitePremiumBlockedSubscribe')}
          </Button>
        </>)}
      {hasPremiumSection && hasSelectableSection && (<Separator className={styles.separator}>
      ...
      {hasSelectableSection && (<>
          ...
          <PeerPicker className={styles.userPicker} itemIds={userIds} selectedIds={selectedMemberIds} lockedUnselectedIds={unselectableIds} lockedUnselectedSubtitle={lang('InvitePremiumBlockedUser')} onSelectedIdsChange={setSelectedMemberIds} isViewOnly={!canSendInviteLink} allowMultiple withStatus itemInputType="checkbox"/>
          {canSendInviteLink && (<Button className={styles.sendInvites} size="smaller" onClick={handleSendInviteLink} disabled={!selectedMemberIds.length}>
