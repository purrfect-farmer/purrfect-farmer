...
const AttachBotInstallModal = ({ modal, }) => {
    ...
    return (<ConfirmDialog isOpen={Boolean(bot)} onClose={cancelAttachBotInstall} title={lang('lng_mini_apps_disclaimer_title')} confirmHandler={handleConfirm} isConfirmDisabled={!isTosAccepted}>
      {renderText(lang('lng_mini_apps_disclaimer_text', bot?.shortName), ['simple_markdown'])}
      <Checkbox className="dialog-checkbox" checked={isTosAccepted} label={tosLabel} onCheck={setIsTosAccepted}/>
