...
function selectOptionalPeer(peerId) {
    return (global) => (peerId ? selectPeer(global, peerId) : undefined);
}
const StarsTransactionItem = ({ transaction, className }) => {
    const { openStarsTransactionModal } = getActions();
    const { date, stars, photo, peer: transactionPeer, extendedMedia, subscriptionPeriod, } = transaction;
    const lang = useLang();
    ...
    const previewContent = useMemo(() => {
        if (isUniqueGift) {
            ...
            return (<>
          <RadialPatternBackground className={styles.uniqueGiftBackground} backgroundColors={backgroundColors}/>
          <AnimatedIconFromSticker className={styles.giftSticker} sticker={giftSticker} size={GIFT_STICKER_SIZE} play={false}/>
        </>);
        }
        if (giftSticker) {
            return (<AnimatedIconFromSticker className={styles.giftSticker} sticker={giftSticker} size={GIFT_STICKER_SIZE} play={false}/>);
        }
        if (extendedMedia) {
        ...
        {Boolean(subscriptionPeriod) && (<StarIcon className={styles.subscriptionStar} type="gold" size="small"/>)}
      </>);
    ...
    return (<div className={buildClassName(styles.root, className)} onClick={handleClick}>
      <div className={styles.preview}>
        {previewContent}
      </div>
      <div className={styles.info}>
        <h3 className={styles.title}>{data.title}</h3>
        {data.description && (<p className={styles.description}>{renderText(data.description)}</p>)}
