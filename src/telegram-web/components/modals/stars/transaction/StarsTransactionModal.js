...
import { selectCanPlayAnimatedEmojis, selectGiftStickerForStars, selectPeer, } from '../../../../global/selectors';
...
const StarsTransactionModal = ({ modal, peer, canPlayAnimatedEmojis, topSticker, paidMessageCommission, }) => {
    ...
    const starModalData = useMemo(() => {
        ...
        const { giveawayPostId, photo, stars, isGiftUpgrade, starGift, isGiftResale, starRefCommision, } = transaction;
        ...
        const uniqueGiftHeader = isUniqueGift && (<div className={buildClassName(styles.header, styles.uniqueGift)}>
        <UniqueGiftHeader backdropAttribute={giftAttributes.backdrop} patternAttribute={giftAttributes.pattern} modelAttribute={giftAttributes.model} title={gift.title} subtitle={lang('GiftInfoCollectible', { number: gift.number })} resellPrice={transaction.stars}/>
      </div>);
        const regularHeader = (<div className={styles.header}>
        {media && (<PaidMediaThumb className={buildClassName(styles.mediaPreview, 'transaction-media-preview')} media={media} onClick={handleOpenMedia}/>)}
        {!media && sticker && (<AnimatedIconFromSticker key={transaction.id} sticker={sticker} play={canPlayAnimatedEmojis} noLoop/>)}
        {shouldDisplayAvatar && (<Avatar peer={avatarPeer} webPhoto={photo} size="giant"/>)}
        {!sticker && (<img className={buildClassName(styles.starsBackground)} src={StarsBackground} alt="" draggable={false}/>)}
        {Boolean(title) && <h1 className={styles.title}>{title}</h1>}
        <p className={styles.description}>{description}</p>
        <p className={styles.amount}>
          <span className={buildClassName(styles.amount, isNegativeStarsAmount(stars) ? styles.negative : styles.positive)}>
            {formatStarsTransactionAmount(lang, stars)}
          </span>
          <StarIcon type="gold" size="middle"/>
          {transaction.isRefund && (<p className={styles.refunded}>{lang('Refunded')}</p>)}
        </p>
        {Boolean(transaction.paidMessages && transaction.starRefCommision && paidMessageCommission) && (<p className={styles.description}>
            {lang('PaidMessageTransactionDescription', { percent: formatPercent(paidMessageCommission / 10) }, {
                    withNodes: true,
                    withMarkdown: true,
                })}
          </p>)}
        ...
        if (transaction.starRefCommision && transaction.paidMessages) {
            tableData.push([
                ...
                formatStarsAsIcon(lang, transaction.stars.amount / ((100 - transaction.starRefCommision) / 100), { asFont: false, className: styles.starIcon, containerClassName: styles.totalStars }),
            ]);
        }
        if (messageLink) {
        ...
        if (transaction.id) {
            tableData.push([
                ...
                (<>
            <div className={styles.tid} onClick={() => {
        ...
        const footer = (<span className={styles.footer}>
        {footerTextParts[0]}
    ...
    return (<TableInfoModal isOpen={Boolean(transaction)} className={styles.modal} hasBackdrop={transaction?.starGift?.type === 'starGiftUnique'} header={renderingModalData?.header} tableData={renderingModalData?.tableData} footer={renderingModalData?.footer} buttonText={oldLang('OK')} onClose={closeStarsTransactionModal}/>);
};
