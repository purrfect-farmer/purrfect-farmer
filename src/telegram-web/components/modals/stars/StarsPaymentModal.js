...
const StarPaymentModal = ({ modal, bot, starsBalanceState, paidMediaMessage, paidMediaChat, }) => {
    ...
    return (<Modal contentClassName={styles.paymentContent} isOpen={isOpen} hasAbsoluteCloseButton isSlim onClose={closeStarsPaymentModal}>
      ...
      <div className={styles.paymentImages} dir={oldLang.isRtl ? 'ltr' : 'rtl'}>
        {paidMediaMessage ? (<PaidMediaThumb media={paidMediaMessage.content.paidMedia.extendedMedia}/>) : inviteCustomPeer ? (<>
          ...
          </>) : (<PeerBadge peer={!photo ? bot : undefined} avatarWebPhoto={photo} avatarSize="giant" badgeIcon="star" badgeText={formatInteger(amount)} badgeClassName={styles.amountBadge} className={styles.paymentPhoto}/>)}
      ...
      {Boolean(disclaimerText) && (<div className={buildClassName(styles.disclaimer, styles.smallerText)}>
          {disclaimerText}
        </div>)}
    </Modal>);
};
