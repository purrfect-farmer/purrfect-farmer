...
const StarsSubscriptionModal = ({ modal, peer, }) => {
    const { closeStarsSubscriptionModal, fulfillStarsSubscription, changeStarsSubscription, checkChatInvite, loadStarStatus, openInvoice, } = getActions();
    ...
    const starModalData = useMemo(() => {
        ...
        const { pricing, until, isCancelled, canRefulfill, photo, title, hasBotCancelled, } = subscription;
        ...
        <img className={buildClassName(styles.starsBackground)} src={StarsBackground} alt="" draggable={false}/>
        ...
        {buttonState !== 'hidden' && (<Button size="smaller" color={buttonState === 'cancel' ? 'danger' : 'primary'} isText={buttonState === 'cancel'} onClick={handleButtonClick}>
            {oldLang(buttonState === 'cancel' ? 'StarsSubscriptionCancel'
    ...
    return (<TableInfoModal isOpen={Boolean(subscription)} className={styles.modal} header={renderingModalData?.header} tableData={renderingModalData?.tableData} footer={renderingModalData?.footer} onClose={closeStarsSubscriptionModal}/>);
};
