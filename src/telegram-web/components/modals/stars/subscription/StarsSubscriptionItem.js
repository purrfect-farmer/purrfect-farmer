...
function selectProvidedPeer(peerId) {
    return (global) => (selectPeer(global, peerId));
}
const StarsSubscriptionItem = ({ subscription }) => {
    const { openStarsSubscriptionModal } = getActions();
    const { peerId, pricing, until, isCancelled, title, photo, } = subscription;
    ...
    return (<div className={styles.root} onClick={handleClick}>
      ...
      <div className={styles.info}>
        ...
        {title && (<p className={styles.subtitle}>
            {photo && <Avatar webPhoto={photo} size="micro"/>}
            {renderText(title)}
          </p>)}
        <p className={styles.description}>
          {lang(hasExpired ? 'StarsSubscriptionExpired'
            : isCancelled ? 'StarsSubscriptionExpires' : 'StarsSubscriptionRenews', formattedDate)}
        </p>
      </div>
      <div className={styles.status}>
        {(isCancelled || hasExpired) ? (<div className={styles.statusEnded}>
