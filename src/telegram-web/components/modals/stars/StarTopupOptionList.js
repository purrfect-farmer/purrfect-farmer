...
const StarTopupOptionList = ({ isActive, className, options, selectedStarOption, selectedStarCount, starsNeeded, onClick, }) => {
    ...
    const [renderingOptions, canExtend] = useMemo(() => {
        ...
        options.forEach((option, index) => {
            ...
            if (starsNeeded && !forceShowAll && option.stars < starsNeeded)
                return;
    ...
    return (<div className={buildClassName(styles.options, className)}>
      {renderingOptions?.map(({ option, starsCount, isWide }) => {
            ...
            return (<div className={buildClassName(styles.option, (!isOdd && isWide) && styles.wideOption, isActiveOption && styles.active)} key={option.stars} onClick={() => onClick?.(option)}>
            <div className={styles.optionTop}>
              +
              {formatInteger(option.stars)}
              <div className={styles.stackedStars} dir={lang.isRtl ? 'ltr' : 'rtl'}>
                {Array.from({ length: starsCount }).map(() => (<StarIcon className={styles.stackedStar} type="gold" size="big"/>))}
              </div>
            </div>
            ...
            {(isActiveOption || (selectedStarOption && 'winners' in selectedStarOption))
                    && Boolean(perUserStarCount) && (<div className={styles.optionBottom}>
      ...
      {!areOptionsExtended && canExtend && (<Button className={styles.moreOptions} isText noForcedUpperCase onClick={markOptionsExtended}>
