...
import { selectIsCurrentUserFrozen, selectIsCurrentUserPremium, selectStickerSet, selectUser, } from '../../../global/selectors';
...
const INTERVAL = 3200;
const EmojiStatusAccessModal = ({ modal, currentUser, stickerSet, isPremium, isAccountFrozen, }) => {
    const { closeEmojiStatusAccessModal, toggleUserEmojiStatusPermission, sendWebAppEvent, openPremiumModal, loadDefaultStatusIcons, } = getActions();
    ...
    const mockPeerWithStatus = useMemo(() => {
        if (!currentUser || !stickerSet?.stickers)
            return undefined;
        return {
            ...currentUser,
            emojiStatus: {
    ...
    const renderPickerItem = useLastCallback(() => {
        return (<PeerChip withEmojiStatus className={styles.chatItem} itemClassName={styles.itemName} mockPeer={mockPeerWithStatus}/>);
    });
    ...
    return (<Modal className={buildClassName('confirm')} contentClassName={styles.content} isOpen={isOpen} onClose={onCloseHandler}>
      {renderPickerItem()}
      <div>
        {renderStatusText()}
        <div className="dialog-buttons mt-2" ref={containerRef}>
          <Button className="confirm-dialog-button" isText onClick={confirmHandler} color="primary">
            {oldLang('lng_bot_allow_write_confirm')}
          </Button>
          <Button className="confirm-dialog-button" isText onClick={onCloseHandler}>
            {lang('Cancel')}
          </Button>
        </div>
      </div>
