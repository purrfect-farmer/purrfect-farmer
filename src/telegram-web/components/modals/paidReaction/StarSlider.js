...
const StarSlider = ({ maxValue, defaultValue, className, onChange, }) => {
    ...
    return (<div className={buildClassName(styles.root, className)} style={`--progress: ${value / points.length}`}>
      ...
      <input className={styles.slider} type="range" min={0} max={points.length} defaultValue={getProgress(points, defaultValue)} step="any" onChange={handleChange}/>
    </div>);
};
