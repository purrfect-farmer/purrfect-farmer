...
import { selectChat, selectChatMessage, selectPeer, selectUser, } from '../../../global/selectors';
...
const PaidReactionModal = ({ modal, chat, message, maxAmount, starBalance, defaultPrivacy, sendPaidReactionsAsPeerIds, currentUserId, currentUser, }) => {
    ...
    const lang = useLang();
    const handleShowInTopSendersChange = useLastCallback((e) => {
        setShouldSendAsAnonymous(!e.target.checked);
    });
    const handleAmountChange = useLastCallback((value) => {
    ...
    const renderMenuItem = useLastCallback((peerId) => {
        ...
        <Icon className={styles.itemIcon} name={isSelected ? 'check' : 'placeholder'}/>
      </MenuItem>);
    });
    const SendAsPeerMenuButton = useMemo(() => {
        return ({ onTrigger, isOpen }) => (<Button ripple={!isMobile} size="smaller" color="translucent" className={buildClassName(styles.sendAsPeerMenuButton, isOpen ? 'active' : '')} onClick={onTrigger} ariaLabel={lang('AccDescrOpenMenu2')}>
        <Avatar className={styles.sendAsPeerButtonAvatar} size="mini" peer={shouldSendAsAnonymous ? ANONYMOUS_PEER : senderPeer}/>
        <Icon name="down" className={styles.buttonDownIcon}/>
      </Button>);
    }, [isMobile, lang, senderPeer, shouldSendAsAnonymous]);
    const sendAsPeersMenu = useMemo(() => {
        if (!canChangeSendAsPeer)
            return undefined;
        return (<DropdownMenu className={buildClassName(styles.sendAsPeerMenu, 'with-menu-transitions')} bubbleClassName={styles.sendAsPeerMenuBubble} trigger={SendAsPeerMenuButton} positionX="right" autoClose>
    ...
    const topReactors = useMemo(() => {
        ...
        all.forEach((reactor) => {
            ...
            if (!peer && !reactor.isAnonymous && !reactor.isMy)
                return;
            if (reactor.isMy) {
                myReactorAmount = reactor.count;
            }
            if (reactor.isMy && (reactor.peerId !== sendAsPeerId || (reactor.isAnonymous && !shouldSendAsAnonymous)))
                return;
    ...
    return (<Modal isOpen={Boolean(modal)} onClose={closePaidReactionModal} isSlim hasAbsoluteCloseButton contentClassName={styles.content} isLowStackPriority>
      ...
      <StarSlider className={styles.slider} defaultValue={DEFAULT_STARS_AMOUNT} maxValue={maxAmount} onChange={handleAmountChange}/>
      ...
      {topReactors && (<div className={styles.top}>
          {topReactors.map((reactor) => {
                ...
                return (<PeerBadge className={styles.topPeer} key={`${reactor.user?.id || 'anonymous'}-${countText}`} peer={peer} badgeText={countText} badgeIcon="star" badgeClassName={styles.topBadge} text={text}/>);
            })}
        </div>)}
      {topReactors && (<Separator className={styles.separator}/>)}
      <Checkbox className={buildClassName(styles.checkBox, 'dialog-checkbox')} checked={!shouldSendAsAnonymous} onChange={handleShowInTopSendersChange} label={oldLang('StarsReactionShowMeInTopSenders')}/>
      <Button size="smaller" onClick={handleSend}>
