...
const BoostModal = ({ modal, chat, chatFullInfo, prevBoostedChat, isCurrentUserPremium, }) => {
    const { applyBoost, closeBoostModal, requestConfetti, openPremiumModal, loadFullChat, } = getActions();
    ...
    const { isStatusLoaded, isBoosted, boost, title, leftText, rightText, value, progress, descriptionText, canBoostMore, } = useMemo(() => {
        ...
        const { hasMyBoost, } = modal.boostStatus;
        ...
        const { boosts, currentLevel, hasNextLevel, levelProgress, remainingBoosts, isMaxLevel, } = getBoostProgressInfo(modal.boostStatus, true);
    ...
    function renderContent() {
        ...
        return (<>
        <PremiumProgress leftText={leftText} rightText={rightText} progress={progress} floatingBadgeText={value} floatingBadgeIcon="boost"/>
        {isBoosted && (<div className={buildClassName(styles.description, styles.bold)}>
    ...
    return (<Modal isOpen={isOpen} title={title} className={styles.modal} contentClassName={styles.content} onClose={closeBoostModal}>
      {renderContent()}
      {isReplacingBoost && boostedChatTitle && (<Modal isOpen={isReplaceModalOpen} className={styles.replaceModal} contentClassName={styles.replaceModalContent} onClose={closeReplaceModal}>
          ...
          <div>
            {renderText(oldLang('ChannelBoost.ReplaceBoost', [boostedChatTitle, chatTitle]), ['simple_markdown', 'emoji'])}
          </div>
          <div className="dialog-buttons">
      ...
      {Boolean(boost?.cooldownUntil) && (<ConfirmDialog isOpen={isWaitDialogOpen} isOnlyConfirm confirmLabel={oldLang('OK')} title={oldLang('ChannelBoost.Error.BoostTooOftenTitle')} onClose={closeWaitDialog} confirmHandler={closeWaitDialog}>
          {renderText(oldLang('ChannelBoost.Error.BoostTooOftenText', formatShortDuration(lang, boost.cooldownUntil - getServerTime())), ['simple_markdown', 'emoji'])}
        </ConfirmDialog>)}
      {!isCurrentUserPremium && (<ConfirmDialog isOpen={isPremiumDialogOpen} confirmLabel={oldLang('Common.Yes')} title={oldLang('PremiumNeeded')} onClose={closePremiumDialog} confirmHandler={handleProceedPremium}>
