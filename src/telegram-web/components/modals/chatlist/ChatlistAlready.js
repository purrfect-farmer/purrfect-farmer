...
const ChatlistAlready = ({ invite, folder }) => {
      ...
      <div className={buildClassName(styles.pickerWrapper, 'custom-scroll')}>
        {Boolean(invite.missingPeerIds.length) && (<>
            <div className={styles.pickerHeader}>
              ...
              <div className={styles.selectionToggle} role="button" tabIndex={0} onClick={handleSelectionToggle}>
                {selectedPeerIds.length === invite.missingPeerIds.length
                ? oldLang('DeselectAll') : oldLang('SelectAll')}
              </div>
            </div>
            <PeerPicker itemIds={invite.missingPeerIds} onSelectedIdsChange={setSelectedPeerIds} selectedIds={selectedPeerIds} allowMultiple withStatus itemInputType="checkbox"/>
          </>)}
        <div className={styles.pickerHeader}>
          <div className={styles.pickerHeaderInfo}>
        ...
        <PeerPicker itemIds={invite.alreadyPeerIds} lockedSelectedIds={invite.alreadyPeerIds} selectedIds={invite.alreadyPeerIds} allowMultiple withStatus itemInputType="checkbox"/>
      </div>
      <Button size="smaller" onClick={handleButtonClick}>
