...
const ChatlistNew = ({ invite }) => {
    const { closeChatlistModal, joinChatlistInvite } = getActions();
    const lang = useLang();
    const oldLang = useOldLang();
    const [selectedPeerIds, setSelectedPeerIds] = useState(invite.peerIds);
    ...
    return (<div className={styles.content}>
      ...
      <div className={buildClassName(styles.pickerWrapper, 'custom-scroll')}>
        ...
        <PeerPicker itemIds={invite.peerIds} lockedSelectedIds={joinedIds} onSelectedIdsChange={setSelectedPeerIds} selectedIds={selectedPeerIds} allowMultiple withStatus itemInputType="checkbox"/>
      </div>
      <Button onClick={handleButtonClick} size="smaller" disabled={!selectedPeerIds.length}>
