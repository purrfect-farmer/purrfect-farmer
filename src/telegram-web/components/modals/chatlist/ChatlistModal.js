...
const ChatlistInviteModal = ({ modal, folder, }) => {
    ...
    const folderTabNumber = useMemo(() => {
        if (!renderingInfo?.invite)
            return undefined;
        if ('missingPeerIds' in renderingInfo.invite)
            return renderingInfo.invite.missingPeerIds.length;
        return undefined;
    }, [renderingInfo]);
    function renderFolders(folderTitle) {
        return (<div className={styles.foldersWrapper}>
    ...
    return (<Modal isOpen={isOpen} title={title} onClose={closeChatlistModal} isSlim hasCloseButton>
