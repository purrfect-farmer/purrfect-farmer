...
import useAutoWalletsRotationMutation from "@/hooks/useAutoWalletsRotationMutation";
...
export default function AutoRotationDialog() {
  const { master } = useAuto();
  const [transferFunds, setTransferFunds] = useState(true);
  const [includeAccounts, setIncludeAccounts] = useState(true);

  const mutation = useAutoWalletsRotationMutation();
